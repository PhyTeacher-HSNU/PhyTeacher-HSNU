...
function init(){
	...
	for(var i =0 ; i<nn ; i++){
		...
		bb1=new TEACHER.ObjSphere(rr,0x00ffff);

		bb1.position.x=-90+i/2;
		bb1.position.y=0;
		bb1.position.z=0;
		scene.add(bb1);
		ballArray1.push(bb1);
	...
	for(var i =0 ; i<nn ; i++){
		...
		bb2=new TEACHER.ObjSphere(rr,0xff0000);

		bb2.position.x=-90+i/2;
		bb2.position.y=0;
		bb2.position.z=0;
		scene.add(bb2);
		ballArray2.push(bb2);
	...
	for(var i =0 ; i<280 ; i++){
		...
		bb3=new TEACHER.ObjSphere(rr,0x00ffff);

		bb3.position.x=-90+i/2;
		bb3.position.y=0;
		bb3.position.z=0;
		scene.add(bb3);
		ballArray3.push(bb3);
	...
	for(var i =0 ; i<80 ; i++){
		...
		bb4=new TEACHER.ObjSphere(rr,0xff00ff);

		bb4.position.x=50+i/2;
		bb4.position.y=0;
		bb4.position.z=0;
		scene.add(bb4);
		ballArray4.push(bb4);
	...
	Arrow1=new TEACHER.ObjArrow(1,0xffffff);
	scene.add(Arrow1);
	Arrow2=new TEACHER.ObjArrow(1,0xffffff);
	scene.add(Arrow2);
	...
	world2D.btn01.setLabel('加偏振片');
	world2D.btn01.on('click' , clickBtn);
	world2D.btn02.setLabel('偏振片旋轉');
	world2D.btn02.on('click' , clickBtn);


	world2D.ch01.setLabel('顯示原電場');
	world2D.ch02.setLabel('顯示原磁場');
...
function tick(){
	...
	for(var i=0 ; i<nn ; i++){
		...
		var zz = AA*Math.sin(2*Math.PI*b2.position.x/lambda - 2*Math.PI*tt/period);
	...
	for(var i=0;i<280;i++){
		...
		var yy = AA*Math.sin(2*Math.PI*b3.position.x/lambda - 2*Math.PI*tt/period);
		b3.position.x=-90+i/2;
		b3.position.y=yy;
		b3.position.z=0;
	}
	...
	for(var i=0;i<80;i++){
		...
		var yy = Math.sqrt(2)/2*AA*Math.sin(2*Math.PI*b4.position.x/lambda - 2*Math.PI*tt/period);
		var zz =-Math.sqrt(2)/2*AA*Math.sin(2*Math.PI*b4.position.x/lambda - 2*Math.PI*tt/period);
		b4.position.y=yy;
		b4.position.z=zz;
	...
	for(var i=0;i<nn;i++){
		ballArray1[i].visible=world2D.ch01.checked;
	}
	for(var i=0;i<nn;i++){
		ballArray2[i].visible=world2D.ch02.checked;
	}
...
function clickBtn(e){
	...
	if(str==='btn01'){
		box=new TEACHER.ObjBox(0.5,20,20);
		box.mat.color.setHSL(0,0.2,0.05);
		box.position.x=50;
		scene.add(box);
	}else if(str==='btn02'){
		...
		for(var i =0 ; i<280 ; i++){
			ballArray3[i].visible=true;
		}
		for(var i =0 ; i<80 ; i++){
			ballArray4[i].visible=true;
		}
	}
