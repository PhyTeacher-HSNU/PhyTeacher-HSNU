...
function init(){
	...
	var superman = new TEACHER.ObjPicPlane(15,20,pics.superman);
	scene.add(superman);
	superman.position.x =55;
	superman.position.z =-50;
	superman.position.y =10;
	var godzila = new TEACHER.ObjPicPlane(30,40,pics.godzila);
	scene.add(godzila);
	godzila.position.x =-55;
	godzila.position.z =55;
	godzila.position.y =-5;
	myname = new TEACHER.ObjTextPlane(56,7,"1533 17 倪德耀","z",0xffffff);
	scene.add(myname);
	for (var i=0; i<nn; i++){
		var bb1 = new TEACHER.ObjSphere(0.6,0xFFFF50);
		bb1.position.x = 2/3*i-50;
		bb1.position.y = 10;
		bb1.position.z = (2/3*i-50)*-1;
		scene.add(bb1);
		ballarray1.push(bb1);
		var bb2 = new TEACHER.ObjSphere(0.6,0xFFFF50);
		bb2.position.x = 2/3*i-50;
		bb2.position.y = 10;
		bb2.position.z = (2/3*i-50)*-1;
		scene.add(bb2);
		ballarray2.push(bb2);
		var bb3 = new TEACHER.ObjSphere(0.6);
		bb3.position.x = 2/3*i-50;
		bb3.position.y = 10;
		bb3.position.z = (2/3*i-50)*-1;
		scene.add(bb3);
		ballarray3.push(bb3);
		var bb4 = new TEACHER.ObjSphere(0.6);
		bb4.position.x = 2/3*i-50;
		bb4.position.y = 10;
		bb4.position.z = (2/3*i-50)*-1;
		scene.add(bb4);
		ballarray4.push(bb4);
	...
	//slide
	world2D.sl01.setLabel("光波波長");
	world2D.sl01.maximum = 40;
	world2D.sl01.minimum = 10;
	world2D.sl01.value = lambda;
	world2D.sl02.setLabel("光波振幅");
	world2D.sl02.maximum = 10;
	world2D.sl02.minimum = 1;
	
	world2D.sl03.setLabel("佔上風");
	world2D.sl03.maximum = 0.5;
	world2D.sl03.minimum = -0.5;
	world2D.sl03.value = dt;

	world2D.slCameraRR.value = 150

	//btn
	/*world2D.btn01.setLabel("STEP");
	world2D.btn02.setLabel("PAUSE");
	world2D.btn01.on("mousedown",ff);
	world2D.btn01.on("pressup",ff);
	world2D.btn02.on("click",ff);*/
...
function tick(){
	...
	dt = world2D.sl03.value*6;
	vs = world2D.sl03.value*100;
	tt += dt
	var random = (Math.random()*-0.5)*10
	for(var i=0; i<nn ;i++){
		var bb1 = ballarray1[i]
		var yy1 = AA*Math.sin( 2*Math.PI*bb1.position.x/lambda - 2*Math.PI*tt/period );
		bb1.position.y = 10+yy1;
		var bb2 = ballarray2[i]
		var yy2 = AA*Math.sin( 2*Math.PI*bb2.position.x/lambda - 2*Math.PI*tt/period );
		bb2.position.y = 10-yy2;
		var bb3 = ballarray3[i]
		var zz3 = AA*Math.sin( 2*Math.PI*bb3.position.x/lambda*3 - 2*Math.PI*tt/period );
		bb3.position.z = (2/3*i-50)*-1+zz3;
		var bb4 = ballarray4[i]
		var zz4 = AA*Math.sin( 2*Math.PI*bb4.position.x/lambda*3 - 2*Math.PI*tt/period );
		bb4.position.z = (2/3*i-50)*-1-zz4;
		if(bb1.position.x<=vs+random){
			bb1.mat.color.setHex(0x00FFFF);
		}
		if(bb1.position.x>=vs+random){
			bb1.mat.color.setHex(0xFFFF50);
		}
		if(bb2.position.x<=vs+random){
			bb2.mat.color.setHex(0x00FFFF);
		}
		if(bb2.position.x>=vs+random){
			bb2.mat.color.setHex(0xFFFF50);
		}
		if(bb3.position.x<=vs+random){
			bb3.mat.color.setHex(0x00FFFF);
		}
		if(bb3.position.x>=vs+random){
			bb3.mat.color.setHex(0xFFFF50);
		}
		if(bb4.position.x<=vs+random){
			bb4.mat.color.setHex(0x00FFFF);
		}
		if(bb4.position.x>=vs+random){
			bb4.mat.color.setHex(0xFFFF50);
		}
