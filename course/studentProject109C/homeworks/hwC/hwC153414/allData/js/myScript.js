...
function init(){
	...
	word=new TEACHER.ObjTextPlane(115,20,"153414孫堃展","z",0xFFFFFF);
	fire=new TEACHER.ObjPicPlane(40,40,pics.fire,"z",2);
	ice=new TEACHER.ObjPicPlane(50,40,pics.ice,"z",2);
	word.position.y=60;
	word.position.x=0;
	word.position.z=-50;
	fire.position.x=0;
	...
	ice.position.z=0;
	ice.position.y=-25;
	scene.add(word,fire,ice);

    for(var i=0;i<nn;i++){
		var bb=new TEACHER.ObjSphere(rr,0xffffff);

		bb.position.y=10+rr+10*Math.floor(i/100);
		bb.position.x=-45+(i%10)*10;
		bb.position.z=-45+Math.floor(i/10)*10-100*Math.floor(i/100);

		bb.vx=5*(Math.random()-0.5);
		bb.vy=5*(Math.random()-0.5);
	...
	world2D.sl02.setLabel("球和球恢復係數");
	world2D.sl02.value=world2D.sl03.value/50;
	world2D.sl02.maximum=2;
	world2D.sl02.minimum=0;
	world2D.sl02.digitN=2;

	world2D.sl03.setLabel("溫度");
	world2D.sl03.value=50;
	world2D.sl03.maximum=60;
	world2D.sl03.minimum=10;
	world2D.sl03.digitN=1;
	
	world2D.ch01.setLabel("顯示箭頭");
	world2D.ch02.setLabel("DISCO");

	world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=world2D.btn01.visible=world2D.btn02.visible=world2D.sl02.visible=fire.visible=ice.visible=false;
...
function tick(){
	...
	for(var i=0;i<nn;i++){
        ...
        if(bb.position.x>50-rr*world2D.sl01.value&&bb.vx>0){
			bb.vx*=-1*world2D.sl03.value/50;
		}else if(bb.position.x<-50+rr*world2D.sl01.value&&bb.vx<0){
			bb.vx*=-1*world2D.sl03.value/50;
		}else if(bb.position.z>50-rr*world2D.sl01.value&&bb.vz>0){
			bb.vz*=-1*world2D.sl03.value/50;
		}else if(bb.position.z<-50+rr*world2D.sl01.value&&bb.vz<0){
			bb.vz*=-1*world2D.sl03.value/50;
		}else if(bb.position.y>60&&bb.vy>0){
			bb.vy*=-1*world2D.sl03.value/50;
		}else if(bb.position.y<rr*world2D.sl01.value&&bb.vy<0){
			bb.vy*=-1*world2D.sl03.value/50;
		}
        
		bb.scale.x=bb.scale.y=bb.scale.z=rr*world2D.sl01.value;;

		var arrow=arrowArray[i];
		arrow.position.x=bb.position.x;
		arrow.position.y=bb.position.y+rr*world2D.sl01.value;
		arrow.position.z=bb.position.z;
		arrow.setArrow(bb.vx*3,bb.vy*3,bb.vz*3);
		arrow.visible=world2D.ch01.checked;

		if(world2D.sl03.value>50){
			world2D.sl03.setLabel("加溫");
			fire.visible=true;
			ice.visible=false;
		}else if(world2D.sl03.value<50){
			world2D.sl03.setLabel("降溫");
			fire.visible=false;
			ice.visible=true;
		}else if(world2D.sl03.value=50){
			world2D.sl03.setLabel("正常");
            fire.visible=false;
			ice.visible=false;
		}
	...
	for(var i=0;i<nn-1;i++){
		for(var j=i+1;j<nn;j++){
			...
			collision(ball1,ball2,2*rr*world2D.sl01.value);
		}
	}
...
function collision(ballA,ballB,dis){
	...
	if(rr<dis && (v1cx*r12x+v1cy*r12y+v1cz*r12z)<0){
		...
		ballA.vx=vcx+v1cx+2*world2D.sl02.value*(v1c_n12*n12x);
		ballA.vy=vcy+v1cy+2*world2D.sl02.value*(v1c_n12*n12y);
		ballA.vz=vcz+v1cz+2*world2D.sl02.value*(v1c_n12*n12z);
		ballB.vx=vcx+v2cx-2*world2D.sl02.value*(v2c_n12*n12x);
		ballB.vy=vcy+v2cy-2*world2D.sl02.value*(v2c_n12*n12y);
		ballB.vz=vcz+v2cz-2*world2D.sl02.value*(v2c_n12*n12z);
		ballA.mat.color.setHSL(hh,1,xx);
		ballB.mat.color.setHSL(hh,1,xx);
	}
