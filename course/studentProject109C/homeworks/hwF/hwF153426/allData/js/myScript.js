...
function init(){
	...
	ballA = new TEACHER.ObjSphere(rrA,0X00FFFF,2)
	scene.add(ballA)
	ballA.position.x=-15
	ballB = new TEACHER.ObjSphere(rrB,0X00FF00,2)
	scene.add(ballB)
	ballB.position.x=0
	springA = new TEACHER.ObjSpring(L0,1,0.2,8,0XFF0000)
	scene.add(springA)
	springA.position.x=0
	
	springB = new TEACHER.ObjSpring(LM,1,0.2,8,0XFF0000)
	scene.add(springB)
	springB.position.x=-15

	ground.position.y=-5

	ballB.position.x+=5

	wallA = new TEACHER.ObjBox(2,10,10,0X00FFFF,2)
	...
	scene.add(wallA)
	scene.add(wallB)
	graph1 = new TEACHER.ObjGraph(0,0,-30,30,0Xffffff,0Xffff00,1000)
	graph1.labelTitle.setText("A位置與時間");
	graph2 = new TEACHER.ObjGraph(0,0,-30,30,0Xffffff,0Xff0000,1000)
	graph2.labelTitle.setText("B位置與時間");
	graph1.position.x=-50
	graph2.position.x=50
	scene.add(graph1)
	scene.add(graph2)
	//graph1.labelTitleX.setText("時間(s)");
	//graph1.labelTitleY.setText("位置");

	ballA.vx = 0
	ballB.vx = 0

	world2D.sl01.setLabel('左彈簧的k')
	world2D.sl01.minimum = 0
	world2D.sl01.maximum = 10
	world2D.sl01.value = 2

	world2D.sl02.setLabel('右彈簧的k')
	world2D.sl02.minimum = 0
	world2D.sl02.maximum = 10
	world2D.sl02.value = 2

	world2D.sl03.setLabel('牆壁的位置')
	world2D.sl03.minimum = -40
	world2D.sl03.maximum = -20
	world2D.sl03.value = -30
...
function tick(){
	...
	kkM = world2D.sl02.value
	wallA.position.x = world2D.sl03.value
	Vc = (mmA*ballA.vx+mmB*ballB.vx)/(mmA+mmB)

	graph1.xMax=tt
	graph1.xMin=tt-100
	graph1.addPoint(tt,ballA.position.x);
	graph2.xMax=tt
	graph2.xMin=tt-100
	graph2.addPoint(tt,ballB.position.x);
	tt+=0.1
	
	ballA.ax = (-kk*(ballA.position.x-wallA.position.x-15)+kkM*(ballB.position.x-ballA.position.x-15))/mmA
	ballB.ax = (+kkM*(ballA.position.x-ballB.position.x+15))/mmB
	ballA.vx+=ballA.ax*dt
	ballB.vx+=ballB.ax*dt
	ballA.position.x+=ballA.vx*dt
	ballB.position.x+=ballB.vx*dt
	
	springA.setSpring((wallA.position.x-ballA.position.x+5),0,0)
	springB.setSpring(-(ballA.position.x-ballB.position.x+10),0,0)
	
	springA.rotation.z=0
	springB.rotation.z=0
	
	springA.position.x=ballA.position.x-5
	springB.position.x=ballA.position.x+5
	
	if(ballB.position.x-ballA.position.x<10 ){
		ballB.position.x=ballA.position.x+10
		ballA.vx = 2*Vc-ballB.vx
		ballB.vx = 2*Vc-ballA.vx
	}
	if(ballA.position.x-wallA.position.x<5 ){
		ballA.position.x=wallA.position.x+5
		ballA.vx=-ballA.vx
	}
