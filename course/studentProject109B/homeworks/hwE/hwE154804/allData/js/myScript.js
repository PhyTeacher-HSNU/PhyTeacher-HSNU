...
var people
var DWater
var sound=new Audio('allData/mp3/song.mp3')
//B.定義init
function init(){
	...
	var text=new TEACHER.ObjTextPlane(90,15,"注意音量!",'z',0xff0000)
	scene.add(text);
	text.position.z=-50;
	text.position.y=40;
	...
	//ball
	for(var i=0 ; i<nn ; i++){
		var b = new TEACHER.ObjSphere(1.8,0x0000ff)
		b.position.x=-50 + i*2
		b.position.y=10
		scene.add(b)
		ballArray1.push(b)
	...
	var horn1=new TEACHER.ObjBox(0.6 , 3 , 0.5,0xa23000)
	horn1.position.y=3
	horn1.position.x+=4
	horn1.position.z=2
	horn1.rotation.z=0.7
	head.add(horn1)
	
	var horn2=new TEACHER.ObjBox(0.6 , 3 , 0.5,0xa23000)
	horn2.position.y=3
	horn2.position.x+=4
	horn2.position.z=-2
	...
	var mouth=new TEACHER.ObjBox(2.5 , 1.5 , 4.5,0xff0000)
	mouth.position.x+=4.5
	mouth.position.y--
	head.add(mouth)

	var face=new TEACHER.ObjBox(5 , 3.6 , 5,0x0000ff)
	face.position.x+=3
	head.add(face)

	var eye1=new TEACHER.ObjSphere(0.8,0x111111)
	eye1.position.x+=5
	eye1.position.y+=1
	eye1.position.z+=1.2
	head.add(eye1)

	var eye2=new TEACHER.ObjSphere(0.8,0x111111)
	eye2.position.x+=5
	eye2.position.y+=1
	eye2.position.z-=1.2
	head.add(eye2)

	DWater=new TEACHER.ObjBox(0.4 , 2 , 1,0x00ffff)
	DWater.position.x+=6.2
	DWater.position.y-=2
	DWater.position.z+=1
	head.add(DWater)
	...
	//people
	people=ballArray1[30]

	var body=new TEACHER.ObjBox(2 , 5 , 2,0x82d900)
	body.position.y+=4
	people.add(body)

	var eye3=new TEACHER.ObjSphere(0.5,0x111111)
	eye3.position.x+=1
	eye3.position.y+=6.2
	eye3.position.z+=0.9
	people.add(eye3)

	var eye4=new TEACHER.ObjSphere(0.5,0x111111)
	eye4.position.x+=1
	eye4.position.y+=6.2
	eye4.position.z-=0.9
	people.add(eye4)

	var pHead=new TEACHER.ObjSphere(1.5,0xffbd9d)
	pHead.position.y+=6
	people.add(pHead)

	//sky
	var sky=new TEACHER.ObjBox(2000,1,200,0x006000)
	scene.add(sky)
	...
	world2D.btn02.setLabel("play/stop")
	world2D.btn02.on("click",clickBtn)

	//checkbox
	world2D.ch01.setLabel("正面")
	world2D.ch02.setLabel("水龍頭")
...
function tick(){
	...
	//camera
	world3D.cameraTarget.x = ballArray1[25].position.x
	...
	if(WD>1){
		sound.play()
		DWater.visible=true
		DWater.scale.y=0.05*WD
		DWater.position.y=-(1+0.05*WD)
	}else{
		sound.pause()
		sound.currentTime=0
		DWater.visible=false
	}
