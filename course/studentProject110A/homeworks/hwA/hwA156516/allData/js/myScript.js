...
function init(){
	...
	pp = new TEACHER.ObjTextPlane(100 , 10 , "1565   16   洪宜蓁", 'z', 0xffff00);
	scene.add(pp);
	...
	bb = new TEACHER.ObjPicPlane(10,10,pics.football,"z",2);
	scene.add(bb);
	bb.position.y = 3;
	bb.position.x = -37;
	...
	aa = new TEACHER.ObjPicPlane(20,40,pics.girl,"z",2);
	scene.add(aa);
	aa.position.x = -40;
	aa.position.y = 20;

	cc = new TEACHER.ObjPicPlane(20,40,pics.boy,"z",2);
	scene.add(cc);
	cc.position.x = 40;
	cc.position.y = 20;

	world2D.btn01.setLabel('左右滾動');
	world2D.btn02.setLabel('原地轉動');
...
function tick(){
	...
	if(isPlay){
		if(gameMD === 1){
			if(bb.position.x>48 && dx > 0){
				dx = -0.5;
			}else if(bb.position.x<-37 && dx<0){
				dx = 0.5;
			}
			bb.position.x += dx;
			bb.rotation.z += dx/rr; 
			...
			bb.rotation.z = 0;
			//smile.position.x = 0;
			bb.position.x = world2D.sl01.value;
			bb.rotation.z = world2D.sl02.value*Math.PI/180;
			bb.scale.x = bb.scale.y = bb.scale.z = world2D.sl03.value;

		}
	}

	isPlay = world2D.ch02.checked;
	bb.visible = world2D.ch01.checked;
