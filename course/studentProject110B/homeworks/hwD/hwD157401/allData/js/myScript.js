...
var rm,rM;
var rx,distP,distM;
var Vm=0,VM=0,sumV,VMVm;
var tri,thetam,thetaM;
var L=20;
var to;
var dt=0.03,tt=0,t=0;
var pause=0,str;
var obs,o;
...
function init(){
	...
	earth=new TEACHER.ObjSphere(1,0x0066FF);
	moon=new TEACHER.ObjSphere(1,0xC0C0C0);
	obs=new TEACHER.ObjCylinder(1.5,3,0xffff00);
	o=new TEACHER.ObjCylinder(1,2,0xffff00);
	scene.add(earth).add(moon).add(obs).add(o);
	

	earth.vz=0;
	moon.vz=0;

	world2D.sl01.setLabel("M質量");
	world2D.sl01.minimum=1000;
	world2D.sl01.maximum=5000;
	world2D.sl01.value=1000;
	world2D.sl02.setLabel("m質量");
	world2D.sl02.minimum=1000;
	world2D.sl02.maximum=5000;
	world2D.sl02.value=1000;
	world2D.sl03.setLabel("兩圓距離");
	world2D.sl03.minimum=5;
	world2D.sl03.maximum=100;
	world2D.sl03.value=40;
	world2D.btn01.setLabel("開始/暫停/重新");
	world2D.btn01.on("click",clickbtn);
	world2D.ch01.setLabel("質點隱藏");
	world2D.ch02.setLabel("157401方世燊");
	world2D.ch03.setLabel("觀察者隱藏");
...
function tick(){
	...
	Vm=Math.PI*rm*rm;
	VM=Math.PI*rM*rM;
	o.visible=!world2D.ch01.checked;
	obs.visible=!world2D.ch03.checked;
	if (pause===1){
		dist=Math.sqrt(Math.pow(earth.position.x-moon.position.x,2)+Math.pow(earth.position.z-moon.position.z,2)+Math.pow(earth.position.y-moon.position.y,2));

		earth.ax=-G*m*(earth.position.x-moon.position.x)/Math.pow(dist,3);
		earth.az=-G*m*(earth.position.z-moon.position.z)/Math.pow(dist,3);

		moon.ax=-G*M*(moon.position.x-earth.position.x)/Math.pow(dist,3);
		moon.az=-G*M*(moon.position.z-earth.position.z)/Math.pow(dist,3);
		...
		moon.position.x+=moon.vx*dt;
		moon.position.z+=moon.vz*dt;

		earth.position.x+=earth.vx*dt;
		earth.position.z+=earth.vz*dt;

		rx=Math.abs(earth.position.x-moon.position.x);
		distP=Math.abs(M/1000+m/1000);
		distM=Math.abs(M/1000-m/1000);
		
		if (rx>distP){
			sumV=Vm+VM;
		}else if (rx<distP&&rx>distM){
			tri=Math.sqrt(((rm+rM+rx)/2)*(((rm+rM+rx)/2)-rm)*(((rm+rM+rx)/2)-rM)*(((rm+rM+rx)/2)-rx));
			thetaM=Math.asin((tri*2)/(rM*rx));
			thetam=Math.asin((tri*2)/(rm*rx));
			VMVm=(rM*rM*thetaM/2+rm*rm*thetam/2-tri)*2;
			sumV=Vm+VM-VMVm;
		}else if (rx<=distM&&moon.position.z>earth.position.z){
			if (Vm<VM)
				sumV=VM-Vm;
			else if (Vm>VM)
				sumV=Vm;
	...
	// if (pause===1){
	// 	M=world2D.sl01.value;
	// 	m=world2D.sl02.value;
	// 	L=world2D.sl03.value;
	// }
	//3D
	world3D.render();
	//2D
...
function clickbtn(e){
	...
	if (str==="btn01"){
		if (pause===0){
			...
			earth.position.x=m/(M+m)*L;
			moon.position.x=-M/(M+m)*L;

			earth.position.z=0;
			moon.position.z=0;

			earth.vx=0;
			earth.vz=Math.sqrt((G*m*m)/(L*(M+m)));
			earth.vy=0;

			moon.vx=0;
			moon.vz=-Math.sqrt((G*M*M)/(L*(M+m)));
			moon.vy=0;

			to=new TEACHER.ObjGraph(0,10,0,100);
			scene.add(to);
			to.labelTitle.setText("L-t");
			to.labelX.setText("t");
			to.labelY.setText("L");
			to.position.z=-50;

			earth.scale.x=earth.scale.y=earth.scale.z=M/1000;
			moon.scale.x=moon.scale.y=moon.scale.z=m/1000;
		}else if (pause===1){
			pause=2;
			sumV=0;
			earth.vx=earth.vz=0;
