...
//B.定義init
function init() {
	a1 = new TEACHER.ObjCylinder(20, 3, 0x777777, false)
	scene.add(a1)
	a1.position.y = 1.5

	a2 = new TEACHER.ObjCylinder(20, 2, 0x777777, true)
	scene.add(a2)
	a2.position.y = 4

	a3 = new TEACHER.ObjCylinder(18, 2, 0x777777, true, "y", 2)
	scene.add(a3)
	a3.position.y = 4

	var geometry = new THREE.RingGeometry(18, 20, 32);
	...
	var geometry = new THREE.CylinderGeometry(15, 15, 11, 32, 1, true);
	var material = new THREE.MeshBasicMaterial({ color: 0x555555 });
	b1 = new THREE.Mesh(geometry, material);
	scene.add(b1);
	b1.position.y = 8

	var geometry = new THREE.CylinderGeometry(2, 2, 11, 32, 1, true);
	var material = new TEACHER.MSMat(0x555555, 1);
	b2 = new THREE.Mesh(geometry, material);
	scene.add(b2);
	b2.position.y = 8

	var geometry = new THREE.RingGeometry(2, 15, 32);
	...
	var geometry = new THREE.CylinderGeometry(15, 15, 2, 32, 1, true);
	var material = new THREE.MeshBasicMaterial({ color: 0x555555 });
	b4 = new THREE.Mesh(geometry, material);
	scene.add(b4);
	b4.position.y = 14.5

	var geometry = new THREE.CylinderGeometry(13.5, 13.5, 2, 32, 1, true);
	var material = new TEACHER.MSMat(0x555555, 1);
	b5 = new THREE.Mesh(geometry, material);
	scene.add(b5);
	b5.position.y = 14.5

	var geometry = new THREE.RingGeometry(13.5, 15, 32);
	var material = new THREE.MeshBasicMaterial({ color: 0x555555, side: THREE.DoubleSide });
	...
	c1 = new TEACHER.ObjBox(14, 3, 3, 0x2A1B1A)
	scene.add(c1)
	c1.position.y = 10.5
	c1.position.x = Math.cos(theta) * c1r
	c1.position.z = Math.sin(theta) * c1r

	c2 = new TEACHER.ObjBox(10, 3, 3, 0x2A1B1A)
	scene.add(c2)
	c2.rotation.z = Math.PI / 2
	c2.position.y = 14
	c2.position.x = Math.cos(theta) * c2r
	c2.position.z = Math.sin(theta) * c2r

	var geometry = new THREE.CylinderGeometry(18, 15, 0.1, 32, 1, true);
	var material = new TEACHER.MSMat(0xeee8aa, 1);
	d1 = new THREE.Mesh(geometry, material);
	scene.add(d1);
	d1.position.y = 2.9

	for (var i = 0; i < n; i++) {
		var ball = new TEACHER.ObjSphere(1, 0xeee8aa)
		ball.position.y = 30
		ball.position.x = (Math.random() - 0.5) * 18
		ball.position.z = (Math.random() - 0.5) * 18
		ball.visible = false
		ballArray.push(ball)
		scene.add(ball)
		Pm = 0
	...
	world2D.btn01.setLabel("倒黃豆")
	world2D.btn02.setLabel("磨/暫停")
	world2D.btn01.on("click", pour)
	world2D.btn02.on("click", yesno)
	world2D.sl01.setLabel("黃豆量")
	world2D.sl02.setLabel("豆漿量")
	...
	nm=new TEACHER.ObjTextPlane(100,10,"1595班17號 吳宗餘")
	scene.add(nm)
	nm.position.z=-50
	nm.position.x=0
	nm.position.y=25
...
//C.定義tick                                                                                                                       
function tick() {
	world2D.sl01.value=b
	world2D.sl02.value=m
	if(m>0){
		m -= 0.5
	}
	d1.position.y = 3 + (m - 10) / 50
	if (Gm === 1) {
		theta += omega
		b1.rotation.y -= omega
		b2.rotation.y -= omega
	...
	if (Pm === 1) {
		for (var i = 0; i < n; i++) {
			...
			if (ball.position.y <= 14.6 && ball.position.x ** 2 + ball.position.z ** 2 > 4) {
				ball.position.x += -ball.position.x / 50
				ball.position.z += -ball.position.z / 50
			}
			if (ball.position.y <= 14.6 && ball.position.x ** 2 + ball.position.z ** 2 <= 4.1 && ball.position.y > 6) {
				v += a
				ball.position.y -= v
			}
		}
