...
//B.定義init
function init(){
	carcar = new TEACHER.ObjPicCylinder(5, 30, pics.car, false, "x", 0);
	scene.add(carcar);
	carcar.position.y = 7;
	oo1 = new TEACHER.ObjPicSphere(rr, pics.wheel1, 0);
	scene.add(oo1);
	oo1.position.x = 10;
	oo1.position.y = rr ;
	oo2 = new TEACHER.ObjPicSphere(rr, pics.wheel2, 0);
	scene.add(oo2);
	oo2.position.x = -10;
	oo2.position.y = rr;
	...
	world2D.btn01.setLabel('順轉');
	world2D.btn02.setLabel('逆轉');
...
//C.定義tick                                                                                                                       
function tick(){
	carcar.position.x += dx;
	oo1.position.x += dx;
	oo1.rotation.z -= dx / rr;
	oo2.position.x += dx;
	oo2.rotation.z -= dx / rr;
	if(oo1.position.x > 50 && dx > 0){
		dx *= -1;
	}else if(oo2.position.x < -50 && dx < 0){
		dx *= -1;
	}
...
function clickBtn(e){
	...
	if (str === 'btnUp'){
		carcar.position.y += 0.5;
	}else if (str === 'btnDown'){
		carcar.position.y -= 0.5;
	}else if (str === 'btn01'){
		carcar.rotation.z += 0.1;
	}else if (str === 'btn02'){
		carcar.rotation.z -=0.1;
	}
