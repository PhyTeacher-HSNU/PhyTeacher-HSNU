...
function tick(){
	...
	if(ball.position.x>50-world2D.sl02.value*2 && ball.vx>0){
		ball.vx*=-1*ratio;
	}else if(ball.position.x<-50+world2D.sl02.value*2 && ball.vx<0){
		ball.vx*=-1*ratio;
	}
	if(ball.position.z>50-world2D.sl02.value*2 && ball.vz>0){
		ball.vz*=-1*ratio;
	}else if(ball.position.z<-50+world2D.sl02.value*2 && ball.vz<0){
		ball.vz*=-1*ratio;
	}
	...
	arrowA.setArrow(ball.ax*4 , ball.ay*4 , ball.az*4);

	var hh=Math.random();;
	ball.mat.color.setHSL(hh,1,0.5)

	if( ball.position.z<10 && ball.position.z>-10 &&ball.position.x<8 && ball.position.x>0){
		smile.rotation.y+=0.5;
		
	}else if( ball.position.z<10 && ball.position.z>-10 && ball.position.x>-8 && ball.position.x<0){
		smile.rotation.y-=0.5;
