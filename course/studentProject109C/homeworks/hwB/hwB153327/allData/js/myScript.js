...
function init(){
	...
	ball= new TEACHER.ObjPicSphere(2,pics.ball,2);
	scene.add(ball);
	ball.position.x=0;	
	ball.ax=0;
	ball.az=0;
	...
	world2D.sl03.maximum=8;
	world2D.sl03.minimum=0;
	world2D.sl03.value=0;
	world2D.btn01.setLabel('暫時停止');
...
//C.定義tick                                                                                                                       
function tick(){
	ball.position.y=world2D.sl01.value*2;
	ball.scale.x=world2D.sl01.value;
	ball.scale.y=world2D.sl01.value;
	ball.scale.z=world2D.sl01.value;
	ball.vx+=ball.ax*dt*world2D.sl02.value/10*(1-world2D.sl03.value);
	ball.vz+=ball.az*dt*world2D.sl02.value/10*(1-world2D.sl03.value);
	ball.position.x+=ball.vx;
	ball.position.z+=ball.vz;
	ball.rotation.x=ball.vx*3.14;
	ball.rotation.z=ball.vz*3.14;
	if(ball.position.x>50-world2D.sl01.value*2 || ball.position.x<-50+world2D.sl01.value*2){
		ball.vx*=-1;
	}
	if(ball.position.z>50-world2D.sl01.value*2 || ball.position.z<-50+world2D.sl01.value*2){
		ball.vz*=-1;
	}
