...
var scoredis=[]
var grounda,wallx=[]
var scoretext,sc,rule,rule2,rule3
var haha=world2D.btn02
//B.定義init
function init(){
	//前置作業
	...
	for(var i=0;i<rand;i++){
		var jj=Math.floor(Math.random()*10)
		scorearray[jj]=4
	}
	//球
	ball=new TEACHER.ObjSphere(2,0x191970,2)
	ball.position.x=0
	ball.position.y=2
	ball.position.z=-48
	ball.vy=0
	scene.add(ball)
	//地板+側牆
	grounda=new TEACHER.ObjPlane(102.5,100,0x9955FF,"y",0)
	scene.add(grounda)
	grounda.position.x=grounda.position.y=grounda.position.z=0
    ground.visible=false
	for(var i=0;i<4;i++){
		var aa=Math.floor(i/2),bb=i%2
		if(aa==0){
			if(bb==0){
				wallx[i]=new TEACHER.ObjPlane(100,10,0x9955FF,"x",2)
				wallx[i].position.y=5
				wallx[i].position.z=0
				wallx[i].position.x=51.25
			}else{
				wallx[i]=new TEACHER.ObjPlane(100,10,0x9955FF,"x",2)
				wallx[i].position.y=5
				wallx[i].position.z=0
			...
			if(bb==0){
				wallx[i]=new TEACHER.ObjPlane(102.5,10,0x9955FF,"z",2)
				wallx[i].position.y=5
				wallx[i].position.z=50
				wallx[i].position.x=0
			}else{
				wallx[i]=new TEACHER.ObjPlane(102.5,10,0x9955FF,"z",2)
				wallx[i].position.y=5
				wallx[i].position.z=-50
	...
	//柱子
	for(var i=0;i<15;i++){
		ha[i]=new TEACHER.ObjCylinder(1,5,0xffff00,false,"y")
		ha[i].position.x=-40+(i%5)*20
		ha[i].position.y=2
		ha[i].position.z=Math.floor(i/5)*20-30
		ha[i].vx=ha[i].vz=0
		ha[i].vy=0
		scene.add(ha[i])
	}
	for(var i=0;i<18;i++){
		var ii=i+15
		ha[ii]=new TEACHER.ObjCylinder(1,5,0xffff00,false,"y")
		ha[ii].position.y=2
		ha[ii].position.x=-50+(i%6)*20
		ha[ii].position.z=Math.floor(i/6)*20-20
		ha[ii].vx=ha[ii].vz=0
		ha[ii].vy=0
		scene.add(ha[ii])		
	}
	...
	var xx=-40
    for(var i=0;i<9;i++){
		wall[i]=new TEACHER.ObjPlane(20,10,0xB94FFF,"x",2)
		wall[i].position.x=xx
		xx+=10
	...
	ball.add(arrowv)
	//視角
	world2D.slCameraRR.value=135
	world3D.cameraTheta=Math.PI/4
	//力量
	world2D.sl01.setLabel("力量大小")
	world2D.sl01.maximum=10
	world2D.sl01.minimum=1
	world2D.sl01.digitN=1
	world2D.sl01.value=4
	//方向
	world2D.btnLeft.on("click",btn)
	world2D.btnRight.on("click",btn)
	//開始
	world2D.btn01.setLabel("施力")
	world2D.btn01.on("click",go)
	//重力
	world2D.sl02.setLabel("重力大小")
	world2D.sl02.maximum=10
	world2D.sl02.minimum=1
	world2D.sl02.digitN=1
	world2D.sl02.value=7
	//施力位置
	world2D.sl03.setLabel("起始位置")
	world2D.sl03.maximum=48
	world2D.sl03.minimum=-48
	world2D.sl03.value=0
	//顯示分數+規則
	scoretext=new TEACHER.ObjTextPlane(60,20,"分數:","y",0x33FFFF)
	scoretext.position.y=0
	scoretext.position.x=-80
	scoretext.position.z=-20
	rule=new TEACHER.ObjTextPlane(160,20,"左右控制發射方向","y",0x33FFFF)
	rule.position.x=0
	rule.position.y=0
	rule.position.z=-80
	rule2=new TEACHER.ObjTextPlane(160,20,"進洞得對應分數","y",0x33FFFF)
	rule2.position.x=0
	rule2.position.y=0
	rule2.position.z=-100
	sc=new TEACHER.ObjTextPlane(60,20,""+score,"y",0x33FFFF)
	sc.position.x=-80
	sc.position.y=0
	sc.position.z=0
	scene.add(scoretext,rule,rule2,sc)
	//各區分數
	var pp=-45
	for(var i=0;i<10;i++){
		scoredis[i]=new TEACHER.ObjTextPlane(10,10,scorearray[i],"y",0x000000)
		scoredis[i].position.x=pp
		pp+=10
		scoredis[i].position.z=35
        scoredis[i].position.y=1
		scene.add(scoredis[i])
	}
	//詛咒
	haha.setLabel("詛咒按鈕")
	world2D.ch02.setLabel("詛咒適用前會先扣2分")
	haha.on("click",cursed)
	rule3=new TEACHER.ObjTextPlane(160,20,"詛咒出好東西機率為50%","y",0x33FFFF)
	rule3.position.x=0
	rule3.position.y=0
	rule3.position.z=-120
	scene.add(rule3)
...
function tick(){
	//速度調整+運動
	force=world2D.sl01.value
	if(started==false){
		world2D.sl03.visible=true
		world2D.sl01.visible=true
		ball.vx=Math.cos(theta)*force;
	    ball.vz=Math.sin(theta)*force;
		for(var i=0;i<33;i++) {
			ha[i].vx=ball.vx
			ha[i].vz=ball.vz
		}
		ball.position.x=world2D.sl03.value
	}else{
		ball.position.x+=ball.vx*dt
		ball.position.z+=ball.vz*dt
		ball.vz+=world2D.sl02.value*0.01
		world2D.sl03.visible=false
		world2D.sl01.visible=false
	}
    arrowv.setArrow(ball.vx*2,0,ball.vz*2)
	//撞牆反彈
	if(ball.position.x>49.25 && ball.vx>0) ball.vx*=-1
	if(ball.position.x<-49.25 && ball.vx<0) ball.vx*=-1
	if(ball.position.z>49.25 && ball.vz>0) ball.vz*=-1
	if(ball.position.z<-49.25 && ball.vz<0) ball.vz*=-1
	//碰撞柱子
	for(var i=0;i<33;i++){
		ha[i].vx=-1*ball.vx
		ha[i].vy=-1*ball.vy
		ha[i].vz=-1*ball.vz
		collision(ball,ha[i],4)
	}
	//是否進洞
	if(ball.position.z>30){
		var place=Math.floor((ball.position.x+50)/10)
        score+=scorearray[place]
		ball.position.x=0
		world2D.sl03.value=0
		ball.position.z=-48
		theta=Math.random()*2
	...
	/*//顯示分數
	if(world2D.ch01.checked==true) world2D.ch01.setLabel("分數:"+score)
	else world2D.ch01.setLabel("顯示分數")*/
	//分數更新
	sc.setText(""+score)
...
function btn(e){
	var haa=e.target.parent.name
	if(haa=="btnLeft") theta+=0.1
	else theta-=0.1
}
...
function cursed(){
	if(score<2){
		world2D.ch02.setLabel("分數要2分才能使用")
		return
	}
	score-=2
    ...
    if(judge==7){
		score*=3
		world2D.ch02.setLabel("本次詛咒:分數三倍")
	}else if(judge==9){
		var aaa=Math.floor(Math.random()*10)
		scorearray[aaa]*=2
		scoredis[aaa].setText(scorearray[aaa])
		world2D.ch02.setLabel("本次詛咒:其中一格分數增加")
	}else if(judge==8){
		ball.visible=false
		theta=Math.random()*2
		world2D.ch02.setLabel("本次詛咒:球隱形一次")
	}else if(judge==6 || judge==5){
		window.open("https://www.youtube.com/channel/UCdBz8unJXpF9cS5T3z6zj0g")
		world2D.ch02.setLabel("本次詛咒:必須訂閱我的頻道")
	}else if(judge==0){
		score+=1569
		world2D.ch02.setLabel("本次詛咒:爽到升天")
	}
	else if(judge<=3 && judge!=0){
		score-=judge
		world2D.ch02.setLabel("本次詛咒:扣"+judge+"分")
	}else{
		var aaa=Math.floor(Math.random()*10)
		scorearray[aaa]/=2
		scorearray[aaa]=Math.round(scorearray[aaa])
		scoredis[aaa].setText(scorearray[aaa])
		world2D.ch02.setLabel("本次詛咒:其中一格分數減半")
	}
}
