...
//var btn01,btn02,btn03;

//B.定義init


function init(){
	introduction1=new TEACHER.ObjTextPlane(80,10,"1.記得開音效","z",0x000000);
	scene.add(introduction1);
	introduction1.position.z=-50;
	introduction1.position.y=50;
	introduction1.position.x=0;

	introduction2=new TEACHER.ObjTextPlane(80,10,"2.勾選換成焦糖口味","z",0x000000);
	scene.add(introduction2);
	introduction2.position.z=-50;
	introduction2.position.y=40;
	introduction2.position.x=0;
	
	introduction3=new TEACHER.ObjTextPlane(80,10,"3.大包和小包可選","z",0x000000);
	scene.add(introduction3);
	introduction3.position.z=-50;
	introduction3.position.y=30;
	introduction3.position.x=0;
	...
	world2D.ch01.setLabel('焦糖');
	world2D.btn01.setLabel('大包爆米花');
	world2D.btn02.setLabel('小包爆米花');
	//world2D.btn03.setLabel('小包');
	...
	world2D.ch02.visible=world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=world2D.sl01.visible=world2D.sl02.visible=world2D.sl03.visible=false;

	sound1=new Audio('allData/wav/bo.wav');
	sound2=new Audio('allData/wav/popcorn.wav');
	sound2.loop = true;
...
function clickBtn(e){
	...
	if (NN>0){
		for (var j=0;j<NN;j++){
			ballArray[j].visible=false;
			objectArray[j].visible=false;
		}
	}
	if(str==='btn01'){	
	...
	if (NN>0){
		ballArray.splice(0, NN); 
		objectArray.splice(0, NN); 	
		preNN = NN;
	}
	NN=popCornSize;
	generatePopcorn(popCornSize);
}

function generatePopcorn(NN){
	for(var i=0; i<NN;i++){
		world2D.sl01.setLabel("半徑");
		world2D.sl01.maximum=6;
		world2D.sl01.minimum=1;
		//rr=world2D.sl01.value;
		ball=new TEACHER.ObjSphere(rr,0xffff00);
		ballArray.push(ball);
		scene.add(ball);
		ball.position.x=0;
		ball.position.x=0;
		...
		ball.vz=10*(Math.random()-0.5);
		ballArray[i].visible=true;
	...
	for(var i=0; i<NN;i++){
		...
		object.rotation.x=3*Math.random();
		object.rotation.y=2*Math.random();
		object.rotation.z=Math.random();

		object.vx=10*(Math.random()-0.5);
		object.vy=10*(Math.random());
		console.log(object.vy);
		object.vz=10*(Math.random()-0.5);
...
function tick(){
	for(var i=0;i<NN;i++){
		...
		if(ball.position.x>40-rr && ball.vx>0){
			ball.vx*=-0.8;
			ball.position.x+=ball.vx*dt;
		}else if(ball.position.x<-40+rr && ball.vx<0){
			ball.vx*=-0.8;
			ball.position.x+=ball.vx*dt;
		}
		...
		if(ball.position.y>40-rr && ball.vy>0){
			...
			ball.vy*=-0.8;
			ball.position.y+=ball.vy*dt;
		}else if(ball.position.y<rr && ball.vy<0){
			ball.vy*=-0.8;
			//ball.vy*=-1;
			if (ball.position.y<0)
				ball.position.y = rr*8;
			ball.position.y+=ball.vy*dt;
			//console.log(ball.position.y);
		}

		if(ball.position.z>40-rr && ball.vz>0){
			ball.vz*=-0.8;
			ball.position.z+=ball.vz*dt;
		}else if(ball.position.z<-40+rr && ball.vz<0){
			ball.vz*=-0.8;
			ball.position.z+=ball.vz*dt;
		}
	...
	for(var i=0;i<NN;i++){
		...
		//var object=objectArray.pop();
		object.vx+=object.ax*dt;
		object.vy+=object.ay*dt;
		object.vz+=object.az*dt;

		object.position.x+=object.vx*dt;
		object.position.y+=object.vy*dt;
		object.position.z+=object.vz*dt;
		
		//object.material.color=colorValue;



		if(object.position.x>40-rr && object.vx>0){
			object.vx*=-0.8;
			object.position.x+=object.vx*dt;
		}else if(object.position.x<-40+rr && object.vx<0){
			object.vx*=-0.8;
			object.position.x+=object.vx*dt;
		}
		if(object.position.y>40-rr && object.vy>0){
			object.vy*=-0.8;
			object.position.y+=object.vy*dt;
		}else if(object.position.y<rr && object.vy<0){
			object.vy*=-0.8;
			if (object.position.y<0)
				object.position.y = rr*8;
			object.position.y+=object.vy*dt;
		}
		if(object.position.z>40-rr && object.vz>0){
			object.vz*=-0.8;
			object.position.z+=object.vz*dt;
		}else if(object.position.z<-40+rr && object.vz<0){
			object.vz*=-0.8;
			object.position.z+=object.vz*dt;
		}
