...
function init(){
        ...
        ground.visible = skyBox.visible = world2D.btn01.visible =world2D.btn02.visible =world2D.ch02.visible =world2D.ch01.visible =false;

        world2D.sl01.setLabel("阻力")
  world2D.sl01.minimum = 0
  world2D.sl01.maximum = 2
  world2D.sl01.value = 0

  world2D.sl02.setLabel("1534")
  world2D.sl02.minimum = 0
  world2D.sl02.maximum = 0
  world2D.sl02.value = 0
  world2D.sl03.setLabel("呂東晉")
  world2D.sl03.minimum = 0
  world2D.sl03.maximum = 0
  world2D.sl03.value = 0

        stars1 = new TEACHER.Points(0xff0000 , 1);
	    scene.add(stars1);
        for(var i=0 ; i<100 ; i++){
 ...
 stars2 = new TEACHER.Points(0x000000 , 1.5);
	    scene.add(stars2);
        for(var i=0 ; i<100 ; i++){
            var xx=100*(Math.random()-0.5); 
            var yy=100*(Math.random()-0.5); 
 ...
 stars3 = new TEACHER.Points(0xFFFF00 , 1.5);
	    scene.add(stars3);
        for(var i=0 ; i<100 ; i++){
            var xx=100*(Math.random()-0.5); 
            var yy=100*(Math.random()-0.5); 
    ...
    ball1=new TEACHER.ObjSphere(5.1,0xff0000)
	scene.add(ball1);
 ball2=new TEACHER.ObjSphere(5,0xffff00)
	scene.add(ball2);
	...
	arrowExtF = new TEACHER.ObjArrow(2,0x00ffff);
	arrowDragF = new TEACHER.ObjArrow(2,0x00ff00);
	arrowV = new TEACHER.ObjArrow(2,0xffff00);
	arrowV.position.x-=20;
	scene.add(arrowExtF);
	scene.add(arrowDragF);
	scene.add(arrowV);
...
function tick(){
 ...
 stars3.position.x=-xx;
	...
	if(zz>50){
        ...
        if(vx>20 || vy>20|| vz>20){
		ball1.visible = false
	}else if(vx<-20 || vy<-20 || vz<-20){
		ball1.visible = false
	}else{
                 ball1.visible =true
}
		...
		arrowExtF.setArrow(extFx*10 , extFy*10 , extFz*10);
	arrowDragF.setArrow(dragFx*10 , dragFy*10 , dragFz*10);
	arrowV.setArrow(vx , vy , vz);

beta = world2D.sl01.value
