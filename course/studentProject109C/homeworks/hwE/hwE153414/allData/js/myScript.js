...
function init(){
	...
	world2D.ch01.visible=world2D.btn02.visible=world2D.ch02.visible=world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=false


    ball=new TEACHER.ObjSphere(5,0xff0000,2);

	arrowFb=new TEACHER.ObjArrow(1,0x00ff00);
	arrowv=new TEACHER.ObjArrow(1,0x00ffff);
	arrowhit=new TEACHER.ObjArrow(1,0xff0000);


	arrowv.position.x=-20;
	arrowv.position.z=-20;

	arrowhit.setArrow(10,0,0);
	arrowhit.position.x=5;

    scene.add(arrowFb).add(arrowv).add(arrowhit);

    stars=new TEACHER.Points(0x00FFff,1);
	scene.add(stars,ball);
	for(var i=0;i<=150;i++){
		var xx=120*(Math.random()-0.5);
		var yy=120*(Math.random()-0.5);
		var zz=120*(Math.random()-0.5);
		for(var xi=-1;xi<=1;xi++){
			for(var yi=0;yi<=1;yi++){
				for(var zi=-1;zi<=1;zi++){
					stars.addPoint(xx+xi*120,yy+yi*120,zz+zi*120);
				}
			}
		}
	}
	world2D.sl01.setLabel("左右角度");
	world2D.sl01.value=0;
	world2D.sl01.maximum=30;
	world2D.sl01.minimum=-30;

	world2D.sl02.setLabel("上下角度");
	world2D.sl02.value=15;
	world2D.sl02.maximum=30;
	world2D.sl02.minimum=0;

	world2D.sl03.setLabel("力量大小");
	world2D.sl03.value=10;
	world2D.sl03.maximum=15;
	world2D.sl03.minimum=0;

	world2D.btn01.setLabel("暫停打擊")
...
function tick(){
    ...
    if(hit){
		arrowFb.visible=arrowv.visible=true;
		arrowhit.visible=false;
		...
		stars.position.x=arrowhit.position.x=-xx;
		stars.position.y=-yy;
		stars.position.z=-zz;


		arrowFb.setArrow(Fbx*8,Fby*8,Fbz*8);
		...
		arrowFb.visible=arrowv.visible=false;
		arrowhit.visible=true;
		arrowhit.rotation.z=world2D.sl02.value/180*Math.PI/2;
		arrowhit.rotation.y=-world2D.sl01.value/180*Math.PI/2;
		Fx=world2D.sl03.value*Math.cos(world2D.sl02.value/180*Math.PI)*Math.cos(world2D.sl01.value/180*Math.PI);
		Fy=world2D.sl03.value*Math.sin(world2D.sl02.value/180*Math.PI);
		Fz=world2D.sl03.value*Math.cos(world2D.sl02.value/180*Math.PI)*Math.sin(world2D.sl01.value/180*Math.PI);
		arrowhit.setArrow(Fx,Fy,Fz);
