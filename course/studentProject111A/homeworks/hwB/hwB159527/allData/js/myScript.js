...
//B.定義init
function init(){
	text=new TEACHER.ObjTextPlane(70,10,'159527黃若洋','z',0xffff00)
	scene.add(text);
	text.position.y=20
	text.position.z=-53
	win=new TEACHER.ObjTextPlane(70,10,'You Win!','z',0xff0000)
	scene.add(win)
	
	win.position.y=5
	win.visible=false;
	loss=new TEACHER.ObjTextPlane(70,10,'You Lose!','z',0xff0000)
	scene.add(loss)
	loss.position.y=5
	loss.visible=false;
	win.position.y=5
	win.visible=false;
	wallL=new TEACHER.ObjBox(2,5,60,0xffffff)
    scene.add(wallL)
	wallL.position.x=-40
	wallL.position.y=2.5
	wallL.position.z=0
	
	wallR=new TEACHER.ObjBox(2,5,60,0xffffff)
    scene.add(wallR)
	wallR.position.x=40
	wallR.position.y=2.5
	wallR.position.z=0
	
	wallB=new TEACHER.ObjBox(80,2,2,0xff0000)
    scene.add(wallB)
	wallB.position.x=0
	wallB.position.y=1
	wallB.position.z=30
	...
	wallfr=new TEACHER.ObjBox(35,5,2,0x00ff00)
    scene.add(wallfr)
	wallfr.position.x=22.5
	wallfr.position.y=2.5
	wallfr.position.z=-30
	
	wallfl=new TEACHER.ObjBox(35,5,2,0x00ff00)
    scene.add(wallfl)
	wallfl.position.x=-22.5
	wallfl.position.y=2.5
	wallfl.position.z=-30
    ...
    ball.position.x=0
    ball.position.y=2
	board=new TEACHER.ObjBox(10,5,2,0xffff00)
	scene.add(board);
...
//board's position
	...
	board.position.z=20;
	board.position.x=vecM.x
	if(board.position.x>35){
		board.position.x=35
	}
	else if(board.position.x<-35){
		board.position.x=-35
	}
//ball rotation
	ball.vx+=ball.ax*dt;
    ...
    if((ball.position.x<wallL.position.x&&ball.vx<0)||(ball.position.x>wallR.position.x&&ball.vx>0)){
		ball.vx*=-1
	}
	else if((ball.position.z<wallfr.position.z&&ball.vz<0)||(ball.position.z<wallfl.position.z&&ball.vz<0)){
		ball.vz*=-1
	...
	else if(((ball.position.x<board.position.x+5)&&(ball.position.x>board.position.x-5))&&(ball.position.z>board.position.z&&ball.vz>0)){
		ball.vz*=-1
	}
	else if(ball.position.z>wallB.position.z&&ball.vz>0){
		ball.vz*=-1
		if(ball.position.z=wallB.position.z){
			loss.visible=true;
			dt*=0
		}
	}
...
function clickBtn(e){
	...
	if(str==='btn02'){
		...
		win.visible=false;
		loss.visible=false;
        ball.vx=4*(Math.random()-5)
	    ball.vy=0
		ball.vz=5*(Math.random()-4)
	}
