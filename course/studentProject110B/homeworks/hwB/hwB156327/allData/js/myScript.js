...
var b,be,s,se
var nn
var g11,g12,g21,g22,g31,g32,g41,g42,g51,g52
var ppw,pp=0
var title
var v
//B.定義init
function init(){
	...
	nn = new TEACHER.ObjTextPlane(150,20,"156327     游奕萱","z",0x444444)
	scene.add(nn)
	nn.position.z=-175
	nn.position.y=60

	
	title=new TEACHER.ObjTextPlane(200,15," 請想辦法將大小調至2.8 牆面及大小牌子皆可利用 ","z",0x550088)
	scene.add(title)
	title.position.z=-200
	title.position.y=80

	v=new TEACHER.ObjTextPlane(300,100,"Victory","z",0xE8CCFF,0x444444)
	v.position.z=70

	b = new TEACHER.ObjTextPlane(80,10,"bigger(+0.4)","z",0x4B0082)
	scene.add(b)
	b.position.x=130
	b.position.y=50

	s = new TEACHER.ObjTextPlane(80,10,"smaller(-0.4)","z",0x4B0082)
	scene.add(s)
	s.position.x=-130
	s.position.y=50

	be = new TEACHER.ObjTextPlane(80,10,"biggest(4)","z",0x191970)
	scene.add(be)
	be.position.z=150
	be.position.y=20

	se = new TEACHER.ObjTextPlane(80,10,"smallest(0.4)","z",0x191970)
	scene.add(se)
	se.position.z=-150
	se.position.y=50

	bb = new TEACHER.ObjPicPlane(20,20,pics.hajime,"z",2)
	scene.add(bb)
	bb.position.y=6
	bb.scale.x=bb.scale.y=bb.scale.z=1
	
	bg=new TEACHER.ObjBox(10000,10000,10000,0xE6E6FA,1)
	scene.add(bg)

	w1=new TEACHER.ObjBox(2,10,350,0x8C0044,0)
	scene.add(w1)
	w1.position.x=-175
	w1.position.y=2.5

	w2=new TEACHER.ObjBox(2,10,350,0x8C0044,0)
	scene.add(w2)
	w2.position.x=175
	w2.position.y=2.5

	w3=new TEACHER.ObjBox(350,5,2,0x8C0044,0)
	scene.add(w3)
	w3.position.z=175

	w4=new TEACHER.ObjBox(350,10,2,0x8C0044,0)
	scene.add(w4)
	w4.position.z=-175
	w4.position.y=2.5

	g11=new TEACHER.ObjBox(20,8,2,0xffffff,2)
	scene.add(g11)
	g11.position.x=-50
	g11.position.z=30
	g11.position.y=30

	g12=new TEACHER.ObjTextPlane(20,8,"2","z",0xB088FF)
	scene.add(g12)
	g12.position.x=-50
	g12.position.z=32
	g12.position.y=30

	g21=new TEACHER.ObjTextPlane(20,8,"3.6","z",0x8C0044)
	scene.add(g21)
	g21.position.x=70
	g21.position.z=-48
	g21.position.y=30

	g22=new TEACHER.ObjBox(20,8,2,0xffffff,2)
	scene.add(g22)
	g22.position.x=70
	g22.position.z=-50
	g22.position.y=30

	g31=new TEACHER.ObjTextPlane(20,8,"1.2","z",0x007799)
	scene.add(g31)
	g31.position.x=-90
	g31.position.z=-68
	g31.position.y=30

	g32=new TEACHER.ObjBox(20,8,2,0xffffff,2)
	scene.add(g32)
	g32.position.x=-90
	g32.position.z=-70
	g32.position.y=30

	g41=new TEACHER.ObjTextPlane(20,8,"0.8","z",0x008844)
	scene.add(g41)
	g41.position.z=-88
	g41.position.y=30

	g42=new TEACHER.ObjBox(20,8,2,0xffffff,2)
	scene.add(g42)
	g42.position.z=-90
	g42.position.y=30

	g51=new TEACHER.ObjTextPlane(20,8,"2.4","z",0x009FCC)
	scene.add(g51)
	g51.position.x=60
	g51.position.z=90
	g51.position.y=30

	g52=new TEACHER.ObjBox(20,8,2,0xffffff,2)
	scene.add(g52)
	g52.position.x=60
	g52.position.z=88
	g52.position.y=30
	...
	world2D.sl02.setLabel("高度")
	world2D.sl02.minimum=0
	world2D.sl02.maximum=80
	world2D.sl02.value=0

	world2D.ch01.setLabel("牆面提示")

	
	
	
	world2D.btn01.setLabel("開始移動")
	world2D.btn02.setLabel("停止原寸")

	world2D.btn01.on("click",clicks)
	world2D.btn02.on("click",clicks)
...
function tick(){
	...
	if(gamemd==1){
		...
		if(bb.position.x>175-10*bb.scale.x && bb.vx>0){
			bb.vx*=-1*ratio
		}else if(bb.position.x<-175+10*bb.scale.x && bb.vx<0){
			bb.vx*=-1*ratio
		}

		if(bb.position.z>172 && bb.vz>0){
			bb.vz*=-1*ratio
		}else if(bb.position.z<-172 && bb.vz<0){
			bb.vz*=-1*ratio
		}
	}else if(gamemd==0){
		bb.scale.x=bb.scale.y=bb.scale.z=1
	...
	//大小轉換
	if(bb.position.x>=175-10*bb.scale.x && bb.scale.x<4){
		bb.scale.x+=0.4
		bb.scale.y+=0.4
		bb.scale.z+=0.4
	}
	
	if(bb.position.x<-175+10*bb.scale.x && bb.scale.x>0.4){
		bb.scale.x-=0.4
		bb.scale.y-=0.4
		bb.scale.z-=0.4
	}
	
	if(bb.position.z>172){
		bb.scale.x=bb.scale.y=bb.scale.z=4
	}if(bb.position.z<-172){
		bb.scale.x=bb.scale.y=bb.scale.z=0.4
	}
	...
	if(bb.position.x<=-40 && bb.position.x>=-60 && bb.position.z>=22 && bb.position.z<=42){
		bb.scale.x=bb.scale.y=bb.scale.z=2
	}if(bb.position.x>=60 && bb.position.x<=80 && bb.position.z>=-58 && bb.position.z<=-38){
		bb.scale.x=bb.scale.y=bb.scale.z=3.6
	}if(bb.position.x>=-10 && bb.position.x<=10 && bb.position.z>=-98 && bb.position.z<=-78){
		bb.scale.x=bb.scale.y=bb.scale.z=0.8
	}if(bb.position.x>=-100 && bb.position.x<=-80 && bb.position.z>=-78 && bb.position.z<=-58){
		bb.scale.x=bb.scale.y=bb.scale.z=1.2
	}if(bb.position.x>=50 && bb.position.x<=70 && bb.position.z>=80 && bb.position.z<=100){
		bb.scale.x=bb.scale.y=bb.scale.z=2.4
	}
	
	if(bb.scale.x===2.8){
		scene.add(v)
		bb.position.x=bb.position.z=0
		ax=ay=az=0
		gamemd=0
		vx=vy=vz=0
		bb.scale.x=bb.scale.y=bb.scale.z=1
	...
	bb.position.y=world2D.sl02.value
	
	b.visible=s.visible=be.visible=se.visible=world2D.ch01.checked
