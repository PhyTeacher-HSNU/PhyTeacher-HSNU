...
var blackMid
var rr = 20
var blackLineallArray = []
var NN =10
var krr
var tt
var craft
var dt = 0.1
var craftvx,craftvy, craftvz, craftax, craftay, craftaz
var mm
var aa
var bb
var cc
...
function init(){
	...
	blackMid = new TEACHER.ObjSphere(15,0XFF5809,2)
	
	scene.add(blackMid)
	pts = new TEACHER.Points(0XCC22CC,2,3000)
	scene.add(pts)
	pts.update();
	for(var j=0;j<NN;j++){
		for(var i=0;i<NN;i++){
			
			var blackLine = new TEACHER.ObjCylinder(0.5,rr,0XF-10*j-5809,false,'x',2) 
			blackLine.position.y = Math.sin(2*Math.PI/NN*j)*rr/2
			krr = Math.sqrt(rr*rr-4*blackLine.position.y*blackLine.position.y)/2
			blackLine.position.x = Math.cos(2*Math.PI/NN*i)*krr/2
			blackLine.position.z = -Math.sin(2*Math.PI/NN*i)*krr/2
			blackLine.rotation.y = 2*Math.PI/NN*i
			blackLine.rotation.z = 2*Math.PI/NN*j-Math.PI
			if(j<8 & j>2){
				blackLine.visible = false
			}
			scene.add(blackLine)
			blackLineallArray.push(blackLine)
	...
	var craftwing = new TEACHER.ObjBox(15,1,5,0X5A5AAD,2)
	var craftbody = new TEACHER.ObjCylinder(3,10,0XFFFFFF,false,'z',2)
	var crafthead = new TEACHER.ObjSphere(3,0XFFFFFF,2)
	crafthead.position.z = 5
	craft.add(craftwing,craftbody,crafthead)
	scene.add(craft)
	craft.position.x =50
	ground.visible = false

	der1 = new TEACHER.ObjPicPlane(70,70,pics.der1,'z',2)
	der2 = new TEACHER.ObjPicPlane(70,70,pics.der2,'z',2)
	der3 = new TEACHER.ObjPicPlane(70,70,pics.der3,'z',2)
	scene.add(der1)
	scene.add(der2)
	scene.add(der3)
	der1.position.z=20
...
if(flame === 1){
	...
	for(var j=0;j<NN;j++){
		for(var i=0;i<NN;i++){
			var blackLine = blackLineallArray[10*j+i] 
			blackLine.position.y = Math.sin(2*Math.PI/NN*j)*rr/2*(1+Math.sin(tt+j))
			krr = Math.sqrt(rr*rr-Math.sin(2*Math.PI/NN*j)*rr*Math.sin(2*Math.PI/NN*j)*rr)/2
			blackLine.position.x = Math.cos(2*Math.PI/NN*i)*krr/2*(1+Math.sin(tt+j))
			blackLine.position.z = -Math.sin(2*Math.PI/NN*i)*krr/2*(1+Math.sin(tt+j))
			
			
			
			blackLineallArray[10*j+i] = blackLine 
	...
	if(blackMid.position.y-craft.position.y!==0){
		
		craftay = mm*1/10/(blackMid.position.y-craft.position.y)*Math.sqrt((blackMid.position.y-craft.position.y)*(blackMid.position.y-craft.position.y)) 
	}
	if(blackMid.position.x-craft.position.x!==0){
		craftax =  mm*1/10/(blackMid.position.x-craft.position.x)*Math.sqrt((blackMid.position.x-craft.position.x)*(blackMid.position.x-craft.position.x)) 
	}
	if(blackMid.position.z-craft.position.z!==0){
		craftaz =  mm*1/10/(blackMid.position.z-craft.position.z)*Math.sqrt((blackMid.position.z-craft.position.z)*(blackMid.position.z-craft.position.z)) 
	}
	craftvx+=craftax*dt
	craftvz+=craftaz*dt
	craftvy+=craftay*dt
	craft.position.x+=craftvx*dt
	craft.position.z+=craftvz*dt
	craft.position.y+=craftvy*dt
	if(craftvz<0){
		craft.rotation.y = Math.atan2(-craftvz,craftvx)+Math.PI/2
	}
	else{
		craft.rotation.y = Math.atan2(-craftvz,craftvx)+Math.PI/2
	}
	
	pts.addPoint(craft.position.x,20,craft.position.z)
	pts.update();	
}

if(cc !== world2D.sl01.value){
	mm = world2D.sl01.value
	cc = world2D.sl01.value
}

if(aa !== world2D.sl02.value){
	craft.position.x = craft.position.x*world2D.sl02.value/aa
	craft.position.z = craft.position.z*world2D.sl02.value/aa
	aa = world2D.sl02.value
	
	
}
if(bb !== world2D.sl03.value){
craftvx = craftvx*world2D.sl03.value/bb
craftvz = craftvz*world2D.sl03.value/bb
bb = world2D.sl03.value
}
...
function clickBtn(e){
	...
	if(str === 'btn01'){
		...
		gameMD = 2
		if(der3.visible === true){
			der1.visible = true
			der3.visible = false
		}
		else if(der2.visible === true){
			der3.visible = true
			der2.visible = false
		}
		else if(der1.visible === true){
			der2.visible = true
			der1.visible = false
		}
