...
function init(){
	//我自己弄的物件
	pizza=new TEACHER.ObjPicCylinder(2,0.5,pics.pizza,false,"y",2);	
	world3D.scene.add(pizza);
	pizza.position.y = 0.5
	//畫出pizza麵皮，並放在舞台上
	rollingpin=new TEACHER.ObjPicCylinder(5,30,pics.rollingpin,false,"x",2);
	rollingpin.rotation.y += 0.5*Math.PI
	rollingpin.right=new TEACHER.ObjPicCylinder(1.5,8,pics.rollingpin,false,"x",2);
	rollingpin.left=new TEACHER.ObjPicCylinder(1.5,8,pics.rollingpin,false,"x",2);
	rollingpin.right.position.z=18
	rollingpin.right.position.y=0.5
	rollingpin.left.position.z=-18
	rollingpin.left.position.y=0.5
	rollingpin.add(rollingpin.right).add(rollingpin.left);
	world3D.scene.add(rollingpin);
	rollingpin.position.y = 5.5
	//畫出擀麵棍，並放在舞台上
...
function tick(){
	...
	if(rollingpin.position.z > 20 && dz > 0){
			dz = -1*k
		}else if(rollingpin.position.z < -20 && dz < 0){
			dz = 1*k
		}
		
	rollingpin.position.z += dz
	rollingpin.rotation.x += dz/r
	pizza.scale.x += 0.1*Math.abs(dz)
	pizza.scale.z += 0.16*Math.abs(dz)
	

	smile.position.x += dx
	smile.rotation.z += -dx/r
	
	smile.position.y = r*smile.scale.y
