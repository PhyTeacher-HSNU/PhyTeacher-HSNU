...
function init(){
	...
	ball=new TEACHER.ObjSphere(1,0x5CADAD)
	bar=new TEACHER.ObjCylinder(0.2,rr,0xC7C7E2,true,"x")
	center=new TEACHER.ObjCylinder(1,4,0x6C3365)

	nn=new TEACHER.ObjTextPlane(100,10,"156327 游奕萱","z",0x484891)
	scene.add(nn)
	nn.position.z=-70
	nn.position.y=10

	clean=new TEACHER.ObjTextPlane(120,50,"Clean!","z",0x003060)
	clean.position.z=45
	
	ii=new TEACHER.ObjTextPlane(150,10,"用水球把髒污清理乾淨吧！","z",0xff0000)
	scene.add(ii)
	ii.position.z=-70
	ii.position.y=25
	...
	ball.position.y=bar.position.y=center.position.y=ball.scale.x*2

	scene.add(center).add(bar).add(ball)

	arrowV=new TEACHER.ObjArrow(0.5,0xFFEEDD)
	ball.add(arrowV)

	bg=new TEACHER.ObjBox(10000,100000,10000,0xFFF3EE,2)
	scene.add(bg)

	d1=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d1.position.x=50
	scene.add(d1)

	d2=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d2.position.x=-10
	d2.position.z=40
	scene.add(d2)

	d3=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d3.position.x=-30
	d3.position.z=-10
	scene.add(d3)

	d4=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d4.position.x=40
	d4.position.z=25
	scene.add(d4)

	d5=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d5.position.x=15
	d5.position.z=-60
	scene.add(d5)

	d6=new TEACHER.ObjBox(2,2,2,0x272727,0)
	d6.position.x=20
	d6.position.z=-20
	scene.add(d6)
...
function tick(){
	...
	var vv=rr*omega
	arrowV.setArrow(-2*vv*Math.sin(theta),0,2*vv*Math.cos(theta))
	arrowV.visible=world2D.ch01.checked

	if(ball.position.x>=45 && ball.position.x<=55){
		d1.visible=false
	}
	else if(ball.position.x>=-15 && ball.position.x<=-5 && ball.position.z<=45 && ball.position.z>=35){
		d2.visible=false
		
	}
	else if(ball.position.x>=-35 && ball.position.x<=-25 && ball.position.z>=-15 && ball.position.z<=-5){
		d3.visible=false
		
	}
	else if(ball.position.x>=35 && ball.position.x<=45 && ball.position.z>=20 && ball.position.z<=30){
		d4.visible=false
	}
	else if(ball.position.x>=10 && ball.position.x<=20 && ball.position.z<=-55 && ball.position.z>=-65){
		d5.visible=false
	}
	else if(ball.position.x<=25 && ball.position.x>=15 && ball.position.z>=-25 && ball.position.z<=-15){
		d6.visible=false
	}

	if(d1.visible==false && d2.visible==false && d3.visible==false && d4.visible==false && d5.visible==false && d6.visible==false){
		scene.add(clean)
