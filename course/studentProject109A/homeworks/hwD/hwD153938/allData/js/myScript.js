...
function init(){
	...
	for(var j=0 ; j<3 ; j++){
		for(var i=0 ; i<441 ; i++){
			var bb = new TEACHER.ObjBox(2,2,2,0x0000ff);
			bb.position.y = (j+1)*3;
			bb.position.x=-50+(i%21)*5;
			bb.position.z=-50+Math.floor(i/21)*5;
			
			scene.add(bb);
			ballArray.push(bb);
			XArray.push(-50+(i%21)*5);
			YArray.push((j+1)*3);
			ZArray.push(-50+Math.floor(i/21)*5);
		}
	}
	...
	line = new TEACHER.Line(0xffff00);
	scene.add(line);

	line2 = new TEACHER.Line(0xffff00);
	scene.add(line2);

	line3 = new TEACHER.Line(0xffff00);
	scene.add(line3);

	box = new TEACHER.ObjSphere(5,0xffff00);
	scene.add(box);
	box.position.x = ballArray[1312].position.x;
	box.position.z = ballArray[1312].position.z-15;

	NAME = new TEACHER.ObjTextPlane(50,6,'1539  38  蘇歆媛');
	scene.add(NAME);
	NAME.position.z = -50;
	NAME.position.x = -20;
	NAME.position.y = 15;
	...
	world2D.ch01.setLabel('軌跡');
	world2D.ch02.setLabel('球');
	world2D.btn01.setLabel('水波');
	world2D.btn02.setLabel('冰淇淋');
	...
	cone = new TEACHER.ObjArrow(7,0xcd853f);
	cone.rotation.z = Math.PI;
	cone.position.y = 40;
	scene.add(cone);
	cone.visible = false;

	ma = new TEACHER.ObjBox(7,15,7,0x708090);
	ma.position.z = 10;
	ma.position.y = 15;
	scene.add(ma);
	ma.visible = false;

	chine = new TEACHER.ObjCylinder(2,10,0x708090);
	ma.add(chine);
	chine.position.y-=5;
	chine.visible = false;

	for(var i=0 ; i<100 ; i++){
		var bb = new TEACHER.ObjSphere(ballRR,0xffefd5);
		scene.add(bb);
		bb.visible = false;
	...
	for(var i=0 ; i<31 ; i++){
		var bb = new TEACHER.ObjSphere(3,0xffefd5);
		scene.add(bb);
		bb.visible = false;

		ICEtheta+=ICEomega*ICEdt;

		bb.position.x=ICErr*Math.cos(ICEtheta);
		bb.position.z=ICErr*Math.sin(ICEtheta);
		bb.position.y=35;
...
function tick(){
	...
	if(cookie===1){
		if(cream===1){
			...
			if(c01<=99){
				ma.position.x = iceArray[c01].position.x;
				ma.position.z = iceArray[c01].position.z;
				ma.position.y = iceArray[c01].position.y+13;
		    }

			if(c01<100){
				setTimeout(c02,1000);
				setTimeout(ice(c01),1000);
			}
		
		
			if(a01<=31){
				setTimeout(a02,1000);
				setTimeout(ice2(a01),1000);
			}
	...
	/*for(var j=0 ; j<3 ; j++){
		for(var i=0 ; i<441 ; i++){
			var bb = ballArray[i+441*j];
			bb.position.x=XArray[i+441*j]+rr*Math.cos(theta);
			bb.position.y=YArray[i+441*j]+rr*(j+1)*0.5*Math.sin(theta);
	...
	line.visible = line2.visible = line3.visible = world2D.ch01.checked;

	box.visible = world2D.ch02.checked;
	box.position.y = ballArray[1312].position.y+6;
	
	/*if(ballArray[1312].position.x>XArray[1312]){
		boxtan = Math.abs(ballArray[1312].position.y-ballArray[1313].position.y)/Math.abs(ballArray[1312].position.x-ballArray[1313].position.x);
		box.rotation.z = Math.atan(boxtan)*Math.PI;
	}else if(ballArray[1312].position.x<XArray[1312]){
		boxtan = Math.abs(ballArray[1311].position.y-ballArray[1312].position.y)/Math.abs(ballArray[1311].position.x-ballArray[1312].position.x);
		box.rotation.z = Math.atan(boxtan)*Math.PI;
	}*/
	//log(boxtan);
	...
	for(var j=0 ; j<21 ; j++){
		...
		thetaArray[j] = thetaArray[j]+omega*dt;
		//theta=j*0.01+theta;

		for(var p=0 ; p<3 ; p++){
			for(var i=0 ; i<21 ; i++){
				var bb = ballArray[i*21+p*441+j];
				bb.position.x=XArray[i*21+p*441+j]+rr*(p+1)*0.5*Math.cos(thetaArray[j]);
				bb.position.y=YArray[i*21+p*441+j]+rr*(p+1)*0.5*Math.sin(thetaArray[j]);
			}
		}
		//log(thetaArray[j]);
	}
	...
	var pp = ballArray[430];
	line.addPoint(pp.position.x,pp.position.y,pp.position.z);

	var pp2 = ballArray[871];
	line2.addPoint(pp2.position.x,pp2.position.y,pp2.position.z);

	var pp3 = ballArray[1312];
	line3.addPoint(pp3.position.x,pp3.position.y,pp3.position.z);
...
TEACHER.ObjArrow=function(_r,_color){
	let ratioHead=0.9;//頭佔全長
	let ratioBody=0;//身寬佔全長
	let material = new THREE.MeshStandardMaterial({ color: _color||0xFF00FF , roughness: 0.4 });
	let geometry = new THREE.ConeGeometry( _r||10, 40*ratioHead, 32 );
	this.ArrowHead=new THREE.Mesh(geometry,material);
	this.ArrowHead.position.y=40*(1-0.5*ratioHead);
