...
function init(){
	...
	world2D.btn01.setLabel('第500顆球');
	for(var i=0;i<n;i++){
		var ball=new TEACHER.ObjSphere(2,0xff0000);
		ball.position.y=2;
		ball.position.x=-45+(i%10)*10;
		ball.position.y=-45+Math.floor(i/100)*10;
		ball.position.z=-45+Math.floor(i/100)*10;
...
function tick(){
	for(var i=0;i<n;i++){
		...
		if(ball.position.y>50 && ball.vy>0){
			ball.vy*=-1;
		}
		else if(ball.position.y<-50 && ball.vy<0){
			ball.vy*=-1;
		}
		var arrow=arrowarray[i];
		arrow.position.x=ball.position.x;
		arrow.position.y=ball.position.y+2;
		arrow.position.z=ball.position.z;
		arrow.setArrow(ball.vx,ball.vy,ball.vz);
	}
