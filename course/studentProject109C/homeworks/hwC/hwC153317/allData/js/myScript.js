...
function init(){
	...
	sun= new TEACHER.ObjPicSphere(rr,pics.sun);
	sunback = new TEACHER.ObjPicPlane(100,50,pics.sunback);
	scene.add(sunback);
	scene.add(sun);
	sun.position.x=0;
	sun.position.y=0;
	sunback.position.z=-50
	sunback.position.y=25
	sun.vx=0;
	sun.vy=0;
	sun.vz=0;
	...
	moon= new TEACHER.ObjPicSphere(rr,pics.moon,1);
	moonback = new TEACHER.ObjPicPlane(100,50,pics.moonback);
	scene.add(moonback);
	scene.add(moon);
	moon.position.x=0;
	moon.position.y=0;
	moonback.position.z=-50
	moonback.position.y=25
	moon.vx=0;
	moon.vy=0;
	moon.vz=0;
	...
	myname = new TEACHER.ObjPicPlane(20,10,pics.name);
	myname.position.y = 5;
	...
	world2D.sl02.setLabel("地球自轉速");

	world2D.ch01.setLabel("地球會轉")
...
//C.定義tick                                                                                                                       
function tick(){
	var isPlay = world2D.ch01.checked
	if (isPlay){
		omega = world2D.sl02.value
		angle+=omega*dt
		sun.scale.x = sun.scale.y = sun.scale.z = world2D.sl01.value/5
		sun.position.x=-RR*Math.cos(angle);
		sun.position.y=Math.sqrt((50-sun.position.x)*(50+sun.position.x));

		sun.vx+=sun.ax;
		sun.vy+=sun.ay;
		sun.vz+=sun.az;

		moon.scale.x = moon.scale.y = moon.scale.z = world2D.sl01.value/5;
		moon.position.x=-RR*Math.cos(angle);
		moon.position.y=Math.sqrt((50-sun.position.x)*(50+sun.position.x));
	...
	if(gameMD === 1 ){
		sun.visible = true;
		moon.visible = false;
		sunback.visible = true;
		moonback.visible = false;
	}else if(gameMD === 2){
		sun.visible = false;
		moon.visible = true;
		sunback.visible = false;
		moonback.visible = true;
	}
