...
function init(){
	...
	kick1=new TEACHER.ObjPicPlane(50,50,pics.kick1,"z",2)
	scene.add(kick1)
	kick1.position.z=-50
	kick1.position.x=-58
	kick1.position.y=kick1.position.y+18

	kick2=new TEACHER.ObjPicPlane(50,50,pics.kick2,"z",2)
	scene.add(kick2)
	kick2.position.x=-58
	kick2.position.z=-50
	kick2.position.y=kick2.position.y+18
	kick2.visible=false

    door=new TEACHER.ObjPicPlane(25,25,pics.door,"z",2)
	scene.add(door)
	door.position.x=50
	door.position.y=5
	door.position.z=-50

	fail=new TEACHER.ObjTextPlane(80,20,"FAIL","z",0xff0000)
	scene.add(fail)
	fail.scale.x=fail.scale.y=3
	fail.position.y+=10
	fail.visible=false

	perfect=new TEACHER.ObjTextPlane(80,20,"PERFECT","z",0xff0000)
	scene.add(perfect)
	perfect.scale.x=perfect.scale.y=3 
    perfect.position.y+=10
	perfect.visible=false

	my=new TEACHER.ObjTextPlane(460,80,"157904 任禹丞","z",0x00CACA,0x007979)
	scene.add(my)
	my.scale.x=my.scale.y=0.3
	my.position.z=0
	my.position.y=-50
	
	info=new TEACHER.ObjPicPlane(120,60,pics.info)
	scene.add(info)
	info.position.z+=50
	info.position.y+=13.5
	info.rotation.x-=30*Math.PI/360
	info.visible=false



	world2D.btn01.setLabel('射門')
	world2D.btn02.setLabel('重製')
	world2D.ch01.setLabel('說明')
...
function tick(){
	...
	if(GMmode===1 && ball.position.x<50 && play===1){
		ball.position.x+=dx
	...
	if(ball.position.x===50){
		...
		if(ball.position.z===door.position.z && door.position.z===50){
            perfect.visible=true
			play=0
		}else if(ball.position.z===door.position.z && door.position.z!==50){
			door.position.z+=10
		}else{
			fail.visible=true
			play=0
    ...
    info.visible=world2D.ch01.checked
...
function clickbtn(e){
	...
	if(str==="btn01"){
		...
		GMmode=0
		ball.position.z=kick1.position.z=kick2.position.z=door.position.z=-50
		ball.position.x=-50
		ball.rotation.z=0
		kick1.visible=true
		kick2.visible=false
		fail.visible=perfect.visible=false
		play=1

	}else if(str==="btnUp" && ball.position.x===-50 && ball.position.z>-50){
		kick1.position.z-=10
		kick2.position.z-=10
		ball.position.z-=10
	}else if(str==="btnDown" && ball.position.x===-50 && ball.position.z<50){
		kick1.position.z+=10
		kick2.position.z+=10
		ball.position.z+=10
