...
//B.定義init
function init(){
	ball= new TEACHER.ObjPicSphere(5,pics.ball,5);
	scene.add(ball);
	ball.rotation.z=PI/2;
	ball.position.y=5;
	body=new TEACHER.ObjBox(6,10,5,0xFF00FF,0);
	scene.add(body);
	body.position.y=12;
	hand=new TEACHER.ObjBox(10,4,4,0xFF00,0);
	scene.add(hand);
	hand.position.y=12;
	head=new TEACHER.ObjPicSphere(4,pics.head,4);
	scene.add(head);
...
//C.定義tick                                                                                                                       
function tick(){
	v=world2D.sl01.value;
	w+=world2D.sl02.value;	
	ball.position.x=head.position.x=hand.position.x=body.position.x += v*Math.sin(w/180)/10;
	ball.position.z=head.position.z=hand.position.z=body.position.z += v*Math.cos(w/180)/10;
	ball.rotation.y=hand.rotation.y=body.rotation.y=Math.sin(w/180)*v/10*PI;
	ball.rotation.z=Math.cos(w/180)*v/10*PI;
	ball.rotation.x=Math.sin(w/180)*v/10*PI;
