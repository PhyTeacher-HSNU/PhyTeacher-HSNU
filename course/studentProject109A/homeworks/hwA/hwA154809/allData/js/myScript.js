...
function init(){
	...
	world2D.btn01.setLabel('縫合');
	world2D.btn01.on("click",clickBtn);

	tt=new TEACHER.ObjTextPlane(100,20,"154809","z",0xffff00,0x0000ff);
	scene.add(tt);
	tt.position.z=-100;
	...
	pp=new TEACHER.ObjPicPlane(8,8,pics.head,"z",2);
	scene.add(pp);
	mm=new TEACHER.ObjPicPlane(10,30,pics.body,"z",2);
	scene.add(mm);
	...
	goal=new TEACHER.ObjPicPlane(20,40,pics.goal,"z",2);
	scene.add(goal);

	goal.position.x=40;
	goal.position.y=12;

	goalword=new TEACHER.ObjPicPlane(40,20,pics.goalword,"z",2);
...
function tick(){
	...
	//pp.position.x+=pp.vx*dt;
	//pp.position.y+=pp.vy*dt;
	//pp.position.z+=pp.vz*dt;
	...
	if(pp.position.x <= 50){
		...
		goalword.position.x=0;
		goalword.position.y=15;
...
function clickBtn(e){
	...
	if(str==="btn01"){
		pp.position.x=-33;
		pp.position.y=28;
		goalword.position.x=1000000;
		goalword.position.y=2827372363273267237632;
