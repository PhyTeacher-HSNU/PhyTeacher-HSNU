...
function init(){
	for(var i=0;i<=6;i++){
		pp=new TEACHER.ObjPicCylinder(5,30,pics.sky,false,"z",2);
		scene.add(pp);
		pp.position.y=5;
		pp.position.x=i*10-30;
		pp.position.z=-20;
	}
	...
	world2D.btn01.setLabel("Do");
	world2D.btn02.setLabel("Re");
	world2D.btn03.setLabel("Mi");
	world2D.btn04.setLabel("Fa");
	world2D.btn05.setLabel("So");
	world2D.btn06.setLabel("Ra");
	world2D.btn07.setLabel("Si");
	world2D.btn01.on('click',initBall);
	world2D.btn02.on('click',initBall);
	world2D.btn03.on('click',initBall);
	world2D.btn04.on('click',initBall);
	world2D.btn05.on('click',initBall);
	world2D.btn06.on('click',initBall);
	world2D.btn07.on('click',initBall);

	sound1=new Audio('allData/wav/Do.wav');
	sound2=new Audio('allData/wav/Re.wav');
	sound3=new Audio('allData/wav/Mi.wav');
	sound4=new Audio('allData/wav/Fa.wav');
	sound5=new Audio('allData/wav/So.wav');
	sound6=new Audio('allData/wav/Ra.wav');
	sound7=new Audio('allData/wav/Si.wav');
...
function initBall(e){
	...
	aBall=new TEACHER.ObjPicSphere(5,pics.sky);
	...
	intervalId=setInterval(ballMove,1000/fps,aBall,intervalId);
}

function ballMove(aBall,intervalId){
	//var dx=0.5;
	//aBall.position.y+=dx;
	//aBall.position.x+=dx
	aBall.position.z+=dx;
	aBall.rotation.x+=dx/rr;
	console.log("aa");
	//if (aBall.position.z>=70) clearInterval(intervalId);//dx=-0.5;//clearInterval(intervalId);
	//else if (aBall.position.z <=0) dx=0.5;
