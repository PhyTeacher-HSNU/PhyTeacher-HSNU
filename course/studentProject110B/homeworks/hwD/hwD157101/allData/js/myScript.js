...
function init(){
	...
	ball = new TEACHER.ObjSphere(2,0x0000ff)
	line = new TEACHER.ObjCylinder(0.2,rr,0x00ff00,true,"x");
	center = new TEACHER.ObjCylinder(1 , 4 , 0xffff00,false,"z")
	world2D.slCameraRR.value=120
	con=new THREE.Object3D;
	arrowv = new TEACHER.ObjArrow(0.3,0xffff00);
	con.add(ball).add(line).add(center).add(arrowv);

	scene.add(con);

	circle=new TEACHER.ObjCylinder(5,0.1,0x010101,false,"y")
	scene.add(circle)
	...
	ball.ay=0;
	ball.az=0;
	ball.position.y=line.position.y=center.position.y=25;
	ground.visible=false;
	can=new TEACHER.ObjCylinder(50,0.1,0xF0F0F0,false,"y",1)
	can.position.y=0
	scene.add(can)

	circle.position.x=(Math.random()*45)*Math.cos(Math.random()*Math.PI*2-Math.PI);
	circle.position.y=0.1;
	circle.position.z=(Math.random()*45)*Math.sin(Math.random()*Math.PI*2-Math.PI);
...
function tick(){
	...
	world2D.ch02.setLabel("分數："+point+"分")

	ballrr=world2D.sl03.value;
	
	ball.scale.x=ball.scale.y=ball.scale.z=ballrr/2

	arrowv.position.x=ball.position.x;
	arrowv.position.y=ball.position.y;
	arrowv.position.z=ball.position.z;
	if (game===1){
		theta += omega*dt;
		ball.position.x=rr*Math.cos(theta);
		ball.position.y=25+rr*Math.sin(theta);
		
		line.position.x=rr*Math.cos(theta)*0.5;
		line.position.y=(50+rr*Math.sin(theta))*0.5;	
		line.rotation.z=theta;


		arrowv.setArrow(vv*Math.sin(theta)*-1,vv*Math.cos(theta)*1,0);
		...
		ball.vx+=ball.ax;		
		ball.vz+=ball.az;
		if(ball.position.x>50-ballrr && ball.vx>0){
			ball.vx*=-1*ratio;
		}else if(ball.position.x<-50+ballrr && ball.vx<0){
			ball.vx*=-1*ratio;
		}	
		if(ball.position.z<-50+ballrr && ball.vz<0){
			ball.vz*=-1*ratio
		}else if(ball.position.z>50-ballrr && ball.vz>0){
			ball.vz*=-1*ratio	
		}
		if(ball.position.y<ballrr && ball.vy<0){
			ball.vy*=-1*0.7
		}
		if(ball.vy<0.1&&ball.position.y<ballrr+0.1){
			ball.vy=0
		}
		arrowv.setArrow(ball.vx*2,ball.vy*2,ball.vz*2)
		world2D.btn01.setLabel("收回旋轉");
	}
	if(game===3){
		ball.position.y+=ball.vy*dt;
		ball.position.x+=ball.vx*dt;
		...
		if(Math.sqrt((ball.position.x-rr*Math.cos(theta))*(ball.position.x-rr*Math.cos(theta))+(ball.position.y-25-rr*Math.sin(theta))*(ball.position.y-25-rr*Math.sin(theta)))<=1 ){
			game=1;
		}
		world2D.btn01.setLabel("丟");
		arrowv.setArrow(ball.vx*2,ball.vy*2,ball.vz*2)
	...
	con.rotation.y+=kk*dt

	xx=ball.position.x*Math.cos(con.rotation.y)
	zz=-ball.position.x*Math.sin(con.rotation.y)


	if(Math.sqrt((xx-circle.position.x)*(xx-circle.position.x)+(zz-circle.position.z)*(zz-circle.position.z))<5&&ball.position.y<=ballrr+0.1){
		point++;
		circle.position.x=(Math.random()*90-45)*Math.cos(Math.random()*720-360);
		circle.position.y=0.1;
		circle.position.z=(Math.random()*90-45)*Math.sin(Math.random()*720-360);
	}
...
function qq(e){
	...
	if (str === 'btn01'){
		...
		if(game===2){
			ball.vx=(rr*Math.cos(theta)-ball.position.x)/4;
			ball.vy=(25+rr*Math.sin(theta)-ball.position.y)/4;
			game=3;
		}else if(game===1){
			ball.vx=vv*Math.sin(theta)*-1/1.5;
			ball.vy=vv*Math.cos(theta)*1/1.5;
			game=2;
		}
	}
	if(game===1){
