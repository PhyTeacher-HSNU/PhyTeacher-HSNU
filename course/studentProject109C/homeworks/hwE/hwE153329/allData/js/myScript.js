...
function init(){
	...
	scene.fog=new THREE.Fog(0x000000,400,450);
	world2D.slCameraRR.value=200;

	me=new TEACHER.ObjPicPlane(20,20,pics.me,"z");
	me.position.z=-50;
	me.position.y=40;
	scene.add(me);

	ball=new TEACHER.ObjSphere(5,0xff0000);
	scene.add(ball);
	ball.position.y=60;
	ballvy=0;
	ballay=-100;
	...
	housearr=[];
	colorarr=[0xff0000,0x00ff00,0x0000ff,0xffff00,0x00ffff,0xff00ff,0x000000,0xffffff];

	for(var i=0;i<20;i++){
		var xx=Math.random()*100-50;
		var zz=Math.random()*100-50;
		var boxx=Math.random()*10+2;
		var boxy=Math.random()*20+5;
		var boxz=Math.random()*10+2;
		for(var j=0;j<100;j++){
			var house=new TEACHER.ObjBox(boxx,boxy,boxz,colorarr[i%8]);
			house.position.x=xx-450+j%10*100;
			house.position.z=zz-450+Math.floor(j/10)*100;
			scene.add(house);
			housearr.push(house);
		}
	}
	floor=new TEACHER.ObjPlane(5000,5000,0x826239,"y",2);
	scene.add(floor);
...
function tick(){
	...
	var b=world2D.sl01.value;
	ballvy+=ballay*dt;
	ball.position.y+=ballvy*dt;
	if(ball.position.y<5 & ballvy<0){
		ballvy*=-b;
	}

	if(btn=='btnUp' & vz<40){
		az=20;
	}else if(btn=='btnDown' & vz>-40){
		az=-20;
	}else if(btn=='btnRight' & vx>-40){
		ax=-20;
	}
	else if(btn=='btnLeft' & vx<40){
		ax=20;
	}else if(btn==0){
		ax=az=0;
	}
	...
	for(var i=0;i<2000;i++){
		...
		house.position.x+=vx*dt;
		house.position.z+=vz*dt;
	}
