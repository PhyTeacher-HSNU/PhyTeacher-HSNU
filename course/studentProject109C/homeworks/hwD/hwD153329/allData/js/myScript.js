...
function init(){
	...
	var name=new TEACHER.ObjPicPlane(20,20,pics.me,"z");
	name.position.z=-50;
	name.position.y=10;
	scene.add(name);
	...
	for(var i=0;i<ballnum;i++){
		var ball=new TEACHER.ObjSphere(1,0x0000ff);
		ball.position.x=-49+i*2;
		ball.position.y=10;
		ball.position.z=20;
		ballarr3.push(ball);
...
function tick(){
	...
	if(world2D.ch01.checked==true){
		for(var i=0;i<ballnum;i++){
			var ball1=ballarr1[i];
			var ball2=ballarr2[i];
			ball1.visible=ball2.visible=false;
		}
	}else{
		for(var i=0;i<ballnum;i++){
			var ball1=ballarr1[i];
			var ball2=ballarr2[i];
			ball1.visible=ball2.visible=true;
		}
	}
...
/*	if(isstep==-1){
	...
	lda1=world2D.sl01.value;

	for(var i=0;i<ballnum;i++){
		var ball=ballarr1[i];
		var y=a*Math.sin(2*Math.PI*ball.position.x/lda1-2*Math.PI*time/t)
		ball.position.y=10+y;
	}

	lda2=world2D.sl02.value;
	move=world2D.sl03.value;
	/*for(var i=0;i<ballnum;i++){
		var ball=ballarr2[i];
		ball.position.x=-49+i*2+move;
	}*/

	for(var i=0;i<ballnum;i++){
		var ball=ballarr2[i];
		var y=a*Math.sin(2*Math.PI*(ball.position.x-move)/lda2-2*Math.PI*time/t)
		ball.position.y=10+y;
	}
	...
	for(var i=0;i<ballnum;i++){
		...
		var ball1=ballarr1[i];
		var ball2=ballarr2[i];
		var y=ball1.position.y-10+ball2.position.y-10;
		ball.position.y=10+y;
	}
