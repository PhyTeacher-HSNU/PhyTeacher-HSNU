...
function init(){
	...
	bar = new TEACHER.ObjCylinder(0.2,rr,0xffff00,true,"y")
	center= new TEACHER.ObjCylinder(1,4,0x00ffff,false,"z")
	
	ball.position.y =  bar.position.y = center.position.y = 20
	ball.position.x =  bar.position.x = center.position.x = 30
	scene.add(center).add(bar).add(ball)

	//wall
	wall1 = new TEACHER.ObjBox(2,5,20,0x009393,2)
	scene.add(wall1)
	wall1.position.x=-35
	wall1.position.y=2.5

	wall2 = new TEACHER.ObjBox(16,5,2,0x009393,2)
	scene.add(wall2)
	wall2.position.z=-10
	wall2.position.x=-42
	wall2.position.y=2.5

	wall3 = new TEACHER.ObjBox(16,5,2,0x009393,2)
	scene.add(wall3)
	wall3.position.z=10
	wall3.position.x=-42
	...
	//arrow
	arrowV = new TEACHER.ObjArrow(0.5,0x00ec00)
	ball.add(arrowV)
...
function tick(){
	...
	//wall
	wall1.scale.y = wall2.scale.y = wall3.scale.y = world2D.sl03.value/5
	wall1.position.y = wall2.position.y = wall3.position.y = world2D.sl03.value/2
	
	if(gameMD===0){

		theta+=omega*dt;
		//ball
		ball.position.y = rr*Math.cos(theta) + 20
		ball.position.x = rr*Math.sin(theta) + 30

		//bar
		bar.position.y = 0.5*rr*Math.cos(theta) + 20
		bar.position.x = 0.5*rr*Math.sin(theta) + 30
		...
		//arrow	
		vv = rr*omega
		arrowV.setArrow( Math.cos(theta)*vv , -Math.sin(theta)*vv , 0 )
		...
		if(ball.position.y<0+2 && ball.vy<0){
			ball.vy*=-0.6
		}

		if(ball.position.y<=world2D.sl03.value+2 && ball.position.x>-38 && ball.position.x<-36 && ball.vx>0){
			ball.vx*=-0.6
		}else if(ball.position.y<=world2D.sl03.value+2 && ball.position.x<-32 && ball.position.x>-34 && ball.vx<0){
			ball.vx*=-0.6
		}
...
function clickBtn(e){
	...
	if(str==="btn01"){
		if(gameMD===0){
			gameMD=1
			ball.vx=Math.cos(theta)*vv
			ball.vy=-Math.sin(theta)*vv
			ball.vz=0
			ball.ay=-2
			world2D.btn01.setLabel("back")
		}else if(gameMD===1){
			gameMD=0
			ball.position.z=0
			world2D.btn01.setLabel("shoot")
