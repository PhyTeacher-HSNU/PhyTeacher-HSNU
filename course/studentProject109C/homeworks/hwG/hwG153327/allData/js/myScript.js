...
function init(){
	...
	planet1=new TEACHER.ObjSphere(1,0XFFFF00);
	planet1.position.x=30;
	planet2=new TEACHER.ObjSphere(1,0XFFFF00);
	planet2.position.x=-30;
	scene.add(planet1);
	scene.add(planet2);
	planet1.vx=0;
	planet1.vz=Math.sqrt(g*m1*m2/30);
	planet2.vx=0;
	planet2.vz=-Math.sqrt(g*m1*m2/30);
	world2D.sl01.setLabel("r");
	world2D.sl01.maximum=2.1;
	world2D.sl01.value=2.0;
	world2D.sl01.minimum=1.9;
	world2D.sl02.setLabel("比例");
	world2D.sl02.maximum=2;
	world2D.sl02.value=1;
	world2D.sl02.minimum=1;	
	line1=new TEACHER.Line(0X00FF00,10000);
	scene.add(line1);
	line2=new TEACHER.Line(0X00FFFF,10000);
	scene.add(line2);
	world2D.btn01.setLabel("清除");	
...
function tick(){
	for(var i=0;i<10;i++){
		...
		var m=world2D.sl02.value;
		r1=Math.sqrt(planet1.position.x*planet1.position.x+planet1.position.z*planet1.position.z);
		planet1.ax=-g*m1*m2*planet1.position.x/Math.pow(r1,3);
		planet1.az=-g*m1*m2*planet1.position.z/Math.pow(r1,3);
		planet1.vx+=planet1.ax*dt;
		planet1.vz+=planet1.az*dt;
		planet1.position.x+=planet1.vx*dt;
		planet1.position.z+=planet1.vz*dt;
		planet2.position.x=-planet1.position.x*m;
		planet2.position.z=-planet1.position.z*m;
		planet1.scale.x=m*m;
		planet1.scale.y=m*m;
		planet1.scale.z=m*m;
		line1.addPoint(planet1.position.x,0,planet1.position.z);
		line2.addPoint(planet2.position.x,0,planet2.position.z);
	}
