...
function init(){
	...
	kk=0 
	rr=2
	says=new TEACHER.ObjPicPlane(50 ,50 ,pics.says,'Z',0)
	der=new TEACHER.ObjPicPlane(64 ,36 ,pics.der,'Z',0)
	scene.add(says)
	scene.add(der)

	says.position.z=10
	says.position.y=-14.7+25+5+10-10
	says.position.x=0
	...
	for(i=0;i<1;i++){
		aa = new TEACHER.ObjSphere(2,0XFF00FF,2);
	    scene.add(aa);
		aa.position.x=30
		aa.position.y=20
		aa.position.z=0
		bb = new TEACHER.ObjArrow(0.4,0Xffff00);
		scene.add(bb);
		bb.position.x=aa.position.x
		bb.position.y=aa.position.y+2
		bb.position.z=aa.position.z
		aa.vx =0
		aa.vy =0
		aa.vz =0
		aa.ax=0
	...
	for(i=0 ;4>i ;i++){
			...
			cc = new TEACHER.ObjBox(10,(i+1)*(10/(3^(1/2))),10,0X0000FF,2)
			scene.add(cc);
			tempArrow.push(cc);
			cc = tempArrow[i] 
			cc.position.x=-20+5+(i+1)*10
			cc.position.y=0+5/((3^(1/2)))*(i+1)
			tempArrow[i] = cc
...
function tick(){
	...
	if(gameMD === 2){
		...
		aa.vy+=aa.ay*dt
		bb.position.x = aa.position.x
		bb.position.y = aa.position.y+rr
		bb.position.z = aa.position.z
		bb.setArrow(2*aa.vx,2*aa.vy,2*aa.vz)
		...
		if (aa.position.z>50-rr && aa.vz>0){
			aa.vz=aa.vz*-1
		}
		if (aa.position.z<-50+rr && aa.vz<0){
			aa.vz=aa.vz*-1
		}
		if (aa.position.x>20 &&aa.position.x<30 &&aa.position.y<(3+1)*(10/(3^(1/2)))+2 && aa.vy<0){
			aa.vy=aa.vy*-1
		}
		if (aa.position.x>10 &&aa.position.x<20 &&aa.position.y<(2+1)*(10/(3^(1/2)))+2 && aa.vy<0){
			aa.vy=-aa.vy*(Math.floor(world2D.sl01.value))*0.3333333 
		}
		if (aa.position.x>0 &&aa.position.x<10 &&aa.position.y<(1+1)*(10/(3^(1/2)))+2 && aa.vy<0){
			aa.vy=-aa.vy*(Math.floor(world2D.sl02.value))*0.3333333  
		}
		if (aa.position.x>-10 &&aa.position.x<0 &&aa.position.y<(0+1)*(10/(3^(1/2)))+2 && aa.vy<0){
			aa.vy=-aa.vy*(Math.floor(world2D.sl03.value))*0.3333333 
		}
		else if (aa.position.x<-10 &&aa.position.y<(-1+1)*(10/(3^(1/2)))+2 && aa.vy<0){
			aa.vy=-aa.vy  
		}
