...
function init(){
	...
	/*world2D.btn01.setLabel("眼睛動");
	world2D.btn02.on("click",button);
	world2D.btn02.setLabel("滾");
	world2D.btn01.on("click",button);
	world2D.btnRight.on("click",button);
	world2D.btnLeft.on("click",button);
	world2D.btnDown.on("click",button);
	world2D.btnUp.on("click",button);
*/
	/*smile.position.y=5;
	smile.scale.x=0.25;
	smile.scale.y=0.25;
	smile.scale.z=0.25;
	world2D.sl01.setLabel("縮放");
	world2D.sl02.setLabel("轉");
	world2D.sl01.maximum=3;
	world2D.sl01.value=1;
	world2D.sl01.minimum=0.2;
	world2D.sl02.maximum=180;
	world2D.sl02.value=0;
	world2D.sl02.minimum=-180;
	world2D.sl01.digitN=1;
	world2D.ch01.setLabel("左眼可視");
	world2D.ch02.setLabel("右眼可視");
	*/

	
	cat=new TEACHER.ObjPicPlane(10,10,pics.cat,"z",2);
	scene.add(cat);
	smile.visible=false;
	cat.position.y=5;
    circle=new TEACHER.ObjPicCylinder(20,10,pics.circle,true,"x",2);
	scene.add(circle);
	circle.position.y=20;
	
	circle.visible=false;
	setInterval(tick,1000/fps);
	world2D.btn01.setLabel("貓咪滾動");
	world2D.btn02.setLabel("貓咪跑滾輪");
	world2D.btn01.on("click",button);
	world2D.btn02.on("click",button);
	world2D.btnUp.on("click",button);
	world2D.btnDown.on("click",button);
	...
	world2D.sl02.setLabel("作者帥氣程度");
	world2D.sl03.setLabel("給這個程式的分數");
	...
	world2D.ch02.setLabel("下方按鈕有沒有用");
	world2D.ch01.setLabel("速度加倍");
...
//C.定義tick                                                                                                                       
function tick(){
	console.log("貓咪大戰爭六周年");



    /*smile.scale.x=world2D.sl01.value;
    smile.scale.y=world2D.sl01.value;
    smile.scale.z=world2D.sl01.value;
	smile.position.y=smile.scale.y*20;
	smile.eyeL.rotation.z=world2D.sl02.value*Math.PI/180;
	smile.eyeR.rotation.z=-(world2D.sl02.value*Math.PI/180);
    smile.eyeL.visible=world2D.ch01.checked;
    smile.eyeR.visible=world2D.ch02.checked;
*/
	/*if(aaa==1){
		smile.rotation.y=0;
		if(aha===0){
			smile.rotation.z-=a/rr;
			smile.position.x+=a;
		...
		if(aha==0 && smile.position.x==45){
			aha=1;
		}else if(aha===1 && smile.position.x==-45){
			aha=0;
		}
	...
	if(aaa==1){
		...
		if(aha==0 && cat.position.x>=45){
			aha=1;
		}else if(aha===1 && cat.position.x<=-45){
			aha=0;
		}
	}else if(aaa==2){
		cat.rotation.y=0;
		cat.rotation.z-=a/rr;
		circle.rotation.z=cat.rotation.z*Math.PI/180*5;
	}
	...
	/*if(aaa==1){
		if(aha===0){
			smile.eyeL.position.x-=a;
			smile.eyeR.position.x+=a;
			
		}else{
			smile.eyeL.position.x+=a;
			smile.eyeR.position.x-=a;
		}
		if(aha==0 && smile.eyeR.position.x>=10){
			aha=1;
		}else if(aha===1 && smile.eyeR.position.x<=0){
			aha=0;
		}
	}else if(aaa==2){
		if(aha===0){
...
function button(e){
	...
	if(world2D.ch02.checked==true){
		if(haha=="btn01"){
			cat.position.x=cat.position.z=0;
			circle.visible=false;
		...
		if(haha=="btn02"){
			cat.position.x=cat.position.z=0;
			circle.position.x=circle.position.z=0;
			circle.visible=true;
			aaa=2;
		}
		if(haha=="btnUp"){
