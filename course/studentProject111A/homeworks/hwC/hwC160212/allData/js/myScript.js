...
var action=0
var stopall=0
var wordend,wordhbase,wordhball,wordbad
var hbase=0
function init(){
    side1=new TEACHER.ObjBox(100,4,4,0x00f0f0)
	side1.position.x=0
	side1.position.y=0
	side1.position.z=-52
	scene.add(side1)
	side2=new TEACHER.ObjBox(100,4,4,0x00f0f0)
	side2.position.x=0
	side2.position.y=0
	side2.position.z=52
	scene.add(side2)
	side3=new TEACHER.ObjBox(4,4,108,0x00f0f0)
	side3.position.x=52
	side3.position.y=0
	side3.position.z=0
	scene.add(side3)
	side4=new TEACHER.ObjBox(4,4,108,0x00f0f0)
	side4.position.x=-52
	side4.position.y=0
	side4.position.z=0
	scene.add(side4)
	...
	for(var i=0;i<6;i++){
	hold=new TEACHER.ObjCylinder(2,0.4,0x0f0f0f,false,"y")
	hold.position.x=(i%3)*48-48
	hold.position.y=0
	hold.position.z=(Math.floor(i/3))*96-48
	holdaa.push(hold)
	scene.add(hold)
	}
	world2D.sl01.setLabel('力道')
	world2D.sl01.maximum=60
    world2D.sl01.minimum=5
	world2D.sl01.value=30
	world2D.sl02.setLabel('起始白球位置')
	world2D.sl02.maximum=50
    world2D.sl02.minimum=-50
	world2D.sl02.value=0
	world2D.sl03.setLabel('旋轉')
	world2D.sl03.maximum=2*Math.PI
    world2D.sl03.minimum=0
	world2D.sl03.value=0
	world2D.btn01.setLabel('開球')
	world2D.btn01.on('click',clickbtn1)
	for(var j=0;j<n;j+=1){
		for(var i=0;i<=j;i++){
			bb=new TEACHER.ObjSphere(2,0xff0000)
			bb.position.x=10+3*j
			bb.position.y=2
			bb.position.z=2*j-(i*3.8)
			h=Math.random()
			s=Math.random()
			l=Math.random()
			bb.mat.color.setRGB(h,s,l)
			scene.add(bb)
			aa.push(bb)
			bb.vx=0
		    bb.vy=0
	...
	wb=new TEACHER.ObjSphere(2,0Xffffff)
	wb.visible=true
	wb.position.x=-30
	wb.position.y=2
	wb.position.z=0
	...
	wb.vz=0
	scene.add(wb)
base=new TEACHER.ObjCylinder(1,30,0x000000,false,"x")
base.position.x=-50
base.position.y=1
base.position.z=0
base.visible=true
...
function tick(){
	...
	for(var i=0;i<mm;i++){
		var bb=aa[i]
	if(Math.abs(wb.vx)<0.1&&Math.abs(wb.vz)<0.1&&Math.abs(bb.vx)<0.1&&Math.abs(bb.vz)<0.1){
		nn+=1
...
nn=0
	if(time===0){
		wb.position.z=world2D.sl02.value
	}
	theta=world2D.sl03.value
	if(ok===0){
	base.position.x=wb.position.x+18*Math.cos(theta)
	base.position.z=wb.position.z+18*Math.sin(theta)
	base.rotation.y=-theta}
	if(action===1){
	wb.position.x+=wb.vx*dt
	wb.position.y+=wb.vy*dt
	wb.position.z+=wb.vz*dt}
	if (wb.position.x>48&&wb.vx>0){
		wb.vx*=-0.7
	}
	if (wb.position.x<-48&&wb.vx<0){
		wb.vx*=-0.7
	...
	bb.position.x+=dt*bb.vx
	bb.position.y+=dt*bb.vy
	bb.position.z+=dt*bb.vz
if (bb.position.x>48&&bb.vx>0){
	bb.vx*=-0.7
}
if (bb.position.x<-48&&bb.vx<0){
	bb.vx*=-0.7
...
for (var i=0;i<mm-1;i++){
	for(var j=i+1;j<mm;j++){
		var balla=aa[i]
		var ballb=aa[j]
		collision(balla,ballb,4)
	}
}
}
for (var i=0;i<mm;i++){
...
for(var i=0;i<mm;i++){
	for(var j=0;j<6;j++){
		var ballt=aa[i]
		var holdt=holdaa[j]
		if(holdt.position.x+2>ballt.position.x&&ballt.position.x>holdt.position.x-2&&holdt.position.z+2>ballt.position.z&&ballt.position.z>holdt.position.z-2){
			ballt.visible=false
			aa.splice(i,1)
			mm-=1
...
for(var t=0;t<6;t++){
	var holdw=holdaa[t]
	if(holdw.position.x+2>wb.position.x&&wb.position.x>holdw.position.x-2&&holdw.position.z+2>wb.position.z&&wb.position.z>holdw.position.z-2){
	wb.visible=false
	wordend=new TEACHER.ObjTextPlane(80,20,"Game over","y",0x0f0f0f)
	wordend.position.x=0
	wordend.position.y=10
	wordend.position.z=-20
	scene.add(wordend)
	wordhbase=new TEACHER.ObjTextPlane(80,20,"用了"+hbase+"杆","y",0x0f0f0f)
	wordhbase.position.x=0
	wordhbase.position.y=10
	wordhbase.position.z=10
	scene.add(wordhbase)
	wordhball=new TEACHER.ObjTextPlane(80,20,"進了"+(10-mm)+"顆球","y",0x0f0f0f)
	wordhball.position.x=0
	wordhball.position.y=10
	wordhball.position.z=30
	scene.add(wordhball)
	scene.add(wordhbase)
	if(10-mm<6||hbase-10+mm>20){
	wordbad=new TEACHER.ObjTextPlane(30,20,"遜啦","y",0x0f0f0f,0xffffff)
	wordbad.position.x=40
	wordbad.position.y=13
	wordbad.position.z=40
	wordbad.rotation.y=Math.PI/6
	scene.add(wordbad)
	}
	stopall=1
	}
	...
	if(mm===0){
		wordend=new TEACHER.ObjTextPlane(80,20,"You win","y",0x0f0f0f)
		wordend.position.x=0
		wordend.position.y=10
		wordend.position.z=-20
		scene.add(wordend)
		...
		wordbad=new TEACHER.ObjTextPlane(30,20,"遜啦","y",0x0f0f0f,0xffffff)
		wordbad.position.x=40
		wordbad.position.y=10
		wordbad.position.z=40
		wordbad.rotation.y=Math.PI/6
		scene.add(wordbad)
		}
		if(hbase-10+mm<10){
			wordbad=new TEACHER.ObjTextPlane(30,20,"真強","y",0x0f0f0f,0xffffff)
			wordbad.position.x=40
			wordbad.position.y=10
			wordbad.position.z=40
			wordbad.rotation.y=Math.PI/6
...
if(stopall===0){
	...
	if(Math.abs(wb.vx)<0.1&&Math.abs(wb.vz)<0.1&&Math.abs(bb.vx)<0.1&&Math.abs(bb.vz)<0.1){
		...
		ok=1
		if(nn===mm){
	wb.vx=world2D.sl01.value*(-Math.cos(theta))
	wb.vy=0
	wb.vz=world2D.sl01.value*(-Math.sin(theta))}
	}
	...
	function move(){
		...
		if(ok===1){
		if(tt<20&&basev>0){
		base.position.x+=Math.cos(theta)
		base.position.z+=Math.sin(theta)
		tt+=1}
		if(tt>=20){
			basev*=-1
		}
