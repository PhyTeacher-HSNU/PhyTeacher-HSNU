...
var nn=70;var ll=30;var dt=0.1;var oo=0.1;var tt=0;var aa;var arra=[];var hh=5;
var tt=0;var xx=0.5;var dd=0.024;var cc=0.1;var rr=0;var yy=0;var ee=0;var yty=20;var ooo=0.07;var ii=0;var nnn=300;var rrr=[];var long;var pts;var n2=60;
//-------------------開始做我的動畫-------------------//
//-------------------開始做我的動畫-------------------//
//-------------------開始做我的動畫-------------------//
/*任務：
...
//B.定義init
function init(){
	world2D.btn01.setLabel('不吃屎')
	world2D.btn02.setLabel('吃屎')
	world2D.btn01.on('click',ccc)
	world2D.btn02.on('click',ccc)

	wall=new TEACHER.ObjBox(1,30,60,0xffff00)
	wall1=new TEACHER.ObjBox(1,30,60,0xffff00)
	wall1.position.x=-70;
	wall.position.x=68;
	wall1.position.y=15;
	wall.position.y=15;
	scene.add(wall1).add(wall)
	ground.visible=false;world2D.sl02.maximum=3;world2D.sl02.minimum=0;world2D.sl02.value=1;var uuu=new TEACHER.ObjSphere(0.01,0x000000)
for(var i=0;i<nnn;i++){xx+=dd;dd-=0.0001;tt+=oo;yty-=ooo;ooo-=0.00008;
	if(cc<0.25){cc+=0.01};cc=cc+rr;rr=rr+ee;ee+=0.00001;aa=new TEACHER.ObjSphere(xx,0x000000);aa.position.x=cc*0.3*Math.cos(tt)
	;aa.position.z=cc*0.3*Math.sin(tt);rrr.push(aa);uuu.add(aa);if(yty>xx-1){aa.position.y=yty;}else{aa.position.y=xx-1;dd=0};}
//pts=new TEACHER.Points(0x0000ff,1,10000);for(var i=0;i<n2*n2;i++){pts.addPoint(1.2*(i%n2-0.5*n2),0,1.2*(Math.floor(i/n2)-0.5*n2))
	
	for(var i=0;i<nn;i++){
	
aa=new TEACHER.ObjBox(2,1,50,0x00ffff)

aa.position.x=-70+(i*2);
aa.position.y=20;
scene.add(aa);
arra.push(aa)
}
...
world2D.sl01.value=0.1;
	arra[34].add(uuu)
uuu.position.y=-5
	arra[34].add(uuu);//pts.update();scene.add(pts)

//魚
for(var i=0;i<3;i++){
var fish=new THREE.Object3D();
body=new TEACHER.ObjSphere(3,0xff0000)
body.scale.x=3;
eye1=new TEACHER.ObjSphere(1,0x00000f)
eye2=new TEACHER.ObjSphere(1,0x00000f)
eye2.position.x=-6
eye1.position.x=-6
eye1.position.y=2
eye2.position.y=2
...
eye2.position.z=1.5
eye1.position.z=-1.5
tail=new TEACHER.ObjBox(2,2.5,1)
tail1=new TEACHER.ObjBox(2,4,1)
tail2=new TEACHER.ObjBox(2,5,1)
tail3=new TEACHER.ObjBox(3,3,1)
tail.position.x=9;
tail1.position.x=11;
tail2.position.x=13;
tail3.position.y=2.5;
tail3.position.x=1.5;
tail3.rotation.z=Math.PI/5;
fish.add(body).add(eye2).add(eye1).add(tail).add(tail1).add(tail2).add(tail3)
fish.position.z=-7+i*7;
fish.position.x=-30+i*30;
scene.add(fish)
fisharray.push(fish)
fish.vx=-5;
}
...
//C.定義tick                                                                                                                       
function tick(){
	if(world2D.ch01.checked){long=world2D.sl02.value;tt+=dt;dt=world2D.sl01.value;for(var i=0;i<nn;i++){

	var b=arra[i];
b.position.x=-70+i*2;
//var pt=pts.getPoint(i);var ptx=pt.x;var ptz=pt.z;var rr=ptx*ptx+ptz*ptz;
var yy=long*Math.sin(2*Math.PI*b.position.x/ll-2*Math.PI*tt/10);
var imy=-long*Math.cos(2*Math.PI*b.position.x/ll-2*Math.PI*tt/10);
b.position.y=20+imy;
b.position.x=-70+i*2+yy;
//pts.setPoint(i,ptx,yy,ptz)
}
	for(var i=0;i<3;i++){
var ff=fisharray[i]
		if(gg===1){
			...
			ff.rotation.y=0;
ff.position.y=0;
ff.position.x=-30+i*30;
		}else if(gg===2){
var theta=Math.atan2(-arra[34].position.y+ff.position.y,Math.sqrt((arra[34].position.x-ff.position.x)*(arra[34].position.x-ff.position.x)+(arra[34].position.z-ff.position.z)*(arra[34].position.z-ff.position.z)))
var theta1=Math.atan2(-arra[34].position.z+ff.position.z,arra[34].position.x-ff.position.x)
ff.rotation.z=theta;
ff.rotation.y=Math.PI+theta1;
ff.theta=theta;
//ff.theta1=theta1;
ff.vx=5*Math.cos(ff.theta);
ff.vz=-5*Math.sin(ff.theta);
//ff.position.x+=ff.vx*0.1;
//ff.position.z+=ff.vz*0.1;
		}
	}
if(gg===2){
for(var i=0;i<3;i++){
	var fis=fisharray[i];
	var ss=arra[34];
	if(fis.position.y<ss.position.y-3){
fis.position.x=fis.position.x*0.95+ss.position.x*0.05;
fis.position.z=fis.position.z*0.95+ss.position.z*0.05;
fis.position.y=fis.position.y*0.95+ss.position.y*0.05;
	}
}}
}
	//3D
