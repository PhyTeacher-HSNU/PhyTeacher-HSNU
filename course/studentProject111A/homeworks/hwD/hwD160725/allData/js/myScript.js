...
//A.宣告全域變數
var ball1,ball2,ball3,ball2M,center;
var rrA=30;
var rrB=60;
var rrC=120;
var rrBM=5;
...
var orbitA,orbitB,orbitC,orbitBM;
//B.定義init
function init(){
	ball1=new TEACHER.ObjPicSphere(1,pics.mercury);
	ball2=new TEACHER.ObjPicSphere(2,pics.mars);
	ball3=new TEACHER.ObjPicSphere(2,pics.uranus);
	ball2M=new TEACHER.ObjPicSphere(0.5,pics.moon);
	center=new TEACHER.ObjPicSphere(10,pics.sun);
	one=new TEACHER.ObjPicPlane(500,500,pics.space,"x");
	two=new TEACHER.ObjPicPlane(500,500,pics.space,"x",1);
	three=new TEACHER.ObjPicPlane(500,500,pics.space,"y");
	four=new TEACHER.ObjPicPlane(500,500,pics.space,"y",1);
	five=new TEACHER.ObjPicPlane(500,500,pics.space,"z");
	six=new TEACHER.ObjPicPlane(500,500,pics.space,"z",1);
	scene.add(ball1).add(ball2).add(ball3).add(ball2M).add(center).add(one).add(two).add(three).add(four).add(five).add(six);

	orbitA=new TEACHER.Line(0xffffff);
	orbitB=new TEACHER.Line(0xffffff);
	orbitC=new TEACHER.Line(0xffffff);
	orbitBM=new TEACHER.Line(0xffffff);
	scene.add(orbitA).add(orbitB).add(orbitC).add(orbitBM);

	text1=new TEACHER.ObjTextPlane(160,20,"1607陳愉","z",0xffffff);
	text2=new TEACHER.ObjTextPlane(160,20,"做動畫學物理   師大附中","z",0xffffff);
	scene.add(text1).add(text2);
	...
	ball1.position.x=30;
	ball2.position.x=60;
	ball3.position.x=120;
	ball2M.position.x=65;

	world2D.sl01.setLabel("行星A軌道半徑");
	world2D.sl01.minimum=20;
	world2D.sl01.maximum=40;
	world2D.sl01.value=30;

	world2D.sl02.setLabel("行星B角速度");
	world2D.sl02.minimum=-0.5;
	world2D.sl02.maximum=0.5;
	world2D.sl02.value=0.1;
	world2D.sl02.digitN=1;

	world2D.ch01.setLabel("顯示行星軌道");
	world2D.ch02.setLabel("顯示衛星軌道");

	world2D.btn01.setLabel("開始");
    world2D.btn02.setLabel("暫停");
	world2D.btn03.setLabel("聚焦恆星");
	world2D.btn04.setLabel("聚焦行星A");
	world2D.btn05.setLabel("聚焦行星B");
	world2D.btn06.setLabel("聚焦行星C");
	...
	world3D.cameraPhi=1.75*Math.PI;
}

//C.定義tick                                                                                                                       
function tick(){
	orbitA.addPoint(ball1.position.x,ball1.position.y,ball1.position.z);
	orbitB.addPoint(ball2.position.x,ball2.position.y,ball2.position.z);
	orbitC.addPoint(ball3.position.x,ball3.position.y,ball3.position.z);
	orbitBM.addPoint(ball2M.position.x,ball2M.position.y,ball2M.position.z);

	orbitA.visible=orbitB.visible=orbitC.visible=world2D.ch01.checked;
	orbitBM.visible=world2D.ch02.checked;

	if(gamemode===1){
		ball1.position.x=rrA*Math.cos(thetaA);
	    ball1.position.z=rrA*Math.sin(thetaA);
		ball2.position.x=rrB*Math.cos(thetaB);
	    ball2.position.z=rrB*Math.sin(thetaB);
		ball3.position.x=rrC*Math.cos(thetaC);
		ball3.position.y=rrC*Math.sin(thetaC);
	    ball3.position.z=rrC*Math.sin(thetaC);
		ball2M.position.x=rrBM*Math.cos(thetaBM)+rrB*Math.cos(thetaB);
	    ball2M.position.z=rrBM*Math.sin(thetaBM)+rrB*Math.sin(thetaB);
		thetaA+=omegaA*dt;
		thetaB+=omegaB*dt;
		thetaC+=omegaC*dt;
		thetaBM+=omegaBM*dt;
	...
	if(targetplanet===0){
		world3D.cameraTarget.x=world3D.cameraTarget.y=world3D.cameraTarget.z=0;
	}
	if(targetplanet===1){
		world3D.cameraTarget.x=ball1.position.x;
		world3D.cameraTarget.y=ball1.position.y;
		world3D.cameraTarget.z=ball1.position.z;
		world3D.cameraPhi=-thetaA;
	}
	if(targetplanet===2){
		world3D.cameraTarget.x=ball2.position.x;
		world3D.cameraTarget.y=ball2.position.y;
		world3D.cameraTarget.z=ball2.position.z;
		world3D.cameraPhi=-thetaB-Math.PI;
	}
	if(targetplanet===3){
		world3D.cameraTarget.x=ball3.position.x;
		world3D.cameraTarget.y=ball3.position.y;
		world3D.cameraTarget.z=ball3.position.z;
		world3D.cameraPhi=-thetaC-Math.PI;
	}
	omegaB=world2D.sl02.value;
	rrA=world2D.sl01.value;
