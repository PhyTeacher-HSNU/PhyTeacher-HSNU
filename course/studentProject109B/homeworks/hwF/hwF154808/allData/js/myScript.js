...
function init(){
	...
	wall=new TEACHER.ObjBox(100,10,1,0x2033a1,2);//牆壁
	scene.add(wall);
	wall.position.z=-51;
	wall=new TEACHER.ObjBox(100,10,1,0x2033a1,2);
	scene.add(wall);
	wall.position.z=50;
	wall=new TEACHER.ObjBox(1,10,100,0x2033a1,"x");
	scene.add(wall);
	wall.position.x=50;
	wall=new TEACHER.ObjBox(1,10,100,0x2033a1,"x");
	scene.add(wall);
	...
	arrow=new TEACHER.ObjArrow(1,0x1c3571);
	scene.add(arrow);




	ball = new TEACHER.ObjSphere(3,0x6ce435);
	ball.vx=0;
	ball.vz=0;
	ball.vy=0;
	ball.ax=0;
	...
	bar = new TEACHER.ObjCylinder(0.2,rr,0x1c3571,true,"x");
	center = new TEACHER.ObjCylinder(1,4,0x00ffff);
	bar.position.y=center.position.y=ball.position.y=2;
	bar.position.x=center.position.x=ball.position.x=0;
	bar.position.z=center.position.z=ball.position.z=-45;
	scene.add(ball)


	for(var i=0; i<nn; i++){
		var ob=new TEACHER.ObjCylinder(0.8,3,0x2033a1,false,"y")

		ob.position.y=2;
		ob.position.x=-50+(i%9)*10.5+8;
		ob.position.z=-45+Math.floor(i/10)*11.5+20;
		ob.vx=2;
		ob.vz=2;
		ob.vy=2;
		scene.add(ob);
	...
	for(var i=0; i<9;i++){
		var ab=new TEACHER.ObjBox(1,3,20,0x2033a1);
		ab.position.y=2;
		ab.position.x=-40+i*10;
		ab.position.z=40;
		ab.vx=2;
		ab.vz=2;
		ab.vy=2;
	...
	pp=new TEACHER.ObjTextPlane(100,10,"154808 吳坤澔","y",0xffffff);//字
	scene.add(pp);
	pp.position.z=-65;
	pp.position.x=0;


	rule=new TEACHER.ObjTextPlane(50,10,'規則',"y",0x0f0e0e,0xffffff)
	scene.add(rule);
	rule.position.z=-30;
	rule.position.x=-80;
	rule.position.y=15;

	rule=new TEACHER.ObjTextPlane(50,8,'左右箭頭控制發射方向',"y",0x0f0e0e,0xffffff)
	scene.add(rule);
	rule.position.z=-20;
	rule.position.x=-80;
	rule.position.y=15;

	rule=new TEACHER.ObjTextPlane(50,8,'右上可以調碰撞的力道',"y",0x0f0e0e,0xffffff)
	scene.add(rule);
	rule.position.z=-10;
	rule.position.x=-80;
	rule.position.y=15;

	rule=new TEACHER.ObjTextPlane(50,4.2,'不同的地點有不同的對應分數',"y",0x0f0e0e,0xffffff)
	scene.add(rule);
	rule.position.z=0;
	rule.position.x=-80;
	rule.position.y=15;
	...
	three=new TEACHER.ObjPicPlane(8,8,pics.three,"y",2)
	scene.add(three);
	three.position.x=-45;
	three.position.z=35;
	three.position.y=5;
	...
	three2=new TEACHER.ObjPicPlane(8,8,pics.three,"y",2)
	scene.add(three2);
	three2.position.x=45;
	three2.position.z=35;
	three2.position.y=5;
	
	three3=new TEACHER.ObjPicPlane(8,8,pics.three,"y",2)
	scene.add(three3);
	three3.position.x=5;
	three3.position.z=35;
	three3.position.y=5;

	three4=new TEACHER.ObjPicPlane(8,8,pics.three,"y",2)
	scene.add(three4);
	three4.position.x=-5;
	three4.position.z=35;
	three4.position.y=5;
	

	two=new TEACHER.ObjPicPlane(8,8,pics.two,"y",2)
	scene.add(two);
	two.position.x=-15;
	two.position.z=35;
	two.position.y=5;
	...
	two2=new TEACHER.ObjPicPlane(8,8,pics.two,"y",2)
	scene.add(two2);
	two2.position.x=15;
	two2.position.z=35;
	two2.position.y=5;
	...
	one=new TEACHER.ObjPicPlane(8,8,pics.one,"y",2)
	scene.add(one);
	one.position.x=25;
	one.position.z=35;
	one.position.y=5;

	 one2=new TEACHER.ObjPicPlane(8,8,pics.one,"y",2)
	scene.add(one2);
	one2.position.x=35;
	one2.position.z=35;
	one2.position.y=5;
	
	 one3=new TEACHER.ObjPicPlane(8,8,pics.one,"y",2)
	scene.add(one3);
	one3.position.x=-25;
	one3.position.z=35;
	one3.position.y=5;
	
	one4=new TEACHER.ObjPicPlane(8,8,pics.one,"y",2)
	scene.add(one4);
	one4.position.x=-35;
	one4.position.z=35;
	one4.position.y=5;
	...
	point=new TEACHER.ObjTextPlane(60,20,"分數",'y');
	point.position.z=-50;
	point.position.x=-70;
	point.position.y=30;
	scene.add(point);
	score=new TEACHER.ObjTextPlane(60,20," ",'y')
	score.position.z=-45;
	score.position.x=-75;
	score.position.y=15;
	scene.add(score);
	...
	world2D.sl01.maximum=2.5;
	world2D.sl01.value=2;
	world2D.sl01.digitN=1;;
	
	
	world2D.slCameraRR.value=170;          //視角
	world3D.cameraTheta=0.252859984112609;
...
function tick(){
	...
	if(gameMD===0){
	ball.vx=Math.cos(theta)*ratio;
	ball.vz=Math.sin(theta)*ratio;
	}
	...
	if(ball.position.x>50-2 && ball.vx>0){
		ball.vx=-1*ratio
	}else if(ball.position.x<-50+2 && ball.vx<0){
		ball.vx=-1*ratio
	}
	if(ball.position.z>50-2&& ball.vz>0){
		ball.vz=-1*ratio
	}else if(ball.position.z<-50+2 && ball.vz<0){
		ball.vz=-1*ratio
	}
	
	
	if(one.position.x-10<ball.position.x && ball.position.x<one.position.x+10 && one.position.z-10<ball.position.z && ball.position.x<one.position.z+10){
	...
	if(one2.position.x-10<ball.position.x && ball.position.x<one2.position.x+10 && one2.position.z-10<ball.position.z && ball.position.x<one2.position.z+10){
	...
	if(one3.position.x-10<ball.position.x && ball.position.x<one3.position.x+10 && one3.position.z-10<ball.position.z && ball.position.x<one3.position.z+10){
	...
	if(one4.position.x-10<ball.position.x && ball.position.x<one4.position.x+10 && one4.position.z-10<ball.position.z && ball.position.x<one4.position.z+10){
	...
	if(three.position.x-10<ball.position.x && ball.position.x<three.position.x+10 && three.position.z-10<ball.position.z && ball.position.x<three.position.z+10){
	...
	if(three2.position.x-11<ball.position.x && ball.position.x<three2.position.x+11 && three2.position.z-11<ball.position.z && ball.position.x<three2.position.z+11){
	...
	if(three3.position.x-10<ball.position.x && ball.position.x<three3.position.x+10 && three3.position.z-10<ball.position.z && ball.position.x<three3.position.z+10){
	...
	if(three4.position.x-10<ball.position.x && ball.position.x<three4.position.x+10 && three4.position.z-10<ball.position.z && ball.position.x<three4.position.z+10){
	...
	if(two.position.x-10<ball.position.x && ball.position.x<two.position.x+10 && two.position.z-10<ball.position.z && ball.position.x<two.position.z+10){
	...
	if(two2.position.x-10<ball.position.x && ball.position.x<two2.position.x+10 && two2.position.z-8<ball.position.z && ball.position.x<two2.position.z+10){
		...
		arrow.setArrow(ball.vx*4,ball.vy*4,ball.vz*4);
...
function collision(ballA,ballB,dis){
	...
	var r=Math.sqrt(r12x*r12x+r12y*r12y+r12z*r12z);
	...
	if(r<dis && (v1cx*r12x+v1cy*r12y+v1cz*r12z)<0){
