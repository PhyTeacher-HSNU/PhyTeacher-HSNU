...
//B.定義init
function init(){
	world2D.slCameraRR.value=190;

	sky = new TEACHER.ObjPicPlane(10000,10000,pics.wallU,'y',1);

	ground.visible = false;
	gd = new TEACHER.ObjPicPlane(150,150,pics.snow,'y');
	scene.add(gd);

	pc = new TEACHER.ObjTextPlane(100,13,'滾動雪球','z');
    scene.add(pc);
	pc.position.z=75;
	pc.position.y=-13;

	pp = new TEACHER.ObjTextPlane(100,13,'大安機械科甲班 林恩廷','z');
    scene.add(pp);
	pp.position.z=-75;
	pp.position.y=20;

	ball = new TEACHER.ObjPicSphere(rr,pics.snow2)
	scene.add(ball);
...
function tick(){
	...
	ball.position.x += ball.ax*dt;
	ball.position.y += ball.ay*dt;
	ball.position.z += ball.az*dt;
...
//ball.scale+=0.5
	...
	if(ball.ax>0){
		...
		ball.position.y=rr*(ball.scale.x=ball.scale.y=ball.scale.z +=0.001);
		
	}if(ball.ax<0){
		ball.rotation.z +=dx/rr;	
		ball.position.y=rr*(ball.scale.x=ball.scale.y=ball.scale.z +=0.005);
