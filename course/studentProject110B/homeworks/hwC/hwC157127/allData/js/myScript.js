...
function init(){
	...
	name1 = new TEACHER.ObjTextPlane(100,20,"1571陳睿杰","z");
	scene.add(name1);
	name1.position.z = -50;
	name1.position.y = 40;
	for(var i=0;i<n;i++){
		var ball = new TEACHER.ObjSphere(2,"#00BFFF",2);
		ball.position.x = -45+Math.floor(i/10)*10;
		ball.position.z = -45+i%10*10;
		ball.position.y = r;
		...
		ball.ay=0;
		scene.add(ball);
		blueballArray.push(ball);

		var arrow = new TEACHER.ObjArrow(0.4,0x00ff00);
		arrow.visible = false;
		scene.add(arrow);
		bluearrowArray.push(arrow);
	} 

	for(var i=0;i<n;i++){
		var ball = new TEACHER.ObjSphere(2,"#00FF00",2);
		ball.position.x = 0+Math.floor(i/10)*10;
		ball.position.z = -50+i%10*10;
		ball.position.y = r;
		ball.vx=5*(Math.random()-0.5);
		ball.vz=5*(Math.random()-0.5);
		ball.vy=5*(Math.random()-0.5);
		...
		ball.visible=false;
		scene.add(ball);
		greenballArray.push(ball);

		var arrow = new TEACHER.ObjArrow(0.4,"#FF4500");
		arrow.visible = false;
		scene.add(arrow);
		greenarrowArray.push(arrow);
	} 
	for(var i=0;i<n;i++){
		var ball = new TEACHER.ObjSphere(2,"#DC143C",2);
		ball.position.x = 45-Math.floor(i/10)*10;
		ball.position.z = 45-i%10*10;
		ball.position.y = 2;
		ball.vx=5*(Math.random()-0.5);
	...
	world2D.sl01.setLabel("球速度");
	world2D.sl02.maximum = 3;
	...
	world2D.sl03.setLabel("恢復係數");
	world2D.btnUp.visible = world2D.btnRight.visible = world2D.btnLeft.visible = world2D.btnDown.visible = world2D.btn01.visible = world2D.btn02.visible = false;
	world2D.btn01.on("click",toDo);
	world2D.ch01.setLabel("加入綠球");
	world2D.ch02.setLabel("箭頭");
...
//C.定義tick                                                                                                                       
function tick(){
	var v = world2D.sl01.value;
	var s = world2D.sl02.value;
	var ration = world2D.sl03.value;
	if(world2D.ch02.checked){
		for(var i=0;i<n;i++){
			var arrow=bluearrowArray[i];
				arrow.visible = true;
			var arrow1=greenarrowArray[i];
				arrow1.visible = true;
			var arrow2=arrowArray[i];
				arrow2.visible = true;
			}
	}
	else{
	...
	for(var i=0;i<n;i++){
		...
		ball.vy += ball.ay*dt;
		ball.vz += ball.az*dt;
		ball.ax = ball.ay = ball.az = v;

		ball.scale.x = ball.scale.y = ball.scale.z = s;
		
		if(ball.position.x>50-r*s&&ball.vx>0){
			ball.vx *=-1*ration;
		}
		else if(ball.position.x<-50+r*s&&ball.vx<0){
			ball.vx *=-1*ration;
		}
		if(ball.position.z>50-r*s&&ball.vz>0){
			ball.vz *=-1*ration;
		}
		else if(ball.position.z<-50+r*s&&ball.vz<0){
			ball.vz *=-1*ration;
		}
		if(ball.position.y>50-r*s&&ball.vy>0){
			ball.vy *=-1*ration;
		}
		else if(ball.position.y<0+r*s&&ball.vy<0){
			ball.vy *=-1*ration;
		}
		...
		arrow.scale.x = arrow.scale.y = arrow.scale.z = s;
		arrow.setArrow(ball.vx*2,ball.vy*2,ball.vz*2);
	}
	if(world2D.ch01.checked){
	for(var i=0;i<n;i++){
		var ball=greenballArray[i];
		ball.visible = true;
	...
	for(var i=0;i<(n-1);i++){
		for(var j=i+1;j<n;j++){
			var ball = blueballArray[i];
			var ball2 = blueballArray[j];
			collision(ball,ball2,4*s);
		}
	}
	for(var i=0;i<(n-1);i++){
		for(var j=i+1;j<n;j++){
			var ball = redballArray[i];
			var ball2 = redballArray[j];
			collision(ball,ball2,4*s);
		}
	}
	for(var i=0;i<(n-1);i++){
