...
function init(){
	...
	//姓名
	{
		n = new TEACHER.ObjTextPlane(50,8,"156820 陳米姍",'z');
		scene.add(n);
		n.position.x=-30;
		n.position.z=-50;
		n.position.y=15;
	...
	//毛線球
	{
		b = new TEACHER.ObjPicSphere(rr, pics.ball);
		scene.add(b);
		b.position.y=rr;
	}
	...
	world2D.ch01.setLabel('不顯示箭頭');
	world2D.ch02.visible = world2D.sl03.visible = false;

	//b的大小
	world2D.sl02.setLabel('毛線球半徑');
	world2D.sl02.minimum=1;
	world2D.sl02.maximum=10;
	world2D.sl02.value=rr=3;
...
function tick(){
	...
	b.scale.x=b.scale.y=b.scale.z=rr;
	b.position.y=rr;
	...
	arrowV.position.y = arrowA.position.y = con.position.y;
	arrowV.position.z = arrowA.position.z = con.position.z;

	
	arrowV.setArrow(con.vx , con.vy , con.vz);
	arrowA.setArrow(con.ax*5 , con.ay*5 , con.az*5);
...
function aa(player1, player2, dis){
	...
	if(rrr<dis){
		k+=1;
		b.position.x=-50+100*Math.random();
