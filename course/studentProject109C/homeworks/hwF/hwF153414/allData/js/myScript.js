...
function init(){
	...
	var text5=new TEACHER.ObjTextPlane(280,40,"153414孫堃展",'z',0xffffff);
	text5.position.x=0;
	text5.position.y=35;
	text5.position.z=-50;
	scene.add(text5);
	...
	ball1=new TEACHER.ObjSphere(rr,0xff0000,2);
	ball2=new TEACHER.ObjSphere(rr,0x00ffff,2);
	ball4=new TEACHER.ObjSphere(rr*Math.pow(3,1/3),0x00ff00,2);

	scene.add(ball1).add(ball2).add(ball4);
	ball1.position.x=0;
	ball1.position.z=-30;
	ball2.position.x=-50;
	ball2.position.z=20;
	...
	ball3=new TEACHER.ObjSphere(3,0xffffff,2);
	scene.add(ball3)
	ball5=new TEACHER.ObjSphere(3,0xffffff,2);
	scene.add(ball5)
	...
	wall1=new TEACHER.ObjBox(2,10,10,0x00ffff,2);
	wall2=new TEACHER.ObjBox(10,10,2,0x00ffff,2);
	wall3=new TEACHER.ObjBox(10,2,10,0x00ffff,2);

	scene.add(wall1).add(wall2).add(wall3);
	wall1.position.x=60;
	wall1.position.z=-30;
	...
	spring1=new TEACHER.ObjSpring(50,0.8*rr,0.5,8,0xffff00);
	spring2=new TEACHER.ObjSpring(50,0.8*rr,0.5,8,0xffff00);
	spring3=new TEACHER.ObjSpring(50,0.8*rr,0.5,8,0xffff00);

	scene.add(spring1).add(spring2).add(spring3);
	spring1.position.x=49;
	spring1.position.z=-30;

	spring2.position.x=-50;
	spring2.position.z=79;

	spring3.position.x=-50;
	spring3.position.z=-30;
	spring3.position.y=1;


	line1=new TEACHER.Line(0xffff00,1.5,3000);
	scene.add(line1);
	line2=new TEACHER.Line(0xffff00,1.5,3000);
	scene.add(line2);
	...
	world2D.sl04.setLabel("紅球質量");
	world2D.sl04.maximum=5;
	world2D.sl04.minimum=1;
	world2D.sl04.value=3;


	world2D.sl01.setLabel("藍球質量");
	world2D.sl01.maximum=5;
	world2D.sl01.minimum=1;
	world2D.sl01.value=3;

	world2D.sl02.setLabel("彈力常數(上)");
	world2D.sl02.maximum=5;
	world2D.sl02.minimum=1;
	world2D.sl02.value=3;

	world2D.sl03.setLabel("彈力常數(左)");
...
function tick(){
	...
	for(var i = 0;i<1;i++){
		if(start===1){
			...
			tt+=dt
			r=Math.abs(Math.sin(tt))*0.5+0.4
			g=Math.abs(Math.cos(tt))*0.5+0.4
			b=Math.abs(Math.cos(tt+0.5))*0.5+0.4
			ball3.mat.color.setRGB(r,g,b)
			ball5.mat.color.setRGB(r,g,b)

			mm1=world2D.sl04.value;
			mm2=world2D.sl01.value;
			kk1=world2D.sl02.value;
			kk2=world2D.sl03.value;
	
	
			ball1.scale.x=Math.pow(world2D.sl04.value,1/3);
			ball1.scale.y=Math.pow(world2D.sl04.value,1/3);
			ball1.scale.z=Math.pow(world2D.sl04.value,1/3);
	
			ball1.ax = -kk1/mm1*ball1.position.x;
			ball1.vx += ball1.ax*dt;
			ball1.position.x += ball1.vx*dt;
	
			ball2.scale.x=Math.pow(world2D.sl01.value,1/3);
			ball2.scale.y=Math.pow(world2D.sl01.value,1/3);
			ball2.scale.z=Math.pow(world2D.sl01.value,1/3);
	
			ball2.az = -kk2/mm2*(ball2.position.z-20);
			ball2.vz += ball2.az*dt;
			ball2.position.z += ball2.vz*dt;
	
			ball4.ay = -3/3*(ball4.position.y-50);
			ball4.vy += ball4.ay*dt;
			ball4.position.y += ball4.vy*dt;
			
			spring1.position.x=59;
			spring1.position.z=-30;
			...
			if(world2D.ch02.checked){
				...
				ball4.visible=spring3.visible=wall3.visible=ball5.visible=line2.visible=true;
				ball3.visible=line1.visible=false;
				world3D.cameraTarget.y=50;
				world3D.cameraTarget.x=0;
				world3D.cameraTarget.z=20;

				ball5.position.x=ball1.position.x;
				ball5.position.z=ball2.position.z;
				ball5.position.y=ball4.position.y;
				line2.visible=world2D.ch02.checked;
				line2.addPoint(ball1.position.x,ball4.position.y,ball2.position.z)
				line2.update();
			}else {

				ball4.visible=spring3.visible=wall3.visible=ball5.visible=line2.visible=false;
				ball3.visible=line1.visible=true;
				world3D.cameraTarget.y=0;
				world3D.cameraTarget.x=0;
				world3D.cameraTarget.z=20;

				ball3.position.x=ball1.position.x;
				ball3.position.z=ball2.position.z;
				ball3.position.y=-10;
				line1.position.y=-10;
				line1.addPoint(ball1.position.x,0,ball2.position.z)
				line1.update();
			}
		}
		if(step===1){
		...
		spring1.setSpring(ball1.position.x-wall1.position.x+rr*Math.pow(world2D.sl04.value,1/3),0,0)
		spring2.setSpring(0,0,ball2.position.z-wall2.position.z+rr*Math.pow(world2D.sl01.value,1/3))
		spring3.setSpring(0,ball4.position.y-wall3.position.y-3,0)
