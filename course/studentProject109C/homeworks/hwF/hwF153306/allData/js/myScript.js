...
//B.定義init
function init(){
	aa=new TEACHER.ObjSphere(5,0xff0000)
	bb=new TEACHER.ObjSphere(5,0xff0000)
	aw=new TEACHER.ObjSpring(20,3.6,0.5,10,0x7f7f7f)
	bw=new TEACHER.ObjSpring(20,3.6,0.5,10,0x7f7f7f)
	ab=new TEACHER.ObjSpring(20,3.6,0.5,10,0x7f7f7f)
	a_w=new TEACHER.ObjBox(2,18,18,0xffff00)
	b_w=new TEACHER.ObjBox(2,18,18,0xffff00)
	ground.position.y=-9
	a_w.position.x=-41
	b_w.position.x=41
	aa.position.x=-15
	...
	aw.position.x=-40
	ab.position.x=aa.position.x+5
	bw.position.x=bb.position.x+5
	aw.setSpring(aa.position.x+35,0,0)
	ab.setSpring(bb.position.x-aa.position.x-10,0,0)
	bw.setSpring(35-bb.position.x,0,0)

	world2D.sl01.maximum=50;
	world2D.sl01.setLabel('k1')
	world2D.sl02.maximum=50;
	world2D.sl02.setLabel('k2')
	world2D.sl03.maximum=50;
	world2D.sl03.setLabel('k3')

	world2D.sl01.value=kk;
	world2D.sl02.value=kk;
	world2D.sl03.value=kk;

	world2D.btn01.setLabel("Run");
	world2D.btn01.on('click',btn01_on_20210618);
	world2D.btn02.setLabel("調三彈簧的k");
	world2D.btn02.on('click',btn02_on_20210618);

	scene.add(aa).add(bb).add(aw).add(bw).add(ab).add(a_w).add(b_w)
...
function tick(){
	if(run_){
		var aw_f=(aa.position.x+15)*aw_k;
		var ab_f=(bb.position.x-aa.position.x-30)*ab_k;
		var bw_f=(15-bb.position.x)*bw_k;
		va=va+(ab_f-aw_f)*dt/ma;
		vb=vb+(bw_f-ab_f)*dt/mb;
		aa.position.x+=va*dt;
		bb.position.x+=vb*dt;
	...
	if(crmxlewsr){
		//world2D.btn01.setLabel("調其他");
		ma=world2D.sl01.value;
		mb=world2D.sl02.value;
	}else{
		//world2D.btn01.setLabel("調三彈簧的k");
		aw_k=world2D.sl01.value;
		ab_k=world2D.sl02.value;
		bw_k=world2D.sl03.value;
	}
	//3D
	world3D.render();
	//2D
...
function btn01_on_20210618(){
	if(run_){
		run_=false;
		world2D.btn01.setLabel("Run");
	}else{
		run_=true;
		world2D.btn01.setLabel("stop");
	}
}

function btn02_on_20210618(){
	if(crmxlewsr){
		crmxlewsr=false;
		world2D.btn02.setLabel("調三彈簧的k");
		...
		world2D.sl01.maximum=50;
		world2D.sl01.setLabel('k1');
		world2D.sl01.value=aw_k;
		world2D.sl02.maximum=50;
		world2D.sl02.setLabel('k2');
		world2D.sl02.value=ab_k;
		world2D.sl03.visible=true;
		world2D.sl03.maximum=50;
		world2D.sl03.setLabel('k3');
		world2D.sl03.value=bw_k;
	}else{
		crmxlewsr=true;
		world2D.btn02.setLabel("調其他");

		world2D.sl01.maximum=5;
		world2D.sl01.setLabel('m1');
		world2D.sl01.value=ma;
		world2D.sl02.maximum=5;
		world2D.sl02.setLabel('m2');
		world2D.sl02.value=mb;
