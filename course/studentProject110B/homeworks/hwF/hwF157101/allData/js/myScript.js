...
function init(){
	...
	for( var i=0 ; i<nn ;i++ ){
		var ball=new TEACHER.ObjSphere(0.2,0x010101)
		ball.position.x=0;
		ball.position.z=0;
		ball.position.y=22;
		scene.add(ball);
		ballarray1.push(ball);
	}
	for( var i=0 ; i<4 ; i++){
		var tire=new TEACHER.ObjPicCylinder(5,0.1,pics.tire,false,"x")
		tire.position.x=-10+i%2*20;
		tire.position.z=-5+(i-i%2)/2*10;
		tire.position.y=5;
		car.add(tire);
		tirearray.push(tire);
	}
	aa=new TEACHER.ObjBox(20,8,10,0x0000FF);
	aa.position.y=8.5
	car.add(aa)
	bb=new TEACHER.ObjBox(10,8,10,0x0000ff);
	bb.position.y=16.5;
	car.add(bb);
	scene.add(car);
	...
	cc=new TEACHER.ObjTextPlane(40,10,"都卜勒效應","z",0x010101)
	cc.position.z=-50;
	cc.position.y=30;
	scene.add(cc)
...
//C.定義tick                                                                                                                       
function tick(){
	car.visible=world2D.ch01.checked;
	rr=world2D.sl01.value*0.1;
	xx=world2D.sl02.value;
	dd=world2D.sl03.value;
	car.scale.x=car.scale.z=car.scale.y=dd/5
	for( var i=0 ; i<nn ;i++ ){
		var ball=ballarray1[i];
		ball.position.x+=Math.sin(i*2*Math.PI/200)*rr*dt;
		ball.position.z+=Math.cos(i*2*Math.PI/200)*rr*dt;
		if (Math.abs(ball.position.x)>50 || Math.abs(ball.position.z)>50 ){
			ball.visible=false;
		}
	}
	car.position.y+=car.vy*dt;
	car.position.x+=car.vx*dt;
	car.position.z+=car.vz*dt;
	car.vx=vv*Math.cos(car.rotation.y);
	car.vz=vv*Math.sin(-car.rotation.y);

	for( var i=0 ; i<4 ; i++){
		var tire=tirearray[i];
		tire.rotation.z-=vv/10/dd
	}
...
function pp(){
	...
	for( var i=0 ; i<200 ;i++ ){
		var ball=new TEACHER.ObjSphere(0.2,0x010101)
		ball.position.x=car.position.x;
		ball.position.z=car.position.z;
		ball.position.y=yy;
		scene.add(ball);
		ballarray1.push(ball);
