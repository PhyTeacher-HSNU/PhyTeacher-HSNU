...
function init(){
	...
	Class = new TEACHER.ObjTextPlane(70,10,"1552 07 周大禾","z",0x00f0ff,0x00000f);
	scene.add(Class);
	Class.position.z=-50;
	Class.position.y=20;
	...
	wall1=new TEACHER.ObjBox(100,8,4,0x00ff00,"z",2);
	wall.add(wall1);
	wall1.position.z=-52;
	wall1.position.y=4;

	wall2=new TEACHER.ObjBox(100,8,4,0x00ff00,"z",2);
	wall.add(wall2);
	wall2.position.z=52;
	wall2.position.y=4;

	wall3=new TEACHER.ObjBox(4,8,100,0x00ff00,"x",2);
	wall.add(wall3);
	wall3.position.x=-52;
	wall3.position.y=4;

	wall4=new TEACHER.ObjBox(4,8,100,0x00ff00,"x",2);
	wall.add(wall4);
	wall4.position.x=52;
	wall4.position.y=4;
	...
	ball=new TEACHER.ObjSphere(r,0xff0000,0);
	scene.add(ball);
	ball.position.y=r;

	ball0=new TEACHER.ObjSphere(r,0xffffff,0);
	scene.add(ball0);
	ball0.visible=false;
	...
	world2D.ch01.setLabel("牆壁顯示");

	world2D.ch02.setLabel("球靜止且回到中心");

	world2D.btn01.setLabel("自由落體");

	world2D.btn02.setLabel("恢復紅球");
...
function tick(){
	...
	if(ball.position.x>wall4.position.x-2-r && ball.vx>0){
		ball.vx*=-1*ratio;
	}else if(ball.position.x<wall3.position.x+2+r && ball.vx<0){
		ball.vx*=-1*ratio;
	}
	if(ball.position.z>wall2.position.z-2-r && ball.vz>0){
		ball.vz*=-1*ratio;
	}else if(ball.position.z<wall1.position.z+2+r && ball.vz<0){
		ball.vz*=-1*ratio;
	}
	if(ball0.position.y<0+r){
		ball0.vy*=-1.03*ratio;
	}
	if(ball0.position.y<0-r){
		ball0.vy=ball0.ay=0;
	...
	arrowV.setArrow(ball.vx*r/3,ball.vy*r/3,ball.vz*r/3);
	arrowA.setArrow(ball.ax*4*r,ball.ay*4*r,ball.az*4*r);

	ball.scale.x=ball.scale.y=ball.scale.z=world2D.sl01.value*ball.scale.x/r;
	...
	if(world2D.ch02.checked){
		if(ball.position.x==0&&ball.position.z==0){
			ball.vx=ball.vy=ball.vz=0;
		}else{
			ball.vx=0-ball.position.x/2;
			ball.vz=0-ball.position.z/2;
		}
	}
...
function clickbtn(e){
	...
	if(str==="btnUp"){
		...
		ball0.position.y=50;
		ball0.visible=true;
		ball.visible=arrowA.visible=arrowV.visible=false;
		ball0.ay=-9.8;
	}else if(str==="btn02"){
		ball0.position.y=2;
		ball0.ay=ball0.vy=0;
		ball0.visible=false;
		ball.visible=arrowA.visible=arrowV.visible=true;
	}
}
...
function wall_pos_sca(e){
	wall1.scale.x*=world2D.sl03.value/dist;
	wall1.position.z=-dist-2;
	wall2.scale.x*=world2D.sl03.value/dist;
	wall2.position.z=dist+2;
	wall3.scale.z*=world2D.sl03.value/dist;
	wall3.position.x=-dist-2;
	wall4.scale.z*=world2D.sl03.value/dist;
	wall4.position.x=dist+2;
	dist=world2D.sl03.value;
}
