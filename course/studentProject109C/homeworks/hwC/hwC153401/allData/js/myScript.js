...
function init(){
  ...
  xx=new TEACHER.ObjPlane(100,50,0x000001,"x",2)
  xx.position.y=25
  xx.position.x=50
  yy=new TEACHER.ObjPlane(100,50,0xccccff,"z",2)
  yy.position.y=25
  yy.position.z=-50
  zz=new TEACHER.ObjPlane(100,50,0x000001,"x",2)
  zz.position.y=25
  zz.position.x=-50
  scene.add(xx)
  scene.add(yy)
  scene.add(zz)
  vv=new TEACHER.ObjPlane(100,100,0x000011,"y",1)
  vv.position.y=50
  scene.add(vv)
	...
	for(var i=0;i<100;i++){
		var bb= new TEACHER.ObjSphere(rr,0x990099)

		scene.add(bb)
		bb.position.x=-24.5+(i%10)*5
		bb.position.z=-24.5+Math.floor(i/10)*5
		
		tempArray.push(bb)
		bb.vz=1*(Math.random()-0.5)
		bb.vx=1*(Math.random()-0.5)
	    bb.vy=1*(Math.random()-0.5)


	var aa=new TEACHER.ObjArrow(0.6,0xff9900)
	scene.add(aa)
	
  arrowArray.push(aa)
  aa.position.x=bb.position.x
  aa.position.z=bb.position.z
  aa.position.y=bb.position.y+2
  bb.position.y=rr 
}
//arrowArray.push()
	...
	i>[(i-1)*i]/2
//btn
	world2D.btn01.setLabel("請按下面")
	world2D.btn02.setLabel("的按鈕")
    tt=0
    uu=0
	...
	oo=new TEACHER.ObjPicCylinder(15,30,pics.me,true,"y",2)
	scene.add(oo)
	oo.position.y=65

	//大球
/*	ss=10
	bbw=new TEACHER.ObjSphere(ss,0xff6688)
	bbw.position.y=25
	scene.add(bbw)*/
...
if(bb.position.x>50-rr && bb.vx>0){
	bb.vx=-bb.vx 
    }else if(bb.position.x<-50+rr+uu && bb.vx<0){
	bb.vx=-bb.vx }
	if(bb.position.z>50-rr && bb.vz>0){
		bb.vz=-bb.vz  
    }else if(bb.position.z<-50+rr+tt && bb.vz<0){
	bb.vz=-bb.vz}
	if(bb.position.y>50-rr && bb.vy>0){
		bb.vy=-bb.vy  
    }else if(bb.position.y<rr && bb.vy<0){
	bb.vy=-bb.vy}
	
  aa.position.x=bb.position.x
  aa.position.z=bb.position.z
  aa.position.y=bb.position.y+2
  aa.setArrow(bb.vx*10,bb.vy*10,bb.vz*10)
   ...
   /* for(var i=1;i<21;i++){
	...
	for(var i=0;i<nn-1;i++){collision(bb1,bbw,rr+10)
	for(var j=i+1;j<nn;j++){	
		var bb1=tempArray[i]
		var bb2=tempArray[j]
		
		collision(bb1,bb2,rr*2)
