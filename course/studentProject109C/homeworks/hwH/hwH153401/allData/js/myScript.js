...
world2D.slCameraRR.maximum=300
	...
	pp=new TEACHER.ObjBox(80,1,80,0xff8888,2,)
	scene.add(pp)
	pp.position.y=-2
	bb =new TEACHER.ObjSphere(2,0xffffff,2)
	scene.add(bb)
    bb.position.x=0
	bb.position.z=0
	bb.vx=0
	bb.vz=0
    aa =new TEACHER.ObjSphere(4,0xffffff,2)
	scene.add(aa)
	aa.position.x=0
	//
    aa.position.y=-40
	...
	sp1=new TEACHER.ObjSpring(A,0.1,0.25,20,0xee44bb)
	scene.add(sp1)
	sp1.position.y=-2
	sp2=new TEACHER.ObjSpring(A,0.1,0.2,20,0xcc2299)
	scene.add(sp2)
	sp2.position.y=-2
	setInterval(tick,1000/fps);
	world2D.btn01.on("click",click)
	world2D.btn02.on("click",click)
	world2D.btn01.setLabel("回復")
	world2D.btn02.setLabel("加重下面的球")
	world2D.ch01.setLabel("按向下按鈕拉下繩子")
	world2D.btnDown.on("mousedown",clickBtn)
	world2D.on("pressup",up)
/*	world2D.sl01.setLabel("位移")
	world2D.sl01.maximum=-10
	world2D.sl01.minimum=-50
	world2D.sl01.value=-40
...
function tick(){
	...
	//aa.position.y=world2D.sl01.value
  

	t+=dt
	
	bb.position.x=A*Math.cos((w)*t)
	bb.position.z=A*Math.sin((w)*t)
	log(bb.vx)
	log(bb.vz)
	if(bb.position.z>0 & bb.vx>0 ){
		bb.vx*=-1
		log("0000000000000")

	}else if(bb.position.z<0 & bb.vx<0){
		bb.vx*=-1

	}
	//rr=Math.sqrt((bb.position.x)*(bb.position.x)+(bb.position.z)*(bb.position.z))
	...
	aa.ay=(mb*A*w*w-ma*g+fn)/ma+Ay
	aa.vy+=aa.ay*dt
	aa.position.y+=aa.vy*dt
	if(aa.position.y<=-(l-0.3)){
		aa.position.y=-(l-1.3)

	

	}else if( aa.position.y>-6){
		aa.position.y=-7
		

	}
	
	sp1.setSpring(bb.position.x,bb.position.y+1.5,bb.position.z)
	sp2.setSpring(0,aa.position.y,0)
	log(aa.vy)
	log(ma)
	log(w)
   log(aa.position.y) 
...
//resize &aa.position.y<-5) &aa.position.y>-60
MyJS.myResize();
