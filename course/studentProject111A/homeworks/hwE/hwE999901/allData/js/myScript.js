...
function init(){
	...
	for(var ii=0;ii<nn;ii++){
		var aa=new TEACHER.ObjSphere(3,0xFF0FF);
		
		aa.position.x=-50+(ii%20)*5;
		aa.position.y=5;
		aa.position.z=5
		scene.add(aa);
		ballArray1.push(aa);
	    ...
	    for(var ii=0;ii<nn;ii++){
			var bb=new TEACHER.ObjSphere(10,0xFF0);
			
			bb.position.x=-50+(ii%20)*5;
			bb.position.y=20;
			bb.position.z=0
			scene.add(bb);
			ballArray2.push(bb);
			...
			for(var ii=0;ii<nn;ii++){
				var cc=new TEACHER.ObjSphere(3,0xFF0FF);
				
				cc.position.x=-50+(ii%20)*5;
				cc.position.y=5;
				cc.position.z=-5
				scene.add(cc);
				ballArray3.push(cc);
				...
				for(var ii=0;ii<nn;ii++){
					var ee=new TEACHER.ObjSphere(5,0xF);
					
					ee.position.x=-55+(ii%20)*5;
					ee.position.y=15;
					ee.position.z=-5
					scene.add(ee);
					ballArray4.push(ee);
					...
					for(var ii=0;ii<nn;ii++){
						var e=new TEACHER.ObjSphere(5,0xF);
						
						e.position.x=-55+(ii%20)*5;
						e.position.y=15;
						e.position.z=5
						scene.add(e);
						ballArray5.push(e);
    ...
    ti=new TEACHER.ObjTextPlane(1000,20,"hwe大安高工一孝24 郭士玄",'z',0xFFFF00);
	scene.add(ti);
	ti.position.z=-50
    ti.position.y=20

	ab=new TEACHER.ObjTextPlane(1000,20,"警惕世人的 嗑藥蟲",'z',0xFFFF00);
	scene.add(ab);
	ab.position.z=-50
    ab.position.y=40

/*
	ab=new TEACHER.ObjTextPlane(1000,20,"電腦不好 請勿使用時間線崩壞",'z',0xFFFF00);
	scene.add(ab);
	ab.position.z=+50
    ab.position.y=40

	ab=new TEACHER.ObjTextPlane(4,4,"整人",'z',0xFFFF0);
	scene.add(ab);
	ab.position.z=+50
    ab.position.y=40
	...
	sunb= new TEACHER.ObjPicSphere(30,pics.sun);
	sunb.position.y=20;
	scene.add(sunb);
...
for(var i=0;i<nn;i++){
   var  earth= new TEACHER.ObjPicSphere(10+i,pics.earth)
	
    earth.position.x=i*2
	
    earth.position.z=i*5
	earth.position.y=20
	earth.vx=5*Math.random()-0.5;
    earth.vz=5*Math.random()-0.5;
	earth.vy=0;
	
	scene.add(earth);
	ballArray.push(earth);
...
//C.定義tick                                                                                                                       
function tick(){
    tt+= world2D.sl01.value;
	
	
	for(var i=0;i<nn;i++){
		var b=ballArray1[i];
		var yy=A*Math.sin(2*Math.PI*b.position.x/lambda - 2*Math.PI*tt/period);
		b.position.y=5+yy;

	}
	for(var i=0;i<nn;i++){
		var a=ballArray2[i];
		var yy=A*Math.sin(2*Math.PI*a.position.x/lambda - 2*Math.PI*tt/period);
		a.position.y=15+yy;

	}
	for(var i=0;i<nn;i++){
		var c=ballArray3[i];
		var yy=A*Math.sin(2*Math.PI*c.position.x/lambda - 2*Math.PI*tt/period);
		c.position.y=5+yy;
   ...
   /*
	theta+=omega*dt
	for(var i=0;i<nn;i++){
	var earth=ballArray[i]
	
	earth.position.x=rr*Math.cos(theta)*i;
	earth.position.z=rr*Math.sin(theta)*i; 
	

    if(sun.position.x>50 && sun.vx>0){
		sun.vx*=-1;
	}else if(sun.position.x<-50 && sun.vx<0){
		sun.vx*=-1;
	}
	if(sun.position.z>50 && sun.vz>0){
		sun.vz*=-1;
	}
...
function collision(earth,sun,dis){
	let isCollision=false;
	let mA= earth.mass||1;
	let mB=sun.mass||1;
	var r12x= earth.position.x-sun.position.x;
	var r12y= earth.position.y-sun.position.y;
	var r12z= earth.position.z-sun.position.z;
	var rr=Math.sqrt(r12x*r12x+r12y*r12y+r12z*r12z);
	var vcx=(mA* earth.vx+mB*sun.vx)/(mA+mB);
	var vcy=(mA* earth.vy+mB*sun.vy)/(mA+mB);
	var vcz=(mA* earth.vz+mB*sun.vz)/(mA+mB);
	var v1cx= earth.vx-vcx;
	var v1cy= earth.vy-vcy;
	var v1cz= earth.vz-vcz;
	var v2cx=sun.vx-vcx;
	var v2cy=sun.vy-vcy;
	var v2cz=sun.vz-vcz;
	...
	if(rr<dis && (v1cx*r12x+v1cy*r12y+v1cz*r12z)<0){
		...
		earth.vx=vcx+v1cx+2*(v1c_n12*n12x);
		earth.vy=vcy+v1cy+2*(v1c_n12*n12y);
		earth.vz=vcz+v1cz+2*(v1c_n12*n12z);
		sun.vx=vcx+v2cx-2*(v2c_n12*n12x);
		sun.vy=vcy+v2cy-2*(v2c_n12*n12y);
		sun.vz=vcz+v2cz-2*(v2c_n12*n12z);
