...
function init(){
    ...
    dy = 0.3;
	//ball
    ball = new TEACHER.ObjSphere(4, 0xff0000);
    bar = new TEACHER.ObjCylinder(0.2 , rr , 0x0000ff, true, "x");
    center = new TEACHER.ObjCylinder(1, 8 , 0xff00ff);
    scene.add(ball).add(bar).add(center);
	//衛星01
	ball01 = new TEACHER.ObjSphere(2, 0xff0000);
	scene.add(ball01);
	//衛星02
    ball02 = new TEACHER.ObjSphere(2.5, 0xff0000);
	scene.add(ball02);
	//arrow
	arrowV = new TEACHER.ObjArrow(0.2, 0xffff00);
    ball.add(arrowV);
    
    //line
	line = new TEACHER.Line(0xff00ff);
	scene.add(line);
	//line01
	line01 = new TEACHER.Line(0xf0ff00);
	scene.add(line01);
    //line02
	line02 = new TEACHER.Line(0x00f0f0);
	scene.add(line02);
    ...
    //ch
	world2D.ch01.setLabel("畫線");
	world2D.ch02.setLabel("bar長短");
    
	//btn
	world2D.btn01.setLabel("上下移動");
	world2D.btn02.setLabel("左右移動");
    world2D.btn01.on('click' , Btn);
    world2D.btn02.on('click' , Btn);
...
function tick(){
  ...
  //ball
  ball.position.x = rr*Math.cos(theta)+x;
  ball.position.z = rr*Math.sin(theta);
  ball.position.y = bar.position.y = center.position.y = 4+y;
  //衛星01
  ball01.position.x = ball.position.x - rr01*Math.cos(theta*3.7);
  ball01.position.z = ball.position.z - rr01*Math.sin(theta*3.7);
  ball01.position.y = 4+y;
  //衛星02
  ball02.position.x = ball.position.x + rr02*Math.cos(theta*2.9);
  ball02.position.z = ball.position.z + rr02*Math.sin(theta*2.9);
  ball02.position.y = 4+y;

  //bar
  bar.position.x = 0.5*rr*Math.cos(theta)+x;
  ...
  //line01
  tt+=dt;
  line01.addPoint(ball01.position.x , ball01.position.y , ball01.position.z);
  //line02 
  tt+=dt;
  line02.addPoint(ball02.position.x , ball02.position.y , ball02.position.z);
  }	
