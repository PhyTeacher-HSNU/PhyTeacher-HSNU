...
var ratio;
var rr=5;
var wallR,wallL,wallU,wallD,wallP;
var long=20;
var place;
var nn;
var point,pointV=0;//分數圖，分數量
var gameMD;

//B.定義init
function init(){

	nn=new TEACHER.ObjTextPlane(90,15,"1548 4 江昶毅",'z',)
	scene.add(nn);
	nn.position.z=-50;
	nn.position.y=20;

	point=new TEACHER.ObjTextPlane(90,15,'0','z',)
	scene.add(point);
	point.position.z=-50;
	point.position.y=30;

	//ball
	ball = new TEACHER.ObjSphere(rr,0xFFFFFF)
	scene.add(ball)
	ball.position.y=rr
	ball.vx=ball.vy=ball.vz=0
	ball.ax=ball.ay=ball.az=0
	//arrow
	arrowA=new TEACHER.ObjArrow(2,0xFFFF00)
	scene.add(arrowA)

	arrowV=new TEACHER.ObjArrow(2,0x00FFFF)
	scene.add(arrowV)
	//wall
	wallU=new TEACHER.ObjBox(100,10,2,0x000000)
	scene.add(wallU)
	wallU.position.y=5
	wallU.position.z=-51

	wallD=new TEACHER.ObjBox(100,10,2,0x000000)
	scene.add(wallD)
	wallD.position.y=5
	wallD.position.z=51

	wallR=new TEACHER.ObjBox(2,10,104,0x000000)
	scene.add(wallR)
	wallR.position.y=5
	wallR.position.x=51

	wallL=new TEACHER.ObjBox(2,10,104,0x000000)
	scene.add(wallL)
	wallL.position.y=5
	wallL.position.x=-51

	wallP=new TEACHER.ObjBox(1,10,1,0x0000FF)
	scene.add(wallP)
	wallP.position.y=5
	change()
	//slider
...
function tick(){
	...
	if(ball.position.x>50-rr && ball.vx>0){//右
		if(gameMD===3 && ball.position.z< place+ long/2 && ball.position.z> place- long/2){
			change()
			pointV++
			point.setText(pointV)
		}
		ball.vx*=-1*ratio
	}
	if(ball.position.x<-50+rr && ball.vx<0){//左
		if(gameMD===2 && ball.position.z< place+ long/2 && ball.position.z> place- long/2){
			change()
			pointV++
			point.setText(pointV)
		}
	...
	if(ball.position.z>50-rr && ball.vz>0){//下
		if(gameMD===1 && ball.position.x< place+ long/2 && ball.position.x> place- long/2){
			change()
			pointV++
			point.setText(pointV)
		}
	...
	if(ball.position.z<-50+rr && ball.vz<0){//上
		if(gameMD===0 && ball.position.x< place+ long/2 && ball.position.x> place- long/2){
			change()
			pointV++
			point.setText(pointV)
		}
	...
	ball.mat.color.setHSL(ball.vx*ball.vz*0.01 , 1 , 0.5)
	...
	arrowA.setArrow(ball.ax*5 , ball.ay*5 , ball.az*5)
...
function change(){
	gameMD=Math.floor(4*Math.random())
	place=Math.floor((-50+long/2)+(101-long)*Math.random())

	if(gameMD===0){//上
		wallP.position.z=-50
		wallP.position.x=place
		wallP.rotation.y=Math.PI/2;
	}else if(gameMD===1){//下
		wallP.position.z=50
		wallP.position.x=place
		wallP.rotation.y=Math.PI/2;
