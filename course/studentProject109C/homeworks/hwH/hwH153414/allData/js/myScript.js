...
function init(){
	...
	ground.visible=world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=false;
	world2D.slCameraRR.value=200;
	world2D.slCameraRR.maximum=250;

	word=new TEACHER.ObjTextPlane(115,20,"153414孫堃展","z",0x000000);
	word.position.y=30;
	word.position.x=0;
	word.position.z=-50;
	scene.add(word);

	c1=new TEACHER.ObjCylinder(1,1,0x0099ff,false,"y");
	c2=new TEACHER.ObjCylinder(5,1,0xff0000,false,"y");
	c3=new TEACHER.ObjCylinder(1,1,0xffff00,false,"y");
	text=new TEACHER.ObjTextPlane(200,200,1,"y",0x222222,0x222222)
	scene.add(text);
	var bar=new TEACHER.ObjBox(5,1,0.5,0x111111);
	bar.position.y=0.5;
	bar.position.x=2.5;
	c2.add(bar);

	scene.add(c1).add(c2).add(c3);
	c1.position.x=0;
	c1.position.y=-2.5;
	c1.position.z=0;
	...
	c3.position.x=r1-r2+r3;
	c3.position.y=-0.75;
	c3.position.z=0;

	world2D.sl01.setLabel("r2");
	world2D.sl01.maximum=30;
	world2D.sl01.minimum=10;
	world2D.sl01.value=10;

	world2D.sl02.setLabel("r3");
	world2D.sl02.maximum=5;
	world2D.sl02.minimum=1;
	world2D.sl02.value=10;

	world2D.sl03.setLabel("r1");
	world2D.sl03.maximum=40;
	world2D.sl03.minimum=20;
	world2D.sl03.value=20;

	line =new TEACHER.Line(0xffff00,3,20000);
	scene.add(line);

	world2D.ch01.setLabel("地板");
	world2D.ch02.setLabel("線");
	world2D.ch03.setLabel("快/慢")

	world2D.btn01.on("click",clickBtn);
	world2D.btn01.setLabel("stop/start");
	world2D.btn02.on("click",function(e){line.clear();});
	world2D.btn02.setLabel("clear");
	world2D.btn03.on("click",clickBtn);
	world2D.btn03.setLabel("內擺線");
	world2D.btn04.on("click",clickBtn);
	world2D.btn04.setLabel("外擺線");
	world2D.btn05.on("click",clickBtn);
	world2D.btn05.setLabel("複雜擺線");
	world2D.btn06.on("click",clickBtn);
	world2D.btn06.setLabel("超橢圓");

	//複雜擺線
	cir1=new TEACHER.ObjCylinder(35,1,0x0099ff,false,"y");
	bar1=new TEACHER.ObjBox(35,2,1,0x111111);
	bar1.position.x=17.5;
	cir1.add(bar1)
	cir2=new TEACHER.ObjCylinder(10,1,0xff1111,false,"y");
	cir2.position.x=45;
	bar2=new TEACHER.ObjBox(10,2,1,0x111111);
	bar2.position.x=5;
	cir2.add(bar2);
	scene.add(cir1).add(cir2);
	cir1.position.y=cir2.position.y=-4;

	p1=new TEACHER.ObjCylinder(1,1,0xffff00,false,"y");
	p2=new TEACHER.ObjCylinder(1,1,0xffff00,false,"y");
	p3=new TEACHER.ObjCylinder(1,1,0xffff00,false,"y");
	p4=new TEACHER.ObjCylinder(1,1,0xffff00,false,"y");
	scene.add(p1).add(p2).add(p3);
	p1.position.y=p2.position.y=p3.position.y=1;
	p1.position.x=-45;
	p3.position.x=30;

	box1 =new TEACHER.ObjBox(50,1,1,0x774455,2);
	box2 =new TEACHER.ObjBox(50,1,1,0x774455,2);
...
function tick(){
	...
	text.visible=world2D.ch01.checked;
	line.visible=world2D.ch02.checked;
	...
	if(mod===1){
		...
		cir1.visible=cir2.visible=box1.visible=box2.visible=p1.visible=p2.visible=p3.visible=ground.visible=false;
		c1.visible=c2.visible=c3.visible=true;
		world2D.sl01.visible=world2D.sl02.visible=world2D.ch01.visible=world2D.ch02.visible=world2D.ch03.visible=true;
		world2D.sl01.setLabel("紅圓半徑");
		world2D.sl01.maximum=30;
		world2D.sl01.minimum=10;
		world2D.sl01.digitN=0;
		world2D.sl02.setLabel("黃距圓心");
		world2D.sl02.minimum=1;
		world2D.sl02.digitN=0;
		world2D.sl02.maximum=world2D.sl01.value;
		world2D.sl01.value=Math.floor(world2D.sl01.value);
		world2D.sl02.value=Math.floor(world2D.sl02.value);

		r2=world2D.sl01.value;
		r3=world2D.sl02.value;
		c1.scale.x=r1;
		c1.scale.z=r1;
		c2.scale.x=r2/5;
		c2.scale.z=r2/5;
		
		c2.position.x=(r1-r2)*Math.cos(th1);
		c2.position.z=(r1-r2)*Math.sin(th1);
		c3.position.x=(r1-r2)*Math.cos(th1)+r3*Math.cos(th2);
		c3.position.z=(r1-r2)*Math.sin(th1)-r3*Math.sin(th2);
		if (start>0){
			for(var i = 0;i<dtnn;i++){
				th1+=dt/dtnn;
				c2.rotation.y=th1*(r1-r2)/r2;
				th2=th1*(r1-r2)/r2;
			}
			line.addPoint(c3.position.x,1,c3.position.z);
			line.rotation.y=0;
			line.update()
		}		
	}else if(mod===2){
		...
		world2D.sl03.visible=c1.visible=c2.visible=c3.visible=world2D.sl01.visible=world2D.sl02.visible=world2D.sl03.visible=world2D.ch01.visible=world2D.ch02.visible=world2D.ch03.visible=true;
		cir1.visible=cir2.visible=box1.visible=box2.visible=p1.visible=p2.visible=p3.visible=ground.visible=false;
		world2D.sl01.setLabel("紅圓半徑");
		world2D.sl01.maximum=30;
		world2D.sl01.minimum=10;
		...
		world2D.sl02.minimum=1;
		world2D.sl02.digitN=0;
		world2D.sl03.setLabel("藍圓半徑");
		world2D.sl03.maximum=40;
		world2D.sl03.minimum=20;
		world2D.sl03.digitN=0;
		world2D.sl02.maximum=world2D.sl01.value;
		world2D.sl01.value=Math.floor(world2D.sl01.value);
		world2D.sl02.value=Math.floor(world2D.sl02.value);
		...
		r2=world2D.sl01.value;
		r3=world2D.sl02.value;
		c1.scale.x=world2D.sl03.value;
		c1.scale.z=world2D.sl03.value;
		rr1=world2D.sl03.value;
		c2.scale.x=r2/5;
		c2.scale.z=r2/5;

		c2.position.x=(rr1+r2)*Math.cos(th1);
		c2.position.z=(rr1+r2)*Math.sin(th1);
		c3.position.x=(rr1+r2)*Math.cos(th1)+r3*Math.cos(th2);
		c3.position.z=(rr1+r2)*Math.sin(th1)+r3*Math.sin(th2);
		if (start>0){
			for(var i = 0;i<dtnn;i++){
				th1+=dt/dtnn;
				c2.rotation.y=-th1*(rr1+r2)/r2;
				th2=th1*(rr1+r2)/r2;
			}
			line.addPoint(c3.position.x,0.5,c3.position.z);
			line.rotation.y=0;
			line.update()
		}
	}else if(mod===3){
		c1.visible=c2.visible=c3.visible=world2D.sl02.visible=world2D.sl03.visible=world2D.ch03.visible=ground.visible=false;
		cir1.visible=cir2.visible=box1.visible=box2.visible=p1.visible=p2.visible=p3.visible=true;
		world2D.sl01.setLabel("距離圓心");
		...
		if(start>0){
			...
			var x=world2D.sl01.value;
			th1+=dt;
			cir1.rotation.y=th1*10/35;
			cir2.rotation.y=-th1;
			p3.position.x=45+x*Math.cos(th1);
			p3.position.z=x*Math.sin(th1);
			//l1:80, len:45
			var l2=Math.pow(x*x*2-2*x*x*Math.cos(th1),1/2);
			var l3=Math.pow(l1*l1+x*x-2*l1*x*Math.cos(th1+Math.PI),1/2);
			phi1=Math.acos((l1+x)/2/len);
			phi2=Math.acos(l3*l3/(2*l3*len));
			phi3=Math.acos((l1*l1+l3*l3-x*x)/(2*l1*l3));

			if((th1%(2*Math.PI))<=Math.PI){
				p2.position.x=-40+len*Math.cos(phi2+phi3);
				p2.position.z=len*Math.sin(phi2+phi3);
			}else if(Math.PI<=(th1%(2*Math.PI))&&(th1%(2*Math.PI))<=2*Math.PI){
				p2.position.x=-40+len*Math.cos(phi2-phi3);
				p2.position.z=len*Math.sin(phi2-phi3);
			}
			box1.position.x=(p1.position.x+p2.position.x)/2;
			box1.position.z=(p1.position.z+p2.position.z)/2;
			box1.rotation.y=-Math.atan((p1.position.z-p2.position.z)/(p1.position.x-p2.position.x))

			box2.position.x=(p3.position.x+p2.position.x)/2;
			box2.position.z=(p3.position.z+p2.position.z)/2;
			box2.rotation.y=-Math.atan((p3.position.z-p2.position.z)/(p3.position.x-p2.position.x))

			p4.position.x=Math.pow(p2.position.x*p2.position.x+p2.position.z*p2.position.z,1/2)*Math.cos(th1*10/35+Math.PI/2);
			p4.position.z=Math.pow(p2.position.x*p2.position.x+p2.position.z*p2.position.z,1/2)*Math.sin(th1*10/35+Math.PI/2);

			line.addPoint(p4.position.x,1,p4.position.z);
			line.rotation.y=th1*10/35;
			line.update()
		}
	}else if(mod===4){
		world2D.sl03.visible=world2D.sl01.visible=world2D.sl02.visible=world2D.sl03.visible=ground.visible=true;
		text.visible=cir1.visible=cir2.visible=box1.visible=box2.visible=p1.visible=p2.visible=p3.visible=c1.visible=c2.visible=c3.visible=world2D.ch01.visible=world2D.ch02.visible=world2D.ch03.visible=false;
		world2D.sl01.setLabel("a");
		world2D.sl01.maximum=30;
		world2D.sl01.minimum=0;
		world2D.sl01.digitN=2;
		world2D.sl02.setLabel("b");
		world2D.sl02.maximum=30;
		world2D.sl02.minimum=0;
		world2D.sl02.digitN=2;
		world2D.sl03.setLabel("n");
		world2D.sl03.maximum=5;
		world2D.sl03.minimum=0.01;
		world2D.sl03.digitN=2;

		a=Math.floor(world2D.sl01.value);
		b=Math.floor(world2D.sl02.value);
		n=world2D.sl03.value;
		...
		if(start>0){
			...
			tt+=dt;
			line.rotation.y=0;
			if((tt%(2*Math.PI))<=(Math.PI/2)){
				x= a*Math.pow(Math.cos(tt),2/n);
				y= b*Math.pow(Math.sin(tt),2/n);
				line.addPoint(x,1,y);
				line.update();
			}else if((Math.PI/2)<=(tt%(2*Math.PI))&&(tt%(2*Math.PI))<(Math.PI)){
				x= -a*Math.pow(-Math.cos(tt),2/n);
				y= b*Math.pow(Math.sin(tt),2/n);
				line.addPoint(x,1,y);
				line.update();
			}else if((Math.PI)<=(tt%(2*Math.PI))&&(tt%(2*Math.PI))<(Math.PI*3/2)){
				x= -a*Math.pow(-Math.cos(tt),2/n);
				y= -b*Math.pow(-Math.sin(tt),2/n);
				line.addPoint(x,1,y);
				line.update();
			}else if((Math.PI*3/2)<=(tt%(2*Math.PI))&&(tt%(2*Math.PI))<(2*Math.PI)){
				x= a*Math.pow(Math.cos(tt),2/n);
				y= -b*Math.pow(-Math.sin(tt),2/n);
				line.addPoint(x,1,y);
				line.update();
			}else if(tt>(2*Math.PI+1)){
				dt=0;
			}else{
				line.addPoint(0,-1,0);
				line.update();
