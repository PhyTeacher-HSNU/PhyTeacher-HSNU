...
function init(){
	...
	pp=new TEACHER.ObjTextPlane(100,20,"1571羅允佑","z",0xffff00);
	scene.add(pp);
	pp.position.x=50;
	pp.position.y=45;
	pp.position.z=-50;
	...
	for(var i=0;i<5;i++){
		var wall=new TEACHER.ObjBox(10,15,5,0x000000);
		wallArray.push(wall);
		scene.add(wall);
		wall.position.z=-100;
		wall.position.y=7.5;
		wall.position.x=-40+20*i;

	}
	
	ball=new TEACHER.ObjSphere(2,0xffff00);
	bar= new TEACHER.ObjCylinder(0.2,rr,0xff0000,true,"x");
	center=new TEACHER.ObjCylinder(1,4,0xffffff);

	ball.position.y=bar.position.y=center.position.y=2;
	
	scene.add(center).add(bar).add(ball);

	arrowV=new TEACHER.ObjArrow(0.8,0x00ffff);
	ball.add(arrowV);
	...
	world2D.btn02.setLabel('初始化');
	world2D.btn02.on('click',clickBtn);

	world2D.sl03.setLabel("得分");
...
function tick(){
	...
	if(gamemd===1){
		...
		ball.position.x+=Math.sin(theta)*-1*omega;
		ball.position.z+=Math.cos(theta)*omega;

		//bar.visible=center.visible=false;
		if(ball.position.z<-100&&-45<ball.position.x<-35){
			world2D.sl03.value=world2D.sl03.value+1;
			gamemd=0;
		}
		if(ball.position.z<-100&&-25<ball.position.x<-15){
			world2D.sl03.value=world2D.sl03.value+1;
			gamemd=0;
		}
		if(ball.position.z<-100&&-5<ball.position.x<5){
			world2D.sl03.value=world2D.sl03.value+1;
			gamemd=0;
		}
		if(ball.position.z<-100&&15<ball.position.x<25){
			world2D.sl03.value=world2D.sl03.value+1;
			gamemd=0;
		}
		if(ball.position.z<-100&&35<ball.position.x<45){
			world2D.sl03.value=world2D.sl03.value+1;
			gamemd=0;
		}
