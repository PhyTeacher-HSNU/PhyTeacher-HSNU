...
function init(){
	...
	//ball
	ball = new TEACHER.ObjCylinder(1,4,0x28004D,true,"z");
	bar = new TEACHER.ObjCylinder(0.2,rr,0x28004D,true,"x");
	center = new TEACHER.ObjCylinder(1,4,0x28004D);

	ball.position.y =2;
	bar.position.y = center.position.y = 2;
	...
	world2D.sl02.maximum = 75;
	world2D.sl02.value = 25;
	//world2D.sl02.digitN = 1;

	world2D.btn01.setLabel("給一顆球");
	world2D.btn01.on('click',clickBtn);
	world2D.btn02.setLabel('距離');

	world2D.ch01.setLabel('規則');
	world2D.ch02.setLabel("圓周運動")
	...
	b = new TEACHER.ObjSphere(1,0x00E3E3);
	
	b.position.y = 2;
	b.position.x = Math.floor(Math.random()*80)-40;
	b.position.z = Math.floor(Math.random()*80)-40;
	scene.add(b);
...
function tick(){
	...
	if(b.position.x+1>ball.position.x && ball.position.x>b.position.x-1 && b.position.z+1>ball.position.z && b.position.z-1<ball.position.z&&b.position.y==ball.position.y||world2D.ch02.checked == false){
		world2D.ch02.checked = false;

		b.vx=-rr*Math.sin(theta)*omega;
		b.vz=rr*Math.cos(theta)*omega;
		...
		b.position.x+=b.vx*dt;
		b.position.y+=b.vy*dt;
		b.position.z+=b.vz*dt;

		var num=Math.sqrt(b.position.x*b.position.x+b.position.z*b.position.z);
		num=num.toFixed(0);
		world2D.btn02.setLabel(num);
...
var logo=new TEACHER.ObjPicPlane(100,50,pics.logo,'z',2);
logo.position.z=-50;
logo.position.y=25;
