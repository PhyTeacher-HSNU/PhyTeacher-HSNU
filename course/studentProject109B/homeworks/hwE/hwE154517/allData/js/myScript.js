...
function init(){
	...
	for(var i=0 ; i<NN ; i++){
		var bb = new TEACHER.ObjSphere(rr,0x0099ff);

		bb.position.y = 0;
		bb.position.x = -48+(i%50)*2;
		bb.position.z = -48+Math.floor(i/50)*2;
	...
	Ball = new TEACHER.ObjSphere(3,0x666666);
	Ball.position.x = 0;
	Ball.position.z = 0;
	Ball.visible = false;
	Ball.vx = 0;
	...
	title = new TEACHER.ObjTextPlane(100,10,"154517張仲佑 療癒3D波","z",0xffffff);
	title.position.x = 0;
	title.position.y = 15;
	title.position.z = -50;
	scene.add(title);
	...
	world2D.ch01.setLabel("丟入石頭");
	world2D.ch02.setLabel("石頭靜止(水平)");

	world2D.btn01.setLabel("可動波源");
	...
	world2D.btnUp.on("pressup",BtnUp);
	world2D.btnDown.on("pressup",BtnUp);
	world2D.btnRight.on("pressup",BtnUp);
	world2D.btnLeft.on("pressup",BtnUp);
...
function tick(){
	...
	for(var i=0 ; i<NN ; i++){
		...
		var z;
		if(Center===1){
			x = b.position.x-vecM.x;		
			z = b.position.z-vecM.z;
		}else{
			x = b.position.x;		
			z = b.position.z;
		}
		var yy = AA*Math.sin(2*Math.PI*Math.sqrt(x*x+z*z)/lambda - 2*Math.PI*tt/period);
	...
	var Bz;
	if(Center===1){
		Bx = Ball.position.x-vecM.x;		
		Bz = Ball.position.z-vecM.z;
	}else{
		Bx = Ball.position.x;		
		Bz = Ball.position.z;
	}
	Ball.position.y = AA*Math.sin(2*Math.PI*Math.sqrt(Bx*Bx+Bz*Bz)/lambda - 2*Math.PI*tt/period);
