...
var dt=0.1;
var r=2;
var xx=100,zz=100,yy=100;
var grd,wall,ro,bal=5,col;
var gg=9.8;
var ste,xxx,yyy,zzz,vxx=0,vyy=0,vzz=0;
var ww=0;
var stars,beta=0.2;
var axx=0,ayy=0,azz=0;
//B.定義init
function init(){
	...
	world2D.sl02.setLabel('球的大小');
	world2D.sl02.minimum=1;
	world2D.sl02.maximum=10;
	world2D.sl02.value=bal;

	world2D.sl03.setLabel('球的顏色');
	...
	stars=new TEACHER.Points(0xffffff,2);
	scene.add(stars);
	for(var i=0 ;i<100;i++){
		 xxx=100*(Math.random()-0.5);
		 yyy=100*(Math.random()-0.5);
		 zzz=100*(Math.random()-0.5);
	...
	wall=new TEACHER.ObjBox(xx,1,zz,0x886600);
	wall.position.y=(yy/2)+0.5;
	scene.add(wall);

	grd=new TEACHER.ObjBox(xx,1,zz,0x886600);
	grd.position.y=-(yy/2)-0.5;
	scene.add(grd);
	...
	for(var i=0;i<nn;i++){
		...
		var bb=new TEACHER.ObjSphere(bal,0xff0000);
		
		
		bb.position.x=-(xx/2)+(i%10)*bal;
		bb.position.z=-(zz/2)+Math.floor(i/10)*bal;
		bb.position.y=-(yy/2)+Math.floor(i/10)*bal;
...
function tick(){
	...
	for(var i=0; i<nn;i++){
		...
		//恢復係數
		col=world2D.sl03.value;
		bal=world2D.sl02.value;
		bb.mat.color.setHSL(col,1,0.5);
		bb.scale.x=bal/6;
		bb.scale.y=bal/6;
		bb.scale.z=bal/6;
		ro=world2D.sl01.value;
		...
		//撞牆反彈
		if(bb.position.x > (xx/2)-bal && bb.vx>=0){
			bb.ax=0;
			bb.vx*=-1*ro;

			
		}else if(bb.position.x < -(xx/2)+bal && bb.vx<=0){
			bb.ax=0;
			bb.vx*=-1*ro;
			
		}
		if(bb.position.z > (zz/2)-bal && bb.vz>=0){
			bb.az=0;
			bb.vz*=-1*ro;
			
		}else if(bb.position.z < -(zz/2)+r && bb.vz<=0){
			bb.az=0;
			bb.vz*=-1*ro;
			
		}
		if(bb.position.y > (yy/2)-bal && bb.vy>=0){
			bb.ay=0;
			bb.vy*=-1*ro;
			
		}else if(bb.position.y < -(yy/2)+bal && bb.vy<=0){
			bb.ay=0;
			bb.vy*=-1*ro;
		...
		arrow.position.y=bb.position.y;
		arrow.position.z=bb.position.z;
		arrow.visible= world2D.ch01.checked;

		arrow.setArrow(2*bb.vx,2*bb.vy,2*bb.vz);
	}
