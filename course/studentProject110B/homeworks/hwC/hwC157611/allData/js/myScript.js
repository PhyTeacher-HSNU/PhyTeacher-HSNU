...
function init(){
	...
	world2D.slCameraRR.setLabel("目標球固定，不會移動。無敵會讓你無視所有球，但不會影響勝負判定。......遠近\n157611林宸宇");

	world2D.sl01.setLabel("障礙物個數");
	world2D.sl01.maximum = 10;
	world2D.sl01.minimum = 0;

	world2D.sl02.setLabel("發射球速");
	world2D.sl02.maximum = 20;
	world2D.sl02.minimum = 3;

	world2D.ch01.setLabel("成功，再玩一次");
	world2D.ch02.setLabel("失敗，再玩一次");
	world2D.ch01.visible = world2D.ch02.visible = false;

	world2D.btn01.setLabel("發射");
	world2D.btn02.setLabel("長壓無敵模式");

	world2D.btn01.on("click", shoot);
	world2D.btn02.on("mousedown", god);
	world2D.btn02.on("pressup", weak);

	me = new TEACHER.ObjSphere(r, 0x00ffff);
	me.position.x = -50;
	me.position.y = r;
	me.position.z = 0;
	me.vx = me.vy = me.vz = 0;
	tgt = new TEACHER.ObjSphere(r, 0xffff00);
	tgt.position.x = 50;
	tgt.position.y = r;
	tgt.position.z = 0;
	tgt.vx = tgt.vy = tgt.vz = 0;
	for (let i = 0; i < n; i++) {
		let b = new TEACHER.ObjSphere(r, 0xff0000);
		b.position.x = i * 2.5 * r;
		b.position.y = r;
		b.position.z = 50 - (100 * Math.random());
		b.vx = b.vy = 0;
		b.vz = 50 * (Math.random() - 0.5);
		while (b.vz < 2 && b.vz > -2) b.vz = 50 * (Math.random() - 0.5);
		obs.push(b);
		scene.add(b);
	}
	scene.add(me).add(tgt);
...
function inits() {
	...
	world2D.ch01.checked = world2D.ch02.checked = world2D.ch01.visible = world2D.ch02.visible = false;
...
function tick(){
	...
	if (world2D.ch01.checked || world2D.ch02.checked) inits();


	for (let i = 0; i < world2D.sl01.value | 0; i++) {
		obs[i].visible = true;
	}
	for (let i = world2D.sl01.value | 0; i < n; i++) {
		obs[i].visible = false;
	}

	me.position.x += me.vx * dt;
	me.position.y += me.vy * dt;
	me.position.z += me.vz * dt;

	for (let i = 0; i < n; i++) {
		obs[i].position.x += obs[i].vx * dt;
		obs[i].position.y += obs[i].vy * dt;
		obs[i].position.z += obs[i].vz * dt;
		if ((obs[i].position.x > 50 && obs[i].vx > 0) || (obs[i].position.x < -50 && obs[i].vx < 0)) obs[i].vx *= -1;
		if ((obs[i].position.z > 50 && obs[i].vz > 0) || (obs[i].position.z < -50 && obs[i].vz < 0)) obs[i].vz *= -1;
	}
	if ((me.position.x > 50 && me.vx > 0) || (me.position.x < -50 && me.vx < 0)) me.vx *= -1;
	if ((tgt.position.x > 50 && tgt.vx > 0) || (tgt.position.x < -50 && tgt.vx < 0)) tgt.vx *= -1;
	if ((me.position.z > 50 && me.vz > 0) || (me.position.z < -50 && me.vz < 0)) me.vz *= -1;
	if ((tgt.position.z > 50 && tgt.vz > 0) || (tgt.position.z < -50 && tgt.vz < 0)) tgt.vz *= -1;

	if (collision(me, tgt, r * 2) && win == -1) win = 1;
	for (let i = 0; i < world2D.sl01.value | 0; i++) {
		if (collision(me, obs[i], r * 2) && win == -1) {
			win = 0;
		}
		collision(obs[i], tgt, r * 2);
	}
	for (let i = 0; i < (world2D.sl01.value | 0) - 1; i++) {
		for (let j = i + 1; j < (world2D.sl01.value | 0); j++) {
			collision(obs[i], obs[j], r * 2);
		}
	}

	if (win == 0) world2D.ch02.visible = true;
	if (win == 1) world2D.ch01.visible = true;
