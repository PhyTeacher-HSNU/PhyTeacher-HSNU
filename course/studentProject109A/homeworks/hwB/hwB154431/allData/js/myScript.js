...
function init(){
	...
	arrowa = new TEACHER.ObjArrow(1,0xFFFF0F);
	...
	wall01.position.x = wall05.position.x = 50;
	wall01.position.y = wall05.position.y = 2.5;
	wall02.position.x = wall06.position.x = -50;
	wall02.position.y = wall06.position.y = 2.5;
	wall03.position.z = wall07.position.z = 50;
	wall03.position.y = wall07.position.y = 2.5;
	wall04.position.z = wall08.position.z = -50;
	wall04.position.y = wall08.position.y = 2.5;

	wall05.visible = false;
	wall06.visible = false;
	wall07.visible = false;
	wall08.visible = false;
...
//C.定義tick                                                                                                                       
function tick(){
	back = world2D.sl01.value;

	//固定視角
	world3D.cameraPhi = 0;
	world3D.cameraTheta = 120.2;
	if(check === 1){
		
		wall02.position.x = wall06.position.x = world2D.sl02.value;
		wall01.position.x = wall05.position.x = world2D.sl03.value;
	}else if(check === 2){
		
		wall04.position.z = wall08.position.z = world2D.sl02.value;
		wall03.position.z = wall07.position.z = world2D.sl03.value;
	}
	...
	if(ball.position.x>wall01.position.x-3 && ball.vx>0 || ball.position.x>wall05.position.x-3 && ball.vx>0){
		ball.vx*=-1*back;
	}else if(ball.position.x<wall02.position.x+3 && ball.vx<0 || ball.position.x<wall06.position.x+3 && ball.vx<0){
		ball.vx*=-1*back;
	}
	if(ball.position.z>wall03.position.z-3 && ball.vz>0 || ball.position.z>wall07.position.z-3 && ball.vz>0){
		ball.vz*=-1*back;
	}else if(ball.position.z<wall04.position.z+3 && ball.vz<0 || ball.position.z<wall08.position.z+3 && ball.vz<0){
		ball.vz*=-1*back;
	}

	arrowa.position.x = arrowv.position.x = ball.position.x;
	arrowa.position.y = arrowv.position.y = ball.position.y;
	arrowa.position.z = arrowv.position.z = ball.position.z;
...
function up(e){
	ball.az = ball.ax = 0;
	wall05.visible = wall06.visible = wall07.visible = wall08.visible = false;
	wall01.visible = wall02.visible = wall03.visible = wall04.visible = true;
}
function gogo(e){
	var str = e.target.parent.name;
	if(str === 'btn01'){
		...
		world2D.sl02.value = wall02.position.x;
		world2D.sl03.value = wall01.position.x;
	}else if(str === 'btn02'){
		check = 2;
		world2D.ch02.checked = true;
		world2D.ch01.checked = false;
		world2D.sl02.value = wall04.position.z;
		world2D.sl03.value = wall03.position.z;
	}
