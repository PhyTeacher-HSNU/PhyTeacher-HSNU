...
//B.定義init
function init(){
	Gameover = new TEACHER.ObjTextPlane(100,10,"GAMEOVER","z",0xffff00)
	scene.add(Gameover)
	Gameover.visible=false
	Gameover.position.z = 30;
	Gameover.position.y = 7.5
	start = new TEACHER.ObjTextPlane(100,10,"TAP TO START","z",0xffff00)
	scene.add(start);
	start.position.z = 20;
	rule = new TEACHER.ObjTextPlane(100,10,"DONT TOUCH IT !","z",0xffff00)
	scene.add(rule);
	rule.position.z = 20;
	rule.position.y = 15;
	myname = new TEACHER.ObjTextPlane(100,10,"1533 17 倪德耀","z",0xffff00)
	scene.add(myname);
	myname.position.z = 20;
	myname.position.y = 7.5;
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray1.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray2.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray3.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray4.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray5.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray6.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray7.push(wall);
	}
	for(var i=-4;i<=4;i++){
		var wall = new TEACHER.ObjPlane(5,1000,0xFFFFFF)
		scene.add(wall);
		wallArray8.push(wall);
	}
	
	ball = new TEACHER.ObjSphere(5,0xFF00FF);
	scene.add(ball);
	ball.position.z = 0
	ball.position.x = 0
	/*arrowExt = new TEACHER.ObjArrow(2);
	arrowDrag = new TEACHER.ObjArrow(2);
	scene.add(arrowExt);
	scene.add(arrowDrag);*/
	//arrowV = new TEACHER.ObjArrow(2);
	//scene.add(arrowV);
	ground.visible = skyBox.visible = false;
	stars = new TEACHER.Points(0xFF00FF,1);
	scene.add(stars);
	for(var i=0;i<1000;i++){	
		var xx = 100*(Math.random()-0.5);
		var yy = 100*(Math.random()-0.5);
		var zz = 100*(Math.random()-0.5);
	...
	//world2D.btn02.setLabel("playagain");
	world2D.on("pressup",press);
...
function tick(){
	...
	stars.position.z = -zz;
	for(var i=0;i<9;i++){
		var wall = wallArray1[i]
		wall.position.x = -xx+10;
		wall.position.z = -zz-(i-4)*50;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
				Gameover.visible = true;
			
			}else if(wall.position.x - ball.position.x <3 && wall.position.x - ball.position.x >0){
				dt = 0;
				isPlay = 0;
				Gameover.visible = true;
		...
		if (wall.position.z - ball.position.z <3 && wall.position.z - ball.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
				Gameover.visible = true;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray2[i]
		wall.position.x = -xx-10;
		wall.position.z = -zz-(i-4)*50+10;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray3[i]
		wall.position.x = -xx+30;
		wall.position.z = -zz-(i-4)*50+20;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray4[i]
		wall.position.x = -xx-30;
		wall.position.z = -zz-(i-4)*50+30;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray5[i]
		wall.position.x = -xx+50;
		wall.position.z = -zz-(i-4)*50+40;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray6[i]
		wall.position.x = -xx-50;
		wall.position.z = -zz-(i-4)*50+50;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray7[i]
		wall.position.x = -xx+70;
		wall.position.z = -zz-(i-4)*50+60;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	for(var i=0;i<9;i++){
		var wall = wallArray8[i]
		wall.position.x = -xx-70;
		wall.position.z = -zz-(i-4)*50+70;
		if (ball.position.z - wall.position.z <3 && ball.position.z - wall.position.z >0){
			if(ball.position.x - wall.position.x <3 && ball.position.x - wall.position.x >0){
				dt = 0;
				isPlay = 0;
	...
	/*arrowExt.setArrow(extFx*10,extFy*10,extFz*10);
	arrowDrag.setArrow(dragFx*10,dragFy*10,dragFz*10);*/
	//arrowV.setArrow(-vecM.x,-vecM.y,-vecM.z);
