...
//波源
var sMoveDir = 0; //1 up , -1 down , 0xx
var ys = 0;
...
var uNow = [];
var uOld = [];
var endMD = 0;//0 fixed , 1 free , 2 absorb
...
//B.定義init
function init(){
	p = new TEACHER.ObjTextPlane(100,20,"1543 陳智紘","z",0xFFFF);
	scene.add(p);
	p.position.z = -50;
	p.position.y = 25;
	...
	sound = new Audio('allData/mp3/Enceladus Hiss audio 256 kbps.mp3');
		...
		for(var ic = 0 ; ic < 10 ; ic++){
			for(var iv = 0 ; iv < 10 ; iv++){
				for(var ig = 0 ; ig < 10 ; ig++){
					stars.addPoint(xx + ic * 100 , yy + iv * 100  , zz + ig * 100 );
				}
			}
		}
	}*/
	...
	pts = new TEACHER.Points(0xff00 , 1.3);
	...
	//s
	world2D.btnUp.on('mousedown' , downBtn);
	world2D.btnDown.on('mousedown' , downBtn);
	...
	*/
	...
	//gravity
	sun = new TEACHER.ObjSphere(2.5 ,0xffff00);
	sun.position.y = 3;
	scene.add(sun);
	for(var i = 0 ; i < nn ; i++){
		var planet = new TEACHER.ObjSphere(1.7 , 0x00ffff);
		planet.position.y = 3;
		planet.position.x = 10 + 10 * i;
		planet.vx = 0;
		planet.vy = 0;
		planet.vz = Math.sqrt(gmm / planet.position.x);
		scene.add(planet);
		planets.push(planet);
...
function tick(){
	...
	for(let i = 0 ; i < NN * NN ; i++){
		let hh = uNow[i] * 0.2 +0.5;
		color.setHSL(hh , 0.5 , 0.5);
	...
	for(var i = 0 ; i < nn ; i++){
		...
		var rr = Math.sqrt(Math.pow(planet.position.x , 2) + Math.pow(planet.position.y , 2) + Math.pow(planet.position.z , 2));
		planet.mat.color.setHSL(x1 , y1 , 0.5);
		...
		planet.ax = -gmm * planet.position.x / Math.pow(rr , 3);
		planet.ay = -gmm * (planet.position.y - 3) / Math.pow(rr , 3);
		planet.az = -gmm * planet.position.z / Math.pow(rr , 3);

		planet.vx += planet.ax * xt;
		planet.vy += planet.ay * xt;
		planet.vz += planet.az * xt;

		planet.position.x +=planet.vx * xt; 
		planet.position.y +=planet.vy * xt; 
		planet.position.z +=planet.vz * xt; 
	...
	uNow[(0.5 * NN) * NN + NN / 2] = ys;
	var pt = pts.getPoint((0.5 * NN) * NN + NN / 2);
	pts.setPoint((0.5 * NN) * NN + NN / 2 , pt.x , ys , pt.z);
	...
	if(world2D.ch01.checked && !world2D.ch02.checked){
		...
		endMD = 1;
		uNow[NN - 1] = uNow[NN - 2];
	}else if(world2D.ch02.checked && !world2D.ch01.checked){
		//absorb
		endMD = 2;
		uNow[NN - 1] = uOld[NN - 2];
	}else{
	...
	*/
	...
	for(var ix = 1; ix < NN - 1 ; ix++){
		for(var iz = 1 ; iz < NN - 1 ; iz++){
			uNew[iz * NN + ix] = 2 * uNow[iz * NN + ix] -uOld[iz * NN + ix] + cc * cc * dt * dt / dx / dx *(uNow[(iz + 1) * NN + ix] + uNow[(iz - 1) * NN + ix] + uNow[iz * NN + ix + 1] + uNow[iz * NN + ix - 1] - 4 * uNow[iz * NN + ix]);
			 var pt = pts.getPoint(iz * NN + ix);
			 pts.setPoint(iz * NN + ix , pt.x , uNew[iz * NN + ix] , pt.z);
	...
	for(var ix = 0; ix < NN ; ix++){
		for(var iz = 0; iz < NN ; iz++){
			uOld[iz * NN + ix] = uNow[iz * NN + ix];
			uNow[iz * NN + ix] = uNew[iz * NN + ix];
		}
	}
