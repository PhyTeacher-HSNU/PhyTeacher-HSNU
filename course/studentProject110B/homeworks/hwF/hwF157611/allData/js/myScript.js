...
//B.定義init
function init(){
	center = new TEACHER.ObjCylinder(1, 30, 0x00ffff);
	scene.add(center);

	center.position.y = ong;

	for (let i = 0; i < mxn; i++) {
		let b = new TEACHER.ObjSphere(br, 0xff0000);
		let bar = new TEACHER.ObjCylinder(0.2, rr, 0xffff00, true, "x");
		...
		for (let j = 0; j < 4; j++) {
			let sb = new TEACHER.ObjSphere(sr, 0x0000ff);
			let sbar = new TEACHER.ObjCylinder(0.2, rr, 0xffff00, true, "x");
			sbar.position.y = ong;
			sb.vx = sb.vy = sb.vz = 0;
			sball[i].push(sb);
			sbars[i].push(sbar);
			stheta[i].push(j * 2 * Math.PI / 4);
			b.add(sb);
			scene.add(sbar);
		}
		
		b.position.y = bar.position.y = ong;
		...
		ball.push(b);
		bars.push(bar);
		theta.push(i * 2 * Math.PI / n);
		dy.push(2 * Math.PI * i / n);
	}
	for (let i = n; i < mxn; i++) {
		ball[i].visible = bars[i].visible = false;
		for (let j = 0; j < 4; j++) sball[i][j].visible = sbars[i][j].visible = false;
	}
	...
	world2D.sl02.setLabel("rotate r");
	world2D.sl02.maximum = 40;
	world2D.sl02.minimum = 10;
	world2D.sl02.value = rr;

	world2D.sl03.setLabel("球數 按確認鍵設定");
	world2D.sl03.maximum = mxn;
	world2D.sl03.minumum = 1;
	world2D.sl03.value = n;
	world2D.sl03.digitN = 0;

	//world2D.btn01.setLabel("甩出去");
	//world2D.btn01.on("click", gogogo);
	world2D.btn01.visible = false;

	world2D.btn02.setLabel("確認");
	world2D.btn02.on("click", bck);

	world2D.slCameraRR.setLabel("遠近 157611林宸宇")
	world2D.slCameraRR.value = 160;

	world2D.ch01.setLabel("線");
	world2D.ch02.setLabel("小球");
...
function tick(){
	...
	if (Math.abs(omega) < 0.05) omega = 0;

	for (let i = 0; i < n; i++) {
		if (goaround) around(i);
		else go(i);
	}
...
function gogogo(e) {
	...
	for (let i = 0; i < n; i++) {
		...
		let v = rr * omega;
		ball[i].vx = -v * Math.sin(theta[i]);
		ball[i].vz = -v * Math.cos(theta[i]);
	}
}

function bck(e) {
	goaround = true;
	n = Math.floor(world2D.sl03.value + 0.5);

	for (let i = 0; i < n; i++) dy[i] = 2 * Math.PI * i / n;
	...
	for (let i = 0; i < n; i++) {
		...
		for (let j = 0; j < 4; j++) {
			sball[i][j].visible = sbars[i][j].visible = true;
		}
	}
	for (let i = n; i < mxn; i++) {
		bars[i].visible = ball[i].visible = false;
		for (let j = 0; j < 4; j++) {
			sball[i][j].visible = sbars[i][j].visible = false;
		}
	}
}
...
function around(i) {
	theta[i] += omega * dt;
	ball[i].position.x = rr * Math.cos(theta[i]);
	ball[i].position.y = 10 * Math.sin(dy[i]) + ong;
	ball[i].position.z = -rr * Math.sin(theta[i]);
	ball[i].scale.x = ball[i].scale.y = ball[i].scale.z = br / 2;
	bars[i].position.x = 0.5 * rr * Math.cos(theta[i]);
	bars[i].position.y = ball[i].position.y;
	bars[i].position.z = -0.5 * rr * Math.sin(theta[i]);
	bars[i].rotation.y = theta[i];
	bars[i].scale.x = rr / 25;
	dy[i] += omega * dt * 5;
	if (dy[i] > Math.PI * 2) dy[i] -= Math.PI * 2;
	bars[i].visible = world2D.ch01.checked;

	
	for (let j = 0; j < 4; j++) {
		stheta[i][j] += omega * dt * 5;
		sball[i][j].position.x = srr * Math.cos(stheta[i][j]);
		sball[i][j].position.z = -srr * Math.sin(stheta[i][j]);
		sbars[i][j].position.x = ball[i].position.x + 0.5 * srr * Math.cos(stheta[i][j]);
		sbars[i][j].position.y = ball[i].position.y;
		sbars[i][j].position.z = ball[i].position.z -0.5 * srr * Math.sin(stheta[i][j]);
		sbars[i][j].rotation.y = stheta[i][j];
		sbars[i][j].scale.x = srr / 25;

		sball[i][j].visible = world2D.ch02.checked;
		sbars[i][j].visible = world2D.ch01.checked && world2D.ch02.checked;
	}
}

function go(i) {
	ball[i].position.x += ball[i].vx * dt;
	ball[i].position.y += ball[i].vy * dt;
	ball[i].position.z += ball[i].vz * dt;
	if ((ball[i].position.x > 50 && ball[i].vx > 0) || (ball[i].position.x < -50 && ball[i].vx < 0)) ball[i].vx *= -1;
	if ((ball[i].position.z > 50 && ball[i].vz > 0) || (ball[i].position.z < -50 && ball[i].vz < 0)) ball[i].vz *= -1;

	for (let j = 0; j < n; j++) {
		if (j != i) collision(ball[i], ball[j], br * 2);
	}
