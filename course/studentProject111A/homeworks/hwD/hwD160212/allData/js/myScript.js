...
//B.定義init
function init(){
word=new TEACHER.ObjTextPlane(100,10,'1602 12林柏諭',"z",0x0000ff)
scene.add(word)	
word.position.z=-60
word.position.y=25
...
world2D.ch01.setLabel("圓盤軌跡/實際路徑")
ball=new TEACHER.ObjPicSphere(2,0xff0000)
ball.position.z=-28
ball.position.y=3
scene.add(ball)
center=new TEACHER.ObjCylinder(rr+2,1,0x0000ff,false,"y")
center.position.y=1
scene.add(center)
center2=new TEACHER.ObjCylinder(1,1.1,0x00ffff,false,"y")
center2.position.y=1
scene.add(center2)
point=new TEACHER.ObjCylinder(2,1.1,0xfff000,false,"y")
point.position.z=rr-0.5
center.add(point)
world2D.btn01.setLabel("開始旋轉")
world2D.btn02.setLabel("發射")
world2D.btn01.on('click',clickbtn1)
world2D.btn02.on('click',clickbtn2)
world2D.ch02.setLabel("視角切換")
...
function clickbtn2(e){
	ismove=1
	thetaball=theta+Math.PI
}                                                                                                                               
function tick(){
	way=world2D.ch01.checked
	ischange=world2D.ch02.checked
	if(ischange){
		world3D.cameraPhi=-theta-Math.PI/2
		world3D.cameraTheta=Math.PI/2.3
	}else{
		world3D.cameraTheta=0
		world3D.cameraPhi==Math.PI/2*3
	}
	if(isrotation===1){
		center.rotation.y+=omaga*dt
		theta-=omaga*dt
	}
	if(ismove===0&&isrotation===1){
		ball.position.x=rr*Math.cos(theta)
		ball.position.z=rr*Math.sin(theta)
	}
	if(ismove===1&&isrotation===1){
		vx=8*Math.cos(thetaball)
		vz=8*Math.sin(thetaball)
		ball.position.x+=vx*dt
		ball.position.z+=vz*dt
		p=new TEACHER.Points(0xffff00,1)
		p.addPoint(ball.position.x,3,ball.position.z)
		pn.push(p)
		scene.add(p)
		n+=1
		if(way){
		for(var i=0;i<n;i++){
			var pnew=pn[i]
			pnew.rotation.y+=omaga*dt
		}
	}
	}
	if(ismove===1&&isrotation===0){
	    vx=8*Math.cos(thetaball)
		vz=8*Math.sin(thetaball)
		ball.position.x+=vx*dt
		ball.position.z+=vz*dt
		p=new TEACHER.Points(0xffff00)
		p.addPoint(ball.position.x,3,ball.position.z)
		pn.push(p)
		scene.add(p)
		n+=1
	}
	if(ismove===1&&Math.pow(Math.pow(ball.position.x,2)+Math.pow(ball.position.z,2),1/2)>=rr){
		isrotation=0
		ismove=0
	}
	//3D
