...
//A.宣告全域變數
var gravity = 39.2;//9.8;

var theta = 10*Math.PI/180;//初始角度
console.log(theta);
...
var ballrr=1;
var timesNum = 100; //20
//var L = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8];//, 0.85];//, 0.9, 0.95];//, 1, 1.05, 1.1, 1.15];//, 1.20, 1.25];//, 1.0, 1.05];
var L = [0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5];
...
function init(){
	...
	for (var i=0; i<n; i++) {
		...
		ballArray.push(ball);
		scene.add(ball);
		ball.position.z=timesNum*i/10;
		stringLen = L[i]*timesNum;
		ballY=ceiling-stringLen;
		ball.position.y=ballY;

		string=new TEACHER.ObjCylinder(0.1,stringLen,0x5f9ea0,false,"y",2);
	...
	//console.log("theta " + 30*Math.PI/180);
	setInterval(tick,10000/fps); 
	//setInterval(tick1,100000/fps);
	//setTimeout(tick,10000/fps);
	//setTimeout(tick1,10000/fps);
}


//C.定義tick                                                                                                                       
function tick(initTheta){
...
*/	/*
	...
	var a = [];
    for (var j=0; j<n; j++) {
        stringArray[j].axis = ballArray[j].position - stringArray[j].position;
        a.push(new TEACHER.Vector3(0,-gravity,0).add(SpringForce(stringArray[j].axis, L[j]).div(m)));
		ballArray[j].v += a[j]*dt;            //計算每一個擺錘的速度
        ballArray[j].position += ballArray[j].v*dt;     //計算每一個擺錘的位置
	}
	
	var tempTheta=0;
	for (var i=0;i<1;i++){
		force = gravity * Math.sin(theta);
		rr=L[i]*timesNum;
		angleA = (-1 * force) / rr;
		angleV += angleA;
		tempTheta += angleV;
		currTheta.push(tempTheta);
	}
	console.log(currTheta);*/
	for (var i=0; i<n; i++) {
		//theta = 30*Math.PI/180;
		var rr=L[i]*timesNum;
		var ball=ballArray[i];
		var string=stringArray[i];
		/*alpha=(-gravity)*Math.sin(currTheta[i])/rr-beta*omega;
		omega+=alpha*dt;
		currTheta[i]+=omega*dt;*/
		...
		ball.position.x = rr * Math.sin(currTheta[i]) ;
		ball.position.y =ceiling-(rr)*Math.cos(currTheta[i]);//len * Math.cos(theta) + ballY;
		string.rotation.z=currTheta[i];
	...
	theta += angleV;
	for (var i=0; i<n; i++) {
		ballArray[i].position.x = rr * Math.sin(theta) ;
		ballArray[i].position.y =ceiling-(rr)*Math.cos(theta);//len * Math.cos(theta) + ballY;
		stringArray[i].rotation.z=theta;
	}*/
	//3D
	world3D.render();
	//2D
...
*/	/*
	...
	*/
	...
	for (var i=1; i<2; i++) {
		...
		var ball=ballArray[i];
		var string=stringArray[i];
		//alpha=(-gravity)*Math.sin(theta)/rr-beta*omega;
		//omega+=alpha*dt;
		//theta+=omega*dt;
