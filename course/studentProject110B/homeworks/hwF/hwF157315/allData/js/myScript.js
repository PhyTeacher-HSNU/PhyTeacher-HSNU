...
function init(){
	//music
	intersound= new Audio('allData/mp3/extra-extended-soundtrack-by-hans-zimmer.mp3')

	intro = new TEACHER.ObjTextPlane(600,90,"本作參考109年下太陽系");
	scene.add(intro);
	intro.position.z=-800;
	intro.position.y=160;
	Classname = new TEACHER.ObjTextPlane(600,90,"157315郁博勛","z",0xfffff);
	scene.add(Classname);
	Classname.position.z=-800;
	Classname.position.y=70;
	//清空場景
	...
	scene.add(lin6);
	lin7=new TEACHER.Line(0x00ff00);
	scene.add(lin7);
	...
	world2D.sl02.setLabel("繞行半徑")
	world2D.sl02.minimum=140;
	world2D.sl02.maximum=400;
	world2D.sl02.value=180;

	world2D.ch01.setLabel("請開聲音");
	world2D.ch02.setLabel("視角回太陽")

	world2D.btn01.setLabel("聚焦地球");
	world2D.btn02.setLabel("聚焦木星");
	world2D.btn03.setLabel("music");



	world2D.btn01.on("click",clickBtn);
	world2D.btn03.on("click",music);
...
function clickBtn(t){
	...
	if(str=="btn01"){
		if(int==0){
			...
			world2D.btn01.setLabel("聚焦水星");
			int++;
		}else if(int==1){
			world2D.btn01.setLabel("聚焦金星");
			world2D.slCameraRR.value=80;
			int++;
		}else if(int==2){
			world2D.btn01.setLabel("聚焦火星");
	...
	//類木行星
		...
		if(int0==0){
			world2D.slCameraRR.value=80;
			world2D.btn02.setLabel("聚焦土星");
			int0++;
		}else if(int0==1){
			world2D.btn02.setLabel("聚焦天王星");
			world2D.slCameraRR.value=80;
			int0++;
		}else if(int0==2){
			world2D.btn02.setLabel("聚焦海王星");
			world2D.slCameraRR.value=70;
			int0++;
		}else{
			world2D.btn02.setLabel("木星聚焦");
			world2D.slCameraRR.value=70;
			int0=0;
		}
		cha=2;
