...
//B.定義init
function init(){
	d=new TEACHER.ObjBox(100,1,1,0xccffff);
	scene.add(d);
	d.position.y=-50;
	d.position.z=50;
	e=new TEACHER.ObjBox(1,1,100,0xccffff);
	scene.add(e);
	e.position.x=50;
	e.position.y=-50;
	f=new TEACHER.ObjBox(100,1,1,0xccffff);
	scene.add(f);
	f.position.y=-50;
	f.position.z=-50;
	g=new TEACHER.ObjBox(1,1,100,0xccffff);
	scene.add(g);
	g.position.x=-50;
	g.position.y=-50;
	h=new TEACHER.ObjBox(100,1,1,0xccffff);
	scene.add(h);
	h.position.z=50;
	h.position.y=50;
	i=new TEACHER.ObjBox(1,1,100,0xccffff);
	scene.add(i);
	i.position.x=50;
	i.position.y=50;
	j=new TEACHER.ObjBox(100,1,1,0xccffff);
	scene.add(j);
	j.position.z=-50;
	j.position.y=50;
	k=new TEACHER.ObjBox(1,1,100,0xccffff);
	scene.add(k);
	k.position.x=-50;
	k.position.y=50;
	l=new TEACHER.ObjBox(1,100,1,0xccffff);
	scene.add(l);
	l.position.x=50;
	l.position.z=50;
	m=new TEACHER.ObjBox(1,100,1,0xccffff);
	scene.add(m);
	m.position.x=-50;
	m.position.z=50;
	n=new TEACHER.ObjBox(1,100,1,0xccffff);
	scene.add(n);
	n.position.x=50;
	n.position.z=-50;
	o=new TEACHER.ObjBox(1,100,1,0xccffff);
	scene.add(o);
	o.position.x=-50;
	o.position.z=-50;
	two=new TEACHER.ObjPicPlane(500,500,pics.two,"z");
	scene.add(two);
	two.position.z=-250;
	one=new TEACHER.ObjPicPlane(500,500,pics.one,"x");
	scene.add(one);
	one.position.x=-250;
	three=new TEACHER.ObjPicPlane(500,500,pics.three,"x",1);
	scene.add(three);
	three.position.x=250;
	four=new TEACHER.ObjPicPlane(500,500,pics.four,"z",1);
	scene.add(four);
	four.position.z=250;
	five=new TEACHER.ObjPicPlane(500,500,pics.five,"y",1);
	scene.add(five);
	five.position.y=250;
	six=new TEACHER.ObjPicPlane(500,500,pics.six,"y");
	scene.add(six);
	six.position.y=-250;

	world2D.slCameraRR.value=200;
	for(var i=0;i<nn;i++){
		var bb=new TEACHER.ObjSphere(2,0xffffff);
		scene.add(bb);
		ballArray.push(bb);
		var arrow=new TEACHER.ObjArrow(1,0xffffff);
		scene.add(arrow);
		arrowArray.push(arrow);

		bb.position.y=45-Math.floor(i/10)*10;
		...
		world2D.ch02.setLabel("撞牆變色");
	    world2D.sl01.setLabel("移動速度");
	...
	pp=new TEACHER.ObjTextPlane(80,10,"1607陳愉","z",0xffffff);
	scene.add(pp);
	pp.position.y=35;
	pp.position.z=-50;
...
function tick(){
	for(var i=0;i<nn;i++){
		...
		arrow.setArrow(bb.vx*2,bb.vy*2,bb.vz*2);
		ballArray[0].mat.color.setHex(0xff8000);

		if(gamemode===1 && world2D.ch02.checked){
			if(bb.position.x>48 && bb.vx>0){
				bb.mat.color.setHex(0xff00ff);
				bb.vx=world2D.sl01.value*-1;
			}else if(bb.position.x<-48 && bb.vx<0){
				bb.mat.color.setHex(0xff00ff);
				bb.vx=world2D.sl01.value;
			}
			if(bb.position.y>48 && bb.vy>0){
				bb.vy=world2D.sl01.value*-1;
				bb.mat.color.setHex(0x00ffff);
			}else if(bb.position.y<-48 && bb.vy<0){
				bb.vy=world2D.sl01.value;
				bb.mat.color.setHex(0x00ffff);
			}
			if(bb.position.z>48 && bb.vz>0){
				bb.vz=world2D.sl01.value*-1;
				bb.mat.color.setHex(0xffff00);
			}else if(bb.position.z<-48 && bb.vz<0){
				bb.vz=world2D.sl01.value;
				bb.mat.color.setHex(0xffff00);
			}
		    ...
		    arrow.position.y=bb.position.y+2;
