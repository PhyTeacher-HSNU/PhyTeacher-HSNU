...
function init(){
	...
	for(var k=0;k<nn;k++){
		var ball1 = new TEACHER.ObjSphere(2,0xffffff);
		ballArray1.push(ball1);
		scene.add(ball1);
		ball1.position.y=2+k;
	}
	...
	for(var n=0 ; n<nn ; n++){
		...
		scene.add(ball);
		ball.position.y=2
		ball.position.x=-45+(n%10)*10;
		ball.position.z=-45+Math.floor(n/10)*10;
		...
		var arrowV=new TEACHER.ObjArrow(1,0xff0000);
		scene.add(arrowV);
    	arrowArrayV.push(arrowV);

		var arrowA=new TEACHER.ObjArrow(1,0xffff00);
		scene.add(arrowA);
    	arrowArrayA.push(arrowA);
...
function tick(){
	 for(var i=0;i<nn;i++){
		...
		if(ball.position.y<2.5&&ball.vy<0){
			ball.vy*=-1;
		}
		...
		var arrowV=arrowArrayV[i];
		var arrowA=arrowArrayA[i];
