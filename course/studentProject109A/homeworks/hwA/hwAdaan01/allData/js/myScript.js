...
//B.定義init
function init(){
	name=new TEACHER.ObjTextPlane(40,20,"圖一乙 林祐德","z");
	scene.add(name);

	bus=new TEACHER.ObjBox(20,10,10,0x0066FF,0);
	wheelL=new TEACHER.ObjPicCylinder(5,0.2,pics.cir,false,'z');
	wheelR=new TEACHER.ObjPicCylinder(5,0.2,pics.cir,false,'z');
	wheelL.position.x=-10;
	wheelL.position.y=-5;
	wheelR.position.x=10;
	wheelR.position.y=-5;
	bus.add(wheelL).add(wheelR);
	scene.add(bus);
...
function tick(){
	...
	bus.vy+=bus.ay*dt;
	bus.vz+=bus.az*dt;
	bus.position.x+=bus.vx*dt;
	bus.position.y+=bus.vy*dt;
	bus.position.z+=bus.vz*dt;

	wheelL.rotation.z-=(bus.vx*dt/5);
	wheelR.rotation.z-=(bus.vx*dt/5);
	
	if(bus.position.x<wallLX && bus.vx<0){
		bus.vx*=-1;
	}else if(bus.position.x>wallRX && bus.vx>0){
		bus.vx*=-1;
	}
	if(bus.position.z<-45 && bus.vz<0){
		bus.vz*=-1;
	}else if(bus.position.z>45 && bus.vz>0){
		bus.vz*=-1;
	}

	vxRatio=world2D.sl01.value;
	if(bus.vx>0){
		bus.vx=vxRatio;
	}else if(bus.vx<0){
		bus.vx=-1*vxRatio;
	}
	vzRatio=world2D.sl02.value;
	if(bus.vz>0){
		bus.vz=vzRatio;
	}else if(bus.vz<0){
		bus.vz=-1*vzRatio;
