...
function init(){
	...
	center = new TEACHER.ObjCylinder(1,2*ballr,0x00ffff,false,"y")
	bar = new TEACHER.ObjCylinder(0.2,barr,0xffff00,true,"x")	
	ball = new TEACHER.ObjSphere(ballr,0xff0000)
	Varrow = new TEACHER.ObjArrow(1,0xffff00)
	point = new TEACHER.ObjSphere(ballr,0x00ffff)
	...
	Varrow.position.y = 0
	point.position.y = 2
	point.position.x = (100*Math.random())-50
	point.position.z = (100*Math.random())-50
	...
	world2D.sl01.setLabel("轉動速度")
	world2D.sl01.maximum = 10
	world2D.sl01.minimum = -10
	world2D.sl01.value = 3

	world2D.sl02.setLabel("長度")
	world2D.sl02.maximum = 35
	world2D.sl02.minimum = -35
	...
	world2D.ch01.setLabel("速度箭頭")
	world2D.ch01.checked = false

	world2D.btn01.setLabel("拋出or收回")
	world2D.btn01.on("click" , clickBtn)
	
	world2D.btn02.setLabel("隨機改變")
...
function tick(){
	...
	barr = world2D.sl02.value
	ballr = world2D.sl03.value

	center.position.y = 2*ballr
	center.scale.y = ballr


	bar.position.x = 0.5*barr*Math.cos(theta)
	bar.position.z = 0.5*barr*Math.sin(theta)
	bar.position.y = 2*ballr
	bar.scale.x = world2D.sl02.value/25
	bar.rotation.y = -theta
	
	ball.position.y = 2*ballr
	ball.scale.x = ball.scale.y = ball.scale.z = ballr
	if(gameMD === 1){
		ball.position.x = barr*Math.cos(theta)
		ball.position.z = barr*Math.sin(theta)
		
		
	}else if(gameMD === 0){
		ball.position.x += xv*dt
		ball.position.z += zv*dt
		ball.position.y += yv*dt

		if(ball.position.x > 50 && xv > 0){
			xv *= -1
		}else if(ball.position.x < -50 && xv < 0){
			xv *= -1
		}

		if(ball.position.y > 50 && yv > 0){
			yv *= -1
		}else if(ball.position.y < -50 && yv < 0){
			yv *= -1
		}

		if(ball.position.z > 50 && zv > 0){
			zv *= -1
		}else if(ball.position.z < -50 && zv < 0){
			zv *= -1
		}
	}
	
	theta += omega*dt
	if(world2D.sl01.value<-1 || world2D.sl01.value>1){
		omega = world2D.sl01.value*0.2
	}else{
		omega = 0
	}

	Varrow.position.x = ball.position.x
	Varrow.position.z = ball.position.z
	Varrow.position.y = ball.position.y
	Varrow.visible = world2D.ch01.checked
	if(gameMD === 1){
		vv = omega*barr
		Varrow.setArrow(-vv*Math.sin(theta) , 0 ,vv*Math.cos(theta))
	}else if (gameMD === 0){
		Varrow.setArrow(xv , yv , zv)
	}
	
	var xxx = Math.abs(ball.position.x-point.position.x)
	var zzz = Math.abs(ball.position.z-point.position.z)
	var xz = 2+(2*world2D.sl03.value)
	if(((xxx*xxx)+(zzz*zzz)) < (xz*xz)){
		console.log("point+1")
		replacepoint()
		points++
	}
	...
	world2D.ch02.setLabel("分數:"+points+"時間:"+Math.floor(times))
...
function clickBtn(e){
	...
	if(str === "btn01"){
		if(gameMD === 1){
			gameMD = 0
			xv = -vv*Math.sin(theta)
			zv = vv*Math.cos(theta)
			yv = 0
		}else if(gameMD === 0){
			gameMD = 1
			xv = 0
		...
		replacepoint()
		world2D.sl01.value = (20*Math.random())-10
		world2D.sl02.value = (70*Math.random()) -35
