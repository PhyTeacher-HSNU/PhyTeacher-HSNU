...
function init(){
	...
	var ar=new TEACHER.ObjArrow(0.4,0xffff00)
	ballArray.push(bb)
	arArray.push(ar)
	scene.add(bb);
	scene.add(ar)
	bb.vx=10*(Math.random()-0.5);
	bb.vz=10*(Math.random()-0.5);
	while((bb.vx*bb.vx+bb.vz*bb.vz)>100){
		bb.vx=10*(Math.random()-0.5);
		bb.vz=10*(Math.random()-0.5);
	}
	bb.vy=0
	bb.position.x=10*(i%10)-45
	bb.position.z=i-i%10-45
	bb.mass=1
	ar.position.y=2
...
function tick(){
	...
	for(var i=0;i<nn;i++){
		arArray[i].position.x=ballArray[i].position.x
		arArray[i].position.z=ballArray[i].position.z
		arArray[i].setArrow(ballArray[i].vx,0,ballArray[i].vz)
	}
	for(var i=0;i<nn;i++){
		var bb=ballArray[i];
		xxx+=(Math.sqrt(bb.vx*bb.vx+bb.vz*bb.vz))
	}
	xxx*=dt
	world2D.btn01.setLabel(xxx)
	//3D
	world3D.render();
	//2D
...
function btn02click(){
	var v0=world2D.sl01.value*1.5
	for(var i=0;i<nn;i++){
		var bb=ballArray[i]
		bb.vx=v0*(Math.random()-0.5);
		bb.vz=v0*(Math.random()-0.5);
		while((bb.vx*bb.vx+bb.vz*bb.vz)>(v0*v0)){
			bb.vx=v0*(Math.random()-0.5);
			bb.vz=v0*(Math.random()-0.5);
		}
		bb.vy=0
