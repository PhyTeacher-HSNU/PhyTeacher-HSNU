...
function init(){
	...
	for(var i=0; i<nn ; i++){
		...
		var balla;
		balla =new TEACHER.ObjSphere(5,0xff0000,2);
		scene.add(balla);
		ballaArray.push(balla);

		balla.position.x=-7.5+(i%5)*4;
		balla.position.y=5;
		balla.position.z=-7.5+Math.floor(i/10)*4;

		
		balla.vx=10*(Math.random()-0.5);
		balla.vy=10*(Math.random()-0.5);
		balla.vz=10*(Math.random()-0.5);
	...
	for(var i=0; i<mm ; i++){
		...
		var ballb;
		ballb =new TEACHER.ObjSphere(0.5,0xffffff,2);
		scene.add(ballb);
		ballbArray.push(ballb);

		ballb.position.x=-7.5+(i%5)*4;
		ballb.position.y=90;
		ballb.position.z=-7.5+Math.floor(i/10)*4;

		
		ballb.vx=10*(Math.random()-0.5);
		ballb.vy=10*(Math.random()-0.5);
		ballb.vz=10*(Math.random()-0.5);
	...
	bb=new TEACHER.ObjPlane(100,100,0x0000ff,"y",);
	scene.add(bb);
	bb.position.x=0;
	bb.position.y=0.1;
	bb.position.z=0;

	bb=new TEACHER.ObjPlane(100,100,0x0000ff,"x",);
	scene.add(bb);
	bb.position.x=50;
	bb.position.y=50;
	bb.position.z=0;
	
	aa=new TEACHER.ObjPlane(100,100,0x0000ff,"x",);
	scene.add(aa);
	aa.position.x=-50;
	aa.position.y=50;
	aa.position.z=0;

	aa=new TEACHER.ObjPlane(100,100,0x0000ff,"z",);
	scene.add(aa);
	aa.position.x=0;
	aa.position.y=50;
	aa.position.z=-50;

	ss=new TEACHER.ObjPlane(100,100,0xffffff,"y",);
	scene.add(ss);
	ss.position.x=0;
	ss.position.y=0.2;
	ss.position.z=0;

	text=new TEACHER.ObjTextPlane(50,8,"156910胡晏慈","z",0xffffff,);
	scene.add(text);
	text.position.x=0;
	text.position.y=20;
	text.position.z=-49;
	...
	/*for(var i=0; i<nn ; i++){
		...
		var balls;
		balls =new TEACHER.ObjSphere(5,0xffffff,2);
		scene.add(balls);
		ballsArray.push(balls);

		balls.position.x=-7.5+(i%5)*4;
		balls.position.y=20;
		balls.position.z=-7.5+Math.floor(i/25)*4;

		balls.vx=(5*Math.random()-0.5);
		balls.vy=0;
		balls.vz=(5*Math.random()-0.5);
	}*/
	/*for(var i=0; i<nn ; i++){
	...
	//按鈕
	world2D.ch01.setLabel('班級姓名');
	world2D.ch02.setLabel('下雪');
	world2D.btn01.setLabel("運動");
	world2D.btn02.setLabel("停下");
...
function tick(){
	...
	if(gameMD === 1){
		for(var i=0;i<nn;i++){
			...
			if(balla.position.x>45 && balla.vx>0){
				balla.vx*=-1;
			}else if(balla.position.x<-40 && balla.vx<0){
				balla.vx*=-1;
			}
			if(balla.position.z>45 && balla.vz>0){
				balla.vz*=-1;
			}else if(balla.position.z<-45 && balla.vz<0){
				balla.vz*=-1;
			}
			if(balla.position.y>95 && balla.vy>0){
				balla.vy*=-1;
			}else if(balla.position.y<5 && balla.vy<0){
				balla.vy*=-1;
			}
	...
	for(var i =0; i<nn-1;i++){
		for(var j=i+1;j<nn ; j++){
			...
			var balla1 = ballaArray[i];
			var balla2 = ballaArray[j];

			collision(balla1,balla2,10);
		}
	}

	for(var i=0;i<mm;i++){
		var ballb = ballbArray[i];
		...
		if(ballb.position.x>45 && ballb.vx>0){
			ballb.vx*=-1;
		}else if(ballb.position.x<-40 && ballb.vx<0){
			ballb.vx*=-1;
		}
		if(ballb.position.z>45 && ballb.vz>0){
			ballb.vz*=-1;
		}else if(ballb.position.z<-45 && ballb.vz<0){
			ballb.vz*=-1;
		}
		if(ballb.position.y>95 && ballb.vy>0){
			ballb.vy*=-1;
		}else if(ballb.position.y<5 && ballb.vy<0){
			ballb.vy*=-1;
		}
	...
	ss.visible=world2D.ch02.checked;
