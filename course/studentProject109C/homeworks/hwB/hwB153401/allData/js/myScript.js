...
var rr=10
var aa
var vy=8*(Math.random()-0.5);
var vx=8*(Math.random()-0.5);
var vz=8*(Math.random()-0.5);
var ax=0
var az=0
var ay=0
var dt=0.5
...
//B.定義init
function init(){
	aa=new TEACHER.ObjPicPlane(100,50,pics.me,2)
	scene.add(aa)
	aa.position.z=-49.5
	aa.position.y=25

	/*nn=new THREE.Object3D()
	scene.add(nn)*/
	ww1=new TEACHER.ObjPicPlane(100,50,pics.wall,"x",2)
	scene.add(ww1)
	ww2=new TEACHER.ObjPicPlane(100,50,pics.wall,"x",2)
	scene.add(ww2)
	ww1.position.x=50
	ww1.position.y=25
	ww2.position.x=-50
	ww2.position.y=25
	gg=new TEACHER.ObjPicPlane(100,100,pics.earth,"y",2)
	scene.add(gg)
	ss=new TEACHER.ObjPicPlane(100,100,pics.sky,"y",2)
	scene.add(ss)
	ss.position.y=50
	
	bb=new TEACHER.ObjPicSphere(rr,pics.bub)
	bb.position.y=40
	scene.add(bb)
	
	/*nn.position.y=bb.position.y
nn.position.z=bb.position.z
nn.position.x=bb.position.x*/
	

	cc=new TEACHER.ObjSphere(1)
	scene.add(cc)
	tt=new TEACHER.ObjSphere(1)
	scene.add(tt)
	uu=new TEACHER.ObjSphere(1)
	scene.add(uu)
	ww=new TEACHER.ObjSphere(1)
	scene.add(ww)
	xx=new TEACHER.ObjSphere(1)
	scene.add(xx)
	yy=new TEACHER.ObjSphere(1)
	scene.add(yy)
	zz=new TEACHER.ObjSphere(1)
	scene.add(zz)
	
myline=new TEACHER.Line(0xff0000,200)
scene.add(myline)	
myline1=new TEACHER.Line(0x00ff00,200)
scene.add(myline1)
myline2=new TEACHER.Line(0x0000ff,200)
scene.add(myline2)
myline3=new TEACHER.Line(0xf990099,200)
scene.add(myline3)
myline4=new TEACHER.Line(0x999900,200)
scene.add(myline4)
myline5=new TEACHER.Line(0xf009999,200)
scene.add(myline5)
myline6=new TEACHER.Line(0x1005000,200)
scene.add(myline6)


//myline.addPoint(cc.position.x,cc.position.y,cc.position.z)


  

world2D.btn01.setLabel('正常重力')
world2D.btn02.setLabel('天地倒轉')
...
function tick(){
	...
	if(bb.position.x>40 && vx>0){
	vx=-vx 
    }else if(bb.position.x<-40 && vx<0){
	vx=-vx }
	if(bb.position.z>40 && vz>0){
		vz=-vz  
    }else if(bb.position.z<-40 && vz<0){
	vz=-vz}
	if(bb.position.y>40 && vy>0){
		vy=-vy* 0.9
    }else if(bb.position.y<10 && vy<0){
	vy=-vy*0.9}
	...
	cc.position.x=bb.position.x+1
	cc.position.y=bb.position.y+1
	cc.position.z=bb.position.z+1;
	tt.position.x=bb.position.x-1
	tt.position.y=bb.position.y+1
	tt.position.z=bb.position.z+1;
	uu.position.x=bb.position.x+1
	uu.position.y=bb.position.y-1
	uu.position.z=bb.position.z+1;
	ww.position.x=bb.position.x+1
	ww.position.y=bb.position.y+1
	ww.position.z=bb.position.z-1;
	xx.position.x=bb.position.x-1
	xx.position.y=bb.position.y+1
	xx.position.z=bb.position.z-1;
	yy.position.x=bb.position.x-1
	yy.position.y=bb.position.y-1
	yy.position.z=bb.position.z+1;
	zz.position.x=bb.position.x+1
	zz.position.y=bb.position.y-1
	zz.position.z=bb.position.z-1;


	myline.addPoint(cc.position.x,cc.position.y,cc.position.z)
	myline1.addPoint(tt.position.x,tt.position.y,tt.position.z)
	myline2.addPoint(uu.position.x,uu.position.y,uu.position.z)
	myline3.addPoint(ww.position.x,ww.position.y,ww.position.z)
	myline4.addPoint(xx.position.x,xx.position.y,xx.position.z)
	myline5.addPoint(yy.position.x,yy.position.y,yy.position.z)
	myline6.addPoint(zz.position.x,zz.position.y,zz.position.z)
