...
var longCat;
var gameMD=0;
var crr=10;//pop cat's r
var lrr=12.5;//long cat's r
var rr=7.5;//ball's r
var dy=0;
var word;
var strike;
...
//B.定義init
function init(){
	word=new TEACHER.ObjTextPlane(90,15,"1548 4 江昶毅",'z',)
	scene.add(word);
	word.position.z=-50;
	word.position.y=20;

	strike=new TEACHER.ObjTextPlane(40,10,"STRIKE","z",0xff0000)
	scene.add(strike);
	strike.position.z=40;
	strike.position.y=5;
	strike.visible=false;

	popCatA=new TEACHER.ObjPicPlane(15,20,pics.popCatA,"z",2);
	scene.add(popCatA);
	popCatA.position.y=10;
	popCatA.position.x=-40;
	
	popCatB=new TEACHER.ObjPicPlane(15,20,pics.popCatB,"z",2);
	scene.add(popCatB);
	popCatB.position.y=10;
	popCatB.position.x=-40;
	popCatB.visible=false;

	longCat=new TEACHER.ObjPicPlane(25,25,pics.longCat,"z",2);
	scene.add(longCat);
	longCat.position.y=12;
	longCat.position.z=2;
	longCat.position.x=40;
	
	ball=new TEACHER.ObjPicPlane(15,15,pics.ball,"z",2);
	scene.add(ball);
	ball.position.y=10;
	ball.position.z=1;
	ball.position.x=-37;
	...
	world2D.ch01.setLabel("是否擊倒");

	world2D.sl01.setLabel("pop cat長度");
	world2D.sl01.maximum=2;
	world2D.sl01.minimum=0.5;
	world2D.sl01.value=1;
	world2D.sl01.digitN=1;

	world2D.sl02.setLabel("long cat長度");
...
//C.定義tick                                                                                                                       
function tick(){
	popCatA.scale.y=popCatB.scale.y=world2D.sl01.value;
	popCatA.position.y=popCatB.position.y=(crr*world2D.sl01.value);
	longCat.scale.y=world2D.sl02.value;
	longCat.position.y=(lrr*world2D.sl02.value);
	if(gameMD===1){
		ball.visible=true;
		ball.position.x+=0.4
		ball.rotation.z-=0.4/rr
		if( ball.position.y>4){
			dy+=0.1
			ball.position.y-=0.1*dy
		}
		if(world2D.ch01.checked===true){
			if(ball.position.x>38 && ball.position.x<40){
				ball.position.x=38
				ball.rotation.z=0
				longCat.position.x+=(world2D.sl02.value)
				longCat.rotation.z-=(world2D.sl02.value)/(lrr*world2D.sl02.value)
				strike.visible=true;
			}
		}
		if(ball.position.x>-26){
			popCatB.visible=false;
			popCatA.visible=true;
		}
	}
...
function clickBtn(e){
	...
	if(str==='btn01'){
		gameMD=1;
		dy=1;
		ball.position.y=(crr*world2D.sl01.value);
		ball.position.x=-37;
		popCatA.visible=false;
		popCatB.visible=true;
		longCat.position.y=12;
		longCat.position.x=40;
		longCat.rotation.z=0;
		strike.visible=false;
	}
}
