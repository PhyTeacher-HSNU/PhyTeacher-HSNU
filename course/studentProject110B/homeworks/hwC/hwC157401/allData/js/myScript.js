...
var arrowarray=[];
var edgearray=[];
var gamemode=0,timegamemode=0;
var scale;
var dt=0.1;
//PV=nRT
// P=N/V*m*1/3*v**2
var P,V,k=1.38E-23,E,m=1,T=0,sumv=0,vbar=0;
var tt=0,t=0;
...
function init(){
	for (let i=0;i<n;i++){
		...
		ball.position.y=(Math.random()-0.5)*60;
		ball.position.x=(Math.random()-0.5)*60;
		ball.position.z=(Math.random()-0.5)*60;
		
		ball.vx=5*(Math.random()-0.5);
		ball.vz=5*(Math.random()-0.5);
		ball.vy=5*(Math.random()-0.5);
		ball.ax=ball.ay=ball.az=0;
		scene.add(ball);
		ballarray.push(ball);
		var arrow=new TEACHER.ObjArrow(1,0xffff00);
		scene.add(arrow);
		arrowarray.push(arrow);
	}

	for (let i=0;i<12;i++){
		var edge=new TEACHER.ObjBox(1,60.5,1,0xffff00);
		edgearray.push(edge);
		if (i===1){
			edge.rotation.z=1/2*Math.PI;
			edge.position.x=0;
			edge.position.y=-30;
			edge.position.z=30;
	...
	world2D.sl01.setLabel("氣體體積V放大倍率");
	world2D.sl01.value=1;
	world2D.sl01.maximum=2;
	world2D.sl01.minimum=0.1;
	world2D.sl01.digitN=3;
	world2D.sl02.setLabel("氣體分子個數");
	world2D.sl02.value=300;
	world2D.sl02.maximum=500;
	world2D.sl02.minimum=0;
	world2D.sl03.setLabel("分子半徑");
	world2D.sl03.value=2;
	world2D.sl03.maximum=10;
	world2D.sl03.minimum=1;
	world2D.btn01.setLabel("暫停");
	world2D.btn01.on("click",clickbtn);
	world2D.btn02.setLabel("速度增加");
	world2D.btn02.on("click",clickbtn);
	world2D.ch01.setLabel("隱藏箭頭");
	world2D.ch02.setLabel("隱藏邊框");
	world2D.ch03.setLabel("157401方世燊");
	
	// P_V=new TEACHER.ObjGraph(0,216,0,150,0xffffff,0xfff00,10000);
	// P_V.labelTitle.setText("P-V");
	// P_V.labelX.setText("V");
	// P_V.labelY.setText("P");
	// P_V.position.z=-500;
	// P_V.visible=false;
	// scene.add(P_V);
	...
	ground.visible=logo.visible=false;
...
//C.定義tick                                                                                                                       
function tick(){
	scale=world2D.sl01.value;
	r=world2D.sl03.value;
	sumv=0;
	n=world2D.sl02.value;
	V=Math.pow(6*scale,3);
	// if (camgamemode===0){
	// 	world3D.cameraTarget.x=0
	// 	world3D.cameraTarget.y=0;
	// 	world3D.cameraTarget.z=0;
	// }else if (camgamemode===1){
	// 	world3D.cameraTarget.x=0
	// 	world3D.cameraTarget.y=0;
	// 	world3D.cameraTarget.z=-500;
	// }
	for (let i=0;i<500;i++){
	...
	if (true){
		for (let i=0;i<n;i++){
			...
			ball.ax=0;
			if (ball.position.x>30*scale-r&&ball.vx>0)
				ball.vx*=-1;
			else if (ball.position.x<-30*scale+r&&ball.vx<0)
				ball.vx*=-1;
			if (ball.position.z>30*scale-r&&ball.vz>0)
				ball.vz*=-1;
			else if (ball.position.z<-30*scale+r&&ball.vz<0)
				ball.vz*=-1;
			if (ball.position.y>30*scale-r&&ball.vy>0)
				ball.vy*=-1;
			else if (ball.position.y<-30*scale+r&&ball.vy<0)
				ball.vy*=-1;
			...
			arrow.setArrow(ball.vx*r,ball.vy,ball.vz);
			if (ball.position.x>30*scale+1||ball.position.x<-30*scale-1||ball.position.z>30*scale+1||ball.position.z<-30*scale-1
				||ball.position.y>30*scale+1||ball.position.y<-30*scale-1){
					ball.position.y=(Math.random()-0.5)*60*scale;
					ball.position.x=(Math.random()-0.5)*60*scale;
					ball.position.z=(Math.random()-0.5)*60*scale;
				}
			sumv+=Math.sqrt(ball.vx*ball.vx+ball.vy*ball.vy+ball.vz*ball.vz);
			ball.scale.x=ball.scale.y=ball.scale.z=world2D.sl03.value;
			// arrow.scale.x=world2D.sl03.value;
		...
		for (let i=0;i<12;i++){
			...
			if (i===1){
				edge.rotation.z=1/2*Math.PI;
				edge.scale.y=scale;
				edge.position.y=-30*scale;
				edge.position.z=30*scale;
			...
			edge.visible=!world2D.ch02.checked;
		}
		for (let i=0;i<n-1;i++){
			for (let j=i+1;j<n;j++){
				...
				collision(ball1,ball2,2*world2D.sl03.value);
			}
		}
	}
	for (let i=0;i<500;i++){
...
function clickbtn(e){
	...
	if (str==="btn01"){
		...
		else if (timegamemode===1){
			dt=1;
			timegamemode=2;
		}else if (timegamemode===2){
			dt=0.1;
			timegamemode=0;
		}
	}
