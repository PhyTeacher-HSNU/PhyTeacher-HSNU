...
function init(){
	...
	sun = new TEACHER.ObjSphere(5,0xffff00)
	scene.add(sun);
	...
	planet.vz = 30;
	scene.add(planet);
	line = new TEACHER.Line(0xffffff);
	scene.add(line)
	world2D.slCameraRR.value  = 100;


	world2D.sl01.setLabel('位置X')
	world2D.sl02.setLabel('位置Y')
	world2D.sl03.setLabel('位置Z')
	world2D.sl01.maximum = 15;
	world2D.sl02.maximum = 10;
	world2D.sl03.maximum = 15;
	world2D.sl01.minimum = 6;
	world2D.sl02.minimum = -10;
	world2D.sl03.minimum = 6;
	world2D.sl01.value = 10;
	world2D.sl02.value = 0;
	world2D.sl03.value = 10;
	world2D.btn01.on('click',ff);
	world2D.btn02.on('click',ff);
	world2D.btn01.setLabel("重來");
	world2D.btn02.setLabel("開始");
	/*world2D.checkBox01.visible = false;
	world2D.checkBox02.visible = false;
...
function tick(){
	...
	if (gamemode===1){
		...
		planet.position.y = world2D.sl02.value;
		planet.position.z = world2D.sl03.value;
		line.clear()
	}	
	/*var rr = Math.sqrt(Math.pow(planet.position.x,2) + Math.pow(planet.position.z,2)+ Math.pow(planet.position.y,2));
		...
		planet.vz = planet.vz + zzz + planet.az*dt
