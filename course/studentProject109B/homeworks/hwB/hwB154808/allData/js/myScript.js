...
//B.定義init
function init(){
	gem=new TEACHER.ObjPicPlane(15,15,pics.gem,"z",2)
	scene.add(gem);
	gem.position.y=5;
	gem.position.x=-20+30*Math.random();
	gem.position.z=-10+40*Math.random();
	point=new TEACHER.ObjTextPlane(60,20,"分數");
	point.position.z=-80;
	point.position.y=30;
	scene.add(point);
	score=new TEACHER.ObjTextPlane(60,20," ")
	score.position.z=-80;
	score.position.y=20;
	scene.add(score);
	wall=new TEACHER.ObjBox(100,10,1,0x00ffff,2);//牆壁
	scene.add(wall);
	wall.position.z=-51;
	wall=new TEACHER.ObjBox(100,10,1,0x00ffff,2);
	scene.add(wall);
	wall.position.z=50;
	wall=new TEACHER.ObjBox(1,10,100,0x00ffff,"x");
	scene.add(wall);
	wall.position.x=50;
	wall=new TEACHER.ObjBox(1,10,100,0x00ffff,"x");
	scene.add(wall);
	...
	pp=new TEACHER.ObjTextPlane(60,10,"154808 吳坤澔","z",0x0000ff);//字
	scene.add(pp);
	pp.position.z=51;
	pp.position.x=-30;
	scene.add(ball);
	ball.position.y=world2D.sl02.value+5;
    //arrow
	...
	world2D.sl02.setLabel('球半徑')
	world2D.sl02.minimum=5;
	world2D.sl02.maximum=10;



	world2D.slCameraRR.value=150;          //視角
	world3D.cameraTheta=0.8164862505571205;
	world3D.cameraTarget.y=25;
	world3D.cameraTarget.z=15;
...
function tick(){
    ...
    arrowA.setArrow(ball.ax*6,ball.ay*6,ball.az*6);

	ball.scale.x=world2D.sl02.value;
	ball.scale.y=world2D.sl02.value;
	ball.scale.z=world2D.sl02.value;
  
	 
		if (world2D.ch01.checked){
			aa=ball.position.z/100;
			ball.mat.color.setHSL(aa,1,0.5);
		}else{ball.mat.color.setHSL(1,1,1)}
    	if(gem.position.x-8<ball.position.x && ball.position.x<gem.position.x+8 && gem.position.z-8<ball.position.z && ball.position.x<gem.position.z+8){
		gem.position.x=-20+40*Math.random();
	    gem.position.z=-10+40*Math.random();
		t++
		score.setText(t);
	}
