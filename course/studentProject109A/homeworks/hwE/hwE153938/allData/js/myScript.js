...
function init(){
	...
	for(var j=0 ; j<NN ; j++){
		for(var i=0 ; i<nn ; i++){
			var b = new TEACHER.ObjBox(2,1,1,0x0000ff);
			b.position.x = -50+i*2
			b.position.y = AA;
			b.position.z = bz;
			scene.add(b);
	...
	pts = new TEACHER.Points(0xffff00,2,50000);
	for(var j=0 ; j<10 ; j++){
		for(var i=0 ; i<NN*50 ; i++){
			pts.addPoint(1.2*(i%NN-0.5*NN),1+j,1.2*(Math.floor(i/NN)-0.5*NN));
			XArray.push(1.2*(i%NN-0.5*NN));
			YArray.push(1+j);
			ZArray.push(1.2*(Math.floor(i/NN)-0.5*NN));
		}
	}
	pts.update();
	scene.add(pts);
	...
	world2D.btn02.setLabel('地震波');
	world2D.ch01.setLabel('箭頭');
	world2D.ch02.setLabel('153938 蘇歆媛');
...
function tick(){
	...
	if(gamemode===1){
		...
		for(var j=0 ; j<NN ; j++){
			for(var i=0 ; i<nn ; i++){
				var b1=ballArray1[j*nn+i];

				var yy1 = AA*Math.sin(2*Math.PI*b1.position.x/lambda + 2*Math.PI*tt/period);

				b1.position.y=AA+yy1;


				if(Math.abs(b1.position.x-ball.position.x)<1.5){
					bally = b1.position.y+5;
				}
			}
		}
		...
		if(ball.position.x>45 && ball.vx>0){
			ball.vx*=-1;
		}else if(ball.position.x<-45 && ball.vx<0){
			ball.vx*=-1;
		}

		if(ball.position.z>45 && ball.vz>0){
			ball.vz*=-1;
		}else if(ball.position.z<-45 && ball.vz<0){
			ball.vz*=-1;
		}
		...
		arrowA.position.x = ball.position.x;
		arrowA.position.y = ball.position.y;
		arrowA.position.z = ball.position.z;

		arrowV.position.x = ball.position.x;
		arrowV.position.y = ball.position.y;
		arrowV.position.z = ball.position.z;

		arrowA.setArrow(ball.vx*2,ball.vy*0.3,ball.vz*2);
		arrowV.setArrow(ball.ax*3,ball.ay*0.3,ball.az*3);
		...
		if(mode===1){
			world2D.btn01.setLabel('P波');
			world2D.btn02.setLabel('換成S波');
			world2D.sl03.visible = true;

			for(var j=0 ; j<50 ; j++){
				for(var p=0 ; p<10 ; p++){
					for(var i=0 ; i<100 ; i++){
						var pt = pts.getPoint(i*50+p*50*100+j);
		
						var xx = XArray[i*50+p*50*100+j]+AA*Math.sin(2*Math.PI*XArray[i*50+p*50*100+j]/lambda + 2*Math.PI*tt/period);
						var zz = pt.z;
						var yy = pt.y;
		
						pts.setPoint(i*50+p*50*100+j,xx,yy,zz);
			...
			world2D.btn01.setLabel('S波');
			world2D.btn02.setLabel('換成洛夫波');
			world2D.sl03.visible = true;

			for(var j=0 ; j<10 ; j++){
				for(var i=0 ; i<NN*50 ; i++){
					var pt = pts.getPoint(j*(NN*50)+i);
					var xx = pt.x;
					var zz = pt.z;
					var yy = 1+j+AA*Math.sin(2*Math.PI*xx/lambda + 2*Math.PI*tt/period);
		
					pts.setPoint(j*(NN*50)+i,xx,yy,zz);
				}
			}
			pts.update();

		}else if(mode===3){
			world2D.btn01.setLabel('洛夫波');
			world2D.btn02.setLabel('換成雷利波');
			world2D.sl03.visible = true;
			...
			for(var j=0 ; j<50 ; j++){
				for(var p=0 ; p<10 ; p++){
					for(var i=0 ; i<100 ; i++){
						...
						var xx = pt.x;
						var zz = ZArray[i*50+p*50*100+j]+(pt.y+1)*0.2*AA*Math.sin(2*Math.PI*XArray[i*50+p*50*100+j]/lambda + 2*Math.PI*tt/period);
						var yy = pt.y;
			...
			world2D.btn01.setLabel('雷利波');
			world2D.btn02.setLabel('換成P波');
			world2D.sl03.visible = false;

			for(var j=0 ; j<50 ; j++){
				theta = theta+omega*dt*0.1;
		
				for(var p=0 ; p<10 ; p++){
					for(var i=0 ; i<100 ; i++){
						var pt = pts.getPoint(i*50+p*50*100+j);
		
						var xx = XArray[i*50+p*50*100+j]+rr*(p+1)*0.25*Math.cos(theta+(XArray[i*50+p*50*100+j]+60)*0.1);
						var zz = pt.z;
						var yy = YArray[i*50+p*50*100+j]+rr*(p+1)*0.5*Math.sin(theta+(XArray[i*50+p*50*100+j]+60)*0.1);
...
function clickBtn(e){
	...
	if(gamemode===1){
		if(str==='btnUp'){
			...
			for(var j=0 ; j<10 ; j++){
				for(var i=0 ; i<NN*50 ; i++){
					var pt = pts.getPoint(i+j*NN*50);
					
					var xx = XArray[i+j*NN*50];
					var yy = YArray[i+j*NN*50];
					var zz = ZArray[i+j*NN*50];

					pts.setPoint(i+j*NN*50,xx,yy,zz);
				}
			}
			pts.update();
