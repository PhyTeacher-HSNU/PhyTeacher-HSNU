...
function init(){
	...
	var t=new TEACHER.ObjTextPlane(100,20,"1553 劉昆泰",'z',0xffff00);
	t.position.z=-100;
	t.position.y=50;
	...
	bar = new TEACHER.ObjCylinder(0.5, rr,0xffff00 ,true,"y");
	center = new TEACHER.ObjCylinder(1, 4, 0x00ffff,false,"z");
	center.position.y=yy;

	ball.position.y=bar.position.y=yy+2;


	
	scene.add(center).add(ball).add(bar).add(t);
	...
	world2D.sl01.minimum=-2.0;
	world2D.sl01.maximum= 2.0;
...
function tick(){
	...
	var bal=world2D.sl03.value;
	ball.scale.x=bal/6;
	ball.scale.y=bal/6;
	ball.scale.z=bal/6;
	
	omg=world2D.sl01.value;
	...
	if(throwBall==false){
		//ball
		ball.position.y=rr*Math.cos(theta)+yy;
		ball.position.x=rr*Math.sin(theta);
		theta+=omg*dt;
		arrowV.setArrow(1.3*vv*Math.cos(theta),-1.3*vv*Math.sin(theta),0 );
		ball.add(arrowV);
		arrowV.visible = world2D.ch01.checked;
	}
	else if(throwBall==true){
		if(throw1==1){
			vx = vv*Math.cos(theta);
			vy = -1*vv*Math.sin(theta);
		}
		throw1++;
		vx+=ax*dt;
		vy+=ay*dt;
		if(ball.position.x<=100 && ball.position.y<-48 && vy<0 && ball.position.x>=-100){
			vy+=-vy*1.7;
		}
		ball.position.x+=vx*dt;
		ball.position.y+=vy*dt;

		arrowV.setArrow(1.5*vx,1.5*vy,0 );
		ball.add(arrowV);
		arrowV.visible = world2D.ch01.checked;
	...
	//bar
	bar.scale.y = rr/25;
	bar.position.y=0.5*rr*Math.cos(theta)+yy;
	bar.position.x=0.5*rr*Math.sin(theta);
...
skyBox.wallU=new TEACHER.ObjPicPlane(10000,10000,pics.blacksky,'y',1);
skyBox.wallD=new TEACHER.ObjPicPlane(10000,10000,pics.redground,'y',0);
skyBox.wallS=new TEACHER.ObjPicCylinder(5000,10000,pics.blacksky,true,'y',1);
...
var ground=new TEACHER.ObjPicPlane(200,200,pics.warground,'y');
