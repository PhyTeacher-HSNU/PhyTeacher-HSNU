...
//B.定義init
function init(){
	aaBall=new TEACHER.ObjSphere(3,0xff00ff);
	bbBall=new TEACHER.ObjSphere(3,0xff00ff);
	pps=new TEACHER.Points(0xffff00,0.4,pointNum*pointNum);
	
	for(var i=0;i<pointNum*pointNum;i++){
		pps.addPoint((i%pointNum)*100/pointNum-50,-10,((i-i%pointNum)/pointNum)*100/pointNum-50)
	}
	aaBall.position.x=-10
	bbBall.position.x=10
	aaBall.position.z=-40
	bbBall.position.z=-40
	...
	world2D.sl03.setLabel('間距');
	world2D.sl03.value=20;
	world2D.ch02.visible=false;
	world2D.ch01.checked=true;
	world2D.ch01.setLabel('Run');
	pps.update();
	scene.add(aaBall).add(bbBall).add(pps);
...
//C.定義tick                                                                                                                       
function tick(){
	aaBall.position.x=world2D.sl03.value/-2;
	bbBall.position.x=world2D.sl03.value/2;
	if(world2D.ch01.checked){
		tt=tt+dt;
	}
	for(var i=0;i<pointNum*pointNum;i++){
		ramx=(i%pointNum)*100/pointNum-50;
		ramz=((i-i%pointNum)/pointNum)*100/pointNum-50;
		ramda=Math.sqrt((ramx-aaBall.position.x)**2+(ramz-aaBall.position.z)**2);
		ramdb=Math.sqrt((ramx-bbBall.position.x)**2+(ramz-bbBall.position.z)**2);
		pps.setPoint(i,ramx,Math.sin(((tt/lt-ramda/lam)*2*Math.PI))*yy+Math.sin(((tt/lt-ramdb/lam)*2*Math.PI))*yy-10,ramz);
	}
	pps.update();
