...
function init(){
	...
	world2D.sl02.setLabel('撞牆反彈恢復係數');
	...
	world2D.btn02.setLabel('回到原點');
	world2D.btn01.on('click' , clickBtn);
	world2D.btn02.on('click' , clickBtn);

	world2D.ch01.setLabel('顯示速度方向');
	world2D.ch02.setLabel('顯示加速度方向');
...
/*	if(ball.position.z < 0){
	...
	ball.ax = -kk *(ball.position.x) - bb*ball.vx;
	ball.vx += ball.ax*dt;
	ball.position.x += ball.vx*dt;
}
*/
	if(ball.position.x > 50-rr &&ball.vx > 0){
	   ball.vx *= -(world2D.sl02.value);
	...
	ball.scale.x = ball.scale.y = ball.scale.z = rr/2;


	arrowV.visible = world2D.ch01.checked;
	arrowA.visible = world2D.ch02.checked;
