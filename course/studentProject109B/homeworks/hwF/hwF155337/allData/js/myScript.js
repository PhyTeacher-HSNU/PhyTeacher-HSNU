...
var ballArray8=[];
var ballArray9=[];
var z1=-40,z2=-30,z3=-20,z4=-10,z5=0,z6=10,z7=20,z8=30,z9=40
var lambda=50;
var period=10;
var tt=0;
var dt=0.1
...
function init(){
	...
	var tittle=new TEACHER.ObjTextPlane(50,10,'1553 蘇昱安')
	scene.add(tittle);
	tittle.position.y=20;
	tittle.position.z=-50;
	//海浪陣列
	for(var i=0;i<nn;i++){
		var b=new TEACHER.ObjSphere(1,0x0000FF);
		b.position.x=-50+i*2;
		b.position.y=3;
		b.position.z=z1;
		scene.add(b);
	...
	world2D.sl01.setLabel('海浪');	
    world2D.sl01.minimum=-0.8;
    world2D.sl01.maximum=0.8;
    world2D.sl01.value=0.1;
    world2D.sl01.digitN=1;
    //海面
	var S=new TEACHER.ObjBox(100,5,100,0x0000FF);
	scene.add(S);
	//船
	B=new TEACHER.ObjBox(30,20,20,0x8b4513,2);
	scene.add(B);
	r=new TEACHER.ObjCylinder(0.5,30,0x8b4513);
	scene.add(r)
	p=new TEACHER.ObjPlane(20,15,0x00ff00,'x',2);
	scene.add(p)
	ro=new TEACHER.ObjCylinder(1,2,0x8b4513);
	scene.add(ro)
	r1=new TEACHER.ObjCylinder(0.9,12,0x8b4513);
	r1.rotation.z=-20
	r1.position.x=19
	scene.add(r1)
	...
	shark=new TEACHER.ObjBox(2,10,10,0x808080)
	shark.position.y=2;
	shark.rotation.x=43;
	con.add(shark);
	scene.add(con)
	world2D.ch01.setLabel('鯊魚');
	//人物
...
function tick(){
	...
	for(var i=0;i<nn;i++){
        ...
        b1.position.y=10+yy1-5;
        b2.position.y=10+yy2-5;
        b3.position.y=10+yy3-5;
        b4.position.y=10+yy4-5;
        b5.position.y=10+yy5-5;
        b6.position.y=10+yy6-5;
        b7.position.y=10+yy7-5;
        b8.position.y=10+yy8-5;
        b9.position.y=10+yy9-5;
	}
	//船動畫
	B.position.y=10+yy1-7;
	r.position.y=10+yy1+8;
	p.position.y=10+yy1+14;
	ro.position.y=10+yy1+23;
	r1.position.y=r.position.y-4
	
	//鯊魚
	con.visible=world2D.ch01.checked;
	theta+=omega*t;
    con.position.x=rr*Math.cos(theta);
    con.position.z=rr*Math.sin(theta);
	con.rotation.y=-theta;
	//人
	if (h.position.x<15 && h.position.x>-15 && h.position.z<10 && h.position.z>-10){
	h.position.y=10+yy1+3.8}
	else if(h.position.y<-20){
    h.position.x=1;
	h.position.z=0;
	}
	else h.position.y=h.position.y-1
