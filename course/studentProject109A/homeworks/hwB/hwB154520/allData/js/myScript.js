...
//B.定義init
function init(){
	score=new TEACHER.ObjTextPlane(300,10,true,'z')
	score1=new TEACHER.ObjTextPlane(300,10,'score',true,'z')
	name1=new TEACHER.ObjTextPlane(300,10,'1545張博崴',true,'z')
	...
	score1.position.z=-50
	scene.add(score)
	scene.add(name1)
	scene.add(score1)
	ball=new TEACHER.ObjSphere(2,0xffffff)
	coin=new TEACHER.ObjPicCylinder(3,1,pics.coin,false,'z')
	coin.position.y=3
	coin.position.x=(Math.random()-0.5)*97
	coin.position.z=(Math.random()-0.5)*97
	scene.add(coin)
	coinarray.push(coin)
	scene.add(ball)
	...
	for(var i=0;i<4;i++){
		wall=new TEACHER.ObjBox(100,10,3,555599)
		log((Math.random-0.5)*97)
	...
	world2D.ch01.setLabel('wall')

	wallarray[0].position.z=-50-1.7
	wallarray[3].position.z=50+1.5
	wallarray[1].position.x=-50-1.5
	
	
	wallarray[2].position.x=50+1.5
	wallarray[1].rotation.y=Math.PI/2
	wallarray[2].rotation.y=Math.PI/2
...
function tick(){
	for(var i=0;i<4;i++){
		wallarray[i].visible=world2D.ch01.checked
	}
	if(ball.position.x>-50+2&&ball.position.x<50-2&&ball.position.z>-50+2&&ball.position.z<50-2){
		vx+=ax-vx/50
		vz+=az-vz/50
	}
	...
	ball.position.z+=vz
	ball.position.y=2
	if(ball.position.x>50-2&&vx>=0){
		vx*=-1
	}
	if(ball.position.x<-50+2&&vx<=0){
		vx*=-1
	}
	if(ball.position.z>50-2&&vz>=0){
		vz*=-1
	}
	if(ball.position.z<-50+2&&vz<=0){
		vz*=-1
	...
	for(var i=0;i<nn;i++){
		var coinx=coinarray[i].position.x
		var coinz=coinarray[i].position.z
		var ballz=ball.position.z
		var ballx=ball.position.x
		coinarray[i].rotation.y+=0.5
		if(-5<ballx-coinx&&ballx-coinx<5&&-5<ballz-coinz&&ballz-coinz<5){
			coinarray[i].position.x=(Math.random()-0.5)*97
			coinarray[i].position.z=(Math.random()-0.5)*97
			sc+=1
			if(nn<50){
				coin=new TEACHER.ObjPicCylinder(3,1,pics.coin,false,'z')
				scene.add(coin)
...
//ground地板
var ground=new TEACHER.ObjPlane(100,100,0x555599,'y');
