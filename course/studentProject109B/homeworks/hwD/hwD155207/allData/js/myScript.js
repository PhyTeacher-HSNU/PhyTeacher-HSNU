...
function init(){
	...
	jupiter=new TEACHER.ObjPicSphere(10,pics.jupiter,0);
	...
	world2D.slCameraRR.maximum=900;
...
function tick(){
	...
	if(cha==1){
		...
		lin7.addPoint(jupiter.position.x+18*Math.cos(theta*50),0,18*Math.sin(theta*50)+jupiter.position.z);
	...
	earth.position.x=dist*Math.cos(theta);
	earth.position.z=dist*Math.sin(theta);

	mercury.position.x=dist*0.5*Math.cos(theta*4);
	mercury.position.z=dist*0.5*Math.sin(theta*4);

	venus.position.x=dist*0.7*Math.cos(theta*1.5);
	venus.position.z=dist*0.7*Math.sin(theta*1.5);

	mars.position.x=dist*1.52*Math.cos(theta*0.5);
	mars.position.z=dist*1.52*Math.sin(theta*0.5);

	jupiter.position.x=dist*2.5*Math.cos(theta/12);
	jupiter.position.z=dist*2.5*Math.sin(theta/12);

	moon.position.x=9*Math.cos(theta*12);
	moon.position.z=9*Math.sin(theta*12);

	io.position.x=18*Math.cos(theta*50);
	io.position.z=18*Math.sin(theta*50);
