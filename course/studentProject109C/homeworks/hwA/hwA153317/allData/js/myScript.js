...
function init(){
	dt=0.01
	car = new TEACHER.ObjPicPlane(40,20,pics.car,"z",0);
	bigtire1 = new TEACHER.ObjPicCylinder(5,6,pics.tire1,false,"y",0);
	bigtire2 = new TEACHER.ObjPicCylinder(5,6,pics.tire2,false,"y",0);
	smalltire1 = new TEACHER.ObjArrow(3,0x600000);
	smalltire2 = new TEACHER.ObjArrow(4,7,0x000000);
	...
	scene.add(bigtire1);
	scene.add(bigtire2);
	//scene.add(smalltire1);
	//scene.add(smalltire2);
	car.position.y=15;
	car.position.x=0;
	
	bigtire1.position.y=5;
	bigtire1.position.x=-10;
	bigtire1.rotation.x=0.5*Math.PI;
	smalltire1.position.y=10;
	smalltire1.position.x=0;
	smalltire1.position.z=-5




	bigtire2.position.y=5;
	bigtire2.position.x=10;
	bigtire2.rotation.x=0.5*Math.PI;
...
function tick(){
	...
	if (car.position.x > 30 && car.vx > 0){
		car.vx = bigtire1.vx = bigtire2.vx = smalltire1.vx = -vv;


	}else if (car.position.x < -30 && car.vx < 0){
		car.vx = bigtire1.vx = bigtire2.vx = smalltire1.vx = vv;

	}
	bigtire1.rotation.y-=bigtire1.vx*dt*rr/5 ;
	bigtire2.rotation.y-=bigtire2.vx*dt*rr/5 ;

	

	car.position.x+=car.vx*dt;
	bigtire1.position.x+=bigtire1.vx*dt;
	bigtire2.position.x+=bigtire2.vx*dt;
	smalltire1.position.x+=bigtire1.vx*dt;
