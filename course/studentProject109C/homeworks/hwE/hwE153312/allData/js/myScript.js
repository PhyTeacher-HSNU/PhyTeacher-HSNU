...
//B.定義init
function init(){
	stars = new TEACHER.Points(0xFF00FF, 2, 10000);
	scene.add(stars);
	for(var i = 0 ; i < 100 ; i ++){
		var xx = 100 * (Math.random()-0.5);
		var yy = 0;
		var zz = 100 * (Math.random()-0.5);
		stars.addPoint(xx, yy, zz);
	...
	ball = new TEACHER.ObjSphere(5, 0x660066);
	scene.add(ball);
	skyBox.visible = false;
	ground.visible = false;
	
	bigg = new TEACHER.ObjPicPlane(100, 50, pics.name, "z", 0);
	scene.add(bigg);
	bigg.position.z = -50;
	
	ball = new TEACHER.ObjSphere(rr, 0xff00ff, 0);
	scene.add(ball);
	ball.position.y = rr;
	...
	world2D.slCameraRR.value = 95;
	scene.fog = new THREE.Fog(0x000000, fogn, fogf);
