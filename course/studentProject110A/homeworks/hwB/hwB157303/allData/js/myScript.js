...
function init(){
    ...
    scene.add(ball);
	ball.position.y = world2D.sl02.value*2
	
	word = new TEACHER.ObjTextPlane(60,10,`157303朱浩鋒`,`z`)
	scene.add(word)
	word.position.z = -50
	word.position.x = -30
	word.position.y = 15

	word01 = new TEACHER.ObjTextPlane(60,10,`鍵盤可控制方向`,`z`)
	scene.add(word01)
	word01.position.z = -50
	word01.position.x = 30
	word01.position.y = 15

    arV = new TEACHER.ObjArrow(1,0xFFFF00)
	scene.add(arV)
	arA = new TEACHER.ObjArrow(1,0x00FFFF)
	scene.add(arA)
...
//C.定義tick                                                                                                                       
function tick(){
	ball.scale.x = ball.scale.y = ball.scale.z = world2D.sl02.value
    
	
	if(ball.position.y<2*world2D.sl02.value && yv<0 && gr == 0){
		ya = 0
		yv *=-0.7*a
		console.log(`反彈`)
		if(yv<0.2){
	...
	}else if(ball.position.y>2.1*world2D.sl02.value && gr == 0){
		ya = -1
		yv += ya*dt
	}
	if(ball.position.y<2*world2D.sl02.value){
		ball.position.y=2*world2D.sl02.value
		console.log(`定位`)
	}
	...
	if(ball.position.x>50-world2D.sl02.value*2 && xv>0){
		xv *= -1*a
	}else if(ball.position.x<-50+world2D.sl02.value*2 && xv<0){
		xv *= -1*a
	}

	if(ball.position.z>50-world2D.sl02.value*2 && zv>0){
		zv *= -1*a
	}else if(ball.position.z<-50+world2D.sl02.value*2 && zv<0){
		zv *= -1*a
	}

	if(gr == 1){
		if(ball.position.y>40-world2D.sl02.value*2 && yv>0){
			yv *= -1*a
		}else if(ball.position.y<0+world2D.sl02.value*2 && yv<0){
			yv *= -1*a
		}
	}

	b = world2D.sl03.value
    xv -= b*xv*0.005
	zv -= b*zv*0.005
	yv -= b*yv*0.005
	...
	if(world2D.ch02.checked === true){
		gr = 1
		world2D.btn02.setLabel(`下降(F鍵)`)
		world2D.btn01.setLabel(`上升(空白鍵)`)
	}else {
		gr = 0
		world2D.btn02.setLabel(`<長按大跳`)
		world2D.btn01.setLabel(`跳(空白鍵)`)
	}

	arV.position.x = arA.position.x = ball.position.x
	arV.position.y = arA.position.y = ball.position.y
	arV.position.z = arA.position.z = ball.position.z
	arV.setArrow(xv*4*world2D.sl02.value,yv*4*world2D.sl02.value,zv*4*world2D.sl02.value)
	arA.setArrow(xa*12*world2D.sl02.value,ya*12*world2D.sl02.value,za*12*world2D.sl02.value)

   if(UD == 38){
	za = -g/world2D.sl02.value
	console.log(`U`)
   }else  if(UD == 40){
	za = g/world2D.sl02.value
	console.log(`D`)
...
function clickbtn(e){
	...
	if(str === `btnUp`){
		...
		xa = g/world2D.sl02.value
	}else if(str === `btn01` && JU == -1){	
	    JU = 1
	}else if(str === `btn02` && gr == 1){	
	    yv = -2/world2D.sl02.value
	}
}
