...
function init(){
	...
	ground.position.y=-ballR;

	ballA=new TEACHER.ObjSphere(5,0xffff00);
	ballB=new TEACHER.ObjSphere(5,0x00ffff);
	ballA.position.x=-15;
	ballB.position.x=15;
	...
	boxA=new TEACHER.ObjBox(2,2*ballR,2*ballR,0xffffff);
	scene.add(boxA);
	boxA.position.x=-41;

	boxB=new TEACHER.ObjBox(2,2*ballR,2*ballR,0xffffff);
	scene.add(boxB);
	boxB.position.x=41;

	spring1=new TEACHER.ObjSpring(L0 , 0.4*ballR , 0.2 ,8,0x090909);
	spring2=new TEACHER.ObjSpring(L0 , 0.4*ballR , 0.2 ,8,0x090909);
	spring3=new TEACHER.ObjSpring(L0 , 0.4*ballR , 0.2 ,8,0x090909);
	scene.add(spring1).add(spring2).add(spring3);
	
	//設k值
	world2D.sl01.setLabel('k左');
	world2D.sl01.maximum=10;
	world2D.sl01.minimum=0.1;
	world2D.sl01.value=2;
	world2D.sl01.digitN=1;
	
	world2D.sl02.setLabel('k中');
	world2D.sl02.maximum=10;
	world2D.sl02.minimum=0.1;
	world2D.sl02.value=2;
	world2D.sl02.digitN=1;

	world2D.sl03.setLabel('k右');
	...
	//設質心
	ballC=new TEACHER.ObjSphere(1.5,0xffffff);
	scene.add(ballC);
	ballC.position.x=0;
	ballC.position.z=ballR
	ballC.visible=false;

	//質心
	world2D.ch01.setLabel('質心');
...
function tick(){
	...
	//設k值
	k1=world2D.sl01.value;
	k2=world2D.sl02.value;
	k3=world2D.sl03.value;

	//ballC
	ballC.position.x=ballA.position.x/2+ballB.position.x/2;
	ballC.visible=world2D.ch01.checked;

	//dxA&dxB
	var dxA = ballA.position.x-(-15);
	var dxB = ballB.position.x-(15);

	ballA.ax=-k1*dxA - k2*(dxA-dxB);
	ballB.ax=-k3*dxB + k2*(dxA-dxB);
