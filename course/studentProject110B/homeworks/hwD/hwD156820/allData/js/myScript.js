...
function init(){
	...
	//姓名
	{
		rule = new TEACHER.ObjTextPlane(80,7,"試著用毛線球打擊老鼠吧!",'z',0xffffff,0x191970);
		scene.add(rule);
		rule.position.x=0;
		rule.position.z=50;
		rule.position.y=15;
	...
	//貓手1
	{
	f = new TEACHER.ObjCylinder(0.3,3,0xffffff,false,"x");
	f.position.y=4;
	f.position.x=2.5;
	f.position.z=0.8;
	f.rotation.z=100;
	...
	//貓手2
	{
	f2 = new TEACHER.ObjCylinder(0.3,3,0xffffff,false,"x");
	f2.position.y=3.5;
	f2.position.x=2.3;
	f2.position.z=-0.8;
	f2.rotation.z=150;
	...
	con2.position.y=50*Math.random();
	}
	...
	//毛線球
	{
	b = new TEACHER.ObjPicSphere(r,pics.ball);
	con.add(b);
	b.position.x=sm;
	}
	...
	//彈簧
	{
	spring=new TEACHER.ObjSpring(25,0.5,0.2,3,0xb0c4de);
	scene.add(spring);
	//spring.position.x=2;
	spring.position.y=2.3;
	spring.position.z=-0.8;
	spring.rotation.z=1.5*Math.PI;
	con.add(spring);
	}
	...
	//arrowV
	{
	arrowV=new TEACHER.ObjArrow(0.8,0x7fffd4);
	arrowV.scale.x=0.3;
	arrowV.scale.z=0.5;
	arrowV.scale.y=0.5;
	b.add(arrowV);
	...
	//btn01,02
	world2D.btn01.setLabel("向上");
	world2D.btn02.setLabel("向下");
	...
	world2D.btnLeft.visible=world2D.btnRight.visible=false;
...
function tick(){
	...
	con3.rotation.y=-theta;
	rule.visible=world2D.ch02.checked;
	//計數
	aa(b,con2,r);

	//毛線球大小
	r = world2D.sl02.value;
	b.scale.x=b.scale.y=b.scale.z=r;
	b.position.y=r;
	b.position.x=sm;

	//毛線長度
	sm = world2D.sl03.value;
	spring.scale.y=sm/25;
	...
	//毛線球轉
	b.position.x=sm*Math.cos(theta);
	b.position.z=sm*Math.sin(theta);
	
	//彈簧
	spring.position.x=0.00000001*sm*Math.cos(theta);
	spring.position.z=0.00000001*sm*Math.sin(theta);
	spring.rotation.y=-theta;
...
function clickbtn(e){
	...
	if(str==='btnUp'){
		con.rotation.x-=1/180*Math.PI;
		con3.rotation.x-=1/180*Math.PI;
