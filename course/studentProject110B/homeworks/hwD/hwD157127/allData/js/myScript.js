...
function init(){
	...
	name1.position.y = 40;
	for(var i=0; i<n ; i++){
		var obs =new TEACHER.ObjCylinder(2,20,"#00C5CD");
		obs.position.y=10;
		obs.position.x=30;
		obs.position.z=-48+(i%15)*15;
		scene.add(obs);
		obsArray.push(obs);	
	}
	for(var i=0; i<n1 ; i++){
		var obs =new TEACHER.ObjCylinder(2,20,"#00868B");
		obs.position.y=10;
		obs.position.x=60;
		obs.position.z=-48+(i%15)*10;
		scene.add(obs);
		obsArray1.push(obs);	
	}
	world2D.slCameraRR.value=180;
	ball = new TEACHER.ObjSphere(2,0xff0000,2);
	bar = new TEACHER.ObjCylinder(0.5,r,0xFFFF00,true,"x");
	center = new TEACHER.ObjCylinder(2,10,"000000");
	scene.add(ball).add(bar).add(center);
	center.position.x = -35;
	
	arrowV = new TEACHER.ObjArrow(1,0xFFFFFF);
	ball.add(arrowV);
	...
	world2D.sl01.value = 0.3 ;
	world2D.sl01.digitN = 1;

	world2D.sl02.setLabel("r");
...
function tick(){
	...
	if(Gamemode == 0){
		bar.visible=center.visible=true;
		ball.position.y=center.position.y=bar.position.y=2*world2D.sl03.value;
		center.scale.y = world2D.sl03.value;
		ball.scale.x=world2D.sl03.value;
		ball.scale.z=world2D.sl03.value;
		ball.scale.y=world2D.sl03.value;
		omega = world2D.sl01.value;
		r = world2D.sl02.value;
		theta += omega*dt;
		ball.position.x = -35+r*Math.cos(theta);
		ball.position.z = r*Math.sin(theta);
		bar.position.x = -35+0.5*r*Math.cos(theta);
		bar.position.z = 0.5*r*Math.sin(theta);
		bar.rotation.y =-theta;
		bar.scale.x = r/25;
		var v = r*omega;
		arrowV.setArrow(1.5*-v*Math.sin(theta),0,1.5*v*Math.cos(theta));
		arrowV.visible = world2D.ch01.checked;
	}
	else if(Gamemode == 1){
		//bar.visible=center.visible=false;
		if(throwball==1){
		ball.position.x+=Math.sin(theta)*-5*omega;
		ball.position.z+=Math.cos(theta)*omega*5;
		}		
	}
	//3D
	world3D.render();
...
//ground地板
var ground=new TEACHER.ObjPicPlane(180,100,pics.ground,'y');
