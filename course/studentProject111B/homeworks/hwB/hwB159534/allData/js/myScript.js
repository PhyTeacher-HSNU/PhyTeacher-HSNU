...
function init(){
	ball=new TEACHER.ObjPicSphere(rr,pics.basketball)
	scene.add(ball)
	ball.position.x=0
	ball.position.y=rr
	ball.position.z=50-rr
	ball.vx=0
	ball.vy=0
	ball.vz=0
	...
	world2D.ch01.setLabel("球出現")
	
	world2D.btn01.setLabel("投球")
	world2D.btn02.setLabel("重置")

	world2D.sl01.setLabel("向上速度")
	...
	world2D.sl02.setLabel("向前速度")
	world2D.sl02.maximum=7
	...
	world2D.btn01.on('click',clickfunction)
	world2D.btn02.on('click',restartfunction)
...
function tick(){
	...
	if(GM===1){
		...
		ball.rotateOnWorldAxis(new THREE.Vector3(Math.sin(phiRot),0,Math.cos(phiRot)),vv/rr)

		if(ball.vz<0.02 && ball.vz>0){
			ball.vz=0
		...
		if(ball.vx<0.02 && ball.vx>0){
			ball.vx=0
			
		}else if(ball.vx>-0.02 && ball.vx<0){
			ball.vx=0
		}
	...
	if(ball.position.z>a && ball.vz>0){
		ball.vz*=-1*xx
		
	}else if(ball.position.z<b && ball.vz<0){
		ball.vz*=-1*xx
		
	}

	if(ball.position.x>a && ball.vx>0){
		ball.vx*=-1*xx
		
	}else if(ball.position.x<b && ball.vx<0){
		ball.vx*=-1*xx
