...
//-------------------開始做我的動畫-------------------//
/*任務：
1. 產生兩波源s1, s2 與干涉位置 p：
	(1) s1, s2 的位置 z0=-30, x 間隔為 d=20。
	(2) p 點的位置用 sl01調整距離10~200，sl02調整方位角-90~+90度

2. 兩波干涉
	(1) 用 TEACHER.Line畫出 s1-p的連線，s2-p的連線。
	(2) 用 TEACHER.Line畫出 s1-p的波動，s2-p的波動。
	(3) 用箭頭表示波源的振動與干涉的結果。

3. 平面上的干涉
	(1) 用 TEACHER.Points 顯示整的平面上干涉的結果
...
//B.定義init
function init(){
	var name=new TEACHER.ObjTextPlane(100,15,'1564 03 王俐惠','z')
	scene.add(name)
	name.position.z=-80
	name.position.y=20

	skyBox.visible=ground.visible=logo.visible=false
	...
	//膜
	pts=new TEACHER.Points(0xffffff,2,90000)
...
function tick(){
	...
	for(var i=0;i<nn*nn;i++){
		...
		var zz=pt.z
		var yy=0
		for(var y=0;y<ttnn;y++){
			var lambda=lambdaarray[y]
			var period=periodarray[y]
			var wave=wavearray[y]
			var xx0=xx0array[y]
			var zz0=zz0array[y]
			var rr=Math.sqrt((xx-xx0)*(xx-xx0)+(zz-zz0)*(zz-zz0))
			yy+=wave*Math.sin(2*Math.PI*rr/lambda-2*Math.PI*tarray[y]/period)
			if(rr>tarray[y]*lambda/period){
				yy-=wave*Math.sin(2*Math.PI*rr/lambda-2*Math.PI*tarray[y]/period)
			}
		}
...
function addwave(x,z){
	var lambda=world2D.sl01.value
	var period=world2D.sl02.value
	var wave=world2D.sl03.value
