...
function init(){
	...
	world2D.ch01.setLabel("相對位置圖表");
	world2D.ch02.setLabel("速度圖表");
	...
	graph1 = new TEACHER.ObjGraph(0,10,-15,15,0xffffff,0xff0000,100);
	scene.add(graph1)
	graph1.labelTitle.setText("")
	graph1.position.z=-50
	graph1.labelY.setText("")

	graph2 = new TEACHER.ObjGraph(0,10,-15,15,0xffffff,0xffff00,100);
	scene.add(graph2)
	graph2.labelTitle.setText("相對位置")
	graph2.position.z=-50
	graph2.labelY.setText("x")
	
	graph3 = new TEACHER.ObjGraph(0,10,-25,25,0xffffff,0xff0000,100);
	scene.add(graph3)
	graph3.labelTitle.setText("")
	graph3.labelY.setText("")
	graph3.position.z=-50

	graph4 = new TEACHER.ObjGraph(0,10,-25,25,0xffffff,0xffff00,100);
	scene.add(graph4)
	graph4.labelTitle.setText("速度")
	graph4.position.z=-50
	graph4.labelY.setText("vx")
	
	graph5 = new TEACHER.ObjGraph(0,10,-10,10,0xffffff,0xffffff,100);
	scene.add(graph5)
	graph5.labelTitle.setText("")
	graph5.labelY.setText("")
	graph5.position.z=-50

	graph6 = new TEACHER.ObjGraph(0,10,-10,10,0xffffff,0x0000AA,100);
	scene.add(graph6)
	graph6.labelTitle.setText("質心x與vx")
	graph6.position.z=-50
	graph6.labelY.setText("x/白 vx/藍")

	
	wallA = new TEACHER.ObjBox(2,2*ballR , 2*ballR , 0x00ffff);
	wallB = new TEACHER.ObjBox(2,2*ballR , 2*ballR , 0x00ffff);
	wallA.position.x=-40-1-mm
	wallB.position.x=40+1+mm
	wallA.position.y=wallB.position.y=ballR
	scene.add(wallA).add(wallB)

    ballA = new TEACHER.ObjSphere(ballR,0xff0000)
	ballB = new TEACHER.ObjSphere(ballR,0xffff00)
	ballA.position.x=-15;
	ballB.position.x=15;
	ballA.position.y=ballR
	ballB.position.y=ballR
	scene.add(ballA).add(ballB)

	ballA.vx=0
	ballB.vx=0
	ballA.position.x+=10;
	
	spring1 = new TEACHER.ObjSpring(L0,0.8*ballR,0.4,8,0xffffff)
	spring2 = new TEACHER.ObjSpring(L0,0.8*ballR,0.4,8,0xffffff)
	spring3 = new TEACHER.ObjSpring(L0,0.8*ballR,0.4,8,0xffffff)
...
function tick(){
	wallA.position.x=-40-1-mm
	wallB.position.x=40+1+mm
	kk=world2D.sl01.value
	mm=world2D.sl02.value
	tt+=dt;
    graph1.xMax = tt;
	graph1.xMin = tt-10;
	graph1.addPoint(tt ,ballA.position.x+12.5)
	graph2.xMax = tt;
	graph2.xMin = tt-10;
	graph2.addPoint(tt ,ballB.position.x-12.5)

	graph3.xMax = tt;
	graph3.xMin = tt-10;
	graph3.addPoint(tt ,ballA.vx)
	graph4.xMax = tt;
	graph4.xMin = tt-10;
	graph4.addPoint(tt ,ballB.vx)

	graph5.xMax = tt;
	graph5.xMin = tt-10;
	graph5.addPoint(tt ,0.5*(ballA.position.x+ballB.position.x))
	graph6.xMax = tt;
	graph6.xMin = tt-10;
	graph6.addPoint(tt ,0.5*(ballA.vx+ballB.vx))

	var dxA = ballA.position.x -(-15)
	var dxB = ballB.position.x -(+15)
	ballA.ax = -kk*dxA-kk*(dxA-dxB)
	ballB.ax = -kk*dxB+kk*(dxA-dxB)
	ballA.vx += ballA.ax*dt
	ballB.vx +=	ballB.ax*dt
	ballA.position.x+=ballA.vx*dt
    ballB.position.x+=ballB.vx*dt

	spring1.position.x=-40-mm
	spring1.setSpring(ballA.position.x-ballR-(-40)+mm,0,0)
	spring2.position.x=ballA.position.x+ballR
	spring2.setSpring(ballB.position.x-ballA.position.x-2*ballR,0,0)
    spring3.position.x=ballB.position.x+ballR
	spring3.setSpring(40-ballB.position.x-ballR+mm,0,0)
	spring3.position.y=spring2.position.y=spring1.position.y=5
