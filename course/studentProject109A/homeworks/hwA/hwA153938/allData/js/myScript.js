...
function init(){
	...
	aa = new TEACHER.ObjBox(100,2,1500,0xFFFFFF);
	scene.add(aa);
	aa.position.z=-300;

	ball = new TEACHER.ObjPicSphere(12,pics.ball);
	scene.add(ball);
	ball.position.y=13;
	ball.position.z=0;

	obstacle01 = new TEACHER.ObjBox(25,25,25,0xFF0000);
	scene.add(obstacle01);
	obstacle01.position.y=13;
	obstacle01.position.z=-200;
	obstacle01.visible=false;

	obstacle02 = new TEACHER.ObjBox(25,25,25,0xFF0000);
	scene.add(obstacle02);
	obstacle02.position.y=13;
	obstacle02.position.z=-350;
	obstacle02.visible=false;

	obstacle03 = new TEACHER.ObjBox(25,25,25,0xFF0000);
	scene.add(obstacle03);
	obstacle03.position.y=13;
	obstacle03.position.z=-50;
	obstacle03.visible=false;

	pp = new TEACHER.ObjTextPlane(115,12,"1539 38     蘇歆媛 ","z",0x00000F,);
	scene.add(pp);
	pp.position.y=4;
	pp.position.z=-200;
	pp.rotation.x=-1;
	...
	gameover = new TEACHER.ObjTextPlane(300,50,"GAME OVER","Z",0xFFFF00,0x00000F);
	scene.add(gameover);
	gameover.position.y=26;
	gameover.visible=false;

	gameover2 = new TEACHER.ObjTextPlane(80,8,"  重整網頁即可再試一次  ","Z",0xFFFFFF);
	scene.add(gameover2);
	gameover2.position.y=10;
	gameover2.position.z=30;
	gameover2.visible=false;

	gameover3 = new TEACHER.ObjPlane(1000,1000,0xFFFFFF);
	scene.add(gameover3);
	gameover3.position.z=-80;
	gameover3.visible=false;

	rule = new TEACHER.ObjPicPlane(300,180,pics.rule);
	scene.add(rule);
	rule.visible=false;
	rule.position.z=-70;

	back1 =  new TEACHER.ObjPlane(20,7,0x00000F);
	scene.add(back1);
	back1.visible=false;
	back1.position.x=116;
	back1.position.y=16;

	back2 =  new TEACHER.ObjPlane(20,7,0x00000F);
	scene.add(back2);
	back2.visible=false;
	back2.position.x=87;
	back2.position.y=62;
	...
	world2D.btn01.setLabel('規則');
	world2D.btn02.setLabel('關閉規則');
	world2D.sl01.setLabel("滾動速度");
	world2D.ch01.setLabel("調整速度");
...
function tick(){
	...
	world2D.sl01.visible=world2D.ch01.checked;
	dx2=world2D.sl01.value;
	if(dx2!==2 && dx!==0){
		dx=dx2;
	}
	...
	if(start === 1){
		...
		var a01=obstacle01.position.x-ball.position.x;
		var a02=obstacle01.position.z-ball.position.z;
		var b01=obstacle02.position.x-ball.position.x;
		var b02=obstacle02.position.z-ball.position.z;
		var c01=obstacle03.position.x-ball.position.x;
		var c02=obstacle03.position.z-ball.position.z;

		if(-24.5<a01 && a01<24.5){
			if(-24.5<a02 && a02<24.5){
				dx=0;
			    start=2;}}

        if(-24.5<b01 && b01<24.5){
			if(-24.5<b02 && b02<24.5){
				dx=0;
			    start=2;}}

        if(-24.5<c01 && c01<24.5){
			if(-24.5<c02 && c02<24.5){
				dx=0;
			    start=2;}}
		...
		gameover3.visible=true;
		back1.visible=true;
		back2.visible=world2D.ch01.checked;
		aa.visible=false;
		ground.visible=false;
		ball.visible=false;
		obstacle03.visible=false;
		world2D.slCameraRR.value=200;
		world3D.cameraTheta=90*Math.PI/180;
		world3D.cameraTarget.z=1;
		dx=0;
	}
	...
	if(ball.position.z<=-500){
		...
		logo.visible=false;
		pp.visible=false;
		obstacle01.visible=true;
		obstacle02.visible=true;
		obstacle03.visible=true;

		function getRandomInt(max) {
			return Math.floor(Math.random() * Math.floor(max));}
	
			var aaa=getRandomInt(3);
			var bbb=getRandomInt(3);
			var ccc=getRandomInt(3);
			//console.log(aaa);
		...
		obstacle01.position.x=obsx01;
		obstacle02.position.x=obsx02;
		obstacle03.position.x=obsx03;
    ...
    if(start!==2 && start!==3){
	world3D.cameraTarget.z=(ball.position.z-30);}
...
skyBox.wallU=new TEACHER.ObjPicPlane(10000,10000,pics.blue,'y',1);
skyBox.wallD=new TEACHER.ObjPicPlane(10000,10000,pics.blue,'y',0);
skyBox.wallS=new TEACHER.ObjPicCylinder(5000,10000,pics.blue,true,'y',1);
