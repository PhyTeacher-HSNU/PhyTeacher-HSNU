...
//A.宣告全域變數
var circlePlane, sinPlane, cosPlane;
var ball, bar, center;
var bar1;
var rrBall=1;
var rrBar=0.2;
...
var dt=1;
var tt=0;
var standardLine1, standardLine2;
var circleLine, sinLine1, sinLine2, cosLine;
var aPoint, topPoint;
//	var pointArray=[];
var NN=10;
var sinGraph, cosGraph;
var myName;
//B.定義init
function init(){
	myName=new TEACHER.ObjTextPlane(50,12,"1590簡郁珊","z",0xf5c738);
	scene.add(myName);
	myName.position.z=-50;
	myName.position.y=55;
	...
	world2D.ch01.setLabel("欣賞大師創作");
	world2D.ch02.setLabel("顯示姓名");
	world2D.btnDown.visible=false;

	//world2D.ch01.on('click',clickCheckBox);

	world2D.sl01.setLabel('omega角速度');
	world2D.sl01.maximum=50;
	world2D.sl01.minimum=0.2;
	world2D.sl01.digitN=2;
	world2D.sl01.value=0.2;

	world2D.sl02.setLabel('r半徑');
	world2D.sl02.maximum=25;
	world2D.sl02.minimum=10;
	world2D.sl02.value=13;

	world2D.sl03.setLabel('color顏色');
	world2D.sl03.maximum=1;
	world2D.sl03.minimum=0;
	world2D.sl03.digitN=3;
	world2D.sl03.value=0.8;
	
	circlePlane=new TEACHER.ObjPlane(50,50,0xFFFFFF,"x",2);
	sinPlane=new TEACHER.ObjPlane(106,50,0xb0c4de,"z",2);
	cosPlane=new TEACHER.ObjPlane(106,50,0xb0c4de,"y",2);
	scene.add(circlePlane).add(sinPlane).add(cosPlane);
	circlePlane.position.x=-50;
	circlePlane.position.z=-25;
	
	circlePlane.position.y=sinPlane.position.y=25;
	sinPlane.position.z=-50;
	cosPlane.position.z=-25;
	sinPlane.position.x=3;
	cosPlane.position.x=3;

	ball=new TEACHER.ObjSphere(rrBall,0xFF0000);
	bar=new TEACHER.ObjCylinder(rrBar,rrCircle,0x5f9ea0,false,"y",2);
	console.log(bar.mesh.scale);
	bar.mesh.scale.y=1;
	center=new TEACHER.ObjCylinder(rrCenter,1,0xffff00,false,"x",2);
	scene.add(ball).add(bar).add(center);
	ball.position.x=-50+rrBall;
	ball.position.z=-25-rrCircle;
	ball.position.y=25+rrCircle;

	bar.position.x=-50+rrBall;
	bar.position.z=-37.5;
	bar.position.y=25;
	
	center.position.x=-50+1;
	center.position.z=-25;
	center.position.y=25;

	circleLine=new TEACHER.Line(0xff00ff);
	scene.add(circleLine);
	circleLine.addPoint(-50,25+(rrCircle-rrBall),-25);
	
	//sin曲線圖
	sinGraph= new TEACHER.ObjGraph(0,10,-1,1,0x191970,0x191970,100000);
	
	sinGraph.labelTitle.setText("sin值");
	sinGraph.labelX.setText("time");
	sinGraph.labelY.setText("sin值");
	sinGraph.position.y=13;
	sinGraph.position.z=-50+0.3;
	sinGraph.position.x=3;
	sinGraph.scale.y=0.6;
	sinGraph.scale.x=0.8;

	scene.add(sinGraph);
	//cos曲線圖
	cosGraph= new TEACHER.ObjGraph(0,100,-1,1,0x191970,0x191970, 100000);
	cosGraph.position.y=0+0.3;
	cosGraph.position.z=-10;
	cosGraph.position.x=3;
	cosGraph.rotation.x=-Math.PI/2;
	cosGraph.scale.y=0.6;
	cosGraph.scale.x=0.8;
	cosGraph.labelTitle.setText("cos值");
	cosGraph.labelX.setText("time");
	cosGraph.labelY.setText("cos值");
	scene.add(cosGraph);
...
function tick(){
	...
	rrCircle=world2D.sl02.value;
	var CC=world2D.sl03.value;
	circleLine.material.color.setHSL(CC,0.5,0.5);
	//theta(x+=v*dt)
	
	tt+=dt;
	bar.mesh.scale.y=world2D.sl02.value/13;
	
	theta+=omega*dt;
	ball.position.y=25+(rrCircle-rrBall)*Math.cos(theta);
	ball.position.z=-25+(rrCircle-rrBall)*Math.sin(theta);

	bar.position.y=25+0.5*rrCircle*Math.cos(theta);
	bar.position.z=-25+0.5*rrCircle*Math.sin(theta);
	bar.rotation.x=theta;
	
	circleLine.addPoint(-50+0.1,25+(rrCircle-rrBall)*Math.cos(theta),-25+(rrCircle-rrBall)*Math.sin(theta));
	
	
	sinGraph.xMax=tt;
	sinGraph.addPoint(tt,Math.cos(theta));
	cosGraph.xMax=tt;
	cosGraph.addPoint(tt,Math.sin(theta));  //,Math.sin(theta)

	
	if(world2D.ch01.checked){
		world3D.cameraTarget.x=-75;
		world3D.cameraTarget.y=25;
		...
		world3D.cameraTarget.x=15;
		world3D.cameraTarget.y=25;
		world3D.cameraTarget.z=0;
		world3D.cameraPhi=0.47;
		world3D.cameraTheta=1.3089969389957472;
	}
