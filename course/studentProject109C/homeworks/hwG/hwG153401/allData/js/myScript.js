...
function init(){
	...
	w=new TEACHER.ObjTextPlane(90,72,"爆炸","z",0xff33bb,)
	rb =new TEACHER.ObjPicSphere(90,pics.ex,2)
	scene.add(rb)
	rb.visible=false
	w.visible=false

	sun =new TEACHER.ObjSphere(4,0xffff00,2)
	scene.add(sun)
	sun.position.y=-30

	line=new TEACHER.Line(0xffffff,2,10000);
	scene.add(line);
	planet =new TEACHER.ObjSphere(2,0xffffff,1)
	scene.add(planet)
    planet.position.x=50
	planet.position.z=0
	planet.position.y=70
	vz=Math.sqrt(gmm/planet.position.x)
	setInterval(tick,1000/fps);
	world2D.slCameraRR.value=400
	if(rr<2){
		plannet.visible=false
	}
}


//C.定義tick                                                                                                                       
function tick(){for(var j=0;j<10;j++){
	rr=Math.sqrt((planet.position.x)*(planet.position.x)+(planet.position.z)*(planet.position.z))
var fx=-1*planet.position.x*gmm/Math.pow(rr,3)
var fz=-1*gmm*planet.position.z/Math.pow(rr,3)
	...
	planet.position.x+=vx*dt
	planet.position.z+=vz*dt
	planet.position.y+=vy*dt
