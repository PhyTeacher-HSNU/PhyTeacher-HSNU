...
function init(){
	...
	scene.add(text);
	Goal=new TEACHER.ObjTextPlane(70,10,'Goal!!!','z',0xff0000)
	scene.add(Goal);
	Goal.visible=false;
	Goal.position.y=5
	text.position.y=20
	text.position.z=-53
    pp=new TEACHER.ObjPicPlane(20,20,pics.ball,'z',2);
    scene.add(pp);
    pp.position.y=4
	pp.position.x=0
    goal=new TEACHER.ObjPicPlane(20,20,pics.door,'z');
	scene.add(goal)
	goal.position.x=45
	goal.position.y=4
	goal.position.z=-1
...
function tick(){
	...
	if(gameMD===1){
		...
		Goal.visible=false;
		pp.position.y=rr;
		pp.position.x=world2D.sl01.value
		dx=world2D.sl02.value
	}
	else{pp.position.x=world2D.sl01.value}
	//3D
	world3D.render();
	//2D
