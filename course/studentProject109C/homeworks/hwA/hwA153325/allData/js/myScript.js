...
function init(){
	...
	p1=new TEACHER.ObjBox(60,20,20,0xFFF000);
	scene.add(p1);
	...
	p2=new TEACHER.ObjPicCylinder(3.75,5,pics.yellow,false,"z",2);
	scene.add(p2);
	...
	p3=new TEACHER.ObjPicCylinder(3.75,5,pics.yellow,false,"z",2);
	scene.add(p3);
	...
	p4=new TEACHER.ObjPicCylinder(3.75,5,pics.yellow,false,"z",2);
	scene.add(p4);
	...
	p5=new TEACHER.ObjPicCylinder(3.75,5,pics.yellow,false,"z",2);
	scene.add(p5);
	...
	world2D.btn01.setLabel('向前');
	world2D.btn02.setLabel('向後');
...
function tick(){
	...
	p1.position.x += p1.vx*dt;
	p1.position.y += p1.vy*dt;
	p1.position.z += p1.vz*dt;

	p2.vx += p2.ax*dt;
	p2.vy += p2.ay*dt;
	p2.vz += p2.az*dt;

	p2.position.x += p2.vx*dt;
	p2.position.y += p2.vy*dt;
	p2.position.z += p2.vz*dt;

	p3.vx += p3.ax*dt;
	p3.vy += p3.ay*dt;
	p3.vz += p3.az*dt;

	p3.position.x += p3.vx*dt;
	p3.position.y += p3.vy*dt;
	p3.position.z += p3.vz*dt;

	p4.vx += p4.ax*dt;
	p4.vy += p4.ay*dt;
	p4.vz += p4.az*dt;

	p4.position.x += p4.vx*dt;
	p4.position.y += p4.vy*dt;
	p4.position.z += p4.vz*dt;

	p5.vx += p5.ax*dt;
	p5.vy += p5.ay*dt;
	p5.vz += p5.az*dt;

	p5.position.x += p5.vx*dt;
	p5.position.y += p5.vy*dt;
	p5.position.z += p5.vz*dt;
...
function clickBtn(e){
	...
	if(str==='btn01'){
		...
		aa=2/3.75*dt;
	/*	p2.rotation.z += 2/3.75*dt;
		p3.rotation.z += 2/3.75*dt;
		p4.rotation.z += 2/3.75*dt;
		p5.rotation.z += 2/3.75*dt;
	*/	
	}else if(str==='btn02'){
		p1.vx = -2;
		p2.vx = -2;
