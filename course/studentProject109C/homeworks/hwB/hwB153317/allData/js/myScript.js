...
function init(){
	...
	rr=2
	for(var i=0 ; i<NN ; i++ ){
		var bb = new TEACHER.ObjPicSphere(rr,pics.ball);
		

		bb.position.y=2;
		bb.position.x=-45+(i%10)*10
		bb.position.z=-45+Math.floor(i/10)*10;
		bb.vx = (Math.random()-0.5)*5;
		bb.vz = (Math.random()-0.5)*5;
		bb.vy = 0;
		bb.ay = -gg;
		bb.ax = ballax;
		bb.az = ballaz;
	...
	world2D.btn02.setLabel('重置速度');
	world2D.btn02.on('mousedown' , mouseDown);
	world2D.btn02.on('pressup' , pressUp);
}
...
function tick(){
	for (var i=0 ; i<NN ; i++ ) {
		...
		if (bb.position.y <world2D.sl01.value  && bb.vy<0){
			bb.position.y = world2D.sl01.value;
		}
		var arrow = arrowArray[i]
		arrow.position.x=bb.position.x
		arrow.position.y=bb.position.y + world2D.sl01.value;
		arrow.position.z=bb.position.z
		arrow.setArrow(bb.vx*2,bb.vy*0,bb.vz*2)
		bb.scale.x= bb.scale.y= bb.scale.z= world2D.sl01.
		value /rr
		arrow.scale.x= arrow.scale.y= arrow.scale.z= world2D.sl01.value /rr
		if (gameMD===1){
			bb.vy = 3;
			bb.ay = -gg;
			bb.vy += bb.ay*dt;
		...
		ballax = world2D.sl02.value;
		ballaz = world2D.sl03.value;
