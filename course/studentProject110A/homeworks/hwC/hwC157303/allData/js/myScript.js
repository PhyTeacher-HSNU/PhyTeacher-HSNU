...
world2D.sl03.setLabel(`球數`)
world2D.sl03.maximum = 100
world2D.sl03.minimum = 0
world2D.sl03.value = 50

world2D.btn01.setLabel(`上`)
world2D.btn02.setLabel(`下`)



world2D.ch01.setLabel(`箭頭顯示`)
...
//B.定義init
function init(){
	var arrow = new TEACHER.ObjArrow(0.5,0x00ff00)
	scene.add(arrow)
	arrowArray.push(arrow)
	ballM = new TEACHER.ObjSphere(1,0x00ffff);
	ballM.position.y = 2
	ballM.position.z = 45
	ballM.position.x = -45
	ballM.vx = 0
	...
	/*for(p=7;p>0;p--){
		...
		for(i=0;i<P;i++){
			...
			for(j=0;j<P;j++){
				var J = j
				var ball = new TEACHER.ObjSphere(2,0xff0000)
				ballArray.push(ball)
				ballArray[n].position.y = 23 - P*3
				ballArray[n].position.x = -5+P*2 - I*4
				ballArray[n].position.z = -5+P*2 - J*4
				scene.add(ballArray[n])
				n += 1
			}
    ...
    for(i=0;i<nn;i++){
        ...
        var ball = new TEACHER.ObjSphere(1,0xff0000)
		ballArray.push(ball)
		

		ball.vx = Math.random()*5-2.5
		ball.vy = Math.random()*5-2.5
		ball.vz = Math.random()*5-2.5
		ball.position.y = 2
		scene.add(ball)
...
//C.定義tick                                                                                                                       
function tick(){
	world2D.slCameraRR.value = 100 + world2D.sl02.value
	ground.scale.x = ground.scale.z = world2D.sl02.value/50
	logo.position.z = -world2D.sl02.value
	logo.position.y =  world2D.sl02.value/5
	logo.scale.x = logo.scale.y = logo.scale.z = world2D.sl02.value/50

	world3D.cameraTarget.y = world2D.sl02.value/2

	nn = world2D.sl03.value

	for(i=0;i<nn+1;i++){
		var ball = ballArray[i]
		var arrow = arrowArray[i]

		arrow.setArrow(ball.vx/g/5*world2D.sl01.value,ball.vy/g/5*world2D.sl01.value,ball.vz/g/5*world2D.sl01.value)
		arrow.position.x = ball.position.x+ball.vx/g*world2D.sl01.value/2
		arrow.position.y = ball.position.y+ball.vy/g*world2D.sl01.value/2
		arrow.position.z = ball.position.z+ball.vz/g*world2D.sl01.value/2
		arrow.scale.x = arrow.scale.y = arrow.scale.z = world2D.sl01.value

		arrow = world2D.ch01.checked
	    ...
	    if(ball.position.x>world2D.sl02.value-world2D.sl01.value && ball.vx>0){
		    ball.vx*= -1
	    }else if(ball.position.x<-world2D.sl02.value+world2D.sl01.value && ball.vx<0){
		    ball.vx*= -1
	    }

	    if(ball.position.z>world2D.sl02.value-world2D.sl01.value && ball.vz>0){
		    ball.vz*= -1
	    }else if(ball.position.z<-world2D.sl02.value+world2D.sl01.value && ball.vz<0){
	        ball.vz*= -1
	    }

		
		if(ball.position.y>world2D.sl02.value-world2D.sl01.value && ball.vy>0){
			ball.vy*= -1
		}else if(ball.position.y<0+world2D.sl01.value && ball.vy<0){
			ball.vy*= -1
		}

		ball.scale.x = ball.scale.y = ball.scale.z = world2D.sl01.value
...
//------------------------------------------------//
	/*if(ballM.position.y<2 && ballM.vy<0 ){
		ballM.ay = 0
		ballM.vy *=-0.7*a
		console.log(`反彈`)
		if(ballM.vy<0.2){
	...
	}else if(ballM.position.y>2.1){
		ballM.ay = -1
		ballM.vy += ballM.ay*dt
	}
	if(ballM.position.y<2){
		ballM.position.y=2
		console.log(`定位`)
	}
//----------------------//
	ballM.position.x += ballM.vx*dt
	ballM.position.y += ballM.vy*dt
	ballM.position.z += ballM.vz*dt
//-------------------------//
	ballM.vx += ballM.ax*dt
	ballM.vz += ballM.az*dt

    if(ballM.position.x>50-2 && ballM.vx>0){
		ballM.vx*= -1
	}else if(ballM.position.x<-50+2 && ballM.vx<0){
		ballM.vx*= -1
	}

	if(ballM.position.z>50-2 && ballM.vz>0){
		ballM.vz*= -1
	}else if(ballM.position.z<-50+2 && ballM.vz<0){
		ballM.vz*= -1
	}

	
	if(ballM.position.y>50-2 && ballM.vy>0){
		ballM.vy*= -1
	}else if(ballM.position.y<0+2 && ballM.vy<0){
		ballM.vy*= -1
	}*/
...
//--------------------------------------------//
	for(i=0;i<nn;i++){
		for(j=i+1;j<nn+1;j++){
			var ballA = ballArray[i]
			var ballB = ballArray[j]
            collision(ballA,ballB,2*world2D.sl01.value)
		}
	}
