...
function init(){
	...
	ff = new TEACHER.ObjBox(20,1,10,0xffff00,2);
	ff.position.x = -40;
	ff.position.z = -45;
	scene.add(ff);

	yy = new TEACHER.ObjPicPlane(40,20,pics.ff,"y");
	yy.position.y = 10;
	scene.add(yy);
	yy.visible = false;

	mm = new TEACHER.ObjPicPlane(40,5,pics.myname,"z");
	scene.add(mm);
	mm.position.z = -50;
	mm.position.y =  20;
	
	for(var i=0 ; i < 4 ; i++){
		var aa = new TEACHER.ObjBox(2,4,90,0x0000ff,2);
		aa.position.x = 30-(i*20);
		if(i%2 === 1){
			aa.position.z = -5;
		}else if(i%2 === 0){
			aa.position.z = 5;
	...
	wallLeft = new TEACHER.ObjBox(2,4,102,0x0000ff,2);
	scene.add(wallLeft);
	wallLeft.position.x=-50;

	wallRight = new TEACHER.ObjBox(2,4,102,0x0000ff,2);
	scene.add(wallRight);
	wallRight.position.x=50;

	wallBack = new TEACHER.ObjBox(100,4,2,0x0000ff,2);
	scene.add(wallBack);
	wallBack.position.z=-50;

	wallFront = new TEACHER.ObjBox(100,4,2,0x0000ff,2);
	scene.add(wallFront);
	wallFront.position.z=50;

	ball = new TEACHER.ObjSphere(rr,0xD1C7BC);
	scene.add(ball);
	ball.position.y = rr;
	ball.position.x = 40;
	ball.position.z = 45;
	...
	world2D.btn02.setLabel("停在原地");
	world2D.btn02.on("mousedown",clickBtn);


	world2D.ch02.setLabel("回到原點");
...
function tick(){
	...
	wallBack.scale.z = world2D.sl02.value;
	wallFront.scale.z = world2D.sl02.value;
	wallLeft.scale.x = world2D.sl02.value;
	wallRight.scale.x = world2D.sl02.value;
	wall01[0].scale.x = wall01[1].scale.x = wall01[2].scale.x = wall01[3].scale.x = world2D.sl02.value;
	
	if(gamemode === 1){

		rotia = world2D.sl01.value;
		...
		if(ball.position.x > (50-(world2D.sl02.value+rr)) && ball.vx > 0){
			ball.vx *= -1*rotia; 
		}else if(ball.position.x < (-50+(world2D.sl02.value+rr)) && ball.vx < 0){
			ball.vx *= -1*rotia;
		}

		if(ball.position.z > (50-(world2D.sl02.value+rr)) && ball.vz > 0){
			ball.vz *= -1*rotia; 
		}else if(ball.position.z < (-50+(world2D.sl02.value+rr)) && ball.vz < 0){
			ball.vz *= -1*rotia;
		}

		if(ball.position.x < 50 && ball.position.x > 30 && ball.position.z > -40 ){
			if(ball.position.x < (30+(world2D.sl02.value+rr)) && ball.vx < 0){
				ball.vx *= -1*rotia;
			}
		}else if(ball.position.x < 30 && ball.position.x > 10 && ball.position.z < 40){
			if(ball.position.x > (30-(world2D.sl02.value+rr)) && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}else if(ball.position.x < (10+(world2D.sl02.value+rr)) && ball.vx < 0){
				ball.vx *= -1*rotia;
			}
		}else if(ball.position.x < 30 && ball.position.x > 10){
			if(ball.position.x > (30-(world2D.sl02.value+rr)) && ball.position.z > 40 && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}
		}else if(ball.position.x < 10 && ball.position.x > -10 && ball.position.z >-40){
			if(ball.position.x > (10-(world2D.sl02.value+rr)) && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}else if(ball.position.x < (-10+(world2D.sl02.value+rr)) && ball.vx < 0){
				ball.vx *= -1*rotia;
			}
		}else if(ball.position.x < 10 && ball.position.x > -10  ){
			if(ball.position.x > (10-(world2D.sl02.value+rr)) && ball.position.z <-40 && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}
		}else if(ball.position.x < -10 && ball.position.x > -30 && ball.position.z < 40){
			if(ball.position.x > (-10-(world2D.sl02.value+rr)) && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}else if(ball.position.x < (-30+(world2D.sl02.value+rr)) && ball.vx < 0){
				ball.vx *= -1*rotia;
			}
		}else if(ball.position.x < -10 && ball.position.x > -30 ){
			if(ball.position.x > (-10-(world2D.sl02.value+rr)) && ball.vx > 0 && ball.position.z > 40){
				ball.vx *= -1*rotia; 
			}
		}else if(ball.position.x < -30 && ball.position.x > -50 && ball.position.z < 40){
			if(ball.position.x > (-30-(world2D.sl02.value+rr)) && ball.vx > 0){
				ball.vx *= -1*rotia; 
			}else if(ball.position.x < (-50+(world2D.sl02.value+rr)) && ball.vx < 0){
				ball.vx *= -1*rotia;
			...
			if(ball.position.x > (-30-(world2D.sl02.value+rr)) && ball.vx > 0 && ball.position.z > 40){
				ball.vx *= -1*rotia; 
			}
		}
	...
	if(ball.position.x < -30 && ball.position.z < -40){
		gamemode = 0;
		yy.visible = true;
	}
