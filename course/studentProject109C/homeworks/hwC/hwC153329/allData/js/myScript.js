...
function init(){
	...
	arr=[];
	for(var i=0;i<100;i++){
		var ball=new TEACHER.ObjPicSphere(r,pics.me);
		ball.position.y=r;
		ball.position.x=(i%10)*10-45;
		ball.position.z=Math.floor(i/10)*10-45;
		scene.add(ball);
		arr.push(ball);
	...
	world2D.sl01.setLabel('大小');

	world2D.btn01.setLabel("加速10%");
	world2D.btn01.on("click",clickBtn);

	world2D.btn02.setLabel("減速10%");
	world2D.btn02.on("click",clickBtn);

	world2D.ch01.setLabel("(console有碰撞次數)")
	setInterval(tick,1000/fps);
}

function clickBtn(e){
	var btn=e.target.parent.name;
	if(btn=="btn01"){
        dv=1;
	}else if(btn=="btn02"){
		dv=2;
...
function tick(){
    ...
    for(var i=0;i <= 99;i++){
		...
		ball.rotation.x+=ball.vz/r/10;
		ball.rotation.z+=ball.vx/r/10;
		if(ball.position.x>50-r && ball.vx>0){
            ball.vx*=-1;
		}else if(ball.position.x<-50+r&&ball.vx<0){
            ball.vx*=-1;
		}
		if(ball.position.z>50-r && ball.vz>0){
            ball.vz*=-1;
		}else if(ball.position.z<-50+r&&ball.vz<0){
            ball.vz*=-1;
		}

		var arrow=arrowarr[i];
		arrow.position.x=ball.position.x;
		arrow.position.z=ball.position.z;
		arrow.position.y=2*r;
