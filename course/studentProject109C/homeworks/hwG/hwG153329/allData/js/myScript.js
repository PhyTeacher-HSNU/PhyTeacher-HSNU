...
function init(){
	...
	sun=new TEACHER.ObjSphere(3,0x00ffff);
	scene.add(sun);

	for(var i=0;i<441;i++){
		var ball=new TEACHER.ObjSphere(0.7,0xffffff);
		scene.add(ball);
		ball.position.z=-50;
		ball.position.x=-30+i%21*3;
		ball.position.y=30-Math.floor(i/21)*3;
	...
	world2D.ch01.setLabel("吸引(不勾則排斥)");

	world2D.ch01.checked=true;

	world2D.btn01.setLabel("Reset");
...
function tick(){
	...
	if(world2D.ch01.checked==true){
		gm=-7000;
	}else if(world2D.ch01.checked==false){
		gm=5000;
	}
