...
function init(){
	...
	wallA.position.x = -40 - 1;
	wallB.position.x = 40 + 1;
	wallA.position.y = wallB.position.y = ballR;
	scene.add(wallA).add(wallB);

	ballA = new TEACHER.ObjSphere(ballR, 0x00ffff);
	ballB = new TEACHER.ObjSphere(ballR, 0x00ffff);
	ballA.position.x = -15;
	ballB.position.x = 15;
	ballA.position.y = ballB.position.y = ballR;
	scene.add(ballA).add(ballB);
	ballA.vx = 0;
	ballB.vx = 0;
	ballA.position.x += 5;

	spring1 = new TEACHER.ObjSpring(L0, 0.8 * ballR, 0.4, 5, 0xffff00);
	spring2 = new TEACHER.ObjSpring(L0, 0.8 * ballR, 0.4, 5, 0xffff00);
	spring3 = new TEACHER.ObjSpring(L0, 0.8 * ballR, 0.4, 5, 0xffff00);
	scene.add(spring1).add(spring2).add(spring3);

	lineA = new TEACHER.Line(0xffff00, 1, 10000);
	lineB = new TEACHER.Line(0xffff00, 1, 10000);
	scene.add(lineA).add(lineB);



	world2D.slCameraRR.value = 120;
	world2D.sl01.setLabel('k');
	world2D.sl01.minimum = 1;
	world2D.sl01.maximum = 10;
	world2D.ch01.setLabel('play');
...
function tick(){
	...
	spring1.position.x = -40;
	spring1.setSpring(ballA.position.x - ballR - (-40), 0, 0);
	...
	spring3.setSpring(40 - ballB.position.x - ballR, 0, 0);
	spring1.position.y = spring2.position.y = spring3.position.y = ballR;
