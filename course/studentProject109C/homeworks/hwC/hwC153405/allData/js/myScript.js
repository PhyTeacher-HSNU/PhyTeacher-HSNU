...
/*	world2D.btn01.on("click" , clickBtn);
	...
	world2D.sl01.setLabel('x軸界線');
	world2D.sl01.minimum = -50;
	world2D.sl01.maximum = 50;
	world2D.sl01.value = 0;

	world2D.sl02.setLabel('z軸界線');
	world2D.sl02.minimum = -50;
	world2D.sl02.maximum = 50;
	world2D.sl02.value = 0;

	world2D.sl03.setLabel('縮放比例');
	world2D.sl03.minimum = 0.2;
	world2D.sl03.maximum = 4.0;
	world2D.sl03.value = 1.0;
	world2D.sl03.digitN = 1;

	world2D.ch01.setLabel('球顯示');
	world2D.ch02.setLabel('無');
...
function tick(){
	...
	if (bb.position.y > 75 && bb.vy > 0){
		bb.vy*=-1;
	} else if (bb.position.y < 0 && bb.vy < 0){
		bb.vy*=-1;
	}
    ...
    if (bb.position.x>aa && bb.position.z>ab){
		bb.mat.color.setHex(0x0000FF);
	} else if (bb.position.x>aa && bb.position.z<ab){
		bb.mat.color.setHex(0xFF9900);
	}
	if (bb.position.x<aa && bb.position.z<ab){
		bb.mat.color.setHex(0x00FF00);
	} else if (bb.position.x<aa && bb.position.z>ab){
		bb.mat.color.setHex(0xFF0000);
	}

	rr=bb.scale.x = bb.scale.y = bb.scale.z = world2D.sl03.value;
    bb.visible = world2D.ch01.checked;
   
	}

	aa = world2D.sl01.value;
	ab = world2D.sl02.value;
	
	for(var i = 0 ; i < nn - 1 ; i ++){
		for(var j = i + 1 ; j < nn ; j ++){
			var ball1 = ballArray[i];
			var ball2 = ballArray[j];
			collision(ball1, ball2, rr);
		}
	}
