...
//B.定義init
function init(){
	stars=new TEACHER.Points(0xffffff,1,100000);
	scene.add(stars);
	ball=new TEACHER.ObjSphere(3,0xff0000);
	scene.add(ball)
	...
	var sz;
	for(var i=0;i<500;i++){
		sx=100*(Math.random()-0.5);
		sy=100*(Math.random()-0.5);
		sz=100*(Math.random()-0.5);
		for(var i1=-1;i1<2;i1++){
			for(var i2=-1;i2<2;i2++){
				for(var i3=-1;i3<2;i3++){
					stars.addPoint(sx+i1*100,sy+i2*100,sz+i3*100);
				}
			}
		}
	}
	...
	world2D.ch01.setLabel('g');
	world2D.sl01.setLabel("gx")
	world2D.sl02.setLabel("gy")
	world2D.sl03.setLabel("gz")
	world2D.sl01.maximum=9;
	world2D.sl02.maximum=9;
	world2D.sl03.maximum=9;
	world2D.sl01.minimum=-9;
	world2D.sl02.minimum=-9;
	world2D.sl03.minimum=-9;
	console.log("Ok");
	world2D.btnRight.on("mousedown",btnrr);
	world2D.btnLeft.on("mousedown",btnll);
	world2D.btnUp.on("mousedown",btnuu);
	world2D.btnDown.on("mousedown",btndd);
	world2D.on("pressup",btnrru);
	aar=new TEACHER.ObjArrow(1,0x0000ff);
	bar=new TEACHER.ObjArrow(1,0xff0000);
	car=new TEACHER.ObjArrow(1,0xf0f0f0);
	scene.add(aar).add(bar).add(car);
	car.position.x=-20;
...
function tick(){
	...
	if(world2D.ch01.checked){
		vx+=world2D.sl01.value*dt;
		vy+=world2D.sl02.value*dt;
		vz+=world2D.sl03.value*dt;
	}
	...
	stars.position.z=-zz
	if(world2D.ch01.checked){
		aar.setArrow(ax+world2D.sl01.value,ay+world2D.sl02.value,az+world2D.sl03.value)
	}else{
		aar.setArrow(ax,ay,az)
	}
	
	bar.setArrow(beta*vx,beta*vy,beta*vz)
	car.setArrow(vx,vy,vz)
	//3D
	world3D.render();
	//2D
