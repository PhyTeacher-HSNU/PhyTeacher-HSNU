...
var isPlay=false;
var isStep=false;
var boat,body,bodyTop;
var car,body1,bodyTop1;
var wheelF,wheelB,wheel;
var dx ;
var kx ;
var zx ;
var dot,dot1;
...
function init(){
	...
	for(var i=0;i<n;i++){
		var wave = new TEACHER.ObjCylinder(1,100,"#0000FF",false,"z");
		scene.add(wave);
		waveArray.push(wave);
		wave.position.y = 1;
		wave.position.x=-55+i*5;
		wave.position.z=100;
	}
	boat = new THREE.Object3D();
	scene.add(boat);
	body = new TEACHER.ObjBox(30,8,20,"#FF8000",2);
	boat.add(body);
	body.position.y = 0;
	body.position.z=75;
	bodyTop = new TEACHER.ObjBox(7,5,8,"#FFF0AC",2);
	boat.add(bodyTop);
	bodyTop.position.y = 7.5;
	bodyTop.position.x=-5;
	bodyTop.position.z=75;
	car = new THREE.Object3D();
	scene.add(car);
	car.position.z = 45;
	wheel = new THREE.Object3D();
	car.add(wheel);
	wheelF = new TEACHER.ObjCylinder(3,15,"#000000",false,"z",2);
	wheel.add(wheelF);
	wheelF.position.x = 10;
	wheelF.position.y = 3;
	wheelB = new TEACHER.ObjCylinder(3,15,"#000000",false,"z",2);
	wheel.add(wheelB);
	wheelB.position.x = -10;
	wheelB.position.y = 3;
	body = new TEACHER.ObjBox(30,10,12,"#0000CD",2);
	car.add(body);
	body.position.y = 8;
	bodyTop = new TEACHER.ObjBox(10,5,12,"#6495ED",2);
	car.add(bodyTop);
	bodyTop.position.y = 15.5;
	dot = new TEACHER.ObjCylinder(2,17,"#FFFFFF",false,"z",2);
	wheel.add(dot);
	dot.position.x = 10;
	dot.position.y = 3;
	dot1 = new TEACHER.ObjCylinder(2,17,"#FFFFFF",false,"z",2);
	wheel.add(dot1);
	dot1.position.x = -10;
	dot1.position.y = 3;
	...
	world2D.btn01.on("click",toDo1);
	world2D.btn02.on("click",toDo);
	world2D.btnUp.on("click",toDo2);
	world2D.btnDown.on("click",toDo2);
	world2D.btnLeft.on("click",toDo2);
	world2D.btnRight.on("click",toDo2);
	car.visible=false;
...
function tick(){
	...
	A = world2D.sl03.value;
	t += ddt;
	for(var i=0;i<n;i++){
		var wave = waveArray[i];
		var	y1 = A*Math.sin(2*Math.PI*wave.position.x/lambda - 2*Math.PI*t/period);
		wave.position.y = 1+y1;
	}
...
function toDo2(e){
	...
	if(str == "btnUp"&& boat.visible == true){
		boat.position.z -=3;
		
	}
	else if(str == "btnDown"&& boat.visible == true){
		boat.position.z +=3;	
	}
	else if(str == "btnRight"&& boat.visible == true){
		boat.position.x +=3;
		
	}
	else if(str == "btnLeft"&& boat.visible == true){
		boat.position.x -=3;
	}
	if(str == "btnUp"&& boat.visible == false){
		car.position.z -=3;
		
	}
	else if(str == "btnDown"&& boat.visible == false){
		car.position.z +=3;	
	}
	else if(str == "btnRight"&& boat.visible == false){
		car.position.x +=3;
		
	}
	else if(str == "btnLeft"&& boat.visible == false){
		car.position.x -=3;
	}
}
