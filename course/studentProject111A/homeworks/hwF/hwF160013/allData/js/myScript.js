...
var timer=0
function init(){
	var pp = new TEACHER.ObjTextPlane(100,10,'160013林浩宇,生命遊戲','z',)
	scene.add(pp)
	scene.add(pp)
	pp.position.z=-50
	pp.position.y=20
	pp.position.x=0
	mainball= new TEACHER.ObjSphere(1.5,0xFF0000) 
	scene.add(mainball)
	mainball.position.y=10
	mainball.position.x=-48
	mainball.position.z=-48
	
	for(var i =0;i<625;i++){
    var deadsqure= new TEACHER.ObjBox(4,4,4,0xFFFFFF)
	deadsqure.position.x=-48+(i%25)*4
	deadsqure.position.z=-48+Math.floor(i/25)*4
	scene.add(deadsqure)
	DeadArray.push(deadsqure)

	var alivesqure= new TEACHER.ObjBox(4,4,4,0x00000F)
	alivesqure.position.x=-48+(i%25)*4
	alivesqure.position.z=-48+Math.floor(i/25)*4
	alivesqure.position.y=-6
	scene.add(alivesqure)
	AliveArray.push(alivesqure)

	var gamer=new TEACHER.ObjSphere(0.5,0xff00ff)
	gamer.position.x=-48+(i%25)*4
	gamer.position.z=-48+Math.floor(i/25)*4
	if(gamer.position.x==alivesqure&&gamer.position.z==alivesqure.position.z&&alivesqure.position.y==0){
		gamer.situation='alive'
	}else {
		gamer.situationy='dead'
	}

	gamerArray.push(gamer)
	gamingArray.push(gamer.situation)
	scene.add(gamer)
	...
	world2D.btn01.on('click',place)
	world2D.btn02.on('click',gaming)
	...
	world2D.btn01.setLabel('放置生命')
	world2D.btn02.setLabel('下一世代')
	world2D.btn03.setLabel('自動播放')
	world2D.btn04.setLabel('重啟')
...
//C.定義tick                                                                                                                       
function tick(){
world2D.ch01.setLabel('第'+G+'世代')

timer++

if(gamemode==-1&& timer%30==0){
	G++
	for(var i =0;i<625;i++){
		var alive=AliveArray[i]
		var dead=DeadArray[i]
		var gamer=gamerArray[i]
		var situation=gamerArray[i].situation
		var x=gamer.position.x
		var z=gamer.position.z
	
		if(gamingArray[i]=='alive'){
		  var aliveCounter=0
		  for(var j=0;j<625;j++){
			var gamer2=gamerArray[j]
			var xb=gamer2.position.x
			var zb=gamer2.position.z
			
			if(Math.abs(x-xb)==4 && z-zb==0 && gamer2.situation=='alive'){
				aliveCounter++
			}
			if(x-xb==0 && Math.abs(z-zb)==4 && gamer2.situation=='alive'){
				aliveCounter++
			}
			if(Math.abs(x-xb)==4 && Math.abs(z-zb)==4 && gamer2.situation=='alive'){
				aliveCounter++
			}
		...
		if(gamingArray[i]=='dead'){
			var aliveCounterB=0
			for(var j=0;j<625;j++){
			var gamer3=gamerArray[j]
			var xc=gamer3.position.x
			var zc=gamer3.position.z
			if(Math.abs(x-xc)==4 && z-zc==0 && gamer3.situation=='alive'){
				aliveCounterB++
			}
			if(x-xc==0 && Math.abs(z-zc)==4 && gamer3.situation=='alive'){
				aliveCounterB++
			}
			if(Math.abs(x-xc)==4 && Math.abs(z-zc)==4 && gamer3.situation=='alive'){
				aliveCounterB++
			}
	...
	for(var i =0;i<625;i++){
		...
		var dead=DeadArray[i]
		var gamer=gamerArray[i]
		if(alive.position.y==0){
			gamer.situation='alive'
		}else {
			gamer.situation='dead'
		}
		gamingArray[i]=gamer.situation	
	}
...
if(str=='btnUp'){
	mainball.position.z-=4
}else if (str=='btnDown'){
	mainball.position.z+=4
}else if (str=='btnRight'){
	mainball.position.x+=4
}else if (str=='btnLeft'){
	mainball.position.x-=4
}else if (str=='btn03'){
	gamemode*=-1
	...
	G=0
	for(var i = 0;i<625;i++){
		var deadsqure= DeadArray[i]
		deadsqure.position.x=-48+(i%25)*4
		deadsqure.position.z=-48+Math.floor(i/25)*4
		deadsqure.position.y=0
	
		var alivesqure= AliveArray[i]
		alivesqure.position.x=-48+(i%25)*4
		alivesqure.position.z=-48+Math.floor(i/25)*4
		alivesqure.position.y=-6
...
for(var i =0;i<1250;i++){
	...
	var gamer=gamerArray[i]
	var dead=DeadArray[i]
	if(alive.position.x==mainball.position.x &&alive.position.z==mainball.position.z&&alive.position.y==-6){
		alive.position.y=0
		dead.position.y=-6	
	}
	if(alive.position.y==0){
