...
function init(){
	//player
	player = new TEACHER.ObjSphere(3,0x0000ff,2)
	scene.add(player)
	...
	//wallL
	wallL = new TEACHER.ObjBox(2050,20,2,0xffffff,0)
	scene.add(wallL)
	wallL.position.x=975
	wallL.position.y=9
	wallL.position.z=-50

	//wallR
	wallR = new TEACHER.ObjBox(2050,20,2,0xffffff,0)
	scene.add(wallR)
	wallR.position.x=975
	wallR.position.y=9
	wallR.position.z=50

	//line
	line = new TEACHER.ObjBox(20,1,100,0xffdc35,0)
	scene.add(line)
	line.position.x=1550
	line.position.y=0.9
	line.position.z=0

	//box
	box = new TEACHER.ObjBox(2000,2,100,0xffffff,0)
	scene.add(box)
	box.position.x=1000	

	//box2
	for(var k=0 ; k<5 ; k++){
	box2 = new TEACHER.ObjBox(10,2,10,0x9d9d9d,0)
	scene.add(box2)
	box2.position.z	=-45+k*20
	box2.position.x=-5
	boxArray.push(box2)
	}
	...
	//box3
	for(var k=0 ; k<5 ; k++){
		box3 = new TEACHER.ObjBox(10,2,10,0x272727,0)
		scene.add(box3)
		box3.position.z	=-35+k*20
		box3.position.x=-5
		boxArray.push(box3)
		}
	...
	//box4
	for(var k=0 ; k<5 ; k++){
		box4 = new TEACHER.ObjBox(10,2,10,0x9d9d9d,0)
		scene.add(box4)
		box4.position.z	=-35+k*20
		box4.position.x=-15
		boxArray.push(box4)
		}
	...
	//box5
	for(var k=0 ; k<5 ; k++){
		box5 = new TEACHER.ObjBox(10,2,10,0x272727,0)
		scene.add(box5)
		box5.position.x=-15
		box5.position.z	=-45+k*20
		boxArray.push(box5)
		}	

	//box6
	box6 = new TEACHER.ObjBox(30,2,100,0xffffff,0)
	scene.add(box6)
	box6.position.x=-35

	//word
	word = new TEACHER.ObjTextPlane(1050,100,'WIN!!!','x',0x00ff00)
	scene.add(word)
	word.position.x=2500
	word.position.y=150
	word.visible=false
		
	//cylinder
	for(var i=0 ; i<NN ; i++){
	cylinder = new TEACHER.ObjCylinder(4,16,0xffffff)

	cylinder.position.x = 1500+1500*(Math.random()-1)
	cylinder.position.y = 8
	cylinder.position.z = 100*(Math.random()-0.5)
	...
	scene.add(cylinder)
	ccArray.push(cylinder)
	}

	//sc
	sc = new TEACHER.ObjTextPlane(1050,100,'1547李瑋晨',"x",)
	sc.position.x=1000
	sc.position.y=100
	scene.add(sc)
...
function tick(){
	...
	for(var i = 0; i<NN; i++){
		...
		//反彈
		if (cc.position.x > 47 && cc.vx > 0) {
			cc.vx *= -1
		} else if (cc.position.x < -47 && cc.vx < 0) {
			cc.vx *= -1
		}
		if (cc.position.z > 47 && cc.vz > 0) {
			cc.vz *= -1
		} else if (cc.position.z < -47 && cc.vz < 0) {
			cc.vz *= -1
		}				
	}
...
/*	for (var i = 0; i < NN - 1; i++) {
		for (var j = i + 1; j < NN; j++){
			...
			var c2 = ccArray[j]
			collision(c1,c2,8)
			collision(player,c1,7)
			collision(player,c2,7)

		}
		var px=player.position.x
		var pz=player.position.z
		if(-4<player.position.x-c1.position.x<4 || -4<player.position.z-c1.position.z<4 ){
			player.position.x=-40	
		} 	 	
	}
*/
	for (var i=0;i<NN;i++) {
		...
		var c1=ccArray[i]
		if (-7<player.position.x-c1.position.x && player.position.x-c1.position.x<7 && -7<player.position.z-c1.position.z && player.position.z-c1.position.z<7 && c1.asd===0){
			c1.asd=1
			player.vx*=-1
			player.vz*=-1
		}else {c1.asd=0}
	...
	player.position.x +=player.vx*ratio
	player.position.z +=player.vz*ratio
	player.vx +=player.ax
	player.vz +=player.az
	...
	if(player.position.z>47 && player.vz>0){
		player.vz*=-1
	}else if(player.position.z<-47 && player.vz<0){
		player.vz*=-1
	...
	word.visible=true
	word.position.x=player.position.x+100
	}
