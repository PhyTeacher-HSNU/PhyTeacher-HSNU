...
function init(){
	...
	back= new TEACHER.ObjPicPlane(20,20,pics.me1);
	scene.add(back);
	back.position.z=-50;
	back.position.y=10;
...
//預設物體1kg
function tick(){
	var f=world2D.sl02.value;
	var g=world2D.sl03.value;
	var c=world2D.sl01.value;
    ball.position.x+=ball.vx*dt;
	ball.vx+=f*dt;
	ball.position.z+=ball.vz*dt;
	ball.vz+=g*dt;
	//碰撞
	if(ball.position.x>48 && ball.vx>0){
	    ball.vx*=-c;
	}else if(ball.position.x<-48 && ball.vx<0){
	    ball.vx*=-c;
	}
	if(ball.position.z>48 && ball.vz>0){
		ball.vz*=-c;
	}else if(ball.position.z<-48 && ball.vz<0){
		ball.vz*=-c;
	}
	...
	/*var s=world2D.sl01.value;
	var k=s/2;
	var f=world2D.sl02.value;
	var g=world2D.sl03.value;
	//判斷是否靜摩擦力
    if(Math.abs(ball.vx)<0.5 && Math.abs(ball.vz)<0.5){
		//if f is bigger than fs
		var tf=Math.sqrt(f*f+g*g);
		if(tf>10*s){
			var ftf=tf-10*k;
			//moving part
			var rate=ftf/tf;
