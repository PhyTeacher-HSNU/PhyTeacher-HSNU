...
function init(){
	...
	var name1=new TEACHER.ObjTextPlane(200,10,'1555 12 林定緯','z',0xffffff)
	scene.add(name1)
	name1.position.x=200
	name1.position.y=25
	name1.position.z=-50


	floor=new TEACHER.ObjBox(400,2,100,0xffff00)
	win=new TEACHER.ObjTextPlane(100,10,'你過關了','x')
	win.position.x=-220
	win.position.y=35
	lose=new TEACHER.ObjTextPlane(200,10,'失敗了 請重來','x',0xff0000)
	lose.position.y=25
	ball=new TEACHER.ObjSphere(2,0xF5DEB3)
	ball.position.x=190
	ball.position.y=3
	ball.mass=1

	var start=new TEACHER.ObjPicPlane(30,120,pics.start,'y')
	scene.add(start)
	start.position.x=190
	start.position.y=2
	start.position.z=-3

	var finish=new TEACHER.ObjBox(10,2,100,0xff0000)
	scene.add(finish)
	finish.position.x=-195
	finish.position.y=0.1

	
	scene.add(floor).add(ball)
	...
	//中間球
	for(var i=0;i<nn;i++){
		bomb=new TEACHER.ObjPicSphere(5,pics.pien)
		scene.add(bomb)
		ballArray.push(bomb)
		bomb.position.x=350*(Math.random()-0.5)
		bomb.position.z=100*(Math.random()-0.5)
		bomb.position.y=3
	}
...
function tick(){
	...
	//鏡頭
	world3D.cameraTarget.x=ball.position.x+10
	world3D.cameraTarget.z=ball.position.z
	world3D.cameraTarget.y=15

	//掉落
	if(ball.position.z>50 | ball.position.z<-50 | ball.position.x>200){
		ball.position.y-=1
	...
	//檢查是否撞到
	for(var i=0;i<nn;i++){ 
		var bomb1=ballArray[i]
		bomb1.mass=10000000000000
		bomb1.vx=0
		bomb1.vy=0
		bomb1.vz=0
		collision(ball,bomb1,world2D.sl02.value+6)
		bomb1.scale.x=world2D.sl02.value
		bomb1.scale.y=world2D.sl02.value
		bomb1.scale.z=world2D.sl02.value
		bomb1.visible=world2D.ch01.checked
