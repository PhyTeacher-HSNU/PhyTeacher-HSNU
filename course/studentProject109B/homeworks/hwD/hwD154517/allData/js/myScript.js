...
function init(){
	...
	Sun = new TEACHER.ObjSphere(20,0xff9900,2);
	Sun.position.x = 0;
	Sun.position.y = 0;
	Sun.position.z = 0;
	scene.add(Sun);
	...
	B2 = new TEACHER.ObjSphere(7,0x0000ff);
	B2.position.y = 0;
	scene.add(B2);

	b1 = new TEACHER.ObjSphere(0.9,0x00ffff);
	b1.position.y = 0;
	scene.add(b1);

	b2 = new TEACHER.ObjSphere(4,0xcccc33);
	b2.position.y = 0;
	scene.add(b2);

	BB = new TEACHER.ObjSphere(4,0x402030);
	BB.position.x = -50;
	BB.position.y = 60;
	BB.position.z = 50;
	scene.add(BB);
	...
	var title = new TEACHER.ObjPicPlane(100,12,pics.title,"z",2);
	title.position.x = 0;
	title.position.y = 18;
	title.position.z = -50;
	scene.add(title);
...
function tick(){
	...
	B1.position.x = 32*Math.cos(theta1)*world2D.sl02.value;
	B1.position.z = 32*Math.sin(theta1)*world2D.sl02.value;

	B2.position.x = 60*Math.cos(theta2)*world2D.sl02.value;
	B2.position.z = 60*Math.sin(theta2)*world2D.sl02.value;

	b1.position.x = B1.position.x+7*Math.cos(theta3)*world2D.sl02.value;
	b1.position.z = B1.position.z+7*Math.sin(theta3)*world2D.sl02.value;
	b1.visible = world2D.ch01.checked;
	
	b2.position.x = B2.position.x+15*Math.cos(theta4)*world2D.sl02.value;
	b2.position.z = B2.position.z+15*Math.sin(theta4)*world2D.sl02.value;
	b2.visible = world2D.ch02.checked;
	...
	if(GM === 1){
		...
		B1.visible = B2.visible = b1.visible = b2.visible = false;			
	}else if(GM === 0){
		Sun.visible = B1.visible = B2.visible = true;
		b1.visible = world2D.ch01.checked;
		b2.visible = world2D.ch02.checked;
		Sun.scale.x = Sun.scale.y = Sun.scale.z	= 1;
		BB.visible = false;
		BB.position.x = -50*world2D.slCameraRR.value/150;
		BB.position.y = 60*world2D.slCameraRR.value/150;
		BB.position.z = 50*world2D.slCameraRR.value/150;
	}else if(GM === 2){
		if(BB.position.x>-15){
			Sun.visible = B1.visible = B2.visible = b1.visible = b2.visible = false;							
			if(world2D.sl03.value>=5){
				BB.position.x += 0.5*world2D.sl03.value;
				BB.position.y += -0.6*world2D.sl03.value;
				BB.position.z += -0.5*world2D.sl03.value;
			}else{
				BB.visible = false;
			}
		}else{
			...
			b2.visible = world2D.ch02.checked;
			BB.visible = true;
			BB.position.x += 0.5*world2D.sl03.value;//-50
			BB.position.y += -0.6*world2D.sl03.value;//60
			BB.position.z += -0.5*world2D.sl03.value;//50
		}		
	}
