...
var range;
var mouse;
var rules,rules1,rules2,rules3,rules4,rules5,rules6;
var rulesnn=7;
var backg;
var g2tt=31;
var eye1,eye2,mouth1,mouth2;
//B.定義init
function init(){
	eye1=new TEACHER.ObjCylinder(1,2,0xffffff,false,'x')
	eye2=new TEACHER.ObjCylinder(1,2,0xffffff,false,'x')
	mouth1=new TEACHER.ObjCylinder(3,2,0xffffff,false,'x')

	world3D.cameraTheta=Math.PI/3;
	world2D.ch01.setLabel('規則')
		rules=new TEACHER.ObjTextPlane(160,10,'規則:','z')
		rules1=new TEACHER.ObjTextPlane(160,10,'鼠標在藍色範圍內時，主角會跟隨鼠')
		rules2=new TEACHER.ObjTextPlane(160,10,'標移動，此狀態下碰觸敵人會減少自')
		rules3=new TEACHER.ObjTextPlane(160,10,'身生命，生命消耗殆盡就會死亡。鼠')
		rules4=new TEACHER.ObjTextPlane(160,10,'標超過範圍時，主角會向前衝鋒，推')
		rules5=new TEACHER.ObjTextPlane(160,10,'開沿路的敵人。將敵人全部推出黃色')
		rules6=new TEACHER.ObjTextPlane(160,10,'範圍即可獲得勝利。(黑球即為你的敵人)')
		backg=new TEACHER.ObjPlane(160,70,0x00000f)
		scene.add(backg)
		backg.position.z=-1;
		rules.position.y=10+10*6;
		rules1.position.y=10+10*5;
		rules2.position.y=10+10*4;
		rules3.position.y=10+10*3;
		rules4.position.y=10+10*2;
		rules5.position.y=10+10;
		rules6.position.y=10;
		scene.add(rules6).add(rules).add(rules1).add(rules2).add(rules3).add(rules4).add(rules5).add(rules6)
	
	
	rules.visible=world2D.ch01.checked;
	rules1.visible=world2D.ch01.checked;
	rules2.visible=world2D.ch01.checked;
	rules3.visible=world2D.ch01.checked;
	rules4.visible=world2D.ch01.checked;
	rules5.visible=world2D.ch01.checked;
	rules6.visible=world2D.ch01.checked;
	world2D.ch01.checked=true;
	mouse=new TEACHER.ObjCylinder(1,1,0xFF00FF)
	scene.add(mouse)
	range=new TEACHER.ObjCylinder(30,0.5,0x00fffff,true,'y',2);
	scene.add(range)
	lose=new TEACHER.ObjTextPlane(100,20,'你已經死了','z',0xff0000)
	win=new TEACHER.ObjTextPlane(40,20,'勝利','z',0xffffff)
	scene.add(lose).add(win);
	
	win.position.y=15+10;
	lose.position.y=25;

	lose.visible=false;
	win.visible=false;
	world2D.sl01.setLabel('生命');
	world2D.sl02.setLabel('敵人數量');
...
ball=new TEACHER.ObjCylinder(5,10,0xff0000,false,'x')
scene.add(ball);
ball.vx=0;
ball.vz=0;
ball.position.y=5;
...
aa=new TEACHER.ObjSphere(5,0x00000f)
aa.position.x=(Math.random()-0.5)*182;
aa.position.z=(Math.random()-0.5)*182;
scene.add(aa);
aa.position.y=5;
aa.vx=20*(Math.random()-0.5);
aa.vz=20*(Math.random()-0.5);
aa.vy=0;
aa.ay=1;
aa.ax=0;
aa.az=0;
aa.an=0;
aa.ar=0;
ball.add(eye2).add(eye1).add(mouth1)
eye1.position.x=eye2.position.x=4;
eye2.position.y=eye1.position.y=1;
eye1.position.z=2;
eye2.position.z=-2;
mouth1.position.x=4;
mouth1.position.y=-1;
mouth1.rotation.z=0.25;
eye1.rotation.z=eye2.rotation.z=-0.25;
aas.push(aa);
}
//floor
for(var i=0;i<NN;i++){

floor=new TEACHER.ObjPlane(25,25,0xffff00,'y')
floor.position.x=-91+26*(i%8);
floor.position.z=-91+26*(Math.floor(i/8))
scene.add(floor)
}
backg.position.y=40;
...
function tick(){
	...
	rules5.visible=world2D.ch01.checked;
	rules6.visible=world2D.ch01.checked;
	backg.visible=world2D.ch01.checked;
if(world2D.ch01.checked){}else{
var vecm=getMouse3D('y',0);

	if(world2D.sl01.value>0){
//var vecm=getMouse3D('y',0);
if(gg===1&&(ball.position.x-vecm.x)*(ball.position.x-vecm.x)+(ball.position.z-vecm.z)*(ball.position.z-vecm.z)<900){
	tt=0;
eye1.rotation.z=eye2.rotation.z=-0.25;
mouth1.rotation.z=0.25;
mouth1.position.y=-1;
...
ball.position.x=vecm.x*0.05+ball.position.x*0.95;
ball.position.z=vecm.z*0.05+ball.position.z*0.95;

	//world3D.cameraTarget.x=ball.position.x;
	//world3D.cameraTarget.y=ball.position.y;
	//world3D.cameraTarget.z=ball.position.z;
var theta=Math.atan2(-vecm.z+ball.position.z,vecm.x-ball.position.x);
ball.theta=theta;
ball.rotation.y=theta;
ball.vx=v0*Math.cos(theta);
ball.vz=-v0*Math.sin(theta);
...
for(var i=0;i<nn;i++){
	var bad=aas[i]
	if(bad.vy===0){
bad.ax=v0*Math.cos(theta);
bad.az=-v0*Math.sin(theta);}
}
}
	else{
		gg=2;
mouth1.rotation.z=-0.25;
eye1.rotation.z=eye2.rotation.z=0.25;
eye1.position.y=eye2.position.y=2.3;
mouth1.position.y=-2.6;

//		for(var i=0;i<nn;i++){
//var bad=aas[i];

//if((ball.position.x-bad.position.x)*(ball.position.x-bad.position.x)+(ball.position.z-bad.position.z)*(ball.position.z-bad.position.z)<100){
//	bad.position.x+=cc*dt;
//	bad.position.z+=dd*dt;
//}
//		}
...
if(sv===0){gg=1}
	...
	for(var i=0;i<nn;i++){
		...
		if((ball.position.x-bad.position.x)*(ball.position.x-bad.position.x)+(ball.position.z-bad.position.z)*(ball.position.z-bad.position.z)<100&&gg===2){
			bad.vy=1;bad.ay=1;bad.as=0
		}
		if(bad.vy===1){
			/*bad.vx=bad.ax;
			bad.vz=bad.az;
			...
			if(bad.ay<0){
				...
				bad.vx=20*(Math.random()-0.5);
				bad.vz=20*(Math.random()-0.5);
				bad.vy=0;
...
if(bad.ar===0){
		if(bad.position.x>104||bad.position.x<-104||bad.position.z<-104||bad.position.z>104){
			world2D.sl02.value-=1;bad.ar=1
		}else if(bad.ar===0){
		bad.position.x+=bad.vx*dt;
		bad.position.z+=bad.vz*dt;}}
		if((ball.position.x-bad.position.x)*(ball.position.x-bad.position.x)+(ball.position.z-bad.position.z)*(ball.position.z-bad.position.z)<100&&gg===1&&bad.an===0&&bad.ar===0){
		world2D.sl01.value-=1;bad.an=1;
		}else if(
(ball.position.x-bad.position.x)*(ball.position.x-bad.position.x)+(ball.position.z-bad.position.z)*(ball.position.z-bad.position.z)>100
		){bad.an=0}
		...
		if(bad.position.x>98&&bad.vx>0){bad.vx*=-1}else if(bad.position.x<-98&&bad.vx<0){bad.vx*=-1}
		if(bad.position.z>98&&bad.vz>0){bad.vz*=-1}else if(bad.position.z<-98&&bad.vz<0){bad.vz*=-1}
	}}else{lose.visible=true}
	if(world2D.sl02.value===0){win.visible=true}

	range.position.x=ball.position.x;
	range.position.z=ball.position.z;

mouse.position.x=vecm.x;
mouse.position.z=vecm.z;
win.position.x=lose.position.x=ball.position.x;
win.position.z=lose.position.z=ball.position.z;
}
