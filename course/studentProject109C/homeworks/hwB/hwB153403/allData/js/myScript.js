...
world2D.on('pressup',clickbt);
world2D.ch01.setLabel('e=0.8');
world2D.slCameraRR.value=120;
world2D.slCameraRR.maximum=300;
world2D.btn01.on('click',clickb);
world2D.btn02.on('click',clickb);
world2D.btn01.setLabel('往上丟');	
world2D.btn02.setLabel('停往上丟');	
world2D.ch02.setLabel('牆壁');
aa=new TEACHER.ObjPicPlane(100,100,pics.abcd,"z",0);
bb=new TEACHER.ObjPicPlane(100,100,pics.abcd,"z",1);
cc=new TEACHER.ObjPicPlane(100,100,pics.abcd,"x",0);
dd=new TEACHER.ObjPicPlane(100,100,pics.abcd,"x",1);
scene.add(aa);
aa.position.y=50;
aa.position.z=-49;
scene.add(bb);
...
//C.定義tick                                                                                                                       
function tick(){
	aa.visible=bb.visible=cc.visible=dd.visible=world2D.ch02.checked;
	ball.scale.x=world2D.sl01.value/2;
	ball.scale.y=world2D.sl01.value/2;
	ball.scale.z=world2D.sl01.value/2;
	if(a===0){
	ball.position.y=rr;
	}
	rr=2*world2D.sl01.value/2;
	vx+=ax*dt;
	vz+=az*dt;
	vy+=-ay*dt;
	if(ball.position.x>50-rr && vx>0){
		vx=-vx*e;
	}
	if(ball.position.x<-50+rr && vx<0){
		vx=-vx*e;
	}
	if(ball.position.z>50-rr && vz>0){
		vz=-vz*e;
	}
	if(ball.position.z<-49+rr && vz<0){
		vz=-vz*e;
	}
	if(ball.position.y<rr && vy<0){
		vy=-vy*e;
	}
	log(ball.position.y);
	ball.position.x+=vx*dt;
	ball.position.z+=vz*dt;
	ball.rotation.z-=vx*dt/rr;
	ball.rotation.x+=vz*dt/rr;
	ball.position.y+=vy*dt;
	e1=world2D.ch01.checked;
	if(e1){
		e=0.8;
	}else{
		e=1;
...
function clickbtn(e){
	...
	if(str==='btnUp'){
az=-world2D.sl03.value;
	}else if(str==='btnDown'){
		az=world2D.sl03.value;
	}else if(str==='btnLeft'){
		ax=-world2D.sl03.value;
	}else{
		ax=world2D.sl03.value;
	}
	}
