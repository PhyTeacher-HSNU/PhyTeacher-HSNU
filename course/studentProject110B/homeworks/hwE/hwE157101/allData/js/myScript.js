...
function init(){
	...
	//1
	for (var i=0;i<nn;i++){
		var ball=new TEACHER.ObjSphere(0.5,0xff0000)
		ball.position.y=10;
		ball.position.x=-50+i*2;
		ball.position.z=z1;
		scene.add(ball);
	...
	//2
	for (var i=0;i<nn;i++){
		var ball=new TEACHER.ObjSphere(0.5,0x0000ff)
		ball.position.y=10;
		ball.position.x=-50+i*2;
		ball.position.z=z2;
		scene.add(ball);
	...
	//3
	for (var i=0;i<nn;i++){
		var ball=new TEACHER.ObjSphere(0.75,0xffffff)
		ball.position.y=10;
		ball.position.x=-50+i*2;
		ball.position.z=z3;
		scene.add(ball);
		ballarray3.push(ball);
	}
	
	for (var i=0;i<nn;i++){
		var ball=new TEACHER.ObjBox(0.5,2,2,0x0000ff)
		ball.position.y=10;
		var xx=-50+i*2
		ball.position.x=xx;
		ball.position.z=0;
		scene.add(ball);
		ballarray4.push(ball);
	}

	for (var i=0;i<nn;i++){
		var ball=new TEACHER.ObjSphere(0.5,0xff0000)
		ball.position.y=10;
		ball.position.x=Math.sin(i*2*Math.PI/nn)*rr;
		ball.position.z=Math.cos(i*2*Math.PI/nn)*rr;
		scene.add(ball);
		ballarray.push(ball);
	}
	...
	world2D.btn01.on("click",pp);
	world2D.btn02.setLabel("STEP");
	world2D.btn02.on("click",pp);
	world2D.btnRight.on("click",pp);

	world2D.btnLeft.on("click",pp);
...
function tick(){
	...
	for (var i=0;i<nn;i++){
		...
		if(gamemode===2){
			b1.visible=b2.visible=b3.visible=true;
		}else{
	...
	for (var i=0;i<nn;i++){
		var b1=ballarray4[i];
		var xx=-50+i*2
		var yy1 = aa*Math.sin( 2*Math.PI*xx/lambda - 2*Math.PI*tt/period )+xx;
		...
		for (var i=0;i<nn;i++){
			...
			if(gamemode===1){
				b.visible=true
				world2D.btn01.setLabel("吸能");
				world2D.btn02.setLabel("釋能");
				world2D.ch01.setLabel("157101王克洋");
			}else{
				if(gamemode===3){
			...
			var yy1 = aa*Math.sin( 2*Math.PI*theta/lambda1 - 2*Math.PI*tt/period );
			var yy2 = aa*Math.sin( 2*Math.PI*theta/lambda1 + 2*Math.PI*tt/period );
			b.position.y=10+yy1+yy2;
		}
	if(gamemode===4){
		gamemode=1
	}else if(gamemode===0){
