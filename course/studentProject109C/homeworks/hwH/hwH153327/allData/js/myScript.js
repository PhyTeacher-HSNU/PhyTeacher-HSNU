...
function init(){
	...
	var table=new TEACHER.ObjBox(30,5,30,0xffffff);
	var leg1=new TEACHER.ObjCylinder(3,20,0xff0000);
	var leg2=new TEACHER.ObjCylinder(3,20,0xff0000);
	var leg3=new TEACHER.ObjCylinder(3,20,0xff0000);
	var leg4=new TEACHER.ObjCylinder(3,20,0xff0000);
	leg1.position.x=12;
	leg1.position.z=12;
	leg2.position.x=12;
	leg2.position.z=-12;
	...
	leg4.position.x=-12;
	leg4.position.z=12;
	leg1.position.y=-12.5;
	leg2.position.y=-12.5;
	leg3.position.y=-12.5;
	leg4.position.y=-12.5;
	scene.add(table);
	scene.add(leg1);
	scene.add(leg2);
	scene.add(leg3);
	...
	ball.position.x=12;
	scene.add(ball);
	rope=new TEACHER.ObjArrow(0.4,0XFF0000);
	rope.position.y=3.5;
	rope.setArrow(10,0,0);
	scene.add(rope);
	rope2=new TEACHER.ObjArrow(0.4,0XFF0000);
	rope2.position.y=4;
	rope2.setArrow(0,-16,0);
	scene.add(rope2);
	box=new TEACHER.ObjBox(3,3,3,0xff0000);
	scene.add(box);
	box.position.y=-10;
	line=new TEACHER.Line(0X00ff00,1,10000);
	scene.add(line);
	line2=new TEACHER.Line(0Xff0000,1,10000);
	scene.add(line2);
	line3=new TEACHER.Line(0XFF0000,1,10000);
	scene.add(line3);
	world2D.sl01.setLabel("b重");
	world2D.sl01.value=1;
	world2D.sl01.minimum=0.5;
	world2D.sl01.maximum=5;
	world2D.sl02.setLabel("下拉");
	world2D.sl02.value=0;
	world2D.sl02.minimum=-2;
	world2D.sl02.maximum=2;
	world2D.btn01.setLabel("clear");
	world2D.btn01.on('click',function(e){line.clear();});
	world2D.btn02.setLabel("平衡");
	world2D.btn02.on('click',function(e){world2D.sl01.value=ma*r*omega*omega/g;
	vc=0;});
...
//C.定義tick                                                                                                                       
function tick(){
	mb=world2D.sl01.value;
	rc0=rc1;
	rc1=world2D.sl02.value;
	ball.position.x+=(rc1-rc0)*Math.cos(count);
	ball.position.z+=(rc1-rc0)*Math.sin(count);
	rope.setArrow(ball.position.x,0,ball.position.z);
	r-=rc1-rc0;
	omega=omega=r0*r0*omega/r/r;
	box.position.y-=rc1-rc0;
	rope2.setArrow(0,box.position.y-6,0);	
	for(var i=1;i<80;i++){
		f=-ma*r*omega*omega+mb*g;
		ac=f/ma;
		vc=vc-ac*dt;
		countr=r0;
		r0=r;
		r-=-vc*dt+0.5*ac*dt*dt;
		vt=r0*vt/r;
		omega=r0*r0*omega/r/r;
		count+=omega*360/2/Math.PI*dt;
		ball.position.x=r*Math.cos(count);
		ball.position.z=r*Math.sin(count);
		rope.setArrow(ball.position.x,0,ball.position.z);
		line.addPoint(ball.position.x,5,ball.position.z);
		line.update();
		box.scale.x=box.scale.y=box.scale.z=Math.pow(world2D.sl01.value,1/3);
		box.position.y-=-vc*dt+0.5*ac*dt*dt;
		rope2.setArrow(0,box.position.y-6,0);		
	}	
