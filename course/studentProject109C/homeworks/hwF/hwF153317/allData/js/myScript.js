...
function init(){
	for(var i=0;i<11;i++){
		var ballA = new TEACHER.ObjCylinder(rrA,2,0x00ffff);
		ballA.vz = 0;
		ballA.position.z = -25
		ballA.position.x = 50 - 10*i
		scene.add(ballA);
		ballarrayA.push(ballA)
		var ballB = new TEACHER.ObjCylinder(rrB,2,0x00ff00)
		ballB.position.x = 50 - 10*i
		ballB.position.z = 25
		ballB.vz = i;
		scene.add(ballB);
		ballarrayB.push(ballB);
		var spring = new TEACHER.ObjSpring(L0,1.8,0.2,20,0xffff00)
		scene.add(spring);
		springarray.push(spring);
		spring.position.x = 50 -10*i

	}
	
	ground.position.y = -1;

	world2D.sl01.setLabel("彈簧原長")
	world2D.sl01.minimum = 30;
	world2D.sl01.maximum = 50;
	world2D.sl01.value = 50;
	world2D.sl02.setLabel("彈力係數")
...
function tick(){
	...
	L0 = world2D.sl01.value;
	kk = world2D.sl02.value;
	

	/*if (ballA.position.x+rrA >= 50){
		ballA.vx *= -1
	}else if (ballA.position.x-rrA <= -50){
		ballA.vx *= -1
	}if (ballB.position.x+rrB >= 50){
		ballB.vx *= -1
	}else if (ballB.position.x-rrB <= -50){
		ballB.vx *= -1
	}*/
	for(var i=0;i<11;i++){
		var ballA = ballarrayA[i]
		var ballB = ballarrayB[i]
		var spring = springarray[i]
		ballB.position.z+= ballB.vz * dt;
		if (ballB.position.z<= 20 && ballB.vz<0 ){
			ballB.vz *= -1
		}else if (ballB.position.z>= 30 && ballB.vz>0 ){
			ballB.vz *= -1
		}
		var dx = (ballB.position.z - ballA.position.z)-(L0+rrA+rrB);
		var ff = 0
		...
		spring.position.z = ballB.position.z - rrB;
		spring.setSpring(0,0,-L0-dx)
