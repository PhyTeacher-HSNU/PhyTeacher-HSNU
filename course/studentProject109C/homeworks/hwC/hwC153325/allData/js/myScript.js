...
function init(){
	...
	world2D.btn01.setLabel('保留一顆');
	world2D.btn01.on('click',clickBtn);

	world2D.btn02.setLabel('變色');
	...
	bb=new TEACHER.ObjSphere(2,0xff0000);
	  ...
	  bb.position.x=-22.5+(i%10)*5;
	  bb.position.y=-22.5+Math.floor(i/200)*5;
	  bb.position.z=-22.5+Math.floor(i/200)*5;
...
function tick(){
	...
	for(var i=0 ; i<nn ; i++){
		...
		if(bb.position.x > 50-rr &&bb.vx > 0){
			bb.vx *= -1;
		}else if(bb.position.x < -50+rr &&bb.vx < 0){
			bb.vx *= -1;
		}if(bb.position.z > 50-rr &&bb.vz > 0){
			bb.vz *= -1;
		}else if(bb.position.z < -50+rr &&bb.vz < 0){
			bb.vz *= -1;
		}if(bb.position.y > 50-rr &&bb.vy > 0){
			bb.vy *= -1;
		}else if(bb.position.y < -50+rr &&bb.vy < 0){
			bb.vy *= -1;
		}

		var arrow=arrowArray[i];
		arrow.position.x=bb.position.x;
		arrow.position.y=bb.position.y+rr;
		arrow.position.z=bb.position.z;

		arrow.setArrow(bb.vx*rr , bb.vy*rr , bb.vz*rr);

		rr = world2D.sl01.value;
		bb.scale.x = bb.scale.y = bb.scale.z = rr/2;
	...
	for(var i=0 ; i<nn-1 ; i++){
		for(var j=i+1 ; j<nn ; j++){
			...
			collision(ball1 , ball2 , rr*2);
		}
	}
/*	hh=world2D.sl02.value;
	ss=world2D.sl02.value;
	ballArray.color=bb.mat.color.setHSL(hh,ss,0.5);
*/	//3D
	world3D.render();
	//2D
...
function clickBtn(e){
	...
	if(str==='btn01'){
		...
		ballArray[399].visible=true;
		arrowArray[399].visible=true;
	}else if(str==='btn02'){
		bb.mat.color.setHSL(Math.random(),Math.random(),0.5);
	}
}
