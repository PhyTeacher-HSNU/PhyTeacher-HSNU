...
function init(){
    ...
    ti=new TEACHER.ObjTextPlane(1000,20,"hwA大安高工一孝24",'z',0xFFFF00);
	scene.add(ti);
	ti.position.z=-50
    ti.position.y=20
	...
	earth= new TEACHER.ObjPicSphere(5,pics.earth)
	scene.add(earth);
    earth.position.y=5
	earth.position.z=-30


    sun= new TEACHER.ObjPicSphere(9,pics.sun)
	scene.add(sun);
    sun.position.x=20
...
function tick(){
	...
	sun.position.x+=sun.vx*dt;
	sun.position.y+=sun.vy*dt;
	sun.position.z+=sun.vz*dt;
    ...
    if(sun.position.z>50 && sun.vz>0){
		sun.vz*=-1*R;
	}else if(sun.position.z<-50 && sun.vz<0){
		ty=new TEACHER.ObjTextPlane(1000,10,"game over",'z',0xFFFF00);
	scene.add(ty);
	ty.position.z=0
    ty.position.y=10
	}
