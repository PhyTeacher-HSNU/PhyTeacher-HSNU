...
var IncidentLine, RefractLine, ReflectLine;
const nNn = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, -50, 0), new THREE.Vector3(0, 50, 0)]), new THREE.LineBasicMaterial({color: 0xffffff}));
...
var PlatformLineArray1 = [], PlatformLineArray2 = [];
var GlassLineArray1 = [], GlassLineArray2 = [];
var WaterSurfaceBallArray = [];
var WaterSurfaceLineArray1 = [], WaterSurfaceLineArray2 = [];

var tt = 0, dt = 0.1;

var CAlabel, TIRlabel, Namelabel;
...
function init(){
	...
	//網格平面
	const material1 = new THREE.LineBasicMaterial({color: 0x00ff3C});
	
	for(var i=-50; i<=50; i=i+5){
		const points = [];
		points.push(new THREE.Vector3(50, 0, i));
		points.push(new THREE.Vector3(-50, 0, i));
		...
		scene.add(line);
		PlatformLineArray1.push(line);
	}
	for(var i=-50; i<=50; i=i+5){
		const points = [];
		points.push(new THREE.Vector3(i, 0, 50));
		points.push(new THREE.Vector3(i, 0, -50));
		...
		const line = new THREE.Line(geometry, material1);
		scene.add(line);
		PlatformLineArray2.push(line);
	}

	//水面
	for(var i=-50; i<=50; i=i+10){
		for(var j=-50; j<=50; j=j+2.5){
			var pt = new TEACHER.ObjSphere(1/2, 0x4700ff, 0);
			pt.position.x = j;
			pt.position.z = i;
			scene.add(pt);
			WaterSurfaceBallArray.push(pt);
			pt.visible = false;
		}
	}
	...
	for(var i=-50; i<=50; i=i+(100/3.0001)){
		const points = [];
		points.push(new THREE.Vector3(50, 0, i));
		points.push(new THREE.Vector3(-50, 0, i));
	...
	for(var i=-50; i<=50; i=i+(100/3.0001)){
		...
		const line = new THREE.Line(geometry, material3);
		scene.add(line);
		GlassLineArray2.push(line);
		line.visible = false;
	}

	//文字平面
	CAlabel = new TEACHER.ObjTextPlane(100, 8, "", "z");
	scene.add(CAlabel);
	CAlabel.position.y = 23;
	CAlabel.position.z = 5;
	CAlabel.visible = false;

	TIRlabel = new TEACHER.ObjTextPlane(28, 8, "全反射", "z", 0xffff00, 0x0000ff);
	scene.add(TIRlabel);
	TIRlabel.position.y = 4;
	TIRlabel.position.z = 5;
	TIRlabel.visible = false;

	Namelabel = new TEACHER.ObjTextPlane(100, 5, "1628班 4號 朱致丞", "z");
	scene.add(Namelabel);
	Namelabel.position.x = 30;
	Namelabel.position.z = 50;
	Namelabel.position.y = 2.5;

	//lines
	IncidentLine = new TEACHER.ObjCylinder(0.3, InRR * 2, 0xffff00);
	scene.add(IncidentLine);
	RefractLine = new TEACHER.ObjCylinder(0.3, InRR * 2, 0x00ffff);
	scene.add(RefractLine);
	ReflectLine = new TEACHER.ObjCylinder(0.3, InRR * 2, 0xff00ff);
	scene.add(ReflectLine);

	//sliders
	world2D.sl01.setLabel("入射光入射角(deg)");
	world2D.sl01.maximum = 89.9;
	world2D.sl01.minimum = -89.9;
	world2D.sl01.digitN = 1;
	world2D.sl01.value = 45.0;
	
	world2D.sl02.setLabel("入射光折射率");
	world2D.sl02.maximum = 2;
	world2D.sl02.minimum = 1;
	world2D.sl02.digitN = 2;
	world2D.sl02.value = 1;

	world2D.sl03.setLabel("折射光折射率");
	world2D.sl03.maximum = 2;
	world2D.sl03.minimum = 1;
	world2D.sl03.digitN = 2;
	world2D.sl03.value = 1.5;
	
	//button
	world2D.btn01.setLabel("水面");
	world2D.btn02.setLabel("玻璃");
	...
	//checkbox
	world2D.ch01.setLabel("法線透明");

	world2D.ch02.setLabel("紓壓用checkbox");
...
function tick(){
	...
	nn1 = world2D.sl02.value;
	nn2 = world2D.sl03.value;
	CriticalAngle = Math.asin(nn2 / nn1) * 180 / Math.PI;
	thetaIN = -1 * world2D.sl01.value / 180 * Math.PI;
	thetaRE = Math.asin(nn1 * Math.sin(thetaIN) / nn2);

	CAlabel.position.x = wxx;
	CAlabel.position.z = 5 + wzz;
	TIRlabel.position.z = 5 + wzz;

	if(nn1 > nn2){
		TIR = true;
		//label
		CAlabel.setText("臨界角 = " + String(CriticalAngle) + " deg");
	...
	//IncidentLine
	IncidentLine.position.x = (-1 * Math.sin(thetaIN) * InRR) + wxx;
	IncidentLine.position.y = Math.cos(thetaIN) * InRR;
	IncidentLine.position.z = wzz;
	IncidentLine.rotation.z = thetaIN;

	//RefractLine & TIR
	if(TIR && (world2D.sl01.value >= CriticalAngle || -1 * world2D.sl01.value >= CriticalAngle)){
		TIRlabel.visible = true;
		if(thetaIN > 0){
			RefractLine.position.x = InRR + wxx;
			RefractLine.position.z = wzz;
			TIRlabel.position.x = 25 + wxx;
		}else if(thetaIN < 0){
			RefractLine.position.x = -1 * InRR + wxx;
			RefractLine.position.z = wzz;
			TIRlabel.position.x = -25 + wxx;
		}
		RefractLine.position.y = 0;
		RefractLine.rotation.z = Math.PI * 1 / 2;
	}else{
		TIRlabel.visible = false;		
		RefractLine.position.x = Math.sin(thetaRE) * InRR + wxx;
		RefractLine.position.y = -1 * Math.cos(thetaRE) * InRR;
		RefractLine.position.z = wzz;
		RefractLine.rotation.z = thetaRE;
	}

	//ReflectLine
	ReflectLine.position.x = Math.sin(thetaIN) * InRR + wxx;
	ReflectLine.position.y = Math.cos(thetaIN) * InRR;
	ReflectLine.position.z = wzz;
	ReflectLine.rotation.z = -1 * thetaIN;

	//GroundMode
	if(GroundMode == 0){
		
	}else if(GroundMode == 1){
		for(var i=0; i<41*11; i++){
			//waving
			WaterSurfaceBallArray[i].position.y = 1.18 * Math.sin(2 * Math.PI * WaterSurfaceBallArray[i].position.x / 30 - 2 * Math.PI * tt / 30);
		}
		world2D.sl03.value = 1.33;
	}else if(GroundMode == 2){
		world2D.sl03.value = 1.52;
	}
...
function clickBtn(e){
	...
	if(str === "btnUp"){
		...
		wxx = wxx + 5;
	}else if(str === "btn01" && GroundMode == 0){ //three options change
		GroundMode = 1;
		world2D.btn01.setLabel("網格平面");
		for(var i=0; i<21; i++){
			PlatformLineArray1[i].visible = false;
			PlatformLineArray2[i].visible = false;
		}
		for(var i=0; i<41*11; i++){
			WaterSurfaceBallArray[i].visible = true;
		}
		for(var i=0; i<4; i++){
			GlassLineArray1[i].visible = false;
			GlassLineArray2[i].visible = false;
		}
		world2D.sl03.setLabel("水面折射率(Locked)");
	}else if(str === "btn02" && GroundMode == 0){
		GroundMode = 2;
		world2D.btn02.setLabel("網格平面");
		for(var i=0; i<21; i++){
			PlatformLineArray1[i].visible = false;
			PlatformLineArray2[i].visible = false;
		}
		for(var i=0; i<41*11; i++){
			WaterSurfaceBallArray[i].visible = false;
		}
		for(var i=0; i<4; i++){
			GlassLineArray1[i].visible = true;
			GlassLineArray2[i].visible = true;
		}
		world2D.sl03.setLabel("玻璃折射率(Locked)");
	}else if(str === "btn01" && GroundMode == 1){
		GroundMode = 0;
		world2D.btn01.setLabel("水面");
		for(var i=0; i<21; i++){
			PlatformLineArray1[i].visible = true;
			PlatformLineArray2[i].visible = true;
		}
		for(var i=0; i<41*11; i++){
			WaterSurfaceBallArray[i].visible = false;
		}
		...
		world2D.sl03.setLabel("折射光折射率");
	}else if(str === "btn02" && GroundMode == 1){
		GroundMode = 2;
		world2D.btn01.setLabel("水面");
		world2D.btn02.setLabel("網格平面");
		for(var i=0; i<21; i++){
		...
		world2D.sl03.setLabel("玻璃折射率(Locked)");
	}else if(str === "btn01" && GroundMode== 2){
		GroundMode = 1;
		world2D.btn02.setLabel("玻璃");
		world2D.btn01.setLabel("網格平面");
		for(var i=0; i<21; i++){
		...
		world2D.sl03.setLabel("水面折射率(Locked)");
	}else if(str === "btn02" && GroundMode == 2){
		GroundMode = 0;
		world2D.btn02.setLabel("玻璃");
		for(var i=0; i<21; i++){
			PlatformLineArray1[i].visible = true;
