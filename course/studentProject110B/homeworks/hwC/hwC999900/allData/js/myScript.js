...
function init(){
	...
	for(var r=0 ; r<ww ; r++){
		var aa=new TEACHER.ObjSphere(5,0x00ff00);

        aa.position.y=5;
		aa.position.x=-45+(i%10)*10;
		aa.position.z=-45+Math.floor(i/10)*10;
    ...
    world2D.ch01.setLabel('綠球');

	for(var q=0 ; q<cc ; q++){
		var kk=new TEACHER.ObjSphere(8,0x0000ff);

        kk.position.y=8;
		kk.position.x=-45+(i%10)*10;
		kk.position.z=-45+Math.floor(i/10)*10;

        kk.vx=5*(Math.random()-0.5);
		kk.vz=5*(Math.random()-0.5);
		kk.vy=5*Math.random();
...
function tick(){
	...
	for(var r=0 ; r<ww ; r++){
		...
		if(aa.position.x > 50 && aa.vx>0){
			aa.vx*=-1;
		}else if(aa.position.x <-50 && aa.vx<0){
			aa.vx*=-1;
		}
		if(aa.position.z > 50 && aa.vz>0){
			aa.vz*=-1;
		}else if (aa.position.z <-50 && aa.vz<0){
			aa.vz*=-1;
		}
		if(aa.position.y > 50 && aa.vy>0){
			aa.vy*=-1;
		}else if (aa.position.y <5 && aa.vy<0){
			aa.vy*=-1;
		}


		aa.visible = world2D.ch01.checked;
	}

    for(var r=0 ; r<ww-1 ; r++){
		for(var t=r+1 ; t<ww ; t++){

			var ball1=sballArray[r];
			var ball2=sballArray[t];
	...
	for(var q=0 ; q<cc ; q++){
		...
		kk.position.x+=kk.vx*dt;
		kk.position.y+=kk.vy*dt;
		kk.position.z+=kk.vz*dt;

		if(kk.position.x > 50 && kk.vx>0){
			kk.vx*=-1;
		}else if(kk.position.x <-50 && kk.vx<0){
			kk.vx*=-1;
		}
		if(kk.position.z > 50 && kk.vz>0){
			kk.vz*=-1;
		}else if (kk.position.z <-50 && kk.vz<0){
			kk.vz*=-1;
		}
		if(kk.position.y > 50 && kk.vy>0){
			kk.vy*=-1;
		}else if (kk.position.y <8 && kk.vy<0){
			kk.vy*=-1;
		}

		kk.visible = world2D.ch02.checked;

	}

	for(var q=0 ; q<cc-1 ; q++){
		for(var e=q+1 ; e<cc ; e++){

			var ball1=sballArray[q];
			var ball2=sballArray[e];

			collision(ball1 , ball2 , 16);
		}
