...
function init(){
	for(var i=0;i<nn;i++){
		var ball=new TEACHER.ObjSphere(0.5,0xffffff);
		ball.position.x=-50+i;
		ball.position.y=10;
		ballArray.push(ball);
		scene.add(ball);
	}
	for(var i=0;i<nn;i++){
		var ball2=new TEACHER.ObjSphere(0.5,0xffffff);
		ball2.position.x=-50+i;
		ball2.position.y=10;
		ballArray2.push(ball2);
		scene.add(ball2);
	}
	
	world2D.sl01.setLabel("波速");
    ...
    world2D.btn01.setLabel("橫波");
	world2D.btn02.setLabel("縱波");
	world2D.btn03.setLabel("駐波");
	world2D.btn04.setLabel("電磁波");
	world2D.btn05.setLabel("開始");
	world2D.btn06.setLabel("暫停");
	...
	blackbox=new TEACHER.ObjBox(300,2,300,0x010101);
	scene.add(blackbox);
	blackbox.position.y=-210;

	text1=new TEACHER.ObjTextPlane(80,10,"1607陳愉","z",0xffffff);
	scene.add(text1);
	text1.position.y=30;
	text1.position.z=-50;
		...
		if(gamemode===1){
			if(wavemode===1){
				for(var i=0;i<nn;i++){
					...
					ball.position.y=10+aa*Math.sin(2*Math.PI*ball.position.x/lambda-2*Math.PI*tt/period);
					ball.mat.color.setHex(0xff3030);
				}
			}else if(wavemode===2){
				for(var i=0;i<nn;i++){
					var ball=ballArray[i];
					ball.position.x=-50+i+aa*Math.sin(2*Math.PI*(-50+i)/lambda-2*Math.PI*tt/period);
					ball.mat.color.setHex(0x30ff30);
				}
			}else if(wavemode===3){
				for(var i=0;i<nn;i++){
					var ball=ballArray[i];
					ball.position.y=10+0.5*(aa*Math.sin(2*Math.PI*ball.position.x/lambda-2*Math.PI*tt/period)+aa*Math.sin(2*Math.PI*ball.position.x/lambda+2*Math.PI*tt/period));
					ball.mat.color.setHex(0x3030ff);
				}
			}else if(wavemode===4){
				for(var i=0;i<nn;i++){
					var ball=ballArray[i];
					...
					ball.position.y=10+aa*Math.sin(2*Math.PI*ball.position.x/lambda-2*Math.PI*tt/period);
					ball.mat.color.setHex(0xffff30);
					ball2.position.z=aa*Math.sin(2*Math.PI*ball.position.x/lambda-2*Math.PI*tt/period);
					ball2.mat.color.setHex(0xffff30);
				}
			}
			tt+=dt;
		}
...
function clickBtn(e){
	if(e.target.parent.name==="btn01"){
		wavemode=1;
		for(var i=0;i<nn;i++){
			ballArray[i].position.x=-50+i;
			ballArray[i].position.y=10;
			ballArray[i].position.z=0;
			ballArray2[i].position.x=-50+i;
			ballArray2[i].position.y=-210;
			ballArray2[i].position.z=0;
		}
	}else if(e.target.parent.name==="btn02"){
		wavemode=2;
		for(var i=0;i<nn;i++){
			ballArray[i].position.x=-50+i;
			ballArray[i].position.y=10;
	...
	}else if(e.target.parent.name==="btn03"){
		wavemode=3;
		for(var i=0;i<nn;i++){
			ballArray[i].position.x=-50+i;
			ballArray[i].position.y=10;
	...
	}else if(e.target.parent.name==="btn04"){
		wavemode=4;
		for(var i=0;i<nn;i++){
			ballArray[i].position.x=-50+i;
			ballArray[i].position.y=10;
			ballArray[i].position.z=0;
			ballArray2[i].position.x=-50+i;
			ballArray2[i].position.y=10;
			ballArray2[i].position.z=0;
		}
	}else if(e.target.parent.name==="btn05"){
		gamemode=1;
	}else if(e.target.parent.name==="btn06"){
		gamemode=2;
	}
}
