...
//B.定義init
function init(){
	pp= new TEACHER.ObjPicPlane(10,10,pics.goal,"y",2);
	scene.add(pp);
	pp1= new TEACHER.ObjPicPlane(100,30,pics.win,"y",2);
	scene.add(pp1);
	pp2= new TEACHER.ObjPicPlane(30,10,pics.word,"y",2);
	scene.add(pp2);
	pp3= new TEACHER.ObjPicPlane(20,10,pics.name,"y",2);
	scene.add(pp3);
	pp1.visible=false;
...
ratio=world2D.sl01.value;
 ...
 if(ball.position.z>50 && ball.vz>0){
	...
	if(ball.position.x<-45 && ball.position.z<-45){
		pp1.visible=true;
		ball.visible=false;
	}
