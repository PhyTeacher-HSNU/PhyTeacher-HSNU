...
function init(){
	...
	jojo=new TEACHER.ObjPicPlane(10,10,pics.jojo,"z",0);
	jojo.position.y=5;
	scene.add(jojo);
	pts= new TEACHER.Points(0x0000ff , 2 , 10000);
	for(var i=0;i<n*n;i++){
		pts.addPoint(1.2*(i%n-0.5*n),0,1.2*(Math.floor(i/n)-0.5*n));	
	}
	pts.update();
	scene.add(pts);	
	world2D.sl01.setLabel("波紋強度");
...
function tick(){
	...
	for(var i=0;i<n*n;i++){
		...
		var xx=pt.x;
		var zz=pt.z;
		var r=Math.sqrt((xx-jojo.position.x)*(xx-jojo.position.x)+(zz-jojo.position.z)*(zz-jojo.position.z));
		var yy=k*Math.sin(2*Math.PI*r/20 - 2*Math.PI*t/10);
		pts.setPoint(i,xx,yy,zz);
	}
	jojo.position.y=k*Math.sin(- 2*Math.PI*t/10)+5;
	pts.update();
