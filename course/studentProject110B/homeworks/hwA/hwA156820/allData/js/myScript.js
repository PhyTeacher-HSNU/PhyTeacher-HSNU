...
function init(){
	...
	//毛線球
	{
	b = new TEACHER.ObjPicSphere(2.5, pics.ball);
	scene.add(b);
	b.position.y=2.5;
	b.position.x=6;
	}
	...
	//按鈕
	world2D.btn01.setLabel("推球");
	world2D.btn02.setLabel("貓咪轉向");
...
//C.定義tick                                                                                                                       
function tick(){
	b.mat.color.setRGB(world2D.sl01.value,world2D.sl02.value,world2D.sl03.value);
	if(gameMD===0){
		con.add(b);
	}else if(gameMD===1){
		b.position.x+=dx;
		b.rotation.z-=(dx/rr);
		if(b.position.x>40 || b.position.x<-40 || b.position.z>40 || b.position.z<-40){
			b.position.y=2.5;
			b.position.x=6;
			con.rotation.y=0;
			gameMD=0;
