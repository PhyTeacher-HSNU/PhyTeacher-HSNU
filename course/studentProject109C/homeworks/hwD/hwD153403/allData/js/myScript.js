...
//B.定義init
function init(){
	world2D.slCameraRR.maximum=100;
	world2D.slCameraRR.value=80;
ground.visible=skyBox.visible=false;
rr=Math.random();
	stars=new TEACHER.Points(0xf0ff0f,rr);
	scene.add(stars);
	for(var i=0;i<100;i++){
		var xxx=100*(Math.random()-0.5);
		var yyy=100*(Math.random()-0.5);
		var zzz=100*(Math.random()-0.5);
for(var ix=-1;ix<=1;ix++){
	for(var iy=-1;iy<=1;iy++){
		for(var iz=-1;iz<=1;iz++){
			stars.addPoint(xxx+ix*100,yyy+iy*100,zzz+iz*100);
}
}
}
}
...
for(var i=0;i<nn;i++){
	var b=new TEACHER.ObjSphere(0.3,0xff0000);
	b.position.x=-75+i*150/nn;
	b.position.y=y1;
	b.position.z=z1;
    scene.add(b);
	ballArray1.push(b);
...
for(var i=0;i<nn;i++){
	var b=new TEACHER.ObjSphere(0.3,0x000fff);
	b.position.x=-75+i*150/nn;
	b.position.y=y2;
	b.position.z=z2;
    scene.add(b);
...
for(var i=0;i<nn;i++){
	var b=new TEACHER.ObjSphere(0.3,0x00ff00);
	b.position.x=-75+i*150/nn;
	b.position.y=y3;
	b.position.z=z3;
    scene.add(b);
...
for(var i=0;i<nn;i++){
	var b4=new TEACHER.ObjSphere(0.3,0xffffff);
	b4.position.x=-75+i*150/nn;
	b4.position.y=y4;
	b4.position.z=z4;
    scene.add(b4);
	b4.visible=false;
...
world2D.ch01.setLabel("合成");	
world2D.ch02.setLabel("縱波");	
world2D.btn01.setLabel("▶ or ||");	
world2D.btn02.setLabel("wave1");	
...
world2D.sl01.setLabel("wave1.λ");
world2D.sl02.setLabel("wave1.A");
world2D.sl03.setLabel("wave1.T");
world2D.sl01.minimum=-50;
...
stars.position.x=-xxx;
stars.position.y=-yyy;
stars.position.z=-zzz;
	if(xxx>50){
		xxx-=100;
	}else if(xxx<-50){
		xxx+=100;
...
if(world2D.sl01.value>-10 &&  world2D.sl01.value<10){
if(Math.abs(world2D.sl01.value-10)>Math.abs(world2D.sl01.value)+10){
	world2D.sl01.value=10;
}else{
	world2D.sl01.value=-10;
}
	...
	if(wave===1){
		lambda1=world2D.sl01.value;
		A1=world2D.sl02.value;
		period1=world2D.sl03.value;
	}
	if(wave===2){
		lambda2=world2D.sl01.value;
		A2=world2D.sl02.value;
		period2=world2D.sl03.value;
	}
	if(wave===3){
		lambda3=world2D.sl01.value;
		A3=world2D.sl02.value;
		period3=world2D.sl03.value;
	}
	if(a===1){
		tt=tt+dt;
	}
	for(var i=0;i<nn;i++){
        ...
        var b1=ballArray1[i];
		var xx1=A1*Math.sin( 2*Math.PI*(b1.position.x-75+i*150/nn)/lambda1*0.2- 2*Math.PI*tt/period1);
		b1.position.y=y1;
		b1.position.z=z1;
		b1.position.x=-75+i*150/nn+xx1;

		var b2=ballArray2[i];
		var xx2= A2*Math.sin( 2*Math.PI*(b2.position.x-75+i*150/nn)/lambda2*0.2 - 2*Math.PI*tt/period2);
		b2.position.y=y2;
		b2.position.z=z2;
		b2.position.x=-75+i*150/nn+xx2;

		var b3=ballArray3[i];
		var xx3= A3*Math.sin( 2*Math.PI*(b3.position.x-75+i*150/nn)/lambda3*0.2 - 2*Math.PI*tt/period3);
		b3.position.y=y3;
		b3.position.z=z3;
		b3.position.x=-75+i*150/nn+xx3;

		var b4=ballArray4[i];
		b4.position.y=(y1+y2+y3)/3;
		b4.position.z=(z1+z2+z3)/3;
		b4.position.x=(b1.position.x+b2.position.x+b3.position.x)/3+xx1+xx2+xx3;

		b1.scale.z=10;
		b2.scale.z=10;
		b3.scale.z=10;
		b4.scale.z=10;	
		var  v1=lambda1/period1;
		var  v2=lambda2/period2;
		var  v3=lambda3/period3;
		if(A1===0){
			v1=0;
			if(wave===1){
		...
		var b1=ballArray1[i];
		var yy1=A1*Math.sin( 2*Math.PI*b1.position.x/lambda1 - 2*Math.PI*tt/period1);
		b1.position.y=y1+yy1;
		b1.position.z=z1;
		b1.position.x=-75+i*150/nn;

		var b2=ballArray2[i];
		var yy2= A2*Math.sin( 2*Math.PI*b2.position.x/lambda2 - 2*Math.PI*tt/period2);
		b2.position.y=y2+yy2;
		b2.position.z=z2;
		b2.position.x=-75+i*150/nn;

		var b3=ballArray3[i];
		var yy3= A3*Math.sin( 2*Math.PI*b3.position.x/lambda3 - 2*Math.PI*tt/period3);
		b3.position.y=y3+yy3;
		b3.position.z=z3;
		b3.position.x=-75+i*150/nn;

		var b4=ballArray4[i];
		b4.position.y=b1.position.y-y1+b2.position.y-y2+b3.position.y-y3+(y1+y2+y3)/3;
		b4.position.z=(z1+z2+z3)/3;
		b4.position.x=-75+i*150/nn;
...
function clickbtn(e){
    ...
    world2D.sl02.setLabel("wave1.A");	
    world2D.sl03.setLabel("wave1.T");
	world2D.sl01.value=lambda1;
	world2D.sl02.value=A1;
	world2D.sl03.value=period1;
	}
	if(wave===2){
	world2D.btn02.setLabel("wave2");
	world2D.sl01.setLabel("wave2.λ");	
    world2D.sl02.setLabel("wave2.A");	
    world2D.sl03.setLabel("wave2.T");
	world2D.sl01.value=lambda2;
	world2D.sl02.value=A2;
	world2D.sl03.value=period2;
	}
	if(wave===3){
	world2D.btn02.setLabel("wave3");
	world2D.sl01.setLabel("wave3.λ");	
    world2D.sl02.setLabel("wave3.A");	
    world2D.sl03.setLabel("wave3.T");
	world2D.sl01.value=lambda3;
	world2D.sl02.value=A3;
	world2D.sl03.value=period3;
	}
	}
}
