...
function init(){
	...
	world2D.sl03.setLabel("水平移動")
	world2D.sl03.minimum = -35
	world2D.sl03.maximum = 35
	world2D.sl03.value = 0

	world2D.btn01.setLabel("Launch")

	
	world2D.slCameraRR.value = 150
	
	world2D.ch01.setLabel("1579_35_劉益全")
	

	//ARROW
	arrowV = new TEACHER.ObjArrow(1,0xFFFF00)
	arrowA = new TEACHER.ObjArrow(1,0x00FFFF)
	arrowL = new TEACHER.ObjArrow(1,0xff00ff)
	scene.add(arrowA)
	scene.add(arrowV)
	scene.add(arrowL)
	arrowL.position.z = 20
	arrowL.position.y = 2


	//wall
	
	wall_right = new TEACHER.ObjBox(2,6,100,0xffffff)
	wall_left = new TEACHER.ObjBox(2,6,100,0xffffff)
	
	wall_front = new TEACHER.ObjBox(100,6,2,0x000000)
	wall_back = new TEACHER.ObjBox(100,6,2,0xffffff)

	scene.add(wall_right).add(wall_left).add(wall_back).add(wall_front)

	wall_back.position.y = wall_front.position.y =wall_right.position.y = wall_left.position.y = 3
	wall_back.position.z = -50
	wall_front.position.z = 50
	wall_left.position.x = -50
	wall_right.position.x = 50

	//target boxes
	for(var i = 0; i < nn; i++){
		var box
		box = new TEACHER.ObjBox(16, 6, 4, 0x0f0f00)
		scene.add(box)
		box_array.push(box)
		box.position.y = 3
		box.position.x = -38 + (i%4)*25
		box.position.z = -40 + Math.floor(i/4)*10
	}
	
	//rebound box
	rebound_box = new TEACHER.ObjBox(30,6,6,0xffff00)
	scene.add(rebound_box)
	rebound_box.position.y = 3
	rebound_box.position.z = 40
...
function tick(){
	...
	power = world2D.sl01.value
	theta = world2D.sl02.value - 90
	rebound_box.position.x = world2D.sl03.value

	//launch
	if(launch == false){
		ball.position.z = 20
		ball.ax = ball.ay = ball.az = ball.vx = ball.vy = ball.vz = 0
		
	}else if(launch == true && ball.vx == 0 && fail == false){
		arrowL.visible = false
		ball.vx = 4*Math.cos(2*Math.PI*theta/360)*power
		ball.vz = 4*Math.sin(2*Math.PI*theta/360)*power
	}

	//btn
	world2D.btnUp.on("mousedown",toDo)
	world2D.btnDown.on("mousedown",toDo)
	world2D.btnRight.on("mousedown",toDo)
	world2D.btnLeft.on("mousedown",toDo)
	world2D.btn01.on("click",release)
	...
	//rebound.ball
	if(ball.position.x > (50-2 * ball.scale.y) && ball.vx >0){
		ball.vx*=(-1)
	}else if(ball.position.x < (-50+2 * ball.scale.y) && ball.vx <0){
		ball.vx*=(-1)
	}

	if(ball.position.z > (50-2 * ball.scale.y) && ball.vz >0 && fail === false){
		fail = true
		ball.vx = ball.vy = ball.vz = 0
		world2D.ch02.setLabel("Failed")
		world2D.ch02.checked = true

	}else if(ball.position.z < (-50+2 * ball.scale.y) && ball.vz <0){
		ball.vz*=(-1)
	}
	
	//rebound.rebound_box

	if(ball.position.z >= 37 && ball.vz > 0 && rebound_box.position.x - 15 <= ball.position.x && ball.position.x <= rebound_box.position.x + 15){
		ball.vz *= (-1)
	}

	

	//rebound.target_boxes
	...
	for(var i = 0; i < nn; i++){
		...
		//z
			//down
		if(ball.position.z >= box.position.z && ball.position.z < box.position.z + 2 && box.position.x - 8 <= ball.position.x && ball.position.x <= box.position.x + 8 && box.visible === true && ball.vz > 0){
			ball.vz *= (-1)
			box.visible = false
		...
		//x
			//right
		if(ball.position.x <= box.position.x + 8 && ball.position.x >= box.position.x + 7 && ball.position.z >= box.position.z - 2 && ball.position.z <= box.position.z +2 && box.visible === true && ball.vx < 0){
			ball.vx *= (-1)
			box.visible = false
		}
	...
	arrowV.position.x = arrowA.position.x = ball.position.x
	arrowV.position.y = arrowA.position.y = ball.position.y
	arrowV.position.z = arrowA.position.z = ball.position.z
	
	arrowV.setArrow(ball.vx,ball.vy,ball.vz)
	arrowA.setArrow(ball.ax*3,ball.ay*3,ball.az*3)
	arrowL.setArrow(5*Math.cos(2*Math.PI*theta/360)*power, 0, 5*Math.sin(2*Math.PI*theta/360)*power)
...
function toDo(e) {
	if (e.target.parent.name === "btnUp"){
		ball.az = -gg
	}else if (e.target.parent.name === "btnDown"){
		ball.az = gg
	}else if (e.target.parent.name === "btnRight"){
		ball.ax = gg
	}else if (e.target.parent.name === "btnLeft"){
		ball.ax = -gg
	}
}
