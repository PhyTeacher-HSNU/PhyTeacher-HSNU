...
//B.定義init
function init(){
	nameboard= new TEACHER.ObjPicPlane(30,30,pics.name,"z",2);
	scene.add(nameboard);
	nameboard.position.z=-50;
	nameboard.position.y=30;

    world2D.slCameraRR.value=120;

	body1 = new TEACHER.ObjPicPlane(15,10,pics.shini,"y",2);
	scene.add(body1);
	body1.position.y=5;
	body1.position.x=0;
	...
	wheel1 = new TEACHER.ObjPicSphere(2.5,pics.shini,0);
	scene.add(wheel1);
	wheel1.position.x=5;
	wheel1.position.y=2.5;
	wheel1.position.z=-3.5;
	wheel2 = new TEACHER.ObjPicSphere(2.5,pics.shini,0);
	scene.add(wheel2);
	
	wheel2.position.x=-5;
	wheel2.position.y=2.5;
	wheel2.position.z=-3.5;
	wheel3 = new TEACHER.ObjPicSphere(2.5,pics.shini,0);
	scene.add(wheel3);
	wheel3.position.x=-5;
	wheel3.position.y=2.5;
	wheel3.position.z=3.5;
	
	wheel4 = new TEACHER.ObjPicSphere(2.5,pics.shini,0);
	scene.add(wheel4);
	wheel4.position.x=5;
	wheel4.position.y=2.5;
	wheel4.position.z=3.5;
	
	world2D.sl01.setLabel("左右加速");
	world2D.sl01.maximum = 2;
	world2D.sl01.minimum = -2;
	world2D.sl01.value=0;
	world2D.sl01.digitN=1;
	world2D.sl02.setLabel("前後加速");
	world2D.sl02.maximum = 2;
	world2D.sl02.minimum = -2;
	world2D.sl02.value=0;
	world2D.sl02.digitN=1;

	world2D.btn01.setLabel("歸零");
...
//C.定義tick                                                                                                                       
function tick(){
	ax=world2D.sl01.value;
	az=world2D.sl02.value;
	...
	body1vx+=ax*dt;
	body1vz+=az*dt;
	body1.position.x+=body1vx*dt;
	body1.position.z+=body1vz*dt;
	
	
	
	wheel1vx+=ax*dt;
	wheel1vz+=az*dt;
	wheel1.position.x+=wheel1vx*dt;
	wheel1.position.z+=wheel1vz*dt;
	wheel1.rotation.z+=wheel1vx*dt/5;
	wheel1.rotation.x+=wheel1vz*dt/5;
	
	wheel2vx+=ax*dt;
	wheel2vz+=az*dt;
	wheel2.position.x+=wheel2vx*dt;
	wheel2.position.z+=wheel2vz*dt;
	wheel2.rotation.z+=wheel2vx*dt/5;
	wheel2.rotation.x+=wheel2vz*dt/5;

	wheel3vx+=ax*dt;
	wheel3vz+=az*dt;
	wheel3.position.x+=wheel3vx*dt;
	wheel3.position.z+=wheel3vz*dt;
	wheel3.rotation.z+=wheel3vx*dt/5;
	wheel3.rotation.x+=wheel3vz*dt/5;
	
	wheel4vx+=ax*dt;
	wheel4vz+=az*dt;
	wheel4.position.x+=wheel4vx*dt;
	wheel4.position.z+=wheel4vz*dt;
	wheel4.rotation.z+=wheel4vx*dt/5;
	wheel4.rotation.x+=wheel4vz*dt/5;
//碰撞
if(body1.position.x>=50){
	body1.position.x=49;
	
	wheel1.position.x=54;
	wheel2.position.x=44;
	wheel3.position.x=54;
	wheel4.position.x=44;
	body1vx=-body1vx/2;
	...
	wheel3vx=-wheel3vx/2;
	wheel4vx=-wheel4vx/2;
	world2D.sl01.value=world2D.sl01.value/2;
}else if(body1.position.x<=-50){
	body1.position.x=-49;
	
	wheel1.position.x=-44;
	wheel2.position.x=-54;
	wheel3.position.x=-54;
	wheel4.position.x=-44;
	body1vx=-body1vx/2;
	...
	wheel4vx=-wheel4vx/2;
	world2D.sl01.value=world2D.sl01.value/2;
}else if(body1.position.z>=50){
	body1.position.z=49;
	
	wheel1.position.z=49-3.5;
	wheel2.position.z=49-3.5;
	wheel3.position.z=49+3.5;
	wheel4.position.z=49+3.5;
    body1vz=-body1vz/2;
	...
	wheel3vz=-wheel3vz/2;
	wheel4vz=-wheel4vz/2;
	world2D.sl02.value=world2D.sl02.value/2;
}else if(body1.position.z<=-50){
	body1.position.z=-49;
...
//結束
if(world2D.sl01.value<0.1 && world2D.sl01.value>-0.1){
	world2D.sl01.value=0;
	body1vx=0;
...
if(world2D.sl02.value<0.1 && world2D.sl02.value>-0.1){
	world2D.sl02.value=0;
	body1vz=0;
