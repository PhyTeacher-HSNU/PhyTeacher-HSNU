...
function init(){
	...
	bigplane = new TEACHER.ObjPlane(1,1,0x00FFFF);
	scene.add(bigplane);

	that = new TEACHER.ObjCylinder(1,1,0x00FFFF);
	that.position.x = RR;
	that.position.y = 0.5;
	scene.add(that)

	OO = new TEACHER.ObjTextPlane(10,5,'展開','z',0xFFFF10);
	OO.position.z = 0;
	scene.add(OO);

	PP = new TEACHER.ObjTextPlane(20,5,'切圓柱','z',0xFFFF10);
	PP.position.z = 0;
	scene.add(PP);

	myname = new TEACHER.ObjTextPlane(40,7,'153317 倪德耀','z');
	myname.position.z = 45;
	myname.position.y = 3.5;
	scene.add(myname); 
	//ground.visible = false;

	for(var i=0;i<NN;i++){
		var ball1 = new TEACHER.ObjSphere(0.3,0xFFFF00)
		ball1.position.x = -(Math.cos(i/NN*2*Math.PI)*RR)+RR;
		ball1.position.z = -(Math.sin(i/NN*2*Math.PI)*RR);
		scene.add(ball1);
		ballarray1.push(ball1);

		var ball2 = new TEACHER.ObjSphere(0.3,0xFFFF00)
		ball2.position.z = 0;
		ball2.position.y = ballarray1[i].position.y;
		scene.add(ball2);
		ballarray2.push(ball2);

		var plane1 = new TEACHER.ObjPlane(0.5,1,0x00FFFF);
		plane1.position.x = ball1.position.x
		plane1.position.z = ball1.position.z
		plane1.position.y = 2.5
		scene.add(plane1)
		plane1array.push(plane1)

		var plane2 = new TEACHER.ObjPlane(0.5,1,0x00FFFF);
		plane2.position.x = ball2.position.x
		plane2.position.z = ball2.position.z
		plane2.position.y = 2.5
		scene.add(plane2)
		plane2array.push(plane2)
	}
	

	world2D.sl01.setLabel('斜率');
	world2D.sl02.setLabel('半徑');
	world2D.sl03.setLabel('高');
	...
	world2D.sl02.value = 8;
	world2D.sl03.value = 5;
	world2D.sl01.digitN = 1;
	
	world2D.ch01.setLabel('顯示全部');
	world2D.ch02.setLabel('顯示下半部');
...
function tick(){
	...
	that.visible = world2D.ch01.checked;
	bigplane.visible = world2D.ch01.checked;


	MM = world2D.sl01.value ;
	RR = world2D.sl02.value;
	HH = world2D.sl03.value;
	
	that.scale.x = that.scale.z = world2D.sl02.value;
	that.scale.y = ballarray1[NN/2+1].position.y;
	that.position.x = world2D.sl02.value;
	that.position.y = ballarray1[NN/2+1].position.y/2;

	bigplane.scale.x = ballarray2[0].position.x - ballarray2[NN-1].position.x;
	bigplane.scale.y = ballarray2[NN/2+1].position.y;
	bigplane.position.x = ballarray2[NN/2+1].position.x;
	bigplane.position.y = ballarray2[NN/2+1].position.y/2;

	OO.position.x = ballarray2[NN/2].position.x;
	OO.position.y = ballarray2[NN/2].position.y+5;

	PP.position.x = ballarray1[NN/2].position.x;
	PP.position.y = ballarray1[NN/2].position.y+5;

	for(var i=0;i<NN;i++){
		var ball1 = ballarray1[i];
		var ball2 = ballarray2[i];
		var plane1 = plane1array[i];
		var plane2 = plane2array[i];
		ball1.position.y = HH + MM*ball1.position.x ;
		ball1.position.x = -(Math.cos(i/NN*2*Math.PI)*RR)+RR;
		ball1.position.z = -(Math.sin(i/NN*2*Math.PI)*RR);
		
		aa = Math.sqrt(Math.pow(MM*RR,2)+Math.pow(RR,2))/2;
		bb = RR/2;
		LL = RR*Math.PI + 4*(aa-bb);
		ball2.position.y = ballarray1[i].position.y;
		ball2.position.x = i/NN * LL-LL-5;
		
		plane1.position.x = ball1.position.x;
		plane1.position.z = ball1.position.z;
		plane2.position.x = ball2.position.x;
		plane2.position.z = ball2.position.z;
		plane1.scale.y = ball1.position.y;
		plane2.scale.y = ball2.position.y;
		plane1.position.y =ball1.position.y/2;
		plane2.position.y =ball2.position.y/2;

		plane1.visible = world2D.ch02.checked;
		plane2.visible = world2D.ch02.checked;
