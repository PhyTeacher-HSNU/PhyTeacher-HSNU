...
function init(){
	...
	for (var i=0;i<n ;i++ ){
		...
		aa = new TEACHER.ObjSphere(2,0xff0000);
		
		aa.position.y= Math.floor(i/100)*10+5;
		aa.position.z=-45+(i%10)*10;
		aa.position.x=-45+Math.floor(i%100/10)*10;

		aa.vx=5*(Math.random()-0.5);
		aa.vz=5*(Math.random()-0.5);
		aa.vy=4*(Math.random()-0.5);
		...
		var arrow = new TEACHER.ObjArrow(0.5,0xffff00);
		scene.add(arrow);
		arrowarray.push(arrow);
	}
	for(var i=0;i<m;i++){
		c1=new TEACHER.ObjBox(100,1,1,0x0000FF)
		c1.position.x=0;
		c1.position.z=-50+(i%2)*100;
		c1.position.y=Math.floor(i/2)*50;
		scene.add(c1);
		c2=new TEACHER.ObjBox(1,50,1,0x0000FF)
		c2.position.x=-50+(i%2)*100;
		c2.position.z=-50+Math.floor(i/2)*100;
		c2.position.y=25;
		scene.add(c2);
		c3=new TEACHER.ObjBox(1,1,100,0x0000FF)
		c3.position.x=-50+(i%2)*100;
		c3.position.z=0;
		c3.position.y=Math.floor(i/2)*50;
		scene.add(c3);
	}
	world2D.sl01.maximum=4;
	world2D.sl01.minimum=1;
	world2D.sl01.setLabel("球半徑")
	world2D.sl01.value=2;	
	world2D.sl02.maximum=50;
	world2D.sl02.minimum=20;
	world2D.sl02.setLabel("熱(冷)源影響範圍")
	world2D.sl02.value=30;	
	world2D.sl03.maximum=1;
	world2D.sl03.minimum=0.1;
	world2D.sl03.digitN=1
	world2D.sl03.setLabel("熱(冷)度")
	world2D.sl03.value=0.5;
	setInterval(tick,1000/fps);
	world3D.camera.position.y=25
	world2D.ch01.setLabel("熱源")
	world2D.ch02.setLabel("冷源")
	world2D.btn01.setLabel("熱(冷)源上")
	world2D.btn02.setLabel("熱(冷)源下")
	world2D.btnUp.on("mousedown",qq)
	world2D.btnDown.on("mousedown",qq)
	world2D.btnRight.on("mousedown",qq)
	world2D.btnLeft.on("mousedown",qq)
	world2D.btn01.on("click",qq)
	world2D.btn02.on("click",qq)
	world2D.on("pressup",pp);

	fire=new TEACHER.ObjPicPlane(20,20,pics.fire,"z",2)
	scene.add(fire)
	ice=new TEACHER.ObjPicPlane(20,20,pics.ice,"z",2)
	scene.add(ice)
	...
	dis=25
	var cc = new TEACHER.ObjTextPlane(50,7,"王克洋 157101","z",0xffffff)
	scene.add(cc)
	cc.position.y=20
	cc.position.z=-50
}
...
function tick(){
	...
	degree=world2D.sl03.value
	dis=world2D.sl02.value
	fire.position.x+=fire.vx
	fire.position.y+=fire.vy
	fire.position.z+=fire.vz
	ice.position.x+=ice.vx
	ice.position.y+=ice.vy
	ice.position.z+=ice.vz
	for(var i=0 ; i<n ; i++){

		aa=ballarray[i];

		aa.position.x+=aa.vx*dt;
		aa.position.z+=aa.vz*dt;
		aa.position.y+=aa.vy*dt;

		if (aa.position.x>50-rr && aa.vx>0){
			aa.vx*=-1;
		}else if (aa.position.x<-50+rr && aa.vx<0){
			aa.vx*=-1;
		}else if (aa.position.z>50-rr && aa.vz>0){
			aa.vz*=-1;
		}else if (aa.position.z<-50+rr && aa.vz<0){
			aa.vz*=-1;
		}else if (aa.position.y>50-rr && aa.vy>0){
			aa.vy*=-1;
		}else if (aa.position.y<rr && aa.vy<0){
			aa.vy*=-1;
		}

		var r12x=fire.position.x-aa.position.x;
		var r12y=fire.position.y-aa.position.y;
		var r12z=fire.position.z-aa.position.z;
		var firerr=Math.sqrt(r12x*r12x+r12y*r12y+r12z*r12z);
		if(firerr<dis && fire.visible===true){
			aa.vy+=degree
		}
		var i12x=ice.position.x-aa.position.x;
		var i12y=ice.position.y-aa.position.y;
		var i12z=ice.position.z-aa.position.z;
		var icerr=Math.sqrt(i12x*i12x+i12y*i12y+i12z*i12z);
		if(icerr<dis && ice.visible===true){
			aa.vy-=degree
		}
		
		aa.scale.x=aa.scale.z=aa.scale.y=rr/2
		var arrow = arrowarray[i];
		arrow.position.x=aa.position.x;
		arrow.position.z=aa.position.z;
		arrow.position.y=aa.position.y;

		arrow.setArrow(aa.vx*(rr-0.5),aa.vy*(rr-0.5),aa.vz*(rr-0.5))
	...
	for(var i=0 ; i<n-1 ; i++){
		for(var j=i+1 ; j<n ; j++){
			...
			collision(b1,b2,rr*2);
		}
	}

	fire.visible=world2D.ch01.checked
	ice.visible=world2D.ch02.checked
