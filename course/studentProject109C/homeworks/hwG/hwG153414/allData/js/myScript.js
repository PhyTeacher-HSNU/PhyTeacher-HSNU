...
function init(){
	...
	skyBox.visible=ground.visible=world2D.ch01.visible=world2D.ch02.visible=world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=false;

	sun = new TEACHER.ObjSphere(2,0xff0000);
	scene.add(sun);

	planet = new TEACHER.ObjSphere(1,0x00ffff);
	planet.vx=0;
	planet.vy=0;
	planet.vz=0;
	scene.add(planet);
	...
	arrow=new TEACHER.ObjArrow(0.5,0xffff00);
	scene.add(arrow);

	world2D.btn01.on("click",clickBtn);
	world2D.btn01.setLabel("start/stop");
	world2D.btn02.on("click",function(e){line.clear();});
	world2D.btn02.setLabel("clear");
	
	
	world2D.sl01.setLabel("星球x位置");
	world2D.sl01.maximum=40;
	world2D.sl01.minimum=-40;
	world2D.sl01.value=20;

	world2D.sl02.setLabel("星球y位置");
	world2D.sl02.maximum=40;
	world2D.sl02.minimum=-40;
	world2D.sl02.value=0;

	world2D.sl03.setLabel("出速度");
	world2D.sl03.maximum=50;
	world2D.sl03.minimum=0;
	world2D.sl03.value=0;

	world2D.sl04.setLabel("角度");
	world2D.sl04.maximum=180;
	world2D.sl04.minimum=-180;
	world2D.sl04.value=0;
...
function tick(){
	...
	for(var i=0;i<10;i++){
		...
		if(start<0){
			line.visible=false;
			arrow.visible=true;	
			arrow.setArrow(world2D.sl03.value,0,0);
			planet.position.x=world2D.sl01.value;
			planet.position.z=world2D.sl02.value;
			arrow.position.x=planet.position.x;
			arrow.position.y=0;
			arrow.position.z=planet.position.z;
			arrow.rotation.y=world2D.sl04.value/180*Math.PI;
			planet.vx=world2D.sl03.value*Math.cos(world2D.sl04.value/180*Math.PI);
			planet.vz=-world2D.sl03.value*Math.sin(world2D.sl04.value/180*Math.PI);
		}
		...
		if (start>0){
			...
			planet.ay=-gmm*planet.position.y/Math.pow(rr,3);
			...
			planet.position.x+=planet.vx*dt;
			planet.position.y+=planet.vy*dt;
			planet.position.z+=planet.vz*dt;

			if( planet.position.x < 3 && planet.position.x > -3){
				if(planet.position.z < 3 &&  planet.position.z > -3){
					start=-1;
				}
			}
			if(planet.position.x > 400){
				start=-1;
			}else if(planet.position.x < -400){
				start=-1;
			}else if(planet.position.z > 400){
				start=-1;
			}else if(planet.position.z < -400){
				start=-1;
			}
