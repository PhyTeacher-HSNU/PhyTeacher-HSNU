...
var lazer;
  ...
  function init(){ 
	...
	afaArray.push(25.43);
	afaArray.push(9.936);
	afaArray.push(13.589);
	afaArray.push(17.004);
	afaArray.push(20.321);
	afaArray.push(23.586);
	afaArray.push(26.82);
	faline=new TEACHER.ObjCylinder(0.35,100,0xffffff);
	scene.add(faline);
	wall=new TEACHER.ObjPlane(200,200,0x898989,"y",2);
	scene.add(wall);
	wall.position.y=150;
	...
	line2=new TEACHER.Line(0x00ff0f,1.5,500);
	scene.add(line1);
	scene.add(line2);
	...
	ground.visible=skyBox.visible=world2D.sl03.visible=world2D.btn02.visible=world2D.btnUp.visible=world2D.btnDown.visible=world2D.btnLeft.visible=world2D.btnRight.visible=world2D.ch02.visible=false; 
	world2D.btn01.on('click',btn);
	world2D.btn01.setLabel('重置');
	world2D.sl01.setLabel("n");
	world2D.sl02.setLabel("m");
	world2D.ch01.setLabel("圓形");
	world2D.sl02.minimum=0;
	world2D.sl01.minimum=1;
	world2D.sl01.maximum=world2D.sl02.maximum=6; 
	world2D.sl01.value=world2D.sl02.value=1; 
	pts=new TEACHER.Points(0xffffff,8); scene.add(pts); 
	...
	pts.update(); 
	setInterval(tick,60/fps);
				 }
//C.定義tick 
function tick(){ 
	n=Math.floor(world2D.sl01.value); 
	m=Math.floor(world2D.sl02.value); 
	world2D.sl01.value=Math.floor(world2D.sl01.value+0.5); 
	world2D.sl02.value=Math.floor(world2D.sl02.value+0.5); 
	tt+=dt; 
	for(var ix=0;ix<N;ix++){ 
		for(var iz=0;iz<N;iz++){ 
			if(world2D.ch01.checked){ 
				...
				jjmm(afaArray[6*m+n-1]/(a/2)*iz*(a/2)/N); 
				var yy=A*Math.cos(cc*afaArray[6*m+n-1]/(a/2)*tt)*Math.cos(m*(ix/N*2*Math.PI+0.7))*jmm; 
				jmm=0; 
			}else{ 
				var xx=-50+100*iz/N; 
				var zz=-50+100*ix/N; 
				var yy=A/2*Math.cos(Math.PI*cc*Math.sqrt(n/a*n/a+m/b*m/b)*tt)*Math.sin(n*Math.PI/a*(xx+50))*Math.sin(m*Math.PI/b*(zz+50))+A/2*Math.cos(Math.PI*cc*Math.sqrt(m/a*m/a+n/b*n/b)*tt)*Math.sin(m*Math.PI/a*(xx+50))*Math.sin(n*Math.PI/b*(zz+50)); 
			} 
	...
	for(let i=0;i<N*N;i++){ 
		let hh=pts.getPoint(i).y; 
		color.setHSL(hh*0.2,0.5,0.5); 
		colors.push(color.r,color.g,color.b); 
	} 
	pts.geometry.setAttribute('color',new THREE.Float32BufferAttribute(colors,3)); pts.update();
...
if(world2D.ch01.checked){
	...
	lazer.position.x=pts.getPoint(mirrormiddle).x;
	...
	line1.setPoint(1,pts.getPoint(mirrormiddle).x,0,pts.getPoint(mirrormiddle).z);
	line1.setPoint(2,pts.getPoint(mirrormiddle).x+150*Math.tan(-2*th1),150,pts.getPoint(mirrormiddle).z+150*Math.tan(2*th2));
	line2.addPoint(pts.getPoint(mirrormiddle).x+150*Math.tan(-2*th1),150,pts.getPoint(mirrormiddle).z+150*Math.tan(2*th2));
	line1.update();
	line2.update();
//3D 
world3D.render(); 
