...
function init(){
	...
	walll = new TEACHER.ObjPlane(50, 50, pics.walll, "z", 0);
	ball = new TEACHER.ObjSphere(rr, pics, 0);
	scene.add(ball);
	ball.position.x = 0;
	ball.position.y = 30;
	ball.position.z = 0;
...
function tick(){
	...
	if(ball.position.y < 0  + rr && ball.vy < 0){
		ball.vy *= -1;
	}else if(ball.position.y > 50 - rr && ball.vy > 0){
		ball.vy *= -1;
	}
	ball.ax = world2D.sl01.value;
	ball.ay = -world2D.sl02.value;
	ball.az = world2D.sl03.value;
