...
//B.定義init
function init(){
	world3D.cameraTheta=0*Math.PI/180;
	world2D.slCameraRR.value = 125;
	ground.visible=logo.visible=false;
	pts=new TEACHER.Points(0xD0D0D0,1.5,40000);
	scene.add(pts);
	for (var i=0;i<n;i++){
		for (var j=0;j<n;j++){
			var xx=-50+100*i/n;
...
function tick(){
	//波源
	tt+=dxt;
	period=world2D.sl01.value;
	A=world2D.sl02.value;
	light=world2D.sl03.value;
	if (gamemode===1||gamemode===2)
		world2D.btn06.visible=true;
	else
		world2D.btn06.visible=false;
	if (pause===1||pause===2){
		if (pause===1){
			ys=A*Math.sin(-2*Math.PI*tt/period);
			uNow[(0.5*n)*n+n/2]=ys;
			var pt=pts.getPoint((0.5*n)*n+n/2);
			pts.setPoint((0.5*n)*n+n/2,pt.x,ys,pt.z);
		}
	

		for(var i=1;i<n-1;i++){
			for (var j=1;j<n-1;j++){
				uNew[j*n+i]=2*uNow[j*n+i]-uOld[j*n+i]+cc*cc*dt*dt/dx/dx*(uNow[(j+1)*n+i]+uNow[(j-1)*n+i]+uNow[j*n+i+1]+uNow[j*n+i-1]-4*uNow[j*n+i]);
				var pt=pts.getPoint(j*n+i);
				pts.setPoint(j*n+i,pt.x,uNew[j*n+i],pt.z);
		...
		for (var i=0;i<n;i++){
			for (var j=0;j<n;j++){
				uOld[j*n+i]=uNow[j*n+i];
				uNow[j*n+i]=uNew[j*n+i];
			}
		}
		if (gamemode===1){
			var colors=[];
			...
			for(var i=0;i<n;i++){
				for (var j=0;j<n;j++){
					var pt=pts.getPoint(j*n+i).y;
					var col=Math.abs(ttarray[j*n+i]-pt);
					var tta=1;
					if (pt<0.1&&pt>-0.1){
						tta+=ttc;
					}else{
					...
					color.setHSL(0,0,tta);
					colors.push(color.r,color.g,color.b);
				}
			}
			
			for (var i=0;i<n*n;i++){
				var pt=pts.getPoint(i).y;
				ttarray[i]=pt;
			}
			pts.geometry.setAttribute('color',new THREE.Float32BufferAttribute(colors,3)); 
			pts.update();
