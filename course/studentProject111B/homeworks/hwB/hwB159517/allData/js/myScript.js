...
function init(){
	...
	ball=new TEACHER.ObjPicSphere(1.25*r,pics.mc)
	scene.add(ball)
	...
	ball.rotation.z=1.675
	ball.rotation.y=-1.675

	world2D.btn01.setLabel("得分0")
	world2D.btn02.setLabel("重新")
	world2D.btn02.on("click",reset)

	nm=new TEACHER.ObjTextPlane(50,6,"1595班17號 吳宗餘","y",0xffffff)
	ru1=new TEACHER.ObjTextPlane(50,6,"請試著逃離紅鬼的追捕","y",0xFF0000)
	ru2=new TEACHER.ObjTextPlane(50,6,"活得越久，得分越高","y",0xFF0000)
	hs=new TEACHER.ObjTextPlane(50,6,"最高得分:"+hn,"y",0xFFFF00)
	...
	for(var i=0;i<p;i++){
		enemy=new TEACHER.ObjPicCylinder(r,0.1,pics.ghost,false)
		enemyArray.push(enemy)
		scene.add(enemy)
		enemy.rotation.y=1.2
		enemy.position.x=(100-2*r)*(Math.random()-0.5)
		enemy.position.z=(100-2*r)*(Math.random()-0.5)
		enemy.position.y=-4
	}
...
function tick(){
	...
	vx=(vecM.x-ball.position.x)*0.1
	vz=(vecM.z-ball.position.z)*0.1
	...
	if(Gm===1){
		...
		if(ball.position.x>50){
			ball.position.x=49.9
		}
		else if(ball.position.x<-50){
			ball.position.x=-49.9
		}
		if(ball.position.z>50){
			ball.position.z=49.9
		}
		...
		n++
		if(n===60){
			enemyArray[k].position.x=(100-2*r)*(Math.random()-0.5)
			enemyArray[k].position.z=(100-2*r)*(Math.random()-0.5)
			enemyArray[k].position.y=1
			n=0
			k++
		}
		for(i=0;i<k;i++){
			if(ball.position.x-enemyArray[i].position.x>0){
				enemyArray[i].position.x+=evx*(i%5*Math.random()+1)*Math.random()*(k-0.5)/(k+0.5)
			}
			else if(ball.position.x-enemyArray[i].position.x<0){
				enemyArray[i].position.x-=evx*(i%5*Math.random()+1)*Math.random()*(k-0.5)/(k+0.5)
			}
			if(ball.position.z-enemyArray[i].position.z>0){
				enemyArray[i].position.z+=evz*(i%5*Math.random()+1)*Math.random()*(k-0.5)/(k+0.5)
			}
			else if(ball.position.z-enemyArray[i].position.z<0){
				enemyArray[i].position.z-=evz*(i%5*Math.random()+1)*Math.random()*(k-0.5)/(k+0.5)
			}
			if(ball.position.x-enemyArray[i].position.x<1.5*r&&ball.position.x-enemyArray[i].position.x>-1.5*r&&ball.position.z-enemyArray[i].position.z<1.5*r&&ball.position.z-enemyArray[i].position.z>-1.5*r){
				Gm=0
				sc=new TEACHER.ObjTextPlane(100,20,"總分"+k+"分","y",0xFF0000)
				scene.add(sc)
				sc.visible=true
			}
		
		}
		world2D.btn01.setLabel("得分"+k)
	}
